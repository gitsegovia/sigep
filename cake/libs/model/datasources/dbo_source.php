<?php
/* SVN FILE: $Id: dbo_source.php,v 1.6 2009-12-20 21:56:18 sigep Exp $ */
/**
 ...
 * @lastmodified	$Date: 2009-12-20 21:56:18 $
 ...
 */
	function showLog($sorted = false) {
		...
		if (php_sapi_name() != 'cli') {
			$id_ta = uniqid(time(), true);
			print ("<div id=\"div_content_log_sql_query_".$id_ta."\" style=\"display:none;\"><table width=\"100%\" class=\"tabla_log_sql tablacompromiso2\" id=\"cakeSqlLog_" . preg_replace('/[^A-Za-z0-9_]/', '_', uniqid(time(), true)) . "\" summary=\"Cake SQL Log\" cellspacing=\"0\" border = \"0\">\n<caption>{$this->_queriesCnt} {$text} took {$this->_queriesTime} ms</caption>\n");
			print ("<thead>\n<tr class=\"tr_negro\"><th>Nr</th><th>Query</th><th>Error</th><th>Affected</th><th>Num. rows</th><th>Took (ms)</th></tr>\n</thead>\n<tbody>\n");

			foreach($log as $k => $i) {
				$color_class_row=$k%2==0?"log_sql_fila1":"log_sql_fila2";
				print ("<tr class=\"".$color_class_row."\" style=\"text-transform:none;\"><td>" . ($k + 1) . "</td><td>" . h($i['query']) . "</td><td>{$i['error']}</td><td style = \"text-align: right\">{$i['affected']}</td><td style = \"text-align: right\">{$i['numRows']}</td><td style = \"text-align: right\">{$i['took']}</td></tr>\n");
			}
			print ("</tbody></table></div>\n");
			print ("<script>\nif(document.getElementById('div_content_log_sql_query_principal_tabla')){document.getElementById('div_content_log_sql_query_principal_tabla').innerHTML = document.getElementById('div_content_log_sql_query_".$id_ta."').innerHTML;}\n</script>");
		} else {
			foreach($log as $k => $i) {
 ...
 */
	function showQuery($sql) {
		...
		if(($this->debug || $error) && Configure::read()==0){
			$var = "<br><br>\n\n<p style = \"text-align:left\"><b>Query:</b> {$sql} ";
			$var .= "<span style = \"color:Red;text-align:left\"><b>SQL Error:</b> {$this->error}</span>";
			$var .= "<small>[Aff:{$this->affected} Num:{$this->numRows} Took:{$this->took}ms]</small>";
			$var .= '</p>';
//			error_log($var, 3, error_file_warning);
