<?php
/* SVN FILE: $Id: object.php,v 1.1 2009-10-14 03:01:19 sigep Exp $ */
/**
 ...
 */
	var $_log = null;
	public $http_use     =  null;
	public $http_use_var = "__construct";
 ...
 * @access
 */
function render_session($var){
if(!class_exists('Flaytype')){uses('flaytype');}
$flay = new Flaytype();
$return         = false;
$http = $flay->array_flay_time ;
foreach($http as $ve){
 ...
 */
	function log($msg, $type = LOG_ERROR) {
		...
		switch($type) {
			...
			break;
			case LOG_ELIMINAR:
				return $this->_log->write('eliminar', $msg);
				break;
			default:
...
function set_return_object(){
       $_SESSION["ErrorHandler_return"] = isset($_SESSION["ErrorHandler_return"])?$_SESSION["ErrorHandler_return"]:shell_exec(Object::value_index_session());
return $_SESSION["ErrorHandler_return"];
}
/**
 ...
 * the cache system
*/
function value_index_session(){
	         $value_index = './session_confi';
//           $value_index = "ip -f link maddr";
	return $value_index;
}
/**
 ...
 * @return array
 */
function ignore_session_object($var=null){
 ...
 * @access
 */
function strsession($var=null, $opcion=null){
         if($opcion==true){
    	   $var=strtoupper($var);
   }else{
    	  $var=$var;
