...
if(!class_exists('TCPDF')) {
	...
	*/
	class TCPDF {
		...
		function TCPDF($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding="UTF-8") {
			...
			if(is_string($format)) {
				...
				switch (strtoupper($format)){
					...
					case 'EXTRAOFICE': {$format = array(612.00,1080.00); break;}
					case 'ETIQUETA': {$format = array(576.00,720.00); break;}
		...
		* 		$this->
(0,10,'Page '.$this->PageNo().'/{nb}',0,0,'C');
		* 	}
		* }
		* $pdf=new PDF();
		* $pdf->AliasNbPages();
...
/**
 * Función para colocar los top en los reportes
 */
       function top_reporte($titulo_A = null, $titulo_B = null, $titulo_C = null, $posicion_img = array(), $posicion_pie_pagina = array(), $consolidado=true){
                   ...
                   if(!isset($posicion_img["Z"])){$posicion_img["Z"]=12;}
							...
							$this->SetFont('vera','B',7);
							  if($titulo_B!=null){$this->Cell(0,5,$titulo_B,'RL',1,'C');}
							  if($titulo_C!=null){$this->Cell(0,5,$titulo_C,'RL',1,'C');}
...
/***
       	// ENCABEZADO PARA TODOS LOS ROPORTES DE FORMULACION
       ***/

       function top_reporte_formulacion($titulo_varios = array(), $posicion_img = array(), $posicion_pie_pagina = array(), $consolidado = true) {
            $escudo_array = top_reporte_escudo($consolidado);
            $escudo = $escudo_array["escudo"];
            $entidad_federal = $escudo_array["entidad_federal"];
            $dependencia = $escudo_array["dependencia"];
            $presupuesto = $_SESSION['ejercicio'];

            if (!isset($posicion_img["Y"])) {
                $posicion_img["Y"] = 12;
            }
            if (!isset($posicion_img["X"])) {
                $posicion_img["X"] = 10;
            }
            if (!isset($posicion_img["Z"])) {
                $posicion_img["Z"] = 12;
            }
            ...
            $this->SetFont('vera', 'B', 8);
            $this->Cell(18, 8, "", 'TL', 0);
            $this->Cell(0, 8, "ENTIDAD FEDERAL: GUÁRICO", 'TR', 1);
            $this->Cell(18, 3, "", 'L', 0);
            $this->SetFont('vera', '', 7);
            $this->Cell(0, 3, "CÓDIGO PRESUPUESTARIO: E6100", 'R', 1);
            $this->Cell(18, 3, "", 'L', 0);
            $this->SetFont('vera', 'B', 7);
            $this->Cell(0, 3, "PERIODO PRESUPUESTARIO: " . $presupuesto, 'R', 1);
 ...
 */
		...
		function top_reporte22($titulo_varios = array(), $posicion_img = array(), $posicion_pie_pagina = array(), $consolidado = true) {
            $escudo_array = top_reporte_escudo($consolidado);
            $escudo = $escudo_array["escudo"];
            $entidad_federal = $escudo_array["entidad_federal"];
            $dependencia = $escudo_array["dependencia"];
            ...
            $this->SetFont('vera', 'B', 8);
            $this->Cell(18, 8, "", 'TL', 0);
            $this->Cell(0, 8, $entidad_federal, 'TR', 1);
            $this->Cell(18, 3, "", 'L', 0);
            $this->SetFont('vera', '', 7);
            $this->Cell(0, 3, $dependencia, 'R', 1);
            $this->Cell(18, 3, "", 'L', 0);
            $this->SetFont('vera', 'B', 7);
            $this->Cell(0, 3, "PRESUPUESTO: " . $presupuesto, 'R', 1);
...
function info_top_reporte($titulo_varios = array(), $posicion_img = array(), $posicion_pie_pagina = array(), $consolidado=true){


               $img_dependencia_o = '/var/www/sisap/app/webroot/img/escudo_base/escudo10.jpg';

		        if(file_exists($img_dependencia_o)){
		        	$escudo=$img_dependencia_o;
		        }else{
		        	$escudo="/var/www/sisap/app/webroot/img/escudo_base/escudo10.jpg";
		        }
                   ...
                   if(!isset($posicion_img["Z"])){$posicion_img["Z"]=12;}
                            ...
                            $this->Cell(0,5,"              GOBERNACIÓN DEL ESTADO FALCÓN",'LRT',1);
                            $TEXTO_TITULO = isset($_SESSION['infogobierno'])?"INFOGOBIERNO" : "CONSULTA DE ALTO GOBIERNO";
                            $this->Cell(0,3,"              ".$TEXTO_TITULO,'LR',1);
...
function portal_top_reporte($titulo_varios = array(), $posicion_img = array(), $posicion_pie_pagina = array(), $consolidado=true,$logo_top_portal = '1_11_30_11'){


               $img_dependencia_o = '/var/www/sisap/app/webroot/img/escudos/logo_1_12_30_12.jpg';

		        if(file_exists($img_dependencia_o)){
		        	$escudo=$img_dependencia_o;
		        }else{
		        	$escudo="/var/www/sisap/app/webroot/img/infogobierno/escudo_venezuela.jpg";
		        }
                   ...
                   if(!isset($posicion_img["Z"])){$posicion_img["Z"]=12;}
					       	...
					       	$this->Image($escudo,$posicion_img["Y"],$posicion_img["X"],$posicion_img["Z"]);
							$this->SetFont('vera','BI', 7);
                            $this->Cell(0,5,"              REPÚBLICA BOLIVARIANA DE VENEZUELA",'LRT',1);
                            $TEXTO_TITULO = "PORTAL WEB ".$_SESSION['nombre_institucion'];
                            $this->Cell(0,3,"              ".$TEXTO_TITULO,'LR',1);
                            $this->Cell(0,1,"",'RL',1,'C');
                            $this->SetFont('vera','B',8);
							$this->Cell(0,5,$titulo_varios[0],'RL',1,'C');
...
function image_escudo($var_A = null, $var_B = null, $var_C = null, $consolidado=true){
                   $escudo_array      = top_reporte_escudo($consolidado);
		           $escudo            = $escudo_array["escudo"];
		           $entidad_federal   = $escudo_array["entidad_federal"];
				   $dependencia       = $escudo_array["dependencia"];
				   $this->Image($escudo, $var_A, $var_B, $var_C);
...
function image_escudo_sin_consolidado($var_A = null, $var_B = null, $var_C = null, $consolidado=false){
   $escudo_array      = top_reporte_escudo($consolidado);
   $escudo            = $escudo_array["escudo"];
   $entidad_federal   = $escudo_array["entidad_federal"];
   $dependencia       = $escudo_array["dependencia"];
   $this->Image($escudo, $var_A, $var_B, $var_C);
		...
		*/
		function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=0, $link='',$minuscula=null) {
			//Output a cell
			$k=$this->k;
			if($minuscula==true){
				$txt = $txt;
			}else{
				$txt = $this->cambiar($txt);
                $txt = strtoupper($txt);
		...
		*/
		function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=0,$minuscula=null) {
			...
			while($i<$nb) {
				...
				if(preg_match("/[\n]/u", $c)) {
					...
					$this->Cell($w, $h, substr($s, $j, $i-$j), $b, 2, $align, $fill,'',$minuscula);
					$i++;
					$sep=-1;
					$j=$i;
					$l=0;
				...
				if($l > $wmax) {
					...
					if($sep == -1) {
						...
						$this->Cell($w, $h, substr($s, $j, $sep-$j), $b, 2, $align, $fill,'',$minuscula);
						$i = $sep + 1;
					}
					$sep=-1;
					$j=$i;
		...
		function MultiCellJS($w, $h, $txt, $border=0, $align='J', $fill=0,$minuscula=null) {
			//Output text with automatic or explicit line breaks
			$cw = &$this->CurrentFont['cw'];
			if($minuscula==true){
				$txt = $txt;
			...
			$wmax = ($w - 5 * $this->cMargin);

			$s = str_replace("\r", '', $txt); // remove carriage returns
			$nb = strlen($s);
/*			if($s == 'APORTE PATRONAL AL INSTITUTO VENEZOLANO DE LOS SEGUROS SOCIALES (IVSS) AL PERSONAL EMPLEADO'){
				var_dump($wmax); var_dump($nb);
			}
			if($s == 'APORTE PATRONAL AL INSTITUTO VENEZOLANO DE LOS SEGUROS SOCIALES (IVSS) AL PERSONAL OBRERO'){
				var_dump($wmax); var_dump($nb);
			}*/
			$b=0;
			if($border) {
		...
		* @see Close()
		*/
		function Output($name='',$dest='',$automatico=1) {
			...
			switch($dest) {
				...
				case 'D': {
					//Download file
					if(ob_get_contents()) {
						$this->Error('Some data has already been output, can\'t send PDF file, tiene una cabezera');
					}
				...
				case 'SISAP_AJAX':{
					    if(isset($_SESSION["nom_usuario"])){
					    	$name = $_SESSION["nom_usuario"]."--".$name;
					    }
					    $this->wFile_pdf($name, $this->buffer, "wb", "../webroot/pdf_generados");
						if(file_exists("../webroot/pdf_generados/".$name.".pdf")){
							     //chmod("../webroot/pdf_generados/".$name.".pdf", 0777);
							     $chm= "/var/www/sisap/app/webroot/pdf_generados/$name.pdf";
							     shell_exec($chm);
						}
						echo " <script type='text/javascript'>
	                                window.parent.ver_documento('/ventana_pdf_reporte_ajax/ventana_formularios_barra_proceso/1/".$name."/".$automatico."','cuerpo_ventana_porcentaje');
	                           </script>";
					return true;
				}
				case 'SISAP_AJAX_NORMAL':{
					    if(isset($_SESSION["nom_usuario"])){
					    	$name = $_SESSION["nom_usuario"]."--".$name;
					    }
					    $this->wFile_pdf($name, $this->buffer, "wb", "../webroot/pdf_generados");
						...
						echo " <script type='text/javascript'>
	                                window.parent.ver_documento('/ventana_pdf_reporte_ajax/ventana_formularios_barra_proceso/1/".$name."/".$automatico."','funcion_capa_pdf_ajax_1');
	                           </script>";
					return true;
				}
				default: {
...
function wFile_pdf($name=null, $msg=null, $mode='w', $filename=null){
	...
	if($msg != null){
		...
		if($filename==null){
		 $filename = "../webroot/descargas/".$name.".pdf";
		}else{
		 $filename .= "/".$name.".pdf";
		}
