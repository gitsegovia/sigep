 ...
 * @version     0.3
 * @date        20 Jan 2005
 * $Id: excel.php,v 1.1 2009-11-02 16:32:29 sigep Exp $
 */
class xlsStream
{
    /* private */
    ...
    function stream_read($byte_count)
    {
        ...
        {
            $dataxxx .= fread($this->fp, $byte_count);
            $this->position = strlen($dataxxx);
        }
        return $dataxxx;
    }
     ...
     * @param   (string)    $dataxxx           serialized array data string
     *                                      representing a tabular worksheet
     */
    function stream_write($dataxxx)
    {
        // buffer the data
        $this->buffer .= $dataxxx;
        $bufsize = strlen($dataxxx);
        return $bufsize;
    }
     ...
     * @access  private
     * @param   (array)     $dataxxx           associative array representing
     *                                      a tabular worksheet
     */
    function _xls_stream_write($dataxxx)
    {
        if (is_array($dataxxx) && !empty($dataxxx))
        {
            $row = 0;
            foreach (array_values($dataxxx) as $_data)
            {
