...
class Cnmd18RecordVacacionesController extends AppController
{
	...
	public function consulta_records(){
		...
		**/
		...
		$data = $this->v_cnmd06_fichas_2->findAll("cedula_identidad= " . $_SESSION['infogobierno']['cedula_identidad'] . " and cod_dep = ".$this->data['cnmd18_record_vacaciones']['cod_dep'], null, "fecha_condicion ASC");

		/**
		*  Consulta para obtener el record de vacaciones del trabajador
		**/

		$records = $this->cnmd18_record_vacaciones->findAll("cedula_identidad = " . $_SESSION['infogobierno']['cedula_identidad'] . " and cod_dep = ".$this->data['cnmd18_record_vacaciones']['cod_dep']);
	...
	public function generar_record_vacaciones(){
		...
		**/
		...
		if($this->data['cnmd18_record_vacaciones']['cod_dep'] != '1'){
			$deno_inst = $this->cugd02_institucion->execute("SELECT denominacion, direccion, cod_area, telefonos FROM cugd02_dependencias WHERE cod_tipo_institucion = ".$data[0]['v_cnmd06_fichas_2']['cod_tipo_inst']." and cod_institucion = ".$data[0]['v_cnmd06_fichas_2']['cod_inst']." and cod_dependencia = ".$data[0]['v_cnmd06_fichas_2']['cod_dep']." LIMIT 1;");
		}else{
			$deno_inst = $this->cugd02_institucion->execute("SELECT denominacion, direccion, cod_area, telefonos FROM cugd02_institucion WHERE cod_tipo_institucion = ".$data[0]['v_cnmd06_fichas_2']['cod_tipo_inst']." and cod_institucion = ".$data[0]['v_cnmd06_fichas_2']['cod_inst']." LIMIT 1;");
		}
		...
		**/
		...
		$datos_firma = $this->cnmd06_constancia_firmante->findAll("cod_dep = ".$data[0]['v_cnmd06_fichas_2']['cod_dep'],'funcionario_firmante, cargo_firmante, resolucion', null, 1);
		

		$cod_defecto = $this->cugd02_institucion->execute("SELECT cod_republica, cod_estado, cod_municipio FROM cugd90_municipio_defecto WHERE cod_dep = ".$data[0]['v_cnmd06_fichas_2']['cod_dep']." LIMIT 1;");
		...
		$datos_constancia = $this->v_cnmd06_fichas_datos_personales->findAll(" cod_dep = ".$data[0]['v_cnmd06_fichas_2']['cod_dep']." and cedula_identidad='".$_SESSION['infogobierno']['cedula_identidad']."'",'cod_presi, cod_entidad, cod_tipo_inst, cod_inst, cod_dep, tipo_nomina, cedula_identidad, nacionalidad, primer_apellido, segundo_apellido, primer_nombre, segundo_nombre, denominacion_clase, fecha_ingreso, sexo', null, 1);
