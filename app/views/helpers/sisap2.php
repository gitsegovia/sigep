...
function top_tabla_modulos($entidad=null,$fecha=false,$titulo=null,$subtitulo=null,$anchoPX=null,$modulo=null) {

	$url             = $this->webroot . $this->themeWeb . IMAGES_URL;
	$contenido       = $this->imagen_top_modulo($modulo);
    if(defined('LOGOINST')){
   	   $dir_escudo_inst=LOGOINST;
    }else{
       $dir_escudo_inst="jl";
...
function imagen_top_modulo($modulo){

	$contenido='<style>
						#tabla_top{
							background-image:url(/img/fondos/top/control_nomina/'.$modulo.'_01.jpg);
							background-repeat:no-repeat;
							 -moz-border-radius: 15px;
						     -webkit-border-radius: 15px;
						      border-radius: 15px;
