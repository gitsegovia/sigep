...
if(isset($_GET['tipo'])){
   ...
   $link = pg_pconnect("host=localhost user=sisap password='sisap' dbname=saul_sisap") or die("error ".pg_last_error($link));
...
?>
<tr><td><a href="#msj_<?=strtolower($CM["username_origen"]);?>"  class="a_msj" onClick="ver_documento('/include/msj/index_msj.php?tipo=verifica_msj<?="=".$username."=".$CM["id_mensaje"];?>','lugar_msj');document.getElementById('c_usuarios').style.display='none';"><?=$CM["username_origen"];?></a></td></tr>
<?
}
?>
</table>
...
<div id="contenido_tabla_msj" style="display:none;">
 <table width="550" border="0" class="tabla_msj" cellpadding="0" cellspacing="0">
       ...
       <tr><td class="td_fecha"><?= enviado($DATA["fecha_emitido"])?></td><td align="right"><a href="#Cerrar_msj" onClick="ver_documento('/include/msj/index_msj.php?tipo=cerrar_msj<?="=".$DATA["username"]."=".$DATA["id_mensaje"];?>','lugar_msj');Control.Modal.current.close(true);"><img src="/img/cancela_fila.png" title="Cerrar Mensaje" border="0" width="24" height="24"/></a></td></tr>
       </table>
</div>
<script>
