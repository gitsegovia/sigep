...
//echo $conexion_variables['login'];
   ...
   $codigo = "";
    ...
    if($_GET["grupo"]){                  $sql_3 =  'where cod_grupo =  '.$_GET["grupo"].'  ';                              $tabla=' cfpd01_grupo ';                             $codigo =$_GET["grupo"];  }
	if($_GET["partida"]){                $sql_3 .= 'and cod_partida = '.$_GET["partida"].'  ';                               $tabla=' cfpd01_partida ';                           $codigo =$_GET["partida"];  }
	if($_GET["generica"]){             $sql_3 .= 'and cod_generica = '.$_GET["generica"].'  ';                         $tabla=' cfpd01_generica ';                        $codigo =$_GET["generica"];  }
	if($_GET["especifica"]){           $sql_3 .= 'and cod_especifica = '.$_GET["especifica"].'  ';                    $tabla=' cfpd01_especifica ';                     $codigo =$_GET["especifica"];  }
	if($_GET["sub_especifica"]){  $sql_3.= 'and cod_sub_espec = '.$_GET["sub_especifica"].'  ';           $tabla=' cfpd01_sub_espec ';                   $codigo =$_GET["sub_especifica"];  }
	if($_GET["auxiliar"]){                 $sql_3.= 'and cod_auxiliar = '.$_GET["auxiliar"].'  ';                                $tabla=' cfpd01_auxiliar ';                          $codigo =$_GET["auxiliar"];   }
