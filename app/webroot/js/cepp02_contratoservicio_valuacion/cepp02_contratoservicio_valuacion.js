...
function cepp02_contratoservicios_valuacion_valida(){

if(verifica_cierre_ano_ejecucion('fecha_valuacion')==false){
	fun_msj('LO SIENTO EL A&Ntilde;O DE LA FECHA DE VALUACI&Oacute;N NO CORRESPONDE AL A&Ntilde;O DE EJECUCI&Oacute;N');
	return false;
}else{
...
monto_iva_411           =  redondear(0,2);//redondear(monto_iva_411,2);
factor_reversion        =  eval(1) + eval(porcentaje_iva);
base                    =  eval(monto_iva_411) / eval(factor_reversion);
monto_iva_411           =  eval(monto_iva_411) - eval(base);

monto_partidas_iva      =  eval(monto_partidas_iva) +  eval(monto_iva_411);
monto_partidas_iva      =  redondear(monto_partidas_iva,2);

monto_partidas_sin_iva  =  eval(monto_partidas_sin_iva) -  eval(monto_iva_411);
...
function cepp02_contratoservicios_valuacion_pregunta_pago_parcial(opc){
...
function cepp02_contratoservicios_valuacion_monto_id_editar_2(id_i,op_i){
...
function cepp02_contratoservicios_valuacion_colocar_iva(id_i,op_i){
...
function cepp02_cal_por_multa(a){
...
function cepp02_contratoservicios_valuacion_detalles_del_pago(){
...
for(ii=0; ii<document.getElementById('cuenta_i').value; ii++){
	if(document.getElementById('partida_401_'+ii)){
		if(document.getElementById('partida_401_'+ii).value=="si"){
        	if(document.getElementById('partida_iva_'+ii)){
				    str = document.getElementById('pago_'+ii).value;
				    var a = redondear(str,2);
...
monto_iva_var_aux2 = monto_iva_var;
...
base             = eval(monto_iva_411) / eval(factor_reversion);
monto_iva_411    = eval(monto_iva_411) - eval(base);
monto_iva_var    = eval(monto_iva_var) +  eval(0);
monto_iva_var    = redondear(monto_iva_var,2);

//if(eval(monto_iva_411)!=0){
  document.getElementById("monto_iva").value = monto_iva_var;
//}
...
//Retención fiel cumplimiento
		...
		retener_multa_y_responsabilidad = eval(monto_laboral) + eval(monto_fiel_cumplimiento);
...
//Amortización anticipo
	...
	if(nuevo_monto==0){
		...
		var str = saldo_anticipo;for(i=0; i<saldo_anticipo.length; i++){str = str.replace('.','');}str   = str.replace(',','.');var saldo_anticipo = str;
		...
		amortizacion_del_anticipo_monto_iva = eval(saldo_anticipo);
			...
			if(document.getElementById('anticipo_con_iva').value=="1"){
				amortizacion_del_anticipo_monto_iva = redondear(eval(monto_a_pagar_con_iva) *  eval(porcentaje_amortizacion_input/100), 2);
			}else{
...
if(document.getElementById('anticipo_con_iva2').value=="1"  && (porce_retencion_laboral_input!=0 || porce_retencion_fiel_cumplimiento_input!=0)){
                  ...
                  A  =  eval(monto_fiel_cumplimiento) + eval(monto_laboral) + eval(monto_iva_var_401);
	            //B  =  eval(monto_opcion_pago222)-eval(A);ORIGINAL - GOB.GUARICO LINEA SIGUIENTE
	              B  =  eval(monto_opcion_pago222);

	if(eval(porcentaje_iva_input) != eval(0)){
		total_retencion  =  redondear(eval(B) / (eval(1) + (eval(porcentaje_iva_input) /eval(100))), 2);
	}else{
		total_retencion  =  redondear(eval(B), 2);
	}

	   monto_iva_var = eval(B) - eval(total_retencion);
}else{
                  A  =  eval(monto_fiel_cumplimiento) + eval(monto_laboral) + eval(monto_iva_var_401);
	            //B  =  eval(monto_opcion_pago222)-eval(A);ORIGINAL - GOB.GUARICO LINEA SIGUIENTE
	              B  =  eval(monto_opcion_pago222);
...
var_cuenta_aux= eval(monto_iva_var) + eval(total_retencion) + eval(monto_fiel_cumplimiento) + eval(monto_laboral);
...
if(nuevo_monto=="0" && (porce_retencion_laboral_input==0 && porce_retencion_fiel_cumplimiento_input==0)){
 monto_iva_var = monto_iva_var_aux_2;
  ...
  document.getElementById('total_retencion_monto_iva').value = redondear(total_retencion,2);
	...
	iva_aux_ccc2   = redondear(eval(iva_aux_ccc) /  (eval(1) + (eval(porcentaje_iva_input) /eval(100))), 2);
...
//Responsabilidad civil
		...
		document.getElementById('retencion_multa_monto').value = retencion_multa_monto;
...
//Responsabilidad social
		...
		document.getElementById('retencion_responsabilidad_social').value = retencion_responsabilidad_social;
...
//////////////////////////////////REDONDEO I.V.A/////////////////////////////////////////
                ...
                }else if(eval(monto_iva_pago_aux_1)<eval(monto_iva_pago_aux_2)  && eval(monto_iva_var)!=eval(0)){
                         for(ii=0; ii<document.getElementById('cuenta_i').value; ii++){
							if(document.getElementById('partida_iva_'+ii)  && eval(monto_iva_pago_aux_1)!=eval(monto_iva_pago_aux_2)){
                               valor_aux_pago[ii]   = eval(valor_aux_pago[ii])   - eval(0.01);
                               monto_iva_pago_aux_2 = eval(monto_iva_pago_aux_2) - eval(0.01);
...
total_cancelado_aux = 0;
                ...
                }else if(eval(monto_de_la_orden_pago)<eval(total_cancelado_aux)){
                         for(ii=0; ii<document.getElementById('cuenta_i').value; ii++){
							if(!document.getElementById('partida_iva_'+ii)  && eval(monto_de_la_orden_pago)!=eval(total_cancelado_aux)){
                               valor_aux_pago[ii]   = eval(valor_aux_pago[ii])  - eval(0.01);
                               total_cancelado_aux  = eval(total_cancelado_aux) - eval(0.01);
...
//}//fin if
				...
				total_reten= eval(document.getElementById('monto_laboral').value) +  eval(document.getElementById('monto_fiel_cumplimiento').value) +  eval(document.getElementById('amortizacion_del_anticipo_monto_iva').value) +  eval(document.getElementById('retencion_incluye_iva_monto_iva').value) +  eval(document.getElementById('impuesto_sobre_la_renta_monto_iva').value) + eval(document.getElementById('timbre_fiscal_monto_iva').value) + eval(document.getElementById('impuesto_municipal_monto_iva').value)+ eval(document.getElementById('retencion_multa_monto').value) + eval(document.getElementById('retencion_responsabilidad_social').value);
				//alert( eval(reemplazarPC(document.getElementById('monto_laboral').value)) + ' ' +   eval(reemplazarPC(document.getElementById('monto_fiel_cumplimiento').value)) + ' ' +   eval(reemplazarPC(document.getElementById('amortizacion_del_anticipo_monto_iva').value)) + ' ' +   eval(reemplazarPC(document.getElementById('retencion_incluye_iva_monto_iva').value)) + ' ' +   eval(reemplazarPC(document.getElementById('impuesto_sobre_la_renta_monto_iva').value)) + ' ' +  eval(reemplazarPC(document.getElementById('timbre_fiscal_monto_iva').value)) + ' ' +  eval(reemplazarPC(document.getElementById('impuesto_municipal_monto_iva').value)) + ' ' +  eval(reemplazarPC(document.getElementById('retencion_multa_monto').value)) + ' ' + eval(reemplazarPC(document.getElementById('retencion_responsabilidad_social').value)));
...
function cepp02_contratoservicios_valuacion_respuesta_pago_parcial(opcion){
...
if(monto_opcion_pago>total){fun_msj('MONTO DE LA VALUACI&oacute;N ES MAYOR AL SALDO DEL CONTRATO');}else{
total2 = 0;
monto_iva_var = 0;
...
function rendondear_decimal_contratos_3(monto_opcion_pago_input, total2, valor_partidas){
...
function cepp02_contratoservicios_valuacion_valida_consulta(){
