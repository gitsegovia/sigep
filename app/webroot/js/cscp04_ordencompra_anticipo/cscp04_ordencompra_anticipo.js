function cscp04_registro_anticipo_ordencompra_valida(){

if(verifica_cierre_ano_ejecucion('fecha_anticipo')==false){
	fun_msj('LO SIENTO EL A&Ntilde;O DE LA FECHA DEL ANTICIPO NO CORRESPONDE AL A&Ntilde;O DE EJECUCI&Oacute;N');
	return false;
}else{
...
val=0;
       ...
       if(diferenciaFecha(document.getElementById('dia_actual').value, document.getElementById('fecha_anticipo').value)){
			...
			fun_msj('Inserte el concepto del anticipo');
...
function cscp04_registro_anticipo_ordencompra_cargar_monto_id_editar(id_a){
...
function cscp04_registro_anticipo_ordencompra_cargar_monto_id_editar_2(id_a){
monto_anticipo_iva = 0;
fr = document.getElementById("iva").value;
str = fr;for(i=0; i<str.length; i++){str = str.replace('.','');}str = str.replace(',','.');  fr = str;
if(document.getElementById('incluye_iva_1').checked!=false || document.getElementById('incluye_iva_2').checked!=false){
...
bb = 0;
      if($('incluye_iva_1').checked==true){var str  =  document.getElementById("monto_actual").value;
}else if($('incluye_iva_2').checked==true){var str  =  document.getElementById("monto_actual_griya").value;
}
for(i=0; i<str.length; i++){
    str = str.replace('.','');
}//fin for
...
function monto_anticipo(op, pa, fr){
...
if(op==1){
				...
				/////////// DIFERENCIA ///////////
				          aux=0;
				          aux = redondear(eval(monto_anticipo_input2) - eval(total), 2);
				if(aux<0){
				            aux = eval(aux) * eval(-1);
				            a = document.getElementById("anticipo_"+0).value;
							var a = redondear(a,2);
				...
				/////////// DIFERENCIA ///////////
				         ...
				         document.getElementById("monto_anticipo").disabled=false;
				         document.getElementById("iva").disabled=false;
				         monto_actual  =  document.getElementById("monto_actual_griya").value;
				         str = monto_actual;for(i=0; i<str.length; i++){str = str.replace('.','');}str = str.replace(',','.');  monto_actual = str;
				         monto_anticipo_input = (monto_actual  * (pa * 0.01));
				         str = monto_anticipo_input;
				         monto_anticipo_input = redondear(str,2);
...
cscp03_cotizacion_cuerpo_moneda('TOTALINGRESOS', total);
if(eval(total)!=eval(monto_anticipo_input_aux) && $('incluye_iva_1').checked==true){fun_msj('FAVOR REVISE EL PORCENTAJE DEL I.V.A. O EL MONTO DEL CONTRATO'); }
...
function recalcular_monto_anticipo(){
...
total = 0;
     ...
     document.getElementById("monto_anticipo").disabled=false;
     document.getElementById("iva").disabled=false;
                                   if($('incluye_iva_1').checked==true){monto_actual  =  document.getElementById("monto_actual").value;
				             }else if($('incluye_iva_2').checked==true){monto_actual  =  document.getElementById("monto_actual_griya").value;
				             }
        str = monto_actual;for(i=0; i<str.length; i++){str = str.replace('.','');}str = str.replace(',','.');
        monto_actual = redondear(str,2);
        iva_anticipo_input  =  document.getElementById("iva_anticipo").value;
        
        if (iva_anticipo_input > 90)
                    {
                        iva_anticipo_input =90;
                        document.getElementById("iva_anticipo").value='90';
                        recalcular_monto_anticipo();
                    }

        str = iva_anticipo_input;
        for(i=0; i<str.length; i++){str = str.replace('.','');}
        str = str.replace(',','.');
        iva_anticipo_input = redondear(str,2);
         monto_anticipo_input = ( monto_actual  * (iva_anticipo_input * 0.01));
         str = monto_anticipo_input;
         monto_anticipo_input = redondear(str,2);
         document.getElementById("monto_anticipo").value=monto_anticipo_input;
         monto_anticipo_input2 = monto_anticipo_input;
...
for(i=0; i<=document.getElementById("cuenta_i").value; i++){
		 ...
		 var monto_anticipo_iva = redondear(monto_anticipo_iva,2);
//////////////////////////////////////////////////////////////
                   if($('incluye_iva_1').checked==true){str = eval(monto_anticipo_input) + eval(monto_anticipo_iva);
             }else if($('incluye_iva_2').checked==true){str = eval(monto_anticipo_input);
             }
             monto_anticipo_input = redondear(str,2);
             document.getElementById("anticipo_"+i).value=monto_anticipo_input;
 			 total =  eval(total) + eval(monto_anticipo_input);
...
function cscp04_registro_anticipo_ordencompra_valida_consulta(){
...
function validar_concepto_anulacion_anticipo_obras(){
	...
	if(verifica_cierre_ano_ejecucion_msj()==false){
		return false;
	        ...
	        document.getElementById('concepto_anulacion').focus();
			return false;
    }else if(!confirm("Esta Seguro que desea anular este Anticipo")){
         return false;
    }//fin
