...
if ($nvars > 0 && $nvars <= 50) {
	...
	//ImageString($image,1,12, 207,"10%",$colores[1]);
	//ImageLine($image,36,230,45,221,$colores[2]);    // diagonal
	//ImageLine($image,46,221,$ancho - 32,149,$colores[3]);

	ImageLine($image,46,221,$ancho - 32,221,$colores[3]);//linea final
...
function Formato2($monto){
	...
	for ($semestre = 0; $semestre < $nvars; $semestre++) {
		...
		$porcentaje_aux_1 = (($valores[$semestre]))/max($valores);
		$aux_por = (100 * $porcentaje_aux_1);

		if($aux_por=100 && $semestre==($nvars-1)){$contador_grafica++;}


				if($aux_por!=100 || $contador_grafica==0){
								$puntos = array();
								...
								$por_real = (100 * $porcentaje) /180;
								ImageString($image,1,$equis1 + 2, 51 + (180 - $porcentaje),Formato2($por_real).'%',$colores[4]);
								...
								$equis1 += $ladoy;
								$equis2 += $ladoy;
								if($aux_por==100){$contador_grafica++;}

				}else{ $contador_grafica++;
...
ImagePNG($image);
$username = $usr;
//rmdir('/tmp/barra_tipo_gasto_'.$username.'_*.png');
//unlink('/tmp/pastel_tipo_gasto_'.$username.'_*.png');
$comm = "rm /var/www/sisap/app/tmp/*$username*.png";
//$output = shell_exec($comm);
$usernameCM=strtolower($username);
$comm = "rm /var/www/sisap/app/tmp/*$usernameCM*.png";
//$output = shell_exec($comm);
ImagePNG($image, '/var/www/sisap/app/tmp/barra_tipo_gasto_'.$username.'_'.$rdm.'.png');
//ImageJPEG($image);
