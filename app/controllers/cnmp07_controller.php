<?php
 ...
 class Cnmp07Controller extends AppController{

	var $name = 'cnmp07';
	var $uses = array('cnmd07_transacciones_actuales', 'cnmd07_transacciones_prenomina', 'cnmd06_fichas', 'Cnmd01', 'cnmd06_datos_personales', 'cnmd03_transacciones','cnmd09_frecuencia',
...
$tipo_nomina=   $this->Session->read('tipo_nomina');
	    ...
	    if($var3==null){
						...
						$var_like = $var2;
						$sql_like = $this->SQLCA(). " and (".$this->sql_like_super_busqueda($var_like, $this->Session->read('pista_opcion')).") and condicion_actividad_ficha = 1 and cod_tipo_nomina   =  '".$tipo_nomina."'    ";
...
if($pag_num==null){$pag_num = 1;}
	...
	if($var != null){
      ...
      $cont_cargo    =   $this->cnmd06_fichas->findCount($condicion.' and cod_tipo_nomina='.$var.' and condicion_actividad = 1 and cedula_identidad='.$var1.'  '.' and cod_cargo='.$var2);
      $fichas        =   $this->cnmd06_fichas->findAll($condicion.' and cod_tipo_nomina='.$var.' and condicion_actividad = 1 and cedula_identidad='.$var1.'  '.' and cod_cargo='.$var2);
      $fichas_aux    =   $fichas;
...
if($cod_cargo[$pag_num]!=""){
	 ...
	 $fichas        =   $this->cnmd06_fichas->findAll($condicion.' and condicion_actividad = 1 and cod_tipo_nomina='.$var.' and cod_cargo='.$cod_cargo[$pag_num].' and cod_ficha='.$cod_ficha[$pag_num].' and cedula_identidad='.$var1.'  ');
...
$this->set('periodo_hasta',$datos_cnmd01[0]['Cnmd01']['periodo_hasta']);
   ...
   $this->set('aceptacion', $acepta);
    ...
    if($Lista == ""){
        ...
        $this->concatena_cuatro_digitos($Lista, 'lista_transacciones');
    }//fin else
...
function observar2transaccion($var2=null, $var3=null, $var4=null, $var1=null, $pista=null){
	$this->layout = "ajax";
	echo "<script>document.getElementById('denominacion_transaccion').value='';</script>";

	if($var1 != null && $var2 != null && $pista != null){
		$pista = strtoupper($pista);

    $Lista = '';
    $Lista = $this->cnmd03_transacciones->generateList("cod_tipo_transaccion=$var1 and (cod_transaccion::text LIKE '%$pista%' or (UPPER(denominacion) LIKE '%$pista%'))", 'cod_transaccion ASC', null, '{n}.cnmd03_transacciones.cod_transaccion', '{n}.cnmd03_transacciones.denominacion');
...
function cod_ficha($var2=null, $var3=null, $var4=null,  $var1=null){
    ...
    if($Lista == ""){
     ...
     echo'<script>';
     		echo'document.getElementById("observa_transaccion").value = ""; ';
			echo'document.getElementById("denominacion_transaccion").value = ""; ';
...
if($var2!=null && $var1!=null){
	...
	$var =   $this->Session->read('tipo_nomina');
							                    ...
							                    if($this->cnmd06_fichas->findCount($condicion.' and condicion_actividad = 1 and cod_tipo_nomina='.$var.' and cedula_identidad='.$var1.'  '.' and cod_cargo='.$var2)!=0){
								  $fichas        =   $this->cnmd06_fichas->findAll($condicion.' and condicion_actividad = 1 and cod_tipo_nomina='.$var.' and cedula_identidad='.$var1.'  '.' and cod_cargo='.$var2);
							      $fichas_aux    =   $fichas;
							      $i = 0;
							      foreach($fichas_aux as $aux){ $i++;
							             $cod_cargo[$i] =  $aux['cnmd06_fichas']['cod_cargo'];
...
if ($status_actual==1){
$sql = "INSERT INTO cnmd07_transacciones_prenomina (cod_presi, cod_entidad, cod_tipo_inst, cod_inst, cod_dep, cod_tipo_nomina, cod_cargo, cod_ficha, cod_tipo_transaccion, cod_transaccion, fecha_transaccion, monto_original, numero_cuotas_descontar, numero_cuotas_cancelar, numero_cuotas_canceladas, monto_cuota, saldo, marca_fin_descuento, fecha_proceso, username )";
$sql.="VALUES ('".$cod_presi."', '".$cod_entidad."', '".$cod_tipo_inst."', '".$cod_inst."', '".$cod_dep."', '".$cod_tipo_nomina."', '".$cod_cargo."', '".$cod_ficha."', '".$cod_tipo_transaccion."', '".$cod_transaccion."', '".$this->Cfecha($fecha_transaccion, 'A-M-D')."', '".$monto_original."', '".$numero_original_cuotas."', '".$numero_cuotas_cancelar."', '".$numero_cuotas_canceladas."', '".$monto_cuota."', '".$saldo."', '".$marca_fin_descuento."', '".$this->Cfecha($fecha_proceso, 'A-M-D')."', '".$username."' )";
$this->cnmd07_transacciones_prenomina->execute($sql);
}
...
function consulta($pagina=null,$tipo=1,$ccod_tipo_nomina=null){
    ...
    $result_cnmd07 = $this->v_cnmd07_transacciones_actuales_frecuencias2->findAll($condicion.$ccod_tipo_nomina, 'DISTINCT cod_tipo_nomina, cod_cargo, cod_ficha ',  'cod_tipo_nomina, cod_cargo, cod_ficha ASC',1,$pagina, null);
...
if($Tfilas!=0){
 ...
 $this->set('ultimo',$Tfilas);
	...
	$fichas        =   $this->cnmd06_fichas->findAll($condicion.' and condicion_actividad = 1 and cod_tipo_nomina='.$cod_tipo_nomina.' and cod_cargo = '.$cod_cargo.' and cod_ficha = '.$cod_ficha.'   ');
...
$fichas        =   $this->cnmd06_fichas->findAll($condicion.' and condicion_actividad = 1 and cod_tipo_nomina='.$numero[$pag_num]['cod_tipo_nomina'].' and cod_cargo = '.$numero[$pag_num]['cod_cargo'].' and cod_ficha = '.$numero[$pag_num]['cod_ficha'].'   ');
$fichas_aux    =   $fichas;
$iiii = 0;
