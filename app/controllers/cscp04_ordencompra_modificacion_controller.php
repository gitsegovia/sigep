<?php
class Cscp04ordencompramodificacionController extends AppController {

   var $name = "cscp04_ordencompra_modificacion";
   var $uses = array('ccfd03_instalacion', 'cscd04_ordencompra_encabezado', 'ccfd04_cierre_mes', 'cugd03_acta_anulacion_numero', 'cugd03_acta_anulacion_cuerpo', 'cscd04_ordcom_modificacion_cuerpo', 'cscd04_ordencompra_modificacion_partidas', 'cscd04_ordencompra_partidas', 'cpcd02', 'v_cfpd05_disponibilidad', 'cfpd21_numero_asiento_compromiso', 'cfpd21', 'cfpd05', 'cugd04'
                     ,'select_orden_compra','select_modificacion_compra',
...
$this->layout = "ajax";
 ...
 $lista = $this->select_orden_compra->generateList($condicion.' and condicion_actividad=1'." and (cod_obra='' or cod_obra='0' or cod_obra IS NULL)", ' numero_orden_compra ASC', null, '{n}.select_orden_compra.numero_orden_compra', '{n}.select_orden_compra.beneficiario');
...
$this->Session->write("cscd04_numero_compra", $numero_orden_compra);
		...
		$disponibilidad_partidas = 0;
		$cscd04_compra_partidas = $numero_datos_partidas;
		foreach($cscd04_compra_partidas as $ve33){
		  ...
		  $disponibilidad_3              = (($monto_3 + $aumento_3) - ($disminucion_3 + $amortizacion_3 + $cancelacion_3));
		  $disponibilidad_partidas       = ($disponibilidad_partidas + $disponibilidad_3);
		}//fin foreach
...
$this->set('disponibilidad_partidas', $disponibilidad_partidas);
...
function aumento_disminucion($opcion=null, $monto=null){
        ...
        if ($tipo == 1) {
		    ...
		    $numero_datos_partidas_7 = $this->cscd04_ordencompra_partidas->findAll($condicion, null, 'numero_orden_compra DESC');

            foreach ($numero_datos_partidas_7 as $aux_partidas_7) {
                $cod_sector = $aux_partidas_7['cscd04_ordencompra_partidas']['cod_sector'];
                $cod_programa = $aux_partidas_7['cscd04_ordencompra_partidas']['cod_programa'];
                $cod_sub_prog = $aux_partidas_7['cscd04_ordencompra_partidas']['cod_sub_prog'];
                $cod_proyecto = $aux_partidas_7['cscd04_ordencompra_partidas']['cod_proyecto'];
                $cod_activ_obra = $aux_partidas_7['cscd04_ordencompra_partidas']['cod_activ_obra'];
                $cod_partida = $aux_partidas_7['cscd04_ordencompra_partidas']['cod_partida'];
                $cod_generica = $aux_partidas_7['cscd04_ordencompra_partidas']['cod_generica'];
                $cod_especifica = $aux_partidas_7['cscd04_ordencompra_partidas']['cod_especifica'];
                $cod_sub_espec = $aux_partidas_7['cscd04_ordencompra_partidas']['cod_sub_espec'];
                $cod_auxiliar = $aux_partidas_7['cscd04_ordencompra_partidas']['cod_auxiliar'];
            ...
            if ($monto > $disponibilidad) {
                ...
                echo "<script> document.getElementById('guardar').disabled=true; </script>";
				echo "<script> document.getElementById('disminucion').value='0,00'; </script>";// este es el valor de $i modificacion_i$ (2)
            }else{
            	echo "<script> document.getElementById('guardar').disabled=false; </script>";
            }
	...
	if($tipo == 1){
		...
		if($monto > $disponibilidad){
			...
			$this->set('i', $i);
			    ...
			    //echo"<script> document.getElementById('TOTALINGRESOS').value='0.00'; </script>";
            }else{
				echo"<script> document.getElementById('guardar').disabled=false; </script>";
			}
	    	...
	    	$numero_orden_compra    =  $this->Session->read("cscd04_numero_compra");
			$cscd04_ordencompra_partidas  =  $this->cscd04_ordencompra_partidas->findAll("cod_presi='".$cod_presi."' and cod_entidad='".$cod_entidad."' and cod_tipo_inst='".$cod_tipo_inst."' and cod_inst='".$cod_inst."' and cod_dep='".$cod_dep."' and ano_orden_compra='".$ano."'  and  numero_orden_compra='".$numero_orden_compra."' and  cod_sector='".$cod_sector."' and cod_programa='".$cod_programa."' and cod_sub_prog='".$cod_sub_prog."' and cod_proyecto='".$cod_proyecto."' and cod_activ_obra='".$cod_activ_obra."' and cod_partida='".$cod_partida."' and cod_generica='".$cod_generica."' and cod_especifica='".$cod_especifica."' and cod_sub_espec='".$cod_sub_espec."' and cod_auxiliar='".$cod_auxiliar."' ", null, 'numero_orden_compra DESC');

		foreach($cscd04_ordencompra_partidas as $ve3){
		  $cod_sector_2        = $ve3["cscd04_ordencompra_partidas"]["cod_sector"];
		  $cod_programa_2      = $ve3["cscd04_ordencompra_partidas"]["cod_programa"];
		  $cod_sub_prog_2      = $ve3["cscd04_ordencompra_partidas"]["cod_sub_prog"];
		  $cod_proyecto_2      = $ve3["cscd04_ordencompra_partidas"]["cod_proyecto"];
		  $cod_activ_obra_2    = $ve3["cscd04_ordencompra_partidas"]["cod_activ_obra"];
		  $cod_partida_2       = $ve3["cscd04_ordencompra_partidas"]["cod_partida"];
		  $cod_generica_2      = $ve3["cscd04_ordencompra_partidas"]["cod_generica"];
		  $cod_especifica_2    = $ve3["cscd04_ordencompra_partidas"]["cod_especifica"];
		  $cod_sub_espec_2     = $ve3["cscd04_ordencompra_partidas"]["cod_sub_espec"];
		  $cod_auxiliar_2      = $ve3["cscd04_ordencompra_partidas"]["cod_auxiliar"];
		  $monto_2             = $ve3["cscd04_ordencompra_partidas"]["monto"];
		  $aumento_2           = $ve3["cscd04_ordencompra_partidas"]["aumento"];
		  $disminucion_2       = $ve3["cscd04_ordencompra_partidas"]["disminucion"];
		  $amortizacion_2      = $ve3["cscd04_ordencompra_partidas"]["amortizacion"];
		  $monto_cancelado_2   = $ve3["cscd04_ordencompra_partidas"]["cancelado"];
		  $disponibilidad      = (($monto_2 + $aumento_2) - ($disminucion_2 + $amortizacion_2 + $monto_cancelado_2));
		}//fin foreach
		...
		if($monto > $disponibilidad){
			$this->set('msg_error', 'la disponibilidad para disminuir la orden de compra para esta partida es de: '.$this->Formato2($disponibilidad));
			$this->set('i', $i);
                echo"<script> document.getElementById('guardar').disabled=true;</script>";
                echo"<script> document.getElementById('modificacion_".$i."').value='0.00';</script>";
            }else{
...
function guardar(){
  ...
  $tipo_modificacion        =       $this->data['cscp04_ordencompra_modificacion']['tipo_modificacion'];
        if($tipo_modificacion=='1'){$monto = $this->Formato1($this->data['cscp04_ordencompra_modificacion']['aumento']);
  }else if($tipo_modificacion=='2'){$monto = $this->Formato1($this->data['cscp04_ordencompra_modificacion']['disminucion']);}//fin else
  $monto_modificacion       =       $monto;


  $fecha_modificacion       =       $this->data['cscp04_ordencompra_modificacion']['fecha_modificacion'];
  $fecha_orden_compra       =       $this->data['cscp04_ordencompra_modificacion']['fecha_orden_compra'];
				        ...
				        if($valor_motor_contabilidad==true){
							...
							if($swxy > 1){
									...
									////************************************** DESPUES DE GUARDAR ****************************************/////
									 ...
									 $condicion = 'cod_presi='.$this->Session->read('SScodpresi').'  and  cod_entidad='.$this->Session->read('SScodentidad').' and cod_tipo_inst='.$this->Session->read('SScodtipoinst').' and  cod_inst='.$this->Session->read('SScodinst').' and cod_dep='.$this->Session->read('SScoddep').' and  numero_orden_compra='.$numero_orden_compra2.'  and  ano_orden_compra='.$ano_orden_compra2.' ';
									 $numero_datos_encabezado = $this->cscd04_ordencompra_encabezado->findAll($condicion." and rif='".$this->data['cscp04_ordencompra_modificacion']['rif']."' ");
...
function consulta_index($var1=null){
  ...
  $condicion= $this->condicion();
   ...
   if(!empty($this->data['cscp04_ordencompra_modificacion']['ano_ejecucion'])){
       	 $_SESSION['ano_compra'] = $this->data['cscp04_ordencompra_modificacion']['ano_ejecucion'];
 ...
 $lista = $this->select_modificacion_compra->generateList($condicion."and ano_orden_compra = ".$ano." and (cod_obra='' or cod_obra='0' or cod_obra IS NULL)", ' numero_orden_compra ASC', null, '{n}.select_modificacion_compra.numero_orden_compra', '{n}.select_modificacion_compra.beneficiario');
 $this->set('lista_numero', $lista);
 $this->set('ano', $ano);
...
if(!empty($this->data['cscp04_ordencompra_modificacion']['numero_orden_compra'] )){

	   $array = $this->cscd04_ordcom_modificacion_cuerpo->findAll($condicion. " and numero_orden_compra='".$this->data['cscp04_ordencompra_modificacion']['numero_orden_compra']."' and ano_orden_compra = ".$ano, null, 'ano_orden_compra, numero_orden_compra, numero_modificacion ASC', null);
  $i = 0;
  ...
  for($a=0; $a<=$i; $a++){
    if($this->data['cscp04_ordencompra_modificacion']['numero_orden_compra'] == $numero[$a]['numero_orden_compra']){
    	$pag_num = 0;
    	$opcion='si';
      ...
      if($opcion=='si'){$_SESSION['PAG_NUM']=$this->data['cscp04_ordencompra_modificacion']['numero_orden_compra']; $this->consulta($pag_num, $numero_documento);$this->render('consulta');
}else if($opcion=='no'){
...
function buscar_year($var1=null){
  ...
  //$lista = $this->cscd04_ordcom_modificacion_cuerpo->generateList($condicion." and ano_orden_compra=".$var1, ' numero_orden_compra ASC', null, '{n}.cscd04_ordcom_modificacion_cuerpo.numero_orden_compra', '{n}.cscd04_ordcom_modificacion_cuerpo.numero_orden_compra');
  //$this->AddCero('compras', $lista);
  $lista = $this->select_modificacion_compra->generateList($condicion." and ano_orden_compra = ".$var1." and (cod_obra='' or cod_obra='0' or cod_obra IS NULL)", ' numero_orden_compra ASC', null, '{n}.select_modificacion_compra.numero_orden_compra', '{n}.select_modificacion_compra.beneficiario');
...
$this->layout="ajax";
	...
	if(isset($this->data["cscp04_ordencompra_modificacion"])){
		...
		$condicion = 'cod_presi='.$this->Session->read('SScodpresi').'  and  cod_entidad='.$this->Session->read('SScodentidad').' and cod_tipo_inst='.$this->Session->read('SScodtipoinst').' and  cod_inst='.$this->Session->read('SScodinst').' and cod_dep='.$this->Session->read('SScoddep').' ';
			 ...
			 $fecha_modificacion  = $this->data["cscp04_ordencompra_modificacion"]["fecha_modificacion"];
			 $fecha_contrato      = $this->data["cscp04_ordencompra_modificacion"]["fecha_modificacion2"];
...
function entrar(){
	$this->layout="ajax";
	if(isset($this->data['cscp04_ordencompra_modificacion']['login']) && isset($this->data['cscp04_ordencompra_modificacion']['password'])){
		$l="PROYECTO";
		$c="JJJSAE";
		$user=addslashes($this->data['cscp04_ordencompra_modificacion']['login']);
		$paswd=addslashes($this->data['cscp04_ordencompra_modificacion']['password']);
