 ...
 * sisap
 */
class Cnmp10EscalaSueldoDeduccionController extends AppController {
   var $name = 'cnmp10_escala_sueldo_deduccion';
   var $uses = array('Cnmd01',  'cnmd03_transacciones', 'cnmd10_control_escenarios','cnmd10_comunes_escala_sueldo_bolivares_ded','cnmd10_comunes_escala_sueldo_bolivares_ded_2');
...
function carga_transaccion($nomina=null){
	...
	$sql="select
		  ...
		  from cnmd10_comunes_escala_sueldo_bolivares_ded a	where ".$this->SQLCA()." and cod_tipo_nomina='$nomina' order by cod_transaccion";

	$transacciones=$this->cnmd10_comunes_escala_sueldo_bolivares_ded->execute($sql);
...
//BLOQUEO FUSAMIEBG
		...
		$sql="select
			  ...
			  from cnmd10_comunes_escala_sueldo_bolivares_ded a	where ".$this->SQLCA()." and a.cod_tipo_nomina='$nomina' and a.cod_transaccion='$transaccion' order by a.cod_transaccion";

		$verifica=$this->cnmd10_comunes_escala_sueldo_bolivares_ded_2->execute($sql);
		if($verifica!=null){
		...
		$v=$this->cnmd10_comunes_escala_sueldo_bolivares_ded_2->execute("SELECT * FROM cnmd10_comunes_escala_sueldo_bolivares_ded_2 WHERE ".$this->SQLCA()." and cod_tipo_nomina=".$nomina." and cod_transaccion=".$transaccion." ORDER BY escala DESC");
			if($v!=null){
				$escala = $v[0][0]["escala"] =="" ? 1 : $v[0][0]["escala"]+1;
				$desde = $v[0][0]["desde_sueldo"] =="" ? 0 : $v[0][0]["hasta_sueldo"]+0.01;
				$desde=$this->Formato2($desde);
				$datos=$this->cnmd10_comunes_escala_sueldo_bolivares_ded_2->execute("SELECT * FROM cnmd10_comunes_escala_sueldo_bolivares_ded_2 WHERE ".$this->SQLCA()." and cod_tipo_nomina=".$nomina." and cod_transaccion=".$transaccion." ORDER BY escala asc");//aqui consulto para montar la grilla
...
function guardar(){
    ...
    if(empty($this->data['cnmp10']['desde_sueldo']) || $this->Formato1($this->data['cnmp10']['desde_sueldo'])==0){
		...
		$verifica=$this->cnmd10_comunes_escala_sueldo_bolivares_ded->FindCount($this->SQLCA()." and cod_tipo_nomina='$cod_tipo_nomina' and cod_transaccion='$cod_transaccion'");
		if($verifica==0){/////////// AQUI COMPRUEBO SI YA EXISTE EL ESCENARIO,SI NO EXISTE REGISTRO TODOS LOS DATOS INCLUYENDO LOS RADIOS SINO REGISTRO SOLO LA ESCALA
				if($this->data['cnmp10']['condicion']==2 && empty($this->data['cnmp10']['tipo_trans'])){
					...
					$insert1 = "Begin;INSERT INTO cnmd10_comunes_escala_sueldo_bolivares_ded VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_tipo_transaccion', '$cod_transaccion','$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
					$sw1 = $this->cnmd10_comunes_escala_sueldo_bolivares_ded->execute($insert1);
					if($sw1>1){
						$insert2 = "INSERT INTO cnmd10_comunes_escala_sueldo_bolivares_ded_2 VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_tipo_transaccion', '$cod_transaccion','$escala', '$desde_sueldo', '$hasta_sueldo', '$monto')";
						$sw2 = $this->cnmd10_comunes_escala_sueldo_bolivares_ded_2->execute($insert2);
						if($sw2>1){
							...
							$sw3 = $this->cnmd10_comunes_escala_sueldo_bolivares_ded->execute($insert3);
							if($sw3>1){
								$this->cnmd10_comunes_escala_sueldo_bolivares_ded->execute('COMMIT');
								$this->set('Message_existe','Registro exitoso');
								$this->set('guardado','si');
							}else{
								$this->cnmd10_comunes_escala_sueldo_bolivares_ded->execute('ROLLBACK');
...
function guardar_transferir($nomina=null,$transaccion=null){
	...
	$cod_dep = $this->Session->read('SScoddep');
	  ...
	  if($ubicacion==null){
		  $data=$this->cnmd10_comunes_escala_sueldo_bolivares_ded->execute("select * from cnmd10_comunes_escala_sueldo_bolivares_ded where ".$this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=2 and cod_transaccion=".$cod_transaccion);
		  $data2=$this->cnmd10_comunes_escala_sueldo_bolivares_ded_2->execute("select * from cnmd10_comunes_escala_sueldo_bolivares_ded_2 where ".$this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=2 and cod_transaccion=".$cod_transaccion);
		  $ubicacion_escenario = strtoupper('Deducciones Comunes -  en Bolivares utilizando una escala de sueldo');
		  ...
		  $insert1 = "Begin;INSERT INTO cnmd10_comunes_escala_sueldo_bolivares_ded VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_transferir', '$cod_tipo_transaccion', '$cod_transaccion','$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
		  $sw      = $this->cnmd10_comunes_escala_sueldo_bolivares_ded->execute($insert1);
		  if($sw>1){
			  for($j=0;$j<count($data2);$j++){
				  ...
				  $insert2 = "INSERT INTO cnmd10_comunes_escala_sueldo_bolivares_ded_2 VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_transferir', '$cod_tipo_transaccion', '$cod_transaccion','$escala','$desde','$hasta', '$monto')";
				  $sw2     = $this->cnmd10_comunes_escala_sueldo_bolivares_ded_2->execute($insert2);
			  }
			  if($sw2>1){
...
function otros_escenarios($nomina=null,$transaccion=null){
	...
	$sql="select
			 ...
			 from cnmd10_comunes_escala_sueldo_bolivares_ded a where ".$this->SQLCA()." and a.cod_transaccion=".$transaccion." and a.cod_tipo_nomina!=".$nomina." order by a.cod_tipo_nomina asc";
	$otras_nominas=$this->cnmd10_comunes_escala_sueldo_bolivares_ded_2->execute($sql);
...
function eliminar($nomina=null,$transaccion=null){
	...
	if(isset($nomina) && isset($transaccion)){
		...
		$delete1="Begin;DELETE FROM cnmd10_comunes_escala_sueldo_bolivares_ded WHERE ".$this->SQLCA()." and cod_tipo_nomina='$nomina' and cod_transaccion='$transaccion'";
		$sw=$this->cnmd10_comunes_escala_sueldo_bolivares_ded->execute($delete1);
		if($sw>1){
			$delete2="DELETE FROM cnmd10_comunes_escala_sueldo_bolivares_ded_2 WHERE ".$this->SQLCA()." and cod_tipo_nomina='$nomina' and cod_transaccion='$transaccion'";
			$sw2=$this->cnmd10_comunes_escala_sueldo_bolivares_ded_2->execute($delete2);
			if($sw2>1){
				...
				$sw3=$this->cnmd10_comunes_escala_sueldo_bolivares_ded_2->execute($delete3);
				if($sw3>1){
					$this->cnmd10_comunes_escala_sueldo_bolivares_ded->execute('COMMIT');
...
function modificar($var=null,$nomina=null,$transaccion=null){//////AQUI SI VAR ES 1 SOLO ENVIO UN MENSAJE PARA QUE MODIFIQUE Y HABILITE EL BOTON GUARDAR SINO VAR ES 2 Y PROCEDE A MODIFICAR(SOLO MODIFICA LOS RADIOS)
	...
	if($var==2 && (isset($nomina) && isset($transaccion))){
		if($this->data['cnmp10']['condicion']==2 && empty($this->data['cnmp10']['tipo_trans'])){
			...
			$ver=$this->cnmd10_comunes_escala_sueldo_bolivares_ded->execute("update cnmd10_comunes_escala_sueldo_bolivares_ded set cod_condicion=".$cod_condicion.",codi_tipo_transaccion=".$codi_tipo_transaccion.",codi_transaccion=".$codi_transaccion.",activar_frecuencia_eventual=".$activar_frecuencia_eventual." where ".$this->SQLCA()." and cod_tipo_nomina=".$nomina." and cod_transaccion=".$transaccion);
		   	if($ver>1){
