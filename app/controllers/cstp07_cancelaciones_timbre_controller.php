...
class Cstp07CancelacionesTimbreController extends AppController{



    var $name = "cstp07_cancelaciones_timbre";
 	var $uses = array('v_cstd01_bancos','v_cstd01_sucursales', 'cstd03_cheque_cuerpo', 'cstd03_cheque_partidas', 'cstd07_retenciones_cuerpo_iva', 'cstd07_retenciones_partidas_iva',
                      'cstd07_retenciones_cuerpo_timbre', 'cstd07_retenciones_partidas_timbre', 'cstd07_retenciones_cuerpo_timbre',
                      'cstd07_retenciones_partidas_timbre', 'cstd07_retenciones_cuerpo_timbre', 'cstd07_retenciones_partidas_timbre',
                      ...
                      'cstd06_comprobante_cuerpo_egreso', 'cstd06_comprobante_cuerpo_timbre', 'cstd06_comprobante_cuerpo_iva',
                      'cstd06_comprobante_cuerpo_timbre', 'cstd06_comprobante_cuerpo_timbre', 'cugd03_acta_anulacion_cuerpo',
                      'cstd06_comprobante_numero_egreso', 'cstd06_comprobante_numero_timbre', 'cstd06_comprobante_numero_iva', 'cstd03_beneficiario_retencion_timbre',
                      'cstd06_comprobante_numero_timbre', 'cstd06_comprobante_numero_timbre','cstd04_movimientos_generales',
                      'cstd06_comprobante_poremitir_egreso', 'cstd06_comprobante_poremitir_timbre', 'cstd06_comprobante_poremitir_iva',
                      'cstd06_comprobante_poremitir_timbre', 'cstd06_comprobante_poremitir_timbre','ccfd03_instalacion', 'cfpd23_numero_asiento_pagado',
                      'cfpd05', 'cugd04', 'cfpd23', 'cugd02_dependencia','cugd02_institucion',
...
if($ano != ''){
	...
	if(isset($var) && $var!=""){
			...
			$cond2   = $this->SQLCA()." and cod_entidad_bancaria=".$cod_1." and cod_sucursal=".$cod_2." and cuenta_bancaria='".$var."' and clase_beneficiario=3";
  ...
  $numero = "";
	...
	if(isset($var) && $var!=""){
     ...
     //$accion =  $this->cstd03_cheque_numero->findAll($cond, null, 'numero_cheque ASC', 1, 1, null);
       ...
       /*$var2[0]['numero_cheque'] = "";

       foreach($accion as $aux){
            $var2[$i]['numero_cheque'] = $aux['cstd03_cheque_numero']['numero_cheque'];
            $i++;
       }$i--;

       $numero =  $var2[0]['numero_cheque'];*/
...
//    cstd03_cheque_cuerpo
  ...
  $ano_movimiento                         =         $this->data['cstp07_cancelaciones_timbre']['ano'];
  $ann = $ano_movimiento;
  $cod_entidad_bancaria                   =         $this->data['cstp07_cancelaciones_timbre']['entidad'];
  $cod_sucursal                           =         $this->data['cstp07_cancelaciones_timbre']['sucursal'];
  $cuenta_bancaria                        =         $this->data['cstp07_cancelaciones_timbre']['cuenta'];
  $numero_cheque                          =         $this->data['cstp07_cancelaciones_timbre']['numero_cheque'];
  $fecha_cheque                           =         $this->Cfecha($this->data['cstp07_cancelaciones_timbre']['fecha'], 'A-M-D');
  $fd                                     =         $this->data['cstp07_cancelaciones_timbre']['fecha'];
  $concepto                               =         $this->data['cstp07_cancelaciones_timbre']['concepto'];
  //$status_cheque                          =         $this->data['cstp07_cancelaciones_timbre']['status_cheque'];
  //$clase_beneficiario                     =         $this->data['cstp07_cancelaciones_timbre']['clase_beneficiario'];

  $status_cheque                          =         1;
  $clase_beneficiario                     =         3;
  ...
  $ejercicio_anterior                     =     2;
	...
	$cinstitucion = $this->cugd02_institucion->findAll('cod_tipo_institucion='.$cod_tipo_inst.' and cod_institucion='.$cod_inst, 'rif');
	if(!empty($cinstitucion)){
		$rif_dinstitucion = str_replace("-", "", $cinstitucion[0]['cugd02_institucion']['rif']);
		$rif_dinstitucion = strtoupper($rif_dinstitucion);
	}else{
		$rif_dinstitucion = "";
	}
...
if($this->cstd03_cheque_cuerpo->execute($sql_cstd03_cheque_cuerpo)>=1){}else{$opcion = 'no';}
...
FROM
         ...
         cstd07_retenciones_cuerpo_timbre a, cstd07_retenciones_partidas_timbre b,  cepd03_ordenpago_cuerpo c
...
if(isset($this->data['cstp07_cancelaciones_timbre']['ano_orden_pago_'.$var_aux])){

									if($ve2[0]['ano_orden_pago']    == $this->data['cstp07_cancelaciones_timbre']['ano_orden_pago_'.$var_aux] &&
									   $ve2[0]['numero_orden_pago'] == $this->data['cstp07_cancelaciones_timbre']['numero_orden_pago_'.$var_aux]
									){
									 ...
									 $var_aux++;
																		...
																		$sql_cstd03_cheque_ordenes.= "VALUES ('".$cod_presi."', '".$cod_entidad."', '".$cod_tipo_inst."', '".$cod_inst."', '".$cod_dep."', '".$ve2[0]['clase_orden']."', '".$ve2[0]['ano_orden_pago']."', '".$ve2[0]['numero_orden_pago']."', '".$ano_movimiento."', '".$cod_entidad_bancaria."', '".$cod_sucursal."', '".$cuenta_bancaria."', '".$numero_cheque."');";

																		if($this->cstd03_cheque_ordenes->execute($sql_cstd03_cheque_ordenes)>=1){}else{$opcion = 'no';}
																		...
																		$sql_cstd06_comprobante_poremitir_egreso.= "VALUES ('".$cod_presi."', '".$cod_entidad."', '".$cod_tipo_inst."', '".$cod_inst."', '".$cod_dep."', '".$username."', '".$ano_comprobante_egreso."', '".$numero_comprobante_egreso."', '".$ve2[0]['ano_orden_pago']."', '".$ve2[0]['clase_orden']."', '".$ve2[0]['numero_orden_pago']."','15'); ";
									...
									ORDER BY
									           ...
									           cod_auxiliar ASC; ");
																		...
																		$datos_orden_pago_partidas = $this->cstd07_retenciones_partidas_timbre->findAll($condicion. ' and  ano_orden_pago='.$ve2[0]['ano_orden_pago'].'  and  numero_orden_pago='.$ve2[0]['numero_orden_pago'], null, 'ano_orden_pago, numero_orden_pago, cod_sector, cod_programa, cod_sub_prog, cod_proyecto, cod_activ_obra, cod_partida, cod_generica, cod_especifica, cod_sub_espec, cod_auxiliar ASC');
																		...
																		$concate = $this->AddCeroR2(substr($ve['cstd07_retenciones_partidas_timbre']['cod_partida'], -2) , substr($ve['cstd07_retenciones_partidas_timbre']['cod_partida'], 0, 1 ) ).'.'.$this->AddCeroR2($ve['cstd07_retenciones_partidas_timbre']['cod_generica']).'.'.$this->AddCeroR2($ve['cstd07_retenciones_partidas_timbre']['cod_especifica']).'.'.$this->AddCeroR2($ve['cstd07_retenciones_partidas_timbre']['cod_sub_espec']);


																		  $ano                                 =         $ve['cstd07_retenciones_partidas_timbre']['ano'];
																		  $cod_sector                          =         $ve['cstd07_retenciones_partidas_timbre']['cod_sector'];
																		  $cod_programa                        =         $ve['cstd07_retenciones_partidas_timbre']['cod_programa'];
																		  $cod_sub_prog                        =         $ve['cstd07_retenciones_partidas_timbre']['cod_sub_prog'];
																		  $cod_proyecto                        =         $ve['cstd07_retenciones_partidas_timbre']['cod_proyecto'];
																		  $cod_activ_obra                      =         $ve['cstd07_retenciones_partidas_timbre']['cod_activ_obra'];
																		  $cod_partida                         =         $ve['cstd07_retenciones_partidas_timbre']['cod_partida'];
																		  $cod_generica                        =         $ve['cstd07_retenciones_partidas_timbre']['cod_generica'];
																		  $cod_especifica                      =         $ve['cstd07_retenciones_partidas_timbre']['cod_especifica'];
																		  $cod_sub_espec                       =         $ve['cstd07_retenciones_partidas_timbre']['cod_sub_espec'];
																		  $cod_auxiliar                        =         $ve['cstd07_retenciones_partidas_timbre']['cod_auxiliar'];
																		  $monto                               =         $this->Formato1($this->data['cstp07_cancelaciones_timbre']['monto_'.$jj]);
																		  $numero_control_compromiso           =         $ve['cstd07_retenciones_partidas_timbre']['numero_control_compromiso'];
																		  $numero_control_causado              =         $ve['cstd07_retenciones_partidas_timbre']['numero_control_causado'];
																...
																// *** DATOS PARA CARGAR ARCHIVO EXCEL:

																		foreach($datos_ordenes as $rdo){
																			$datosXml[$contador][0] = $rif_dinstitucion;
																			$periodo = cambiar_formato_fecha($rdo['cepd03_ordenpago_cuerpo']['fecha_orden_pago']);
																			$datosXml[$contador][1] = $periodo."  ".$periodo;
																			$datosXml[$contador][2] = "0125";
																			$datosXml[$contador][3] = strtoupper(str_replace("-", "", $rdo['cepd03_ordenpago_cuerpo']['rif']));
																			$datosXml[$contador][4] = $rdo['cepd03_ordenpago_cuerpo']['numero_comprobante_timbre'];
																			$datosXml[$contador][5] = $this->Formato2($rdo['cepd03_ordenpago_cuerpo']['monto_timbre_fiscal']);
																			$contador++;
																		}

																	// FIN DE DATOS PARA CARGA EXCEL ***
...
if(isset($datosXml) && !empty($datosXml)){

	$_SESSION["DATAXMLEX"]=$datosXml;

// $this->set('datosXml',$datosXml);
...
if($this->cstd03_cheque_numero->execute("UPDATE cstd03_cheque_numero SET situacion=3   WHERE ".$condicion." and  cod_entidad_bancaria = '".$cod_entidad_bancaria."' and   cod_sucursal='".$cod_sucursal."' and  cuenta_bancaria='".$cuenta_bancaria."' and  numero_cheque=".$numero_cheque."; " )>=1){}else{$opcion = 'no';}//fin else
...
if($this->cstd02_cuentas_bancarias->execute("UPDATE cstd02_cuentas_bancarias SET cheque_dia=".$cheque_dia.", cheque_mes=".$cheque_mes.", cheque_ano=".$cheque_ano.", monto_cheque_por_emitir=".$monto_cheque_por_emitir.", disponibilidad_libro=".$disponibilidad_libro."   WHERE cod_presi='".$cod_presi."' and cod_entidad='".$cod_entidad."' and  cod_tipo_inst='".$cod_tipo_inst."' and  cod_inst='".$cod_inst."' and cod_dep='".$cod_dep."' and cod_entidad_bancaria = '".$cod_entidad_bancaria."' and   cod_sucursal='".$cod_sucursal."' and  cuenta_bancaria='".$cuenta_bancaria."'; ")>=1){}else{$opcion = 'no';}//fin else
	...
	}else{$this->cstd03_cheque_cuerpo->execute("ROLLBACK;"); $this->set('errorMessage', 'La datos no fueron almacenados correctamente'); }//fin else



}else{	$this->cstd03_cheque_cuerpo->execute("ROLLBACK;"); $this->set('errorMessage', 'Los datos no pueden ser almacenado verifique el n&uacute;mero de comprobante egreso');  }//fin else
}else{	$this->cstd03_cheque_cuerpo->execute("ROLLBACK;"); $this->set('errorMessage', 'Los datos no pueden ser almacenados');}//fin else
}else{	$this->cstd03_cheque_cuerpo->execute("ROLLBACK;"); $this->set('errorMessage', 'Los datos ya existen'); }//fin else
   ...
   // $this->render('index');
    ...
    //$this->generar_cheque_formato_libre($ano,'2');
    //$this->render('generar_cheque_formato_libre');

    // $this->redirect("/reporte/generar_cheque_formato_libre/");

}//fin guardar



function descargar_formato($contador) {
   $this->layout="ajax";
   $this->set('contador',$contador);
...
if(isset($pag_num)){
			...
			$cond2   = $this->SQLCA()." and cod_entidad_bancaria=".$cod_1." and cod_sucursal=".$cod_2." and cuenta_bancaria='".$cod_3."' and numero_cheque=".$pag_num."";
...
$datos_cheque_ordenes  = $this->cstd03_cheque_ordenes->findAll($condicion. " and  cod_entidad_bancaria='".$numero[0]['cod_entidad_bancaria']."'  and  cod_sucursal='".$numero[0]['cod_sucursal']."' and  cuenta_bancaria='".$numero[0]['cuenta_bancaria']."'  and  ano_movimiento=".$numero[0]['ano_movimiento']."  and  numero_cheque=".$numero[0]['numero_cheque']);
$datos_cheque_partidas = $this->cstd03_cheque_partidas->findAll($condicion. " and  cod_entidad_bancaria='".$numero[0]['cod_entidad_bancaria']."'  and  cod_sucursal='".$numero[0]['cod_sucursal']."' and  cuenta_bancaria='".$numero[0]['cuenta_bancaria']."'  and  ano_movimiento=".$numero[0]['ano_movimiento']."  and  numero_cheque=".$numero[0]['numero_cheque']);
		   ...
		   $cond2   =  $this->SQLCA()."  and ano_movimiento='".$numero[0]['ano_movimiento']."' and cod_entidad_bancaria=".$numero[0]['cod_entidad_bancaria']." and cod_sucursal=".$numero[0]['cod_sucursal']." and cuenta_bancaria='".$numero[0]['cuenta_bancaria']."' and clase_beneficiario=3 ";
...
function guardar_anulacion2($var=null) {
  ...
  $condicion = "cod_presi = ".$cod_presi." and cod_entidad = ".$cod_entidad." and cod_tipo_inst = ".$cod_tipo_inst." and cod_inst = ".$cod_inst." and cod_dep = ".$cod_dep;
   	...
   	if(!empty($this->data["cstp07_cancelaciones_timbre"]["fecha_cheque"]) && !empty($this->data["cstp07_cancelaciones_timbre"]["concepto_anulacion"]) && !empty($this->data["cstp07_cancelaciones_timbre"]["cod_sucursal"]) && !empty($this->data["cstp07_cancelaciones_timbre"]["cuenta_bancaria"]) && !empty($this->data["cstp07_cancelaciones_timbre"]["cod_entidad_bancaria"]) && !empty($this->data["cstp07_cancelaciones_timbre"]["numero_cheque"])){
		...
		$condicion = 'cod_presi='.$this->Session->read('SScodpresi').'  and  cod_entidad='.$this->Session->read('SScodentidad').' and cod_tipo_inst='.$this->Session->read('SScodtipoinst').' and  cod_inst='.$this->Session->read('SScodinst').' and cod_dep='.$this->Session->read('SScoddep').' ';
		      ...
		      $tipo_documento           =  251;
			  $concepto_anulacion       =  $this->data["cstp07_cancelaciones_timbre"]["concepto_anulacion"];
			  $fecha_proceso_anulacion  =  date("d/m/Y");
			  $condicion_documento      =  2;//cuando se guarda es Activo=1
			  $fecha_cheque            =   $this->data["cstp07_cancelaciones_timbre"]["fecha_cheque"];
			  $fd = $fecha_cheque;
			  $cod_sucursal            =   $this->data["cstp07_cancelaciones_timbre"]["cod_sucursal"];

			  $ano_movimiento_canc          =   $this->data["cstp07_cancelaciones_timbre"]["ano_movimiento"];
			  $ano_movimiento          =   $this->ano_ejecucion();

			  $cuenta_bancaria         =   $this->data["cstp07_cancelaciones_timbre"]["cuenta_bancaria"];
			  $numero_cheque           =   $this->data["cstp07_cancelaciones_timbre"]["numero_cheque"];
			  $cod_entidad_bancaria    =   $this->data["cstp07_cancelaciones_timbre"]["cod_entidad_bancaria"];
...
foreach($datos_partidas as $row){
										 	...
										 	$numero_control_compromiso = $row['cstd03_cheque_partidas']['numero_control_compromiso'];
										 	$numero_control_causado = $row['cstd03_cheque_partidas']['numero_control_causado'];
										 	$numero_control_pagado = $row['cstd03_cheque_partidas']['numero_control_pagado'];
										 	$opago = $row['cstd03_cheque_partidas']['numero_orden_pago'];
										 	$cbanco = $row['cstd03_cheque_partidas']['cod_entidad_bancaria'];
							...
							$ano_orden_pago_1     =  $row['cstd03_cheque_partidas']['ano_orden_pago'];
                            $numero_orden_pago_1  =  $row['cstd03_cheque_partidas']['numero_orden_pago'];
...
$monto_cheque    =   $this->data["cstp07_cancelaciones_timbre"]["monto_cheque"];
$beneficiario    =   $this->data["cstp07_cancelaciones_timbre"]["beneficiario"];
$rif_cedula      =   $this->data["cstp07_cancelaciones_timbre"]["rif_cedula"];
...
function agregar_orden_pago_session($var=null){
  ...
  if(!isset($_SESSION['opcion_emitir'])){$_SESSION['opcion_emitir']=1;}
   ...
   $beneficiario = "";
   $beneficiario = $this->cstd03_beneficiario_retencion_timbre->findAll($this->SQLCA());



if(!isset($beneficiario[0]['cstd03_beneficiario_retencion_timbre']['beneficiario'])){
...
ORDER BY
 ...
 if(isset($dato[0][0]['ano_orden_pago'])){
	...
	if(!isset($dato[0][0]['numero_orden_pago'])){
		   ...
		   $this->set('beneficiario',   $beneficiario[0]['cstd03_beneficiario_retencion_timbre']['beneficiario']);
		   $_SESSION['ORDEN_PAGO_TOTAL']['autorizado'] = $beneficiario[0]['cstd03_beneficiario_retencion_timbre']['beneficiario'];
	}//fin else
}//fin else
