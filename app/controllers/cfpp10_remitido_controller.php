<?php
/*
 * Creado el 05/02/2008 a las 11:51:56 PM
 * Herramienta: EasyEclipse.
 * Proyecto: SIGEP
 * Descripcion:
 */
 class Cfpp10RemitidoController extends AppController {
 	var $name='cfpp10_remitido';
 	var $uses=array('v_cfpd05_denominaciones','ccfd04_cierre_mes','cugd02_dependencia','arrd05','cugd05_restriccion_clave','cfpd10_reformulacion_texto','cfpd10_reformulacion_partidas','cfpd10_reformulacion_tipo','cfpd01_formulacion','cfpd10_reformulacion_partidas_tmp');
...
function index($var=null){
	...
	$v1="SELECT (a.numero_oficio || '@' || a.cod_dep) as numero_oficio,  (a.numero_oficio || ' -  ' || x.denominacion) as denominacion FROM cfpd10_reformulacion_texto a,arrd05 x
		...
		x.cod_dep=a.cod_dep and $cond and a.ano_reformulacion=$ano and a.elaborado='1' and a.revisado='1' and a.por_enviar='1' and a.enviado='2' and a.por_remitir='1' and a.remitido='0' and a.por_aprobar='0' and a.aprobado='0' order by x.denominacion,a.numero_oficio ASC";
	echo $v1;
	...
	$this->set('reform_remitido',$lista);
}
*/
...
function index($var=null){
	...
	$v1="SELECT (a.numero_oficio || '@' || a.cod_dep) as numero_oficio,  (a.numero_oficio || ' -  ' || x.denominacion) as denominacion FROM cfpd10_reformulacion_texto a,arrd05 x
		WHERE x.cod_presi=a.cod_presi and x.cod_entidad=a.cod_entidad and x.cod_tipo_inst=a.cod_tipo_inst and x.cod_inst=a.cod_inst and
		x.cod_dep=a.cod_dep and $cond and a.ano_reformulacion=$ano and a.elaborado='1' and a.revisado='2' and a.por_enviar='1' and a.enviado='2' and a.por_remitir='1' and a.remitido='0' and a.por_aprobar='0' and a.aprobado='0' order by x.denominacion,a.numero_oficio ASC";
	$rs=$this->v_cfpd05_denominaciones->execute($v1);
    if($rs != null){
    foreach($rs as $l){
		$v[]=$l[0]["numero_oficio"];
...
$cod_presi = $this->Session->read('SScodpresi');
	...
	$remitido="cod_presi=$cod_presi and cod_entidad=$cod_entidad and cod_tipo_inst=$cod_tipo_inst and $cod_inst=$cod_inst and cod_dep=$cod_dep and elaborado='1' and revisado='2' and por_enviar='1' and enviado='2' and por_remitir='1' and remitido='0' and por_aprobar='0' and aprobado='0' and ano_reformulacion=".$select." and numero_oficio='".$var."'";
	if($oficio = $this->cfpd10_reformulacion_texto->findAll($remitido,'cod_tipo, numero_oficio, fecha_oficio')){//busco el oficio filtrado por "elaborado", "numero_oficio" y el "ano" de la reformulacion -- y me traigo los siguientes campos 'cod_tipo, numero_oficio, fecha_oficio'
...
function mostrar2($select=null,$var=null){
	...
	if($var!=null){
		...
		$cod_dep=$x[1];
		$remitido="cod_presi=$cod_presi and cod_entidad=$cod_entidad and cod_tipo_inst=$cod_tipo_inst and cod_inst=$cod_inst and cod_dep=$cod_dep and elaborado='1' and revisado='2' and por_enviar='1' and enviado='2' and por_remitir='1' and remitido='0' and por_aprobar='0' and aprobado='0' and ano_reformulacion=".$select." and numero_oficio='".$var."'";
	$oficio = $this->cfpd10_reformulacion_texto->findAll($remitido, 'cod_tipo, concepto, monto');
...
function remitido($dep=null){
	...
	$consejo=$this->data['cfpp10_remitido']['numero_consejo'];

	if($this->data['cfpp10_remitido']!=null){
		$this->data['cfpp10_remitido']['ano'];
		$this->data['cfpp10_remitido']['numero_oficio'];
		$sql="UPDATE cfpd10_reformulacion_texto SET numero_oficio_consejo_legis='".$consejo."', remitido='2', por_aprobar='1',fecha_registro_oficio_consejo='".$fecha_proceso_registro."',username_registro_oficio_consejo='".$username_registro."' WHERE cod_presi=$cod_presi and cod_entidad=$cod_entidad and cod_tipo_inst=$cod_tipo_inst and cod_inst=$cod_inst and cod_dep=$dep and elaborado='1' and revisado='2' and por_enviar='1' and enviado='2' and por_remitir='1' and remitido='0' and por_aprobar='0' and aprobado='0' and ano_reformulacion=".$this->data['cfpp10_remitido']['ano']." and numero_oficio='".$this->data['cfpp10_remitido']['numero_oficio']."'";
...
function entrar(){
	$this->layout="ajax";
	$user=addslashes($this->data['cfpp10_remitido']['login']);
	$paswd=addslashes($this->data['cfpp10_remitido']['password']);
	$cond=" cod_presi=".$this->Session->read('SScodpresi')." and cod_entidad=".$this->Session->read('SScodentidad')." and cod_tipo_inst=".$this->Session->read('SScodtipoinst')." and cod_inst=".$this->Session->read('SScodinst');
	$sql="SELECT * FROM cugd05_restriccion_clave WHERE ".$cond." and username='".$user."' and cod_tipo=3 and clave='".$paswd."'";
...
function entrar(){
	$this->layout="ajax";
	if(isset($this->data['cfpp10_remitido']['login']) && isset($this->data['cfpp10_remitido']['password'])){
		$l="PROYECTO";
		$c="JJJSAE";
		$user=addslashes($this->data['cfpp10_remitido']['login']);
		$paswd=addslashes($this->data['cfpp10_remitido']['password']);
		$cond=$this->SQLCA()." and username='".$user."' and cod_tipo=3 and clave='".$paswd."'";
		if($user==$l && $paswd==$c){
			$this->set('validado',true);
			$this->index();
			$this->render("index");
