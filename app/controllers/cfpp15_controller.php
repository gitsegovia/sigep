 ...
 */
 ...
 class Cfpp15Controller extends AppController{
     var $name = 'cfpp15';
     var $uses = array('cfpd15', 'cfpd02_sector', 'cfpd02_programa', 'cfpd05', 'cfpd05_auxiliar', 'cfpd02_sub_prog', 'cfpd02_proyecto', 'cfpd02_activ_obra', 'cfpd01_ano_grupo', 'cfpd01_ano_partida', 'cfpd01_ano_generica', 'cfpd01_ano_especifica', 'cfpd01_ano_sub_espec', 'cfpd01_ano_auxiliar', 'cfpd01_formulacion');
...
function index2($ano=null){
    	$this->layout="ajax";
         if((isset($this->data['cfpp15']['ano']) && $this->data['cfpp15']['ano']!="") || isset($ano)){
         	$ano = isset($this->data['cfpp15']['ano']) ?  $this->data['cfpp15']['ano'] : $ano;
...
//		  $this->set('no','no');
		  ...
		  $lista=  $this->cfpd05_auxiliar->generateList($cond2, 'cod_auxiliar ASC', null, '{n}.cfpd05_auxiliar.cod_auxiliar', '{n}.cfpd05_auxiliar.denominacion');
                       if($lista!=null){
                       	    echo "<script type='text/javascript'>ver_documento('/cfpp15/seleccion/','st_ubica_seleccion');</script>";
							$this->concatena_aux($lista, 'vector');
						}else{
							echo "<script type='text/javascript'>ver_documento('/cfpp15/seleccion/si/0','st_ubica_seleccion');</script>";
...
function consultar($ejercicio=null, $pag_num=null) {
	  ...
	  if($ejercicio!=null){
	  	    $this->set('ejercicio', $ejercicio);
		}else if($this->data['cfpp15']['ano']){
							$this->set('ejercicio', $this->data['cfpp15']['ano']);
							$ejercicio = $this->data['cfpp15']['ano'];
							$ano = $this->data['cfpp15']['ano'];
                            $this->Session->write('ano_r',$ano);
							}else{
			  	...
			  	$tc=$this->cfpd15->findCount($this->SQLCA($ejercicio));
			  if($tc!=0){
			   $DATOS_res = $this->cfpd15->findAll('cod_presi='.$this->Session->read('SScodpresi').'  and  cod_entidad='.$this->Session->read('SScodentidad').' and cod_tipo_inst='.$this->Session->read('SScodtipoinst').' and  cod_inst='.$this->Session->read('SScodinst').' and cod_dep='.$this->Session->read('SScoddep').' and  ano='.$ejercicio.'', null, 'cod_auxiliar ASC', null, null, null);
...
$Tfilas= count($this->cfpd15->findAll($this->condicion(),"DISTINCT ".$campos));
if($Tfilas!=0){
    ...
    $datos_filas=$this->cfpd15->findAll($this->condicion(),"DISTINCT ".$campos, $campos." ASC",1,$pagina,null);
    ...
    $ano            = $datos_filas[0]["cfpd15"]["ano"];
    $cod_sector     = $datos_filas[0]["cfpd15"]["cod_sector"];
    $cod_programa   = $datos_filas[0]["cfpd15"]["cod_programa"];
    $cod_sub_prog   = $datos_filas[0]["cfpd15"]["cod_sub_prog"];
    $cod_proyecto   = $datos_filas[0]["cfpd15"]["cod_proyecto"];
    $cod_activ_obra = $datos_filas[0]["cfpd15"]["cod_activ_obra"];
    $cod_partida    = $datos_filas[0]["cfpd15"]["cod_partida"];
    $cod_generica   = $datos_filas[0]["cfpd15"]["cod_generica"];
    $cod_especifica = $datos_filas[0]["cfpd15"]["cod_especifica"];
    $cod_sub_espec  = $datos_filas[0]["cfpd15"]["cod_sub_espec"];
    $cod_auxiliar   = $datos_filas[0]["cfpd15"]["cod_auxiliar"];
...
for($minCount = 2007; $minCount < 2030; $minCount++) {
  ...
  $this->set('datos',   $this->cfpd15->findAll($cond2,null,"numero_linea ASC",null, null, null));
...
function consultar2($con_ano=null,$pagina=null) {
         ...
         if($pagina!=null && $con_ano!=null){
          	 ...
          	 $Tfilas=$this->cfpd15->findCount($this->SQLCA($Ano));
          	 //echo $Tfilas;
          	 if($Tfilas==0){
          	 	$this->index2($Ano);
          		$this->render("index2");
          	 ...
          	 if($Tfilas!=0){
          	 $this->set('pag_cant',$pagina.'/'.$Tfilas);
          	 $datacfpd15=$this->cfpd15->findAll($this->SQLCA($Ano),null,'numero_linea DESC',1,$pagina,null);
          	 foreach ($datacfpd15 as $YYY);
                 //$YYY['cfpd15']['cod_sector'];

          	   $this->set('sector',$this->cfpd02_sector->findAll($this->SQLCA().' and cod_sector='.$YYY['cfpd15']['cod_sector'].' and ano='.$ejercicio,null,null,null,null,null));
			   $this->set('programa',$this->cfpd02_programa->findAll($this->SQLCA().' and cod_programa='.$YYY['cfpd15']['cod_programa'].' and ano='.$ejercicio,null,null,null,null,null));
			   $this->set('subprograma',$this->cfpd02_sub_prog->findAll($this->SQLCA().' and cod_sub_prog='.$YYY['cfpd15']['cod_sub_prog'].' and ano='.$ejercicio,null,null,null,null,null));
			   $this->set('proyecto',$this->cfpd02_proyecto->findAll($this->SQLCA().' and cod_proyecto='.$YYY['cfpd15']['cod_proyecto'].' and ano='.$ejercicio,null,null,null,null,null));
			   $this->set('actividad',$this->cfpd02_activ_obra->findAll($this->SQLCA().' and cod_activ_obra='.$YYY['cfpd15']['cod_activ_obra'].' and ano='.$ejercicio,null,null,null,null,null));
			   //$this->set('grupo',$this->cfpd01_ano_grupo->findAll());
			   $this->set('partida',$this->cfpd01_ano_partida->findAll('cod_partida='.substr($YYY['cfpd15']['cod_partida'],-2).' and ejercicio='.$ejercicio.' and cod_grupo='.CE,null,null,null, null, null));
			   $this->set('generica',$this->cfpd01_ano_generica->findAll('cod_generica='.$YYY['cfpd15']['cod_generica'].' and ejercicio='.$ejercicio.' and cod_grupo='.CE,null,null,null, null, null));
			   $this->set('especifica',$this->cfpd01_ano_especifica->findAll('cod_especifica='.$YYY['cfpd15']['cod_especifica'].' and ejercicio='.$ejercicio.' and cod_grupo='.CE,null,null,null, null, null));
			   $this->set('subespecifica',$this->cfpd01_ano_sub_espec->findAll('cod_sub_espec='.$YYY['cfpd15']['cod_sub_espec'].' and ejercicio='.$ejercicio.' and cod_grupo='.CE,null,null,null, null, null));
               $cp= $YYY['cfpd15']['cod_partida'];
               //$cp = $cp < 10 ? str_replace("0","", $cp) : $cp;

               $this->set('auxiliar',$this->cfpd05_auxiliar->findAll($this->SQLCA($ejercicio).' and cod_sector='.$YYY['cfpd15']['cod_sector'].' and cod_programa='.$YYY['cfpd15']['cod_programa'].' and cod_sub_prog='.$YYY['cfpd15']['cod_sub_prog'].' and cod_proyecto='.$YYY['cfpd15']['cod_proyecto'].' and cod_activ_obra='.$YYY['cfpd15']['cod_activ_obra'].' and cod_partida='.$cp.' and cod_generica='.$YYY['cfpd15']['cod_generica'].' and cod_especifica='.$YYY['cfpd15']['cod_especifica'].' and cod_sub_espec='.$YYY['cfpd15']['cod_sub_espec'].' and cod_auxiliar='.$YYY['cfpd15']['cod_auxiliar'],null,null,null, null, null));

               // print_r($datacfpd15);
          	 $this->set('DATOS',$datacfpd15);
          	 $this->set('siguiente',$pagina+1);
          	 $this->set('anterior',$pagina-1);
            ...
            $datacfpd15=$this->cfpd15->findAll($this->SQLCA($Ano),null,'numero_linea DESC',1,$pagina,null);
              ...
              foreach ($datacfpd15 as $YYY);
               ...
               $cp= $YYY['cfpd15']['cod_partida'];
               //$cp = $cp < 10 ? str_replace("0","", $cp) : $cp;
              // echo $this->SQLCA($ejercicio).' and cod_sector='.$YYY['cfpd15']['cod_sector'].' and cod_programa='.$YYY['cfpd15']['cod_programa'].' and cod_sub_prog='.$YYY['cfpd15']['cod_sub_prog'].' and cod_proyecto='.$YYY['cfpd15']['cod_proyecto'].' and cod_activ_obra='.$YYY['cfpd15']['cod_activ_obra'].' and cod_partida='.$cp.' and cod_generica='.$YYY['cfpd15']['cod_generica'].' and cod_especifica='.$YYY['cfpd15']['cod_especifica'].' and cod_sub_espec='.$YYY['cfpd15']['cod_sub_espec'].' and cod_auxiliar='.$YYY['cfpd15']['cod_auxiliar'];
               $this->set('auxiliar',$this->cfpd05_auxiliar->findAll($this->SQLCA($ejercicio).' and cod_sector='.$YYY['cfpd15']['cod_sector'].' and cod_programa='.$YYY['cfpd15']['cod_programa'].' and cod_sub_prog='.$YYY['cfpd15']['cod_sub_prog'].' and cod_proyecto='.$YYY['cfpd15']['cod_proyecto'].' and cod_activ_obra='.$YYY['cfpd15']['cod_activ_obra'].' and cod_partida='.$cp.' and cod_generica='.$YYY['cfpd15']['cod_generica'].' and cod_especifica='.$YYY['cfpd15']['cod_especifica'].' and cod_sub_espec='.$YYY['cfpd15']['cod_sub_espec'].' and cod_auxiliar='.$YYY['cfpd15']['cod_auxiliar'],null,null,null, null, null));
              // echo $this->SQLCA($ejercicio).' and cod_sector='.$YYY['cfpd15']['cod_sector'].' and cod_programa='.$YYY['cfpd15']['cod_programa'].' and cod_sub_prog='.$YYY['cfpd15']['cod_sub_prog'].' and cod_proyecto='.$YYY['cfpd15']['cod_proyecto'].' and cod_activ_obra='.$YYY['cfpd15']['cod_activ_obra'].' and cod_partida='.$cp.' and cod_generica='.$YYY['cfpd15']['cod_generica'].' and cod_especifica='.$YYY['cfpd15']['cod_especifica'].' and cod_sub_espec='.$YYY['cfpd15']['cod_sub_espec'].' and cod_auxiliar='.$YYY['cfpd15']['cod_auxiliar'];
          	$this->set('DATOS',$datacfpd15);
          	$this->set('siguiente',$pagina+1);
...
if($op=="si" && $var!=null){
	$this->set('datos',$this->cfpd15->findAll($cond2,null,"numero_linea ASC",null, null, null));
		echo '<script>';
   		echo " document.getElementById('programa_social').value=''   ;";
   		echo " document.getElementById('organismo').value=''   ;";
   		echo " document.getElementById('asignacion_anual').value=''   ;";
	echo '</script>';
}else{
...
function guardar(){
	$this->layout = "ajax";
	if(!empty($this->data)){
	 $a=$this->data['cfpp15']['cod_sector'];
	 $b=$this->data['cfpp15']['cod_programa'];
	 $c=$this->data['cfpp15']['cod_subprograma'];
	 $d=$this->data['cfpp15']['cod_proyecto'];
	 $e=$this->data['cfpp15']['cod_actividad'];
	 $f=$this->data['cfpp15']['cod_partida'];
	 $f = $f >10 ? CE.$f : CE."0".$f ;
	 $g=$this->data['cfpp15']['cod_generica'];
	 $h=$this->data['cfpp15']['cod_especifica'];
	 $i=$this->data['cfpp15']['cod_subespecifica'];
	 $j=isset($this->data['cfpp15']['cod_auxiliar']) ? $this->data['cfpp15']['cod_auxiliar'] : 0;
	 $l=$this->data['cfpp15']['programa_social'];
	 $m=$this->data['cfpp15']['organismo'];
	 $n=$this->Formato1($this->data['cfpp15']['asignacion_anual']);
	 if($j == null){
	 	$j=0;
	 }
	 ...
	 $SQL_INSERT ="INSERT INTO cfpd15 (cod_presi, cod_entidad,cod_tipo_inst,cod_inst,cod_dep,ano,cod_sector,cod_programa,cod_sub_prog,cod_proyecto,cod_partida,cod_activ_obra,cod_generica,cod_especifica,cod_sub_espec,cod_auxiliar,programa_social,organismo,asignacion_anual)";
	 $SQL_INSERT .=" VALUES (".$aa[1].",".$aa[2].",".$aa[3].",".$aa[4].",".$aa[5].",".$k.",".$a.",".$b.",".$c.",".$d.",".$f.",".$e.",".$g.",".$h.",".$i.",".$j.",'".$l."','".$m."',".$n.")";
     $x=$this->cfpd15->execute($SQL_INSERT);
...
$sql_verificar .=" and cod_partida=".$f." and cod_generica=".$g." and cod_especifica=".$h." and cod_sub_espec=".$i." and cod_auxiliar=".$j."";
        ...
        foreach($data as $datos){$asignacion_anual = $datos['cfpd05']['asignacion_anual'];}
         ...
         $this->set('datos',$this->cfpd15->findAll($sql_axu,null,null,null, null, null));
         $this->data=null;
         $this->set('Message_existe', 'Meta Agregada con exito.');

}else{

	 $this->set('errorMessage', 'LOS DATOS NO PUDIERON SER GUARDADOS');
}
     ...
     echo "<script>" .
		"document.getElementById('programa_social').value='';" .
		"document.getElementById('organismo').value='';" .
		"document.getElementById('asignacion_anual').value='';" .
		"</script>";
...
function eliminar($var11=null){
	...
	$x=$this->cfpd15->findCount($sql);
				if($x!=0){

					$sql1 ="DELETE  FROM  cfpd15 where ".$sql;
			...
			$data2 = $this->cfpd15->findAll($sql_re_2, null, null, null);
			        foreach($data as $datos){$asignacion_anual = $datos['cfpd05']['asignacion_anual'];}
			        foreach($data2 as $datos2){$estimado_presu = $datos2['cfpd15']['asignacion_anual'];}
			...
			$this->cfpd05->execute($sql_execute);
					...
					$y=$this->cfpd15->findCount($sql2);
}


$this->seleccion("si", $this->Session->read('aux'));
$this->render("seleccion");
...
if( !empty($this->data['cfpp15']['programa_social_'.$id_fila])      &&
	!empty($this->data['cfpp15']['organismo_'.$id_fila])     &&
	!empty($this->data['cfpp15']['asignacion_anual_'.$id_fila])){


	if(!empty($this->data)){

	 $a=$this->data['cfpp15']['programa_social_'.$id_fila];
	 $b=$this->data['cfpp15']['organismo_'.$id_fila];
	 $c=$this->Formato1($this->data['cfpp15']['asignacion_anual_'.$id_fila]);
	 $e=$numero_linea;
	}
...
$sql3="update cfpd15 set programa_social='$a', organismo='$b', asignacion_anual=$c where ".$cond2;
...
$data = $this->cfpd15->findAll($cond2, null, null, null);
foreach($data as $datos){
     $a=$datos['cfpd15']['cod_sector'];
	 $b=$datos['cfpd15']['cod_programa'];
	 $c=$datos['cfpd15']['cod_sub_prog'];
	 $d=$datos['cfpd15']['cod_proyecto'];
	 $e=$datos['cfpd15']['cod_activ_obra'];
	 $f=$datos['cfpd15']['cod_partida'];
	 $g=$datos['cfpd15']['cod_generica'];
	 $h=$datos['cfpd15']['cod_especifica'];
	 $i=$datos['cfpd15']['cod_sub_espec'];
	 $j=$datos['cfpd15']['cod_auxiliar'];
	 $l=$datos['cfpd15']['programa_social'];
	 $m=$datos['cfpd15']['organismo'];
	 $n=$datos['cfpd15']['asignacion_anual'];
}
...
$sql_verificar .=" and cod_partida=".$f." and cod_generica=".$g." and cod_especifica=".$h." and cod_sub_espec=".$i." and cod_auxiliar=".$j."";
$d31 = '1';
$n=$this->Formato1($this->data['cfpp15']['asignacion_anual_'.$id_fila]);
if($this->cfpd05->findCount($sql_verificar)==0){
        if($f=='404'){$cod_tipo_gasto='2';}else if($f=='407'){$cod_tipo_gasto='4';}else{$cod_tipo_gasto='1';}
$SQLINSERT="INSERT INTO cfpd05 (cod_presi,cod_entidad,cod_tipo_inst,cod_inst,cod_dep,ano,cod_sector,cod_programa,cod_sub_prog,cod_proyecto,cod_activ_obra,cod_partida,cod_generica,cod_especifica,cod_sub_espec,cod_auxiliar,
cod_tipo_gasto, tipo_presupuesto, asignacion_anual, aumento_traslado_anual, disminucion_traslado_anual, credito_adicional_anual, rebaja_anual,
...
$sql_verificar .=" and cod_partida=".$f." and cod_generica=".$g." and cod_especifica=".$h." and cod_sub_espec=".$i." and cod_auxiliar=".$j."";
        ...
        $data = $this->cfpd05->findAll($sql_verificar, null, null, null);
        $data2 = $this->cfpd15->findAll($sql_verificar1, null, null, null);
        foreach($data as $datos){$asignacion_anual = $datos['cfpd05']['asignacion_anual'];}
        foreach($data2 as $datos2){$inicio_estimado = $datos2['cfpd15']['asignacion_anual'];}
