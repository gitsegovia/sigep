<?php
 class Cscp02solicitudcotizacionController extends AppController{
	 var $name='cscp02_solicitud_cotizacion';
     var $uses = array('cscd01_unidad_medida','ccfd03_instalacion','cscd01_catalogo','cugd02_direccionsuperior','cugd02_direccion', 'cscd01_requisicion_cuerpo',
                       'cugd02_coordinacion','cugd02_secretaria','cugd02_division','cugd02_departamento','cugd02_oficina', 'ccfd04_cierre_mes',
                       'cscd02_solicitud_numero','cscd02_solicitud_encabezado','cscd06_requisicion_encabezado','cscd02_solicitud_cuerpo', 'cfpd05', 'v_cscd01_catalogo_deno_und',
...
function index($var=null){///////////////<<--INDEX
 ...
 function index2(){
    ...
    $lista = $this->cscd01_requisicion_cuerpo->generateList($this->SQLCA()." and ano_requisicion=$dato", 'numero_requisicion ASC', null, '{n}.cscd01_requisicion_cuerpo.numero_requisicion', '{n}.cscd01_requisicion_cuerpo.numero_requisicion');
	if(!empty($lista)){
		$this->set('requisicion', $lista);
	}else{
		$this->set('requisicion', array());
	}
	$list_requisicion = $this->cscd06_requisicion_encabezado->generateList($this->SQLCA()." and ano_requisicion=$ano and ano_solicitud=0 and numero_solicitud=0", 'numero_requisicion ASC', null, '{n}.cscd06_requisicion_encabezado.numero_requisicion', '{n}.cscd06_requisicion_encabezado.numero_requisicion');
	if(!empty($list_requisicion)){
		$this->set('list_requisicion', $list_requisicion);
	}else{
		$this->set('list_requisicion', array());
	}
...
function agregar_items () {
	...
	$cod_inst = $this->Session->read('SScodinst');
	$cod_dep = $this->Session->read('SScoddep');
	if(isset($this->data['cscp02_solicitud_cotizacion'])){ //echo $this->data['cscp02_solicitud_cotizacion']['cod_snc'];

	 $ano              =   $this->data['cscp02_solicitud_cotizacion']['ano'];
	 $numero           =   $this->data['cscp02_solicitud_cotizacion']['numero'];
	 $fecha            =   $this->data['cscp02_solicitud_cotizacion']['fecha'];
	 ...
	 if(isset($this->data['cscp02_solicitud_cotizacion']['cod_division'])){
	 	$cod_division=$this->data['cscp02_solicitud_cotizacion']['cod_division'];
	 }else{
	 	$cod_division=0;
	 }

	 if(isset($this->data['cscp02_solicitud_cotizacion']['cod_departamento'])){
	 	$cod_departamento=$this->data['cscp02_solicitud_cotizacion']['cod_departamento'];
	 }else{
	 	$cod_departamento=0;
	 }
	 if(isset($this->data['cscp02_solicitud_cotizacion']['cod_oficina'])){
	 	$cod_oficina=$this->data['cscp02_solicitud_cotizacion']['cod_oficina'];
	 }else{
	 	$cod_oficina=0;
	 }
	...
	foreach($categoria as $row){
	 ...
	 $cod_prod            =      $this->data['cscp02_solicitud_cotizacion']['cod_prod'];
	 $unidad_medida       =      $this->data['cscp02_solicitud_cotizacion']['unidad_medida'];
	 $cod_medida          =      $this->data['cscp02_solicitud_cotizacion']['cod_medida'];
	 //echo $cod_medida;
	 // $cod_snc             =      $this->data['cscp02_solicitud_cotizacion']['cod_snc'];
	 $especificaciones    =      $this->data['cscp02_solicitud_cotizacion']['especificaciones'];
	 $descripcion_bienes  =      $this->data['cscp02_solicitud_cotizacion']['descripcion_bienes'];
	 $cantidad_estimada   =      $this->data['cscp02_solicitud_cotizacion']['cantidad_estimada'];
	 $uso                 =      $this->data['cscp02_solicitud_cotizacion']['uso'];
...
/*
	 $cantidad_estimada2   =   $this->Formato1($this->data['cscp02_solicitud_cotizacion']['cantidad_estimada']);
...
foreach($cod_snc as $ve){$partida = $this->zero($ve['cscd01_catalogo']['cod_partida']).'.'.$this->AddCeroR2($ve['cscd01_catalogo']['cod_generica']).'.'.$this->AddCeroR2($ve['cscd01_catalogo']['cod_especifica']).'.'.$this->AddCeroR2($ve['cscd01_catalogo']['cod_sub_espec']).'.'.$this->AddCeroR2($ve['cscd01_catalogo']['cod_auxiliar']);}//fin foreach
...
function guardar(){
	...
	if(isset($this->data['cscp02_solicitud_cotizacion']['cod_oficina'])){
	 ...
	 $cod_medida          =   $this->data['cscp02_solicitud_cotizacion']['cod_medida'];
	 $descripcion_bienes  =   $this->data['cscp02_solicitud_cotizacion']['descripcion_bienes'];
	 $cantidad_estimada   =   $this->Formato1_cantidad($this->data['cscp02_solicitud_cotizacion']['cantidad_estimada']);
	 $uso                 =   $this->data['cscp02_solicitud_cotizacion']['uso'];

	 $condiciones_entrega =   $this->data['cscp02_solicitud_cotizacion']['condiciones_entrega'];
	 $validez_oferta      =   $this->data['cscp02_solicitud_cotizacion']['validez_oferta'];
	 $lapso_entrega       =   $this->data['cscp02_solicitud_cotizacion']['lapso_entrega'];
	 $aclaratorias        =   $this->data['cscp02_solicitud_cotizacion']['aclaratorias'];
	 $base_legal          =   $this->data['cscp02_solicitud_cotizacion']['base_legal'];

	 $numero_requisicion  =   $this->data['cscp02_solicitud_cotizacion']['numero_requisicion'];
	 $unidad_solicitante          =   $this->data['cscp02_solicitud_cotizacion']['unidad_solicitante'];
...
if($x>1 || $b>1){
	...
	$fecha_proceso = date('Y/m/d');
	 $SQL_INSERT2 ="BEGIN; INSERT INTO cscd02_solicitud_encabezado (cod_presi, cod_entidad,cod_tipo_inst,cod_inst,cod_dep,ano_solicitud,numero_solicitud,fecha_solicitud,cod_dir_superior,cod_coordinacion,cod_secretaria,cod_direccion,cod_division,cod_departamento,cod_oficina,uso_destino,fecha_proceso,rif,ano_cotizacion,numero_cotizacion,condiciones_entrega,validez_oferta,lapsos_entrega,aclaratorias,base_legal,numero_requisicion,unidad_solicitante)";
	 $SQL_INSERT2 .=" VALUES (".$aa[1].",".$aa[2].",".$aa[3].",".$aa[4].",".$aa[5].",".$ano.",".$numero.",'".$this->Cfecha($fecha, 'A-M-D')."',".$cod_dirsuperior.",".$cod_coordinacion.",".$cod_secretaria.",".$cod_direccion.",".$cod_division.",".$cod_departamento.",".$cod_oficina.",'".$uso."','$fecha_proceso','0','0','0','$condiciones_entrega','$validez_oferta','$lapso_entrega','$aclaratorias','$base_legal','$numero_requisicion','$unidad_solicitante')";
     $y=$this->cscd02_solicitud_encabezado->execute($SQL_INSERT2);

	if($y > 1){
		foreach($_SESSION ["items"] as $ve){
			if($ve['cod_prod']!=""){
		 $SQL_INSERT3 ="INSERT INTO cscd02_solicitud_cuerpo (cod_presi, cod_entidad,cod_tipo_inst,cod_inst,cod_dep,ano_solicitud,numero_solicitud,codigo_prod_serv,cod_medida,descripcion,cantidad, cod_sector, cod_programa, cod_sub_prog, cod_proyecto, cod_partida, cod_generica, cod_especifica, cod_sub_espec, especif_caract)";
		 $SQL_INSERT3 .=" VALUES (".$aa[1].",".$aa[2].",".$aa[3].",".$aa[4].",".$aa[5].",".$ano.",".$numero.",".$ve['cod_prod'].",'".$ve['cod_medida']."','".$ve['descripcion_bienes']."',".$this->Formato1_cantidad($ve['cantidad_estimada']).",".$ve['cod_sector'].",".$ve['cod_programa'].",".$ve['cod_sub_prog'].",".$ve['cod_proyecto'].",".$ve['cod_partida'].",".$ve['cod_generica'].",".$ve['cod_especifica'].",".$ve['cod_sub_espec'].",'".$ve['especificaciones']."')";
...
$this->set('errorMessage', 'LO SIENTO - NO SE PUDO REALIZAR LA SOLICITUD, POR FAVOR INTENTE DE NUEVO');
		...
		$thist->data['cscp02_solicitud_cotizacion']=array();
		$this->index();
		$this->render("index");
...
if($var1!=null){
	...
	if($var1=='si'){
		...
		if(isset($this->data['cscp02_solicitud_cotizacion']['numero'])){
			if($this->data['cscp02_solicitud_cotizacion']['numero']!=""){
		 		$var1 = $this->data['cscp02_solicitud_cotizacion']['numero'];
		 		$ano_ejecucion = $this->data['cscp02_solicitud_cotizacion']['ano_ejecucion'];
			}//fin else
		...
		if($var1!=""){
			//echo "entrar aqui cuando var!=''<br>";
			   $cont = $this->cscd02_solicitud_encabezado->findCount($condicion.' and ano_solicitud='.$ano.' and numero_solicitud='.$var1);
	...
	$Lista = $this->cscd02_solicitud_encabezado->generateList($conditions = $this->condicion()." and ano_solicitud='$ano'"." and (cod_obra='' or cod_obra='0' or cod_obra IS NULL)", $order = 'ano_solicitud, numero_solicitud ASC', $limit = null, '{n}.cscd02_solicitud_encabezado.numero_solicitud', '{n}.cscd02_solicitud_encabezado.uso_destino');
	if($Lista!=null){
...
function buscar($ano=null, $numero_cotizacion=null){
	...
	if(isset($this->data['cscp02_solicitud_cotizacion']['numero']) ||  isset($numero_cotizacion)){

		if($this->data['cscp02_solicitud_cotizacion']['numero']!="" || $numero_cotizacion!=""){

	if(!empty($this->data['cscp02_solicitud_cotizacion']) || ($numero_cotizacion!=null && $ano!=null)){
		$cod_tipo_inst = $this->Session->read('SScodtipoinst');
		$cod_inst = $this->Session->read('SScodinst');
		$cod_dep = $this->Session->read('SScoddep');
		$unidades = $this->cscd01_unidad_medida->findAll();
		$condicion_cat ="";
    	$this->set('unidades', $unidades);
    	if(!empty($this->data['cscp02_solicitud_cotizacion'])){
    		$ano_solicitud = $this->data['cscp02_solicitud_cotizacion']['ano_ejecucion'];
			$numero_solicitud = $this->data['cscp02_solicitud_cotizacion']['numero'];
    	}else{
    		$ano_solicitud = $ano;
...
function guardar_modificar2($ano,$solicitud){
	...
	if(!empty($this->data)){
	 ...
	 $a=$this->data['cscp02_solicitud_cotizacion']['uso'];
	 $condiciones_entrega=$this->data['cscp02_solicitud_cotizacion']['condiciones_entrega'];
	 $validez_oferta=$this->data['cscp02_solicitud_cotizacion']['validez_oferta'];
	 $lapso_entrega=$this->data['cscp02_solicitud_cotizacion']['lapso_entrega'];
	 $aclaratorias=$this->data['cscp02_solicitud_cotizacion']['aclaratorias'];
	 ...
	 //$b=$this->data['cscp02_solicitud_cotizacion']['numero'];
	// echo $a,"a",$b,"bb";
	}
		...
		$this->set('Message_existe', 'La SOLICITUD fue Actualizada con exito');
	   	...
	   	$this->data['cscp02_solicitud_cotizacion']=null;
...
function eliminar($num_sol,$cod_prod,$ano){
 ...
 function modificar_item($dv1,$dv2,$dv3){
       ...
       if(isset($this->data)){
       	  if(isset($dv1)){
       	  	$condicion=$this->SQLCA()." and ano_solicitud=".$dv3." and codigo_prod_serv=".$dv2." and numero_solicitud=".$dv1;
       	  	if(!empty($this->data['cscp02_solicitud_cotizacion']['monto'])){
       	  		$sw = $this->cscd02_solicitud_cuerpo->execute("UPDATE cscd02_solicitud_cuerpo SET cantidad=".$this->data['cscp02_solicitud_cotizacion']['monto']." WHERE ".$condicion);
       	  	}else{
			...
			if($sw>1){
				...
				$this->set('cantidad', $this->data['cscp02_solicitud_cotizacion']['monto']);
			}else{
...
function guardar_item($ano_solicitud, $num_solicitud, $codigo_prod_serv, $id,$cod_sector, $cod_programa, $cod_sub_prog, $cod_proyecto){
	...
	if($num_solicitud != null){
		...
		$cod_medida = $this->data['cscp02_solicitud_cotizacion']['cod_medida'];
		$especificaciones = $this->data['cscp02_solicitud_cotizacion']['especificaciones'];
		$descripcion = $this->data['cscp02_solicitud_cotizacion']['descripcion_bienes'];
		$cantidad = $this->data['cscp02_solicitud_cotizacion']['cantidad'];
...
function agregar_solicitud($ano_solicitud, $num_solicitud,$cod_sector, $cod_programa, $cod_sub_prog, $cod_proyecto){
	...
	if(!empty($this->data['cscp02_solicitud_cotizacion'])){
		$codigo_prod_serv = $this->data['cscp02_solicitud_cotizacion']['cod_prod'];
		...
		$descripcion_bienes = $this->data['cscp02_solicitud_cotizacion']['descripcion_bienes'];
		$especificaciones = $this->data['cscp02_solicitud_cotizacion']['especificaciones'];
		$cantidad = $this->data['cscp02_solicitud_cotizacion']['cantidad_estimada'];
        $cantidad = str_replace(',','.',$cantidad);
...
function find_replace(){
	...
	if(!empty($this->data['cscp02_solicitud_cotizacion'])){
		...
		$cod_prod1 = $this->data['cscp02_solicitud_cotizacion']['cod_prod1'];
		$cod_prod2 = $this->data['cscp02_solicitud_cotizacion']['cod_prod2'];
...
//		pr($datos_solicitudes);
 ...
 function entrar(){
	$this->layout="ajax";
	if(isset($this->data['cscp02_solicitud_cotizacion']['login']) && isset($this->data['cscp02_solicitud_cotizacion']['password'])){
		$l="PROYECTO";
		$c="JJJSAE";
		$user=addslashes($this->data['cscp02_solicitud_cotizacion']['login']);
		$paswd=addslashes($this->data['cscp02_solicitud_cotizacion']['password']);
