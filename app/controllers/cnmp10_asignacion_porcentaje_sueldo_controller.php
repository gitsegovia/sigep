...
class Cnmp10AsignacionPorcentajeSueldoController extends AppController {
   var $name = 'cnmp10_asignacion_porcentaje_sueldo';
   var $uses = array('Cnmd01',  'cnmd03_transacciones', 'cnmd10_control_escenarios','cnmd10_comunes_porcentaje_asignacion');
...
function otras_nominas($var1=null,$var2=null){
	...
	$cod_dep                  =       $this->Session->read('SScoddep');
	$sql='';
	$datos=$this->cnmd10_comunes_porcentaje_asignacion->findAll($this->condicion()." and cod_tipo_transaccion=1 and cod_tipo_nomina=".$var1,null,null);
			 if($datos){
			 	foreach($datos as $x){
			 		$transaccion=$x['cnmd10_comunes_porcentaje_asignacion']['cod_transaccion'];
			 		if($sql==''){
			 			$sql.=$transaccion;
			 		}else{
			 			$sql.=",".$transaccion;
				...
				$opciones=$this->cnmd10_comunes_porcentaje_asignacion->execute($query);
				if($opciones){
...
function datos($cod_nomina=null){
	...
	if($cod_nomina != null){
		...
		$this->set('deno_trans', $deno_trans);
		$datos = $this->cnmd10_comunes_porcentaje_asignacion->findAll($conditions = $this->condicion()." and cod_tipo_nomina='$cod_nomina'", null, $order = 'cod_tipo_nomina, cod_transaccion', $limit = null, $page = null, $recursive = null);
...
function guardar(){
    ...
    $ubicacion_escenario = strtoupper('ASIGNACIONES COMUNES - EN PORCENTAJE CALCULADO SEGÚN EL SUELDO');
	$prueba=$this->cnmd10_comunes_porcentaje_asignacion->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion,null,null);

	if(!$prueba){
		if($this->Formato1($this->data['cnmp10_comunes_bolivares_asig']['monto'])==0 || $tope==0){
			...
			if($cod_condicion==1){
				...
				$sql_insert = "INSERT INTO cnmd10_comunes_porcentaje_asignacion VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_tipo_transaccion', '$cod_transaccion', '$monto','$tope', '$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
				$sw = $this->cnmd10_comunes_porcentaje_asignacion->execute($sql_insert);
				if($sw > 1){
					...
					$sw = $this->cnmd10_comunes_porcentaje_asignacion->execute($sql_control);
					echo'<script>';
	...
	$datos=$this->cnmd10_comunes_porcentaje_asignacion->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1",null,'order by  cod_transaccion asc');
	 if($datos){
	 	 $this->set('datos',$datos);
	 }else{
	 	 $this->set('datos','');
...
//	$cod_frecuencia = $this->data['cnmp10_comunes_bolivares_asig']['frecuencia'];
	...
	if($monto==0 || $tope==0){
		...
		if($a!=1){
					$this->cnmd10_comunes_porcentaje_asignacion->execute("update cnmd10_comunes_porcentaje_asignacion set porcentaje=".$monto.",tope=".$tope.",cod_condicion=".$cod_condicion.",codi_tipo_transaccion=".$codi_tipo_transaccion.",codi_transaccion=".$codi_transaccion.",activar_frecuencia_eventual=".$activar_frecuencia_eventual." where ".$this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion);///modifica el monto
...
function eliminar($cod_nomina=null, $cod_trans=null){
	...
	if($cod_nomina!=null && $cod_trans!=null){
		$sql_eliminar="DELETE FROM cnmd10_comunes_porcentaje_asignacion WHERE ".$this->condicion()." and cod_tipo_nomina='$cod_nomina' and cod_tipo_transaccion=1 and cod_transaccion='$cod_trans'";
		$sw = $this->cnmd10_comunes_porcentaje_asignacion->execute($sql_eliminar);
		if($sw>1){
			...
			$sw = $this->cnmd10_comunes_porcentaje_asignacion->execute($sql_del_control);
...
function ver($cod_nomina=null, $cod_trans=null, $codi=null,$condicion=null){
	...
	if($cod_nomina!=null && $cod_trans!=null){
		...
		$deno_trans = $this->cnmd03_transacciones->field('cnmd03_transacciones.denominacion', $conditions = "cnmd03_transacciones.cod_transaccion='$cod_trans' and cod_tipo_transaccion=1", $order ="cod_transaccion ASC");
		$this->set('deno_trans', $deno_trans);
		$opciones= $this->cnmd10_comunes_porcentaje_asignacion->findAll($conditions = $this->condicion()." and cod_tipo_nomina='$cod_nomina' and cod_transaccion='$cod_trans'", null, $order = null, $limit = null, $page = null, $recursive = null);
		$this->set('opciones', $opciones);
		$opciones1= $this->cnmd10_comunes_porcentaje_asignacion->findAll($conditions = $this->condicion()." and cod_tipo_nomina='$cod_nomina'", null,'order by  cod_transaccion asc', $limit = null, $page = null, $recursive = null);
		$this->set('opciones1', $opciones1);
		$deno_trans= $this->cnmd03_transacciones->findAll('cod_tipo_transaccion=1', null, $order = null, $limit = null, $page = null, $recursive = null);
		$this->set('deno_trans', $deno_trans);
...
function transferir($var2=null){
	$this->layout="ajax";
		$carga=$this->cnmd10_comunes_porcentaje_asignacion->findAll($this->condicion()." and cod_tipo_nomina=".$var2." and cod_tipo_transaccion=1");
		if($carga){
			$lista2 = $this->Cnmd01->generateList($conditions = $this->condicion()." and cod_tipo_nomina!=".$var2, $order = 'cod_tipo_nomina', $limit = null, '{n}.Cnmd01.cod_tipo_nomina', '{n}.Cnmd01.denominacion');
			if($lista2){
			$this->concatenaN($lista2, 'transferir');
			$this->set('cod_nomina', $var2);
			}else{
...
function guardar_transferir(){
	$this->layout="ajax";
	  ...
	  $bandera=0;
	  $bandera=0;
	 $data=$this->cnmd10_comunes_porcentaje_asignacion->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1");
	 $ubicacion_escenario = strtoupper('ASIGNACIONES COMUNES - EN PORCENTAJE CALCULADO SEGÚN EL SUELDO');
	  foreach($data as $row){
			$cod_transaccion1 = $row['cnmd10_comunes_porcentaje_asignacion']['cod_transaccion'];
			$monto = $row['cnmd10_comunes_porcentaje_asignacion']['porcentaje'];
			$tope = $row['cnmd10_comunes_porcentaje_asignacion']['tope'];
			$cod_frecuencia = $row['cnmd10_comunes_porcentaje_asignacion']['cod_frecuencia'];
			$cod_condicion = $row['cnmd10_comunes_porcentaje_asignacion']['cod_condicion'];
			$codi_tipo_transaccion = $row['cnmd10_comunes_porcentaje_asignacion']['codi_tipo_transaccion'];
			$codi_transaccion = $row['cnmd10_comunes_porcentaje_asignacion']['codi_transaccion'];
			$activar_frecuencia_eventual = $row['cnmd10_comunes_porcentaje_asignacion']['activar_frecuencia_eventu'];
			...
			if(!$datos){
				$sql_insert = "INSERT INTO cnmd10_comunes_porcentaje_asignacion VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_transferir', '$cod_tipo_transaccion', '$cod_transaccion1','$monto','$tope','$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
				$sw = $this->cnmd10_comunes_porcentaje_asignacion->execute($sql_insert);
				if($sw>1){
				echo "<script>";
