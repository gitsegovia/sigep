 ...
 * sisap
 */
class Cnmp10DeduccionBolivaresSegunPuestoController extends AppController {
   var $name = 'cnmp10_deduccion_bolivares_segun_puesto';
   var $uses = array('Cnmd01',  'cnmd03_transacciones', 'cnmd10_control_escenarios','Cnmd02_empleados_puestos','Cnmd02_obreros_puestos','cnmd02_varios_puestos','cnmd10_comunes_puestos_bolivares_ded','cnmd10_comunes_puestos_bolivares_ded_2','v_cnmd05');
...
function cod_puesto($cod=null){
	...
	if($cod!=null){
		...
		if(!isset($_SESSION['transaccion'])){
			...
			$datos1 = $this->cnmd10_comunes_puestos_bolivares_ded->findCount($this->SQLCA()." and cod_tipo_nomina=".$x1." and cod_transaccion=".$x2);

			$datos = $this->cnmd10_comunes_puestos_bolivares_ded_2->findCount($this->SQLCA()." and cod_tipo_nomina=".$x1." and cod_transaccion=".$x2." and cod_puesto=".$cod);
...
//		 $var2=$this->Session->read('transaccion');
		...
		$carga=$this->cnmd10_comunes_puestos_bolivares_ded_2->findAll($this->condicion()." and cod_tipo_nomina=".$var1." and cod_tipo_transaccion=2 and cod_transaccion=".$var2);
		if($carga){
			$lista2 = $this->Cnmd01->generateList($conditions = $this->condicion()." and cod_tipo_nomina!=".$var1." and clasificacion_personal=".$cod1, $order = 'cod_tipo_nomina', $limit = null, '{n}.Cnmd01.cod_tipo_nomina', '{n}.Cnmd01.denominacion');
			if($lista2){
	...
	$datos=$this->cnmd10_comunes_puestos_bolivares_ded->findAll($this->SQLCA()." and cod_tipo_transaccion=2 and cod_transaccion=".$var2." and cod_tipo_nomina!=".$var1,null,null);
			 if($datos){
...
function guardar_transferir(){
	$this->layout="ajax";
	  ...
	  $cod_tipo_nomina = $this->data['cnmp10_deduccion_bolivares_segun_puesto']['cod_nomina'];
	  $cod_transaccion = $this->data['cnmp10_deduccion_bolivares_segun_puesto']['cod_trans'];
	  $cod_transferir = $this->data['cnmp10_deduccion_bolivares_segun_puesto']['cod_transferir'];
	  ...
	  if(!$ubicacion){
	  $data=$this->cnmd10_comunes_puestos_bolivares_ded->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=2 and cod_transaccion=".$cod_transaccion);
	  $data2=$this->cnmd10_comunes_puestos_bolivares_ded_2->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=2 and cod_transaccion=".$cod_transaccion);
	  $ubicacion_escenario = strtoupper('Deduccion Comunes - en Bolivares segun el puesto que ocupa');
	  foreach($data as $row){
			$cod_frecuencia = $row['cnmd10_comunes_puestos_bolivares_ded']['cod_frecuencia'];
			$cod_condicion = $row['cnmd10_comunes_puestos_bolivares_ded']['cod_condicion'];
			$codi_tipo_transaccion = $row['cnmd10_comunes_puestos_bolivares_ded']['codi_tipo_transaccion'];
			$codi_transaccion = $row['cnmd10_comunes_puestos_bolivares_ded']['codi_transaccion'];
			$activar_frecuencia_eventual = $row['cnmd10_comunes_puestos_bolivares_ded']['activar_frecuencia_eventu'];
			$sql_insert = "INSERT INTO cnmd10_comunes_puestos_bolivares_ded VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_transferir', '$cod_tipo_transaccion', '$cod_transaccion','$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
			$sw = $this->cnmd10_comunes_puestos_bolivares_ded->execute($sql_insert);
			if($sw>1){
				echo "<script>";
				...
				if($sw1 > 1){
					foreach($data2 as $row){
						$cod_puesto = $row['cnmd10_comunes_puestos_bolivares_ded_2']['cod_puesto'];
						$monto = $row['cnmd10_comunes_puestos_bolivares_ded_2']['monto'];
						$sql_insert2 = "INSERT INTO cnmd10_comunes_puestos_bolivares_ded_2 VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_transferir', '$cod_tipo_transaccion', '$cod_transaccion','$cod_puesto', '$monto')";
						$sw2 = $this->cnmd10_comunes_puestos_bolivares_ded_2->execute($sql_insert2);
					}// fin foreach data2
				}//fin $sw1
			}//fin $sw
	  }//fin foreach data
...
//////////////////////////////AGREGAR DESDE AQUI////////////////////
	...
	$datos=$this->cnmd10_comunes_puestos_bolivares_ded->findAll($this->SQLCA()." and cod_tipo_transaccion=2 and cod_transaccion=".$cod_transaccion." and cod_tipo_nomina!=".$cod_tipo_nomina,null,null);
			 if($datos){
			 	 $this->set('opciones',$datos);
			 }else{
			 	 $this->set('opciones','');
...
function verifica($cod_nomina=null, $cod_trans=null){
	...
	if($cod_trans!=null && $cod_trans!=null && $cod_trans!=""){
		$ubicacion = $this->cnmd10_control_escenarios->field('cnmd10_control_escenarios.ubicacion_escenario', $conditions = $this->condicion()." and cod_tipo_nomina='$cod_nomina' and cod_tipo_transaccion=2 and cod_transaccion='$cod_trans'", $order =null);
		$this->set('ubicacion', $ubicacion);
		$veri=$this->cnmd10_comunes_puestos_bolivares_ded->findAll($this->condicion()." and cod_tipo_nomina=".$cod_nomina." and cod_tipo_transaccion=2 and cod_transaccion=".$cod_trans);
		$this->set('opciones',$veri);
		if($veri!=null){
			$v=$this->cnmd10_comunes_puestos_bolivares_ded->execute("SELECT * FROM cnmd10_comunes_puestos_bolivares_ded WHERE ".$this->condicion()." and cod_tipo_nomina=".$cod_nomina." and cod_tipo_transaccion=2 and cod_transaccion=".$cod_trans);
...
function grilla($var=null,$var2=null){
	...
	if($var2!=""){
		$cond= $this->SQLCA();
		$datos = $this->cnmd10_comunes_puestos_bolivares_ded_2->findAll($this->SQLCA()." and cod_tipo_nomina=".$var." and cod_tipo_transaccion=2 and cod_transaccion=".$var2,null,' cod_puesto ASC');
		if($datos){
...
function guardar_items(){
	$this->layout="ajax";
	  ...
	  $cod_tipo_transaccion = 2;
	  $cod_transaccion = $this->data['cnmp10_deduccion_bolivares_segun_puesto']['cod_trans'];
	  $monto = $this->Formato1($this->data['cnmp10_deduccion_bolivares_segun_puesto']['monto']);
	   $cod_puesto=$this->data['cnmp10_deduccion_bolivares_segun_puesto']['cod_puesto'];

	    	$sql_insert2 = "INSERT INTO cnmd10_comunes_puestos_bolivares_ded_2 VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_tipo_transaccion', '$cod_transaccion','$cod_puesto', '$monto')";
			$sw = $this->cnmd10_comunes_puestos_bolivares_ded_2->execute($sql_insert2);
			if($sw>1){
				$datos = $this->cnmd10_comunes_puestos_bolivares_ded_2->findAll($this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=2 and cod_transaccion=".$cod_transaccion,null,'cod_puesto ASC');
				$this->set("datos",$datos);
				$cod1=$this->Session->read('clasi_personal');
				if($cod1==1){
					$table="Cnmd02_empleados_puestos";
...
function guardar(){
	...
	$cod_condicion = $this->data['cnmp10_deduccion_bolivares_segun_puesto']['condicion'];
	$cod_puesto = $this->data['cnmp10_deduccion_bolivares_segun_puesto']['cod_puesto'];
    $ubicacion_escenario = strtoupper('Deduccion Comunes - en Bolivares segun el puesto que ocupa');
	$prueba=$this->cnmd10_comunes_puestos_bolivares_ded->findAll($this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion,null,null);

	if(!$prueba){
		 $monto = $this->Formato1($this->data['cnmp10_deduccion_bolivares_segun_puesto']['monto']);

		if($this->data['cnmp10_deduccion_bolivares_segun_puesto']['monto']==0){
		...
		if  (!empty($this->data['cnmp10_deduccion_bolivares_segun_puesto'])){

			if(empty($this->data['cnmp10_deduccion_bolivares_segun_puesto']['tipo_trans'])){
				$codi_tipo_transaccion = 0;
			}else{
				$codi_tipo_transaccion = $this->data['cnmp10_deduccion_bolivares_segun_puesto']['tipo_trans'];
			}

		if($cod_condicion==1){
			$codi_transaccion = 0;
		}else{
			if(empty($this->data['cnmp10_deduccion_bolivares_segun_puesto']['select4'])){
				 ...
				 $codi_transaccion = $this->data['cnmp10_deduccion_bolivares_segun_puesto']['select4'];
			}

		}
			if(empty($this->data['cnmp10_deduccion_bolivares_segun_puesto']['escenario'])){
				$activar_frecuencia_eventual = 2;
			}else{
				$activar_frecuencia_eventual = $this->data['cnmp10_deduccion_bolivares_segun_puesto']['escenario'];
			}
			$sql_insert = "INSERT INTO cnmd10_comunes_puestos_bolivares_ded VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_tipo_transaccion', '$cod_transaccion','$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
			$sw = $this->cnmd10_comunes_puestos_bolivares_ded->execute($sql_insert);
			$sql_insert2 = "INSERT INTO cnmd10_comunes_puestos_bolivares_ded_2 VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_tipo_transaccion', '$cod_transaccion','$cod_puesto','$monto')";
			$sw = $this->cnmd10_comunes_puestos_bolivares_ded_2->execute($sql_insert2);
			if($sw > 1){
		...
		$this->data['cnmp10_deduccion_bolivares_segun_puesto'] = array();
		}//FIN ELSE VALIDA MONTO CERO
	}else{

	}
$datos=$this->cnmd10_comunes_puestos_bolivares_ded_2->FindAll($this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=2 and cod_transaccion=".$cod_transaccion,null,null);
	 if($datos){
	 	 $this->set('datos',$datos);
	 }else{
	 	 $this->set('datos','');
...
function modificar(){
	...
	$cod_tipo_nomina = $this->data['cnmp10_deduccion_bolivares_segun_puesto']['cod_nomina'];
	$cod_transaccion = $this->data['cnmp10_deduccion_bolivares_segun_puesto']['cod_trans'];
//	$cod_frecuencia = $this->data['cnmp10_deduccion_bolivares_segun_puesto']['frecuencia'];
	$cod_condicion = $this->data['cnmp10_deduccion_bolivares_segun_puesto']['condicion'];
	$a=0;
	...
	if(empty($this->data['cnmp10_deduccion_bolivares_segun_puesto']['tipo_trans'])){
		...
		if($cod_condicion==1){
			...
			if($a!=1){
				$this->cnmd10_comunes_puestos_bolivares_ded->execute("update cnmd10_comunes_puestos_bolivares_ded set cod_condicion=".$cod_condicion.",codi_tipo_transaccion=".$codi_tipo_transaccion.",codi_transaccion=".$codi_transaccion.",activar_frecuencia_eventual=".$activar_frecuencia_eventual." where ".$this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion);///modifica el monto
		   		 $this->set('Message_existe', 'El registro se ha modificado');
			}

			$datos=$this->cnmd10_comunes_puestos_bolivares_ded->findAll($this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=2 and cod_transaccion=".$cod_transaccion,null,null);
			 if($datos){
			 	 $this->set('opciones',$datos);
			 }else{
			 	 $this->set('opciones','');
...
function eliminar($cod_nomina=null, $cod_trans=null){
	$this->layout="ajax";
	    ...
	    $verificar=$this->cnmd10_comunes_puestos_bolivares_ded_2->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=2 and cod_transaccion=".$cod_transaccion);
		if($verificar!=null){
			if(($cod_tipo_nomina!="") && ($cod_transaccion!="")){
				$sql_eliminar="DELETE FROM cnmd10_comunes_puestos_bolivares_ded WHERE ".$this->condicion()." and cod_tipo_nomina='$cod_tipo_nomina' and cod_tipo_transaccion=2 and cod_transaccion='$cod_transaccion'";
				$sw = $this->cnmd10_comunes_puestos_bolivares_ded->execute($sql_eliminar);
				$sql_eliminar2="DELETE FROM cnmd10_comunes_puestos_bolivares_ded_2 WHERE ".$this->condicion()." and cod_tipo_nomina='$cod_tipo_nomina' and cod_tipo_transaccion=2 and cod_transaccion='$cod_transaccion'";
				...
				if($sw>1){
					...
					$sw = $this->cnmd10_comunes_puestos_bolivares_ded_2->execute($sql_del_control);
...
function transaccion($var=null){
	...
	if($var!='guarda'){
		$nomina=$var;
		 ...
		 $nomina = $this->data['cnmp10_deduccion_bolivares_segun_puesto']['cod_nomina'];
	}
	$datos=$this->cnmd10_comunes_puestos_bolivares_ded->findAll($this->SQLCA()." and cod_tipo_transaccion=2 and cod_tipo_nomina=".$nomina,null,null);
			 if($datos){
			 	 $this->set('opciones',$datos);
			 }else{
			 	 $this->set('opciones','');
