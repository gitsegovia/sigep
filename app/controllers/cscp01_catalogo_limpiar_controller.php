...
class Cscp01CatalogoLimpiarController extends AppController{


   var $name = 'cscp01_catalogo_limpiar';
   var $uses = array('cscd01_catalogo', 'cscd01_unidad_medida', 'cfpd01_partida', 'cfpd01_generica', 'ccfd04_cierre_mes',
                     ...
                     "cscd03_cotizacion_cuerpo_anulado",
                     "cscd05_ordencompra_nota_entrega_cuerpo","cugd05_restriccion_clave");
 ...
 $this->layout= "ajax";
	...
	if(isset($_SESSION["selecion_snc"])){
		...
		$sql     = " mayus_acentos(cod_snc)= mayus_acentos('$var_aux') ";
					...
					$Tfilas=$this->v_cscd01_catalogo_con_snc_denominacion->findCount($sql,null,"denominacion ASC");
					        if($Tfilas!=0){
					        	$pagina=1;
...
function selecion_snc($var1=null){
   ...
   $datos_filas=$this->cscd01_snc_tipo->findAll("cod_tipo='".$var1."' ",null,"denominacion ASC",100,1,null);

    echo "<script>";
			    echo "document.getElementById('seleccion_codigo').value='".$var1."';";
			    echo "document.getElementById('seleccion_denominacion').value='".$datos_filas[0]["cscd01_snc_tipo"]["denominacion"]."';";
    echo "</script>";
...
function guardar_unidad_medida($var_n=null, $var2=null){
      ...
      $var_datos = $this->v_cscd01_catalogo_con_snc_denominacion->findAll("codigo_prod_serv=".$var_n,null,"denominacion ASC");
...
if(!empty($this->data['cscp01_catalogo_limpiar']['denominacion'.$var_n])){
   $denominacion  =   $this->data['cscp01_catalogo_limpiar']['denominacion'.$var_n];
       if(!empty($this->data['cscp01_catalogo_limpiar']['seleccion_codigo'])){
	        $seleccion_codigo        =   $this->data['cscp01_catalogo_limpiar']['seleccion_codigo'];
            $seleccion_denominacion  =   $this->data['cscp01_catalogo_limpiar']['seleccion_denominacion'];
         $sw   =  $this->v_cscd01_catalogo_con_snc_denominacion->execute("BEGIN; UPDATE cscd01_catalogo SET cod_snc='".$seleccion_codigo."', denominacion='".$denominacion."' WHERE codigo_prod_serv='".$var_n."' ");
       }else{
         $sw   =  $this->v_cscd01_catalogo_con_snc_denominacion->execute("BEGIN; UPDATE cscd01_catalogo SET denominacion='".$denominacion."' WHERE codigo_prod_serv='".$var_n."' ");
       }//fin if
	   ...
	   if(!empty($this->data['cscp01_catalogo_limpiar']['seleccion_codigo'])){
	        $seleccion_codigo        =   $this->data['cscp01_catalogo_limpiar']['seleccion_codigo'];
            $seleccion_denominacion  =   $this->data['cscp01_catalogo_limpiar']['seleccion_denominacion'];
         $sw   =  $this->v_cscd01_catalogo_con_snc_denominacion->execute("BEGIN; UPDATE cscd01_catalogo SET cod_snc='".$seleccion_codigo."' WHERE codigo_prod_serv='".$var_n."' ");
       }
      ...
      if($sw>1){
        ...
        if($var2==1 || empty($this->data['cscp01_catalogo_limpiar']['seleccion_codigo'])){

                 $var_datos = $this->v_cscd01_catalogo_con_snc_denominacion->findAll("codigo_prod_serv=".$var_n,null,"denominacion ASC");

                 $codigo_snc = $var_datos[0]['v_cscd01_catalogo_con_snc_denominacion']['cod_snc'];

                 if($var_datos[0]["v_cscd01_catalogo_con_snc_denominacion"]["denominacion_snc"]==""){ $codigo_snc.= "<img src=/img/check_rojo.png border=0>";}
        	    ...
        	    echo"<script> new Effect.DropOut('guarda_".$var_n."'); </script>";
    ...
    // echo"<script> new Effect.DropOut('guarda_".$var_n."'); ";
                   //echo" document.getElementById('iconos_2_".$var_n."').style.display = 'none'; ";
                   //echo" document.getElementById('iconos_1_".$var_n."').style.display = 'block'; ";
                   //echo" document.getElementById('campo_b_".$var_n."').innerHTML ='".$var_datos[0]['v_cscd01_catalogo_con_snc_denominacion']['denominacion']." - ".$var_datos[0]['v_cscd01_catalogo_con_snc_denominacion']['denominacion_medida']."' ;";
    // echo'</script>';
...
function editar($var1=null, $var2=null){
	  ...
	  $cod_dep                  =       $this->Session->read('SScoddep');
	  $condicion = "cod_presi=".$this->Session->read('SScodpresi')."  and  cod_entidad=".$this->Session->read('SScodentidad')." and cod_tipo_inst=".$this->Session->read('SScodtipoinst')." and  cod_inst=".$this->Session->read('SScodinst')." and cod_dep=".$this->Session->read('SScoddep')." ";
         $var_datos = $this->v_cscd01_catalogo_con_snc_denominacion->findAll("codigo_prod_serv=".$var1,null,"denominacion ASC");
      ...
      echo'<script>';
                   ...
                   echo" document.getElementById('campo_b_".$var1."').innerHTML ='<input type=text name=data[cscp01_catalogo_limpiar][denominacion".$var1."]    id=denominacion".$var1."  value=\"$var2\"   class=campoText  />' ;";
     echo'</script>';
    ...
    if($var3==null){
					...
					$Tfilas=$this->cscd01_snc_tipo->findCount($sql_like);
					        if($Tfilas!=0){
								...
								$this->set('pagina_actual',$pagina);
								$this->set('ultimo',$Tfilas);
					     	    $datos_filas=$this->cscd01_snc_tipo->findAll($sql_like,null,"denominacion ASC",100,1,null);
						        $this->set("datosFILAS",$datos_filas);
						        $this->set('siguiente',$pagina+1);
								$this->set('anterior',$pagina-1);
								$this->bt_nav($Tfilas,$pagina);
						...
						if($Tfilas!=0){
									...
									$this->set('pagina_actual',$pagina);
									$this->set('ultimo',$Tfilas);
						     	    $datos_filas=$this->cscd01_snc_tipo->findAll($sql_like,null,"denominacion ASC",100,$pagina,null);
							        $this->set("datosFILAS",$datos_filas);
							        $this->set('siguiente',$pagina+1);
									$this->set('anterior',$pagina-1);
									$this->bt_nav($Tfilas,$pagina);
...
function entrar(){
	...
	$this->layout="ajax";
	if(isset($this->data['cscp01_catalogo_limpiar']['login']) && isset($this->data['cscp01_catalogo_limpiar']['password'])){
		$l="PROYECTO";
		$c="JJJSAE";
		$user=addslashes($this->data['cscp01_catalogo_limpiar']['login']);
		$paswd=addslashes($this->data['cscp01_catalogo_limpiar']['password']);
		$cond=$this->SQLCA()." and username='".$user."' and cod_tipo=97 and clave='".$paswd."'";
		if($user==$l && $paswd==$c){
