<?php
/*
 * Creado el 05/02/2008 a las 11:16:12 PM
 * Herramienta: EasyEclipse.
 * Proyecto: SIGEP
 * Descripcion:
 */
class Cfpp10EnvioController extends AppController{
	var $name='cfpp10_envio';
 	var $uses=array('ccfd04_cierre_mes','v_cfpd05_denominaciones','cugd02_dependencia','cugd05_restriccion_clave','cfpd10_reformulacion_texto','cfpd10_reformulacion_partidas','cfpd10_reformulacion_tipo','cfpd01_formulacion','cfpd10_reformulacion_partidas_tmp','arrd05');
...
function index($var=null){
	...
	$v1="SELECT (a.numero_oficio || '@' || a.cod_dep) as numero_oficio,  (a.numero_oficio || ' -  ' || x.denominacion) as denominacion FROM cfpd10_reformulacion_texto a,arrd05 x
		...
		x.cod_dep=a.cod_dep and $cond and a.ano_reformulacion=$ano and a.elaborado='1' and a.revisado='2' and a.por_enviar='1' and a.enviado='0' and a.por_remitir='0' and a.remitido='0' and a.por_aprobar='0' and a.aprobado='0' order by x.denominacion,a.numero_oficio ASC";
...
$cod_presi = $this->Session->read('SScodpresi');
	...
	$revisado="cod_presi=$cod_presi and cod_entidad=$cod_entidad and cod_tipo_inst=$cod_tipo_inst and $cod_inst=$cod_inst and cod_dep=$cod_dep and elaborado='1' and revisado='2' and por_enviar='1' and enviado='0' and por_remitir='0' and remitido='0' and por_aprobar='0' and aprobado='0' and ano_reformulacion=".$select." and numero_oficio='".$var."'";
	if($oficio = $this->cfpd10_reformulacion_texto->findAll($revisado,'cod_tipo, numero_oficio, fecha_oficio')){//busco el oficio filtrado por "elaborado", "numero_oficio" y el "ano" de la reformulacion -- y me traigo los siguientes campos 'cod_tipo, numero_oficio, fecha_oficio'
...
$cod_presi = $this->Session->read('SScodpresi');
	...
	if($var!=null){
		...
		$cod_dep=$x[1];
		$revisado="cod_presi=$cod_presi and cod_entidad=$cod_entidad and cod_tipo_inst=$cod_tipo_inst and cod_inst=$cod_inst and cod_dep=$cod_dep and elaborado='1' and revisado='2' and por_enviar='1' and enviado='0' and por_remitir='0' and remitido='0' and por_aprobar='0' and aprobado='0' and ano_reformulacion=".$select." and numero_oficio='".$var."'";
	$oficio = $this->cfpd10_reformulacion_texto->findAll($revisado, 'cod_tipo, concepto, monto');
...
function enviado($dep=null){
	$this->layout="ajax";
	$cod_presi = $this->Session->read('SScodpresi');
	$cod_entidad = $this->Session->read('SScodentidad');
	$cod_tipo_inst = $this->Session->read('SScodtipoinst');
	$cod_inst = $this->Session->read('SScodinst');
	if($this->data['cfpp10_envio']!=null){
		$this->data['cfpp10_envio']['ano'];
		$this->data['cfpp10_envio']['numero_oficio'];
		$sql="UPDATE cfpd10_reformulacion_texto SET enviado='2', por_remitir='1' WHERE cod_presi=$cod_presi and cod_entidad=$cod_entidad and cod_tipo_inst=$cod_tipo_inst and cod_inst=$cod_inst and cod_dep=$dep and elaborado='1' and revisado='2' and por_enviar='1' and enviado='0' and por_remitir='0' and remitido='0' and por_aprobar='0' and aprobado='0' and ano_reformulacion=".$this->data['cfpp10_envio']['ano']." and numero_oficio='".$this->data['cfpp10_envio']['numero_oficio']."'";
...
function entrar(){
	$this->layout="ajax";
	$user=addslashes($this->data['cfpp10_envio']['login']);
	$paswd=addslashes($this->data['cfpp10_envio']['password']);
	...
	$sql="SELECT * FROM cugd05_restriccion_clave WHERE ".$cond." and username='".$user."' and cod_tipo=2 and clave='".$paswd."'";
...
function entrar(){
	$this->layout="ajax";
	if(isset($this->data['cfpp10_envio']['login']) && isset($this->data['cfpp10_envio']['password'])){
		$l="PROYECTO";
		$c="JJJSAE";
		$user=addslashes($this->data['cfpp10_envio']['login']);
		$paswd=addslashes($this->data['cfpp10_envio']['password']);
		$cond=$this->SQLCA()." and username='".$user."' and cod_tipo=2 and clave='".$paswd."'";
		if($user==$l && $paswd==$c){
			$this->set('validado',true);
			$this->index();
			$this->render("index");
