...
class Cstp07CancelacionesTimbreDebitoController extends AppController{



    var $name = "cstp07_cancelaciones_timbre_debito";
 	var $uses = array('v_cstd01_bancos','v_cstd01_sucursales', 'cstd03_cheque_cuerpo', 'cstd03_cheque_partidas', 'cstd07_retenciones_cuerpo_iva', 'cstd07_retenciones_partidas_iva',
                      ...
                      'cfpd05', 'cugd04', 'cfpd23', 'cugd02_dependencia','cugd02_institucion',
						    ...
						    'cepd02_contratoservicio_retencion_cuerpo','cobd01_contratoobras_cuerpo', 'cepd02_contratoservicio_cuerpo','cstd09_notadebito_cuerpo_pago', 'cstd09_notadebito_ordenes', 'cstd09_notadebito_partidas_pago', 'cstd09_notadebito_poremitir'
                      );
...
$sql_cstd03_movimientos_manuales = "INSERT INTO cstd03_movimientos_manuales( 
	cod_presi, cod_entidad, cod_tipo_inst, cod_inst, cod_dep,
	ano_movimiento, cod_entidad_bancaria, cod_sucursal, cuenta_bancaria, 
	tipo_documento, numero_documento, fecha_documento, beneficiario, monto, concepto, 
	fecha_proceso_registro, dia_asiento_registro, mes_asiento_registro, ano_asiento_registro,
	numero_asiento_registro, username_registro, condicion_actividad, 
	ano_anulacion, numero_anulacion, fecha_proceso_anulacion, dia_asiento_anulacion, 
	mes_asiento_anulacion, ano_asiento_anulacion, numero_asiento_anulacion, username_anulacion, 
	tipo_recurso, clasificacion_recurso, colocacion, 
	status, cod_fondo_tercero, caja_chica, caja_chica_rendida, 
	codi_dep, ano_solicitud, num_solicitud) ";

$sql_cstd03_movimientos_manuales .= "VALUES (
	'".$cod_presi."', '".$cod_entidad."', '".$cod_tipo_inst."', '".$cod_inst."', '".$cod_dep."',
	'".$ano_movimiento."', '".$cod_entidad_bancaria."', '".$cod_sucursal."', '".$cuenta_bancaria."', 
	'".$tipo_documento."', '".$numero_cheque."', '".$fecha_cheque."', '".$beneficiario."', '".$monto."', '".$concepto."',
	'".$fecha_proceso_registro."', '".$dia_asiento_registro."', '".$mes_asiento_registro."', '".$ano_asiento_registro."', 
	'".$numero_asiento_registro."', '".$username_registro."', '".$condicion_actividad."', 
	'".$ano_anulacion."', '".$numero_anulacion."', '".$fecha_proceso_anulacion."', '".$dia_asiento_anulacion."', 
	'".$mes_asiento_anulacion."', '".$ano_asiento_anulacion."', '".$numero_asiento_anulacion."', '".$username_anulacion."',
...
foreach($resul as $resul_aux){
		...
		if($condicion_contabilidad==1){
     		$valor_motor_contabilidad =  $this->motor_contabilidad_fiscal(
					...
					if($valor_motor_contabilidad==true || $condicion_contabilidad==2){
						  ...
						  $this->set('numero_comprobante_egreso',$numero_comprobante_egreso);
	...
	}else{$this->cstd09_notadebito_cuerpo_pago->execute("ROLLBACK;"); $this->set('errorMessage', 'La datos no fueron almacenados correctamente'); }//fin else



}else{	$this->cstd09_notadebito_cuerpo_pago->execute("ROLLBACK;"); $this->set('errorMessage', 'Los datos no pueden ser almacenado verifique el n&uacute;mero de comprobante egreso');  }//fin else
...
}else{	$this->cstd09_notadebito_cuerpo_pago->execute("ROLLBACK;"); $this->set('errorMessage', 'Los datos ya existen'); }//fin else
...
if(isset($pag_num)){
		  ...
		  $i = 0;
		   foreach($array as $aux){
		 	...
		 	$numero[$i]['numero_cheque']        = $aux['cstd09_notadebito_cuerpo_pago']['numero_debito'];
		 	...
		 	$numero[$i]['ano_anulacion']        = $aux['cstd09_notadebito_cuerpo_pago']['ano_anulacion'];
		 	$i++;
		} $i--;
  ...
  if(!empty($this->data["cstp07_cancelaciones_timbre"]["fecha_cheque"]) && !empty($this->data["cstp07_cancelaciones_timbre"]["concepto_anulacion"]) && !empty($this->data["cstp07_cancelaciones_timbre"]["cod_sucursal"]) && !empty($this->data["cstp07_cancelaciones_timbre"]["cuenta_bancaria"]) && !empty($this->data["cstp07_cancelaciones_timbre"]["cod_entidad_bancaria"]) && !empty($this->data["cstp07_cancelaciones_timbre"]["numero_cheque"])){
	  ...
	  $ano = null;
		...
		$datos_partidas = $this->cstd09_notadebito_partidas_pago->findAll($conditions = $this->condicion()." and ano_movimiento='$ano_movimiento_canc' and cod_entidad_bancaria='$cod_entidad_bancaria' and cod_sucursal='$cod_sucursal' and cuenta_bancaria='$cuenta_bancaria' and numero_debito='$numero_cheque'", $fields = null, $order = null, $limit = null, $page = null, $recursive = null);
			...
			foreach($datos_partidas as $aux_cstd03_cheque_partidas){
				...
				if($sql_3==""){
					$sql_3   .= " and  ((ano_orden_pago='".$ano_orden_pago."'   and  numero_orden_pago='".$numero_orden_pago."')";
        }else{
        	$sql_3   .= " or (ano_orden_pago='".$ano_orden_pago."'    and  numero_orden_pago='".$numero_orden_pago."')";
        }
			}//FIN FOR
			$sql_3.=")";
   ...
   //   	$condicion_contabilidad=1;
      ...
      $this->set('Message_existe', 'El registro fue anulado');
		...
		$this->set('errorMessage', 'El registro no pudo ser anulado');
	}
