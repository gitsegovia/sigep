<?php
 ...
 class Cfpp97Controller extends AppController{
 	var $name="cfpp97";
	var $uses = array('Cnmd01','arrd05','Cnmd02_empleados_puestos','Cnmd02_obreros_puestos','cnmd02_varios_puestos',
                      'cfpd97','cugd02_direccionsuperior', 'cugd02_coordinacion', 'cugd02_secretaria', 'cugd02_direccion', 'cugd02_division',
                      ...
                      'cnmd04_tipo', 'v_cfpd97','cfpd05_auxiliar','cnmd06_fichas','cnmd06_datos_personales','cfpd01_formulacion', 'v_cfpd05_denominaciones',
                      'v_cfpd97_cargos');
...
function index($var=null){
    ...
    //print_r($datos);
    //$datos__ii = $this->cnmd04_ocupacion->generateList(null, 'cod_nivel_ii ASC', null, '{n}.cnmd04_ocupacion.cod_nivel_ii', '{n}.cnmd04_ocupacion.cod_nivel_ii');
...
function cargo($var1=null, $var2=null){
	...
	if($var2 != null){
		if($var1=='1'){
			...
			$cargo="SELECT MAX(cod_cargo) as cod_cargo FROM cfpd97 WHERE ".$condicion." and cod_tipo_nomina='$var2'";
			$cargo = $this->cfpd97->execute($cargo);
...
function mostrar_cod_puesto($var=null, $var2=null) {
    ...
    if(isset($var2) && !empty($var2) && isset($var) && !empty($var)){
        ...
        if($var == 1){
			...
			/* $condicion="cod_puesto=".$var2;
        	$a=$this->cnmd02_confianza_puestos->findAll($condicion);
        	if($a!=null){
		  	 	echo "<input type='text' name='data[ccfp01_division][cod_div_contable1]' value='".$a[0]['cnmd02_confianza_puestos']['denominacion_clase']."' id='cod_puesto1'   readonly='readonly' class='inputtext' />";
			  }else{
			  	 echo "<input type='text' name='data[ccfp01_division][cod_div_contable1]' value=''   readonly='readonly' class='inputtext' />";
			  }
//			echo $a[0]['cnmd02_confianza_puestos']['denominacion_clase']; */
...
function mostrar_cod_dir_superior($var=null) {
  ...
  $SQL_INSERT="INSERT INTO cfpd97 (".$CAMPOS.") VALUES (".$VALORES.")";
  $total_filas=$this->cfpd97->findCount($this->SQLCA()." and cod_tipo_nomina=".$vdatos[1]." and cod_cargo=".$vdatos[2]);
  $aux_cargo=$vdatos[2];
  if($total_filas==0){
    $this->cfpd97->recursive=0;
    for($i=0;$i<$ciclo;$i++){
  		$this->cfpd97->execute($SQL_INSERT);
  		$vdatos[2] += 1;
  		$VALORES = $S[1].",".$S[2].",".$S[3].",".$S[4].",".$S[5].",".$vdatos[1].",".$vdatos[2].",".$vdatos[3].",".$vdatos[28].",".$vdatos[29].",".$vdatos[30].",".$vdatos[31].",".$vdatos[4].",".$vdatos[5].",".$vdatos[6].",".$vdatos[7].",".$vdatos[8].",".$vdatos[9].",".$vdatos[10].",".$vdatos[11].",".$vdatos[12].",".$vdatos[13].",".$vdatos[14].",1,".$vdatos[27].",".$vdatos[15].",".$vdatos[16].",".$vdatos[17].",".$vdatos[18].",".$vdatos[19].",".$vdatos[20].",".$vdatos[21].",".$vdatos[22].",".$vdatos[23].",".$vdatos[24].",".$vdatos[25].",".$vdatos[26]."";
  		$SQL_INSERT="INSERT INTO cfpd97 (".$CAMPOS.") VALUES (".$VALORES.")";
    }
...
function reporte($var=null, $var2=null){
	...
	if($consolidacion=="1"){
          ...
          $var_orde = " cod_tipo_nomina,cod_cargo,denominacion_clase ASC ";

	}else if($consolidacion=="2"){

          $var_orde = " cod_tipo_nomina,cod_cargo,dir_superior, coordinacion, secretaria, direccion, division, departamento, oficina ASC ";
...
//	$datos = $this->v_cnmd05_cargos->findAll($conditions = $condicion, $fields = null, $var_orde, $limit = null, $page = null, $recursive = null);
	$sql="select * from v_cfpd97_cargos where ".$condicion." order by ".$var_orde;
	$datos = $this->v_cfpd97_cargos->execute($sql);
	if($datos!=null){
...
function consulta1($cod_tipo_nomina=null,$pagina=null){
	...
	if(isset($pagina)){
		$Tfilas=$this->cfpd97->findCount($this->SQLCA()." and cod_tipo_nomina=".$nomina);
        if($Tfilas!=0){
        	$x=$this->cfpd97->findAll($this->SQLCA()." and cod_tipo_nomina=".$nomina,null,"cod_tipo_nomina,cod_cargo ASC",1,$pagina,null);
			$cod_cargo = $x[0]['cfpd97']['cod_cargo'];
	...
	$sql="select * from v_cfpd97_cargos where ".$this->SQLCA()." and cod_tipo_nomina=".$x[0]["cfpd97"]["cod_tipo_nomina"]." and cod_cargo=".$x[0]["cfpd97"]["cod_cargo"];
	$datos=$this->v_cfpd97_cargos->execute($sql);
	$this->set('datos',$datos);
...
//					$sql     =" (".$this->busca_separado(array("cod_tipo_nomina", "cod_cargo", "cod_puesto", "tipo_nomina","denominacion_clase"), $var1).") ";
		...
		$Tfilas=$this->v_cfpd97_cargos->findCount($condicion." and ".$sql);
        if($Tfilas!=0){
								...
								$this->set('pagina_actual',$pagina);
								$this->set('ultimo',$Tfilas);
					     	    $datos_filas=$this->v_cfpd97_cargos->findAll($condicion." and ".$sql,null,"cod_tipo_nomina, cod_cargo, cod_puesto ASC",100,$pagina,null);
						        $this->set("datosFILAS",$datos_filas);
						        $this->set('siguiente',$pagina+1);
								$this->set('anterior',$pagina-1);
								$this->bt_nav($Tfilas,$pagina);
...
function busqueda1($tipo_nomina=null,$cod_cargo=null,$cod_puesto=null){
    ...
    $this->set('dixx',$dixx);
	$sql="select * from v_cfpd97_cargos where ".$this->SQLCA()." and cod_tipo_nomina='$tipo_nomina' and cod_cargo='$cod_cargo' and cod_puesto='$cod_puesto'";
	$datos=$this->v_cfpd97_cargos->execute($sql);
	$this->set('datos',$datos);
...
function eliminar($cod_tipo_nomina=null, $cod_cargo=null,$cod_puesto=null,$pagina=null){
	...
	$this->cfpd97->execute("DELETE FROM cfpd97 WHERE ".$this->SQLCA()." and cod_tipo_nomina='$cod_tipo_nomina' and cod_cargo='$cod_cargo'");
...
function modificar($cod_tipo_nomina=null, $cod_cargo=null,$cod_puesto=null,$pagina=null){
	...
	$sql="select * from v_cfpd97_cargos where ".$this->SQLCA()." and cod_tipo_nomina='$cod_tipo_nomina' and cod_cargo='$cod_cargo' and cod_puesto='$cod_puesto'";
	$datos=$this->v_cfpd97_cargos->execute($sql);
	$this->set('datos',$datos);
	$this->set('pagina',$pagina);
...
function guardar_modificar($cod_tipo_nomina=null, $cod_cargo=null,$cod_puesto=null, $pagina=null){
		...
		$sql = "UPDATE cfpd97 SET cod_puesto='$cod_puesto', sueldo_basico='$sueldo_basico', compensaciones='$compensaciones', primas='$primas', bonos='$bonos', cod_dir_superior=$cod_dir_superior, cod_coordinacion=$cod_coordinacion, ";
