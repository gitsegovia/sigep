 ...
 */
 ...
 class Ciap01ControlSalidasNumeroController extends AppController{
	var $name = "ciap01_control_salidas_numero";
 	var $uses = array('ciad01_inventario_salidas_numero','ccfd04_cierre_mes','cugd05_restriccion_clave','ciad01_inventario_usuarios','ciad01_inventario_almacen','cugd05_restriccion_clave');
...
$this->verifica_entrada('76');
	...
	if($ver!=null){
		...
		$Tfilas=$this->ciad01_inventario_salidas_numero->findCount($this->SQLCA()." and cod_almacen_salida=".$a[0][0]['cod_almacen']." and ano_orden_salida=".$dato);
    if($Tfilas!=0){
    	$pagina=1;
		...
		$this->set('ultimo',$Tfilas);
 	    $datos_filas=$this->ciad01_inventario_salidas_numero->findAll($this->SQLCA()." and cod_almacen_salida=".$a[0][0]['cod_almacen']." and ano_orden_salida=".$dato,null,"numero_orden_salida ASC",1000,1,null);
	 ...
	 $max=$this->ciad01_inventario_salidas_numero->execute("SELECT MAX(numero_orden_salida) as numero_mayor FROM ciad01_inventario_salidas_numero WHERE ".$this->SQLCA()." and cod_almacen_salida=".$almacen." and ano_orden_salida=".$dato);
     ...
     if($var2!=null){
		$this->set('msg_error1', $msg_error1 = 'NECESITA CREAR LOS NUMEROS DE CONTROL DE SALIDAS PARA CONTINUAR');
		return;
	 }
     if(isset($var)){
     	if($var=="guardado"){
             ...
             $this->set("Message_existe","Puede crear los números de CONTROL DE SALIDAS");
        }else{
        	if($var=="xy"){
				$this->set('msg_error1', $msg_error1 = 'NECESITA CREAR O DESCONGELAR LOS NUMEROS DE CONTROL DE SALIDAS PARA CONTINUAR');
			}else{
    ...
    $this->set('modelo','ciad01_inventario_salidas_numero');
}//fin index
...
function guardar(){
	$this->layout="ajax";
	$modelo = 'ciad01_inventario_salidas_numero';
	if(isset($this->data[$modelo])){
			 ...
			 $camposT3="cod_presi,cod_entidad,cod_tipo_inst,cod_inst,cod_dep,cod_almacen_salida,ano_orden_salida,numero_orden_salida,situacion";
			 $values="";
			 $monto=0;
			 $SQLAIN = $this->SQLCAIN();
			 ...
			 $R3=$this->ciad01_inventario_salidas_numero->execute("INSERT INTO ciad01_inventario_salidas_numero (".$camposT3.") VALUES ".$valores.";");
             if($R3 > 1){
             	$msg="guardado";
             }else{
                 $msg=false;
...
function cambiar_situacion ($codigo,$ano,$var) {
	$this->layout="ajax";
    $resultado=$this->ciad01_inventario_salidas_numero->execute("UPDATE ciad01_inventario_salidas_numero SET situacion=1 WHERE ".$this->SQLCA()." and numero_orden_salida=".$codigo." and ano_orden_salida=".$ano);
	if($resultado>1){
        $this->set("Message_existe","Situacion del n&uacute;mero actualizada con exito");
        $m="si";
	}else{
...
function cambiar_situacion_por_emitir ($almacen,$codigo,$ano,$var) {
	$this->layout="ajax";
    $resultado=$this->ciad01_inventario_salidas_numero->execute("UPDATE ciad01_inventario_salidas_numero SET situacion=1 WHERE ".$this->SQLCA()." and numero_orden_salida=".$codigo." and ano_orden_salida=".$ano." and cod_almacen_salida=".$almacen);
	if($resultado>1){
        $this->set("Message_existe","Situacion del n&uacute;mero actualizada con exito");
        $m="si";
	}else{
...
function cambiar_situacion_celdacompleta($almacen,$codigo,$ano,$var,$opc){
	...
	if($opc==1){//anular el numero de solicitud.
	    $resultado=$this->ciad01_inventario_salidas_numero->execute("UPDATE ciad01_inventario_salidas_numero SET situacion=4 WHERE ".$this->SQLCA()." and numero_orden_salida=".$codigo." and ano_orden_salida=".$ano." and cod_almacen_salida=".$almacen);
	    $consulta_numero =$this->ciad01_inventario_salidas_numero->execute("SELECT cod_almacen_salida,ano_orden_salida, numero_orden_salida, situacion FROM ciad01_inventario_salidas_numero WHERE ".$this->SQLCA()." and numero_orden_salida=".$codigo." and ano_orden_salida=".$ano." and cod_almacen_salida=".$almacen);
		if($resultado>1){
		...
		$this->set('codigo_almacen',$consulta_numero[0][0]["cod_almacen_salida"]);
		$this->set('codigo',$consulta_numero[0][0]["numero_orden_salida"]);
		$this->set('ano',$consulta_numero[0][0]["ano_orden_salida"]);
		$this->set('i',$var);
		$this->set('opc',$opc);

	}elseif($opc==2){//colocar como emitido el numero de solicitud.
	    $resultado=$this->ciad01_inventario_salidas_numero->execute("UPDATE ciad01_inventario_salidas_numero SET situacion=3 WHERE ".$this->SQLCA()." and numero_orden_salida=".$codigo." and ano_orden_salida=".$ano." and cod_almacen_salida=".$almacen);
	   $consulta_numero =$this->ciad01_inventario_salidas_numero->execute("SELECT cod_almacen_salida,ano_orden_salida, numero_orden_salida, situacion FROM ciad01_inventario_salidas_numero WHERE ".$this->SQLCA()." and numero_orden_salida=".$codigo." and ano_orden_salida=".$ano." and cod_almacen_salida=".$almacen);
		if($resultado>1){
	    ...
	    $resultado=$this->ciad01_inventario_salidas_numero->execute("UPDATE ciad01_inventario_salidas_numero SET situacion=5 WHERE ".$this->SQLCA()." and numero_orden_salida=".$codigo." and ano_orden_salida=".$ano." and cod_almacen_salida=".$almacen);
	    $consulta_numero =$this->ciad01_inventario_salidas_numero->execute("SELECT cod_almacen_salida,ano_orden_salida, numero_orden_salida, situacion FROM ciad01_inventario_salidas_numero WHERE ".$this->SQLCA()." and numero_orden_salida=".$codigo." and ano_orden_salida=".$ano." and cod_almacen_salida=".$almacen);
		if($resultado>1){
...
$this->layout = "ajax";
     ...
     if(isset($pagina)){
		...
		$Tfilas=$this->ciad01_inventario_salidas_numero->findCount($this->SQLCA()." and cod_almacen_salida=".$_SESSION['cod_almacen']." and ano_orden_salida=".$dato);
        if($Tfilas!=0){
        	//$Tfilas=$Tfilas/1000;
        	$Tfilas=(int)ceil($Tfilas/1000);
        	$this->set('total_paginas',$Tfilas);
			...
			$this->set('pag_cant',$pagina.'/'.$Tfilas);
			$this->set('ultimo',$Tfilas);
     	    $datos_filas=$this->ciad01_inventario_salidas_numero->findAll($this->SQLCA()." and cod_almacen_salida=".$_SESSION['cod_almacen']." and ano_orden_salida=".$dato,null,"numero_orden_salida ASC",1000,$pagina,null);
	        $this->set("datosFILAS",$datos_filas);
	        $this->set('siguiente',$pagina+1);
			$this->set('anterior',$pagina-1);
			$this->bt_nav($Tfilas,$pagina);
...
function entrar(){
	...
	if(isset($this->data['cscp02_solicitud_numero']['login']) && isset($this->data['cscp02_solicitud_numero']['password'])){
		...
		$cond=$this->SQLCA()." and username='".$user."' and cod_tipo=76 and clave='".$paswd."'";
		if(($user==$l && $paswd==$c)){
			$this->set('autor_valido',true);
