 ...
 * sisap
 */
class Cnmp10AsignacionBolivaresEscalaSueldoController extends AppController {
   var $name = 'cnmp10_asignacion_bolivares_escala_sueldo';
   var $uses = array('Cnmd01',  'cnmd03_transacciones', 'cnmd10_control_escenarios','cnmd10_comunes_dia_deduccion','v_cnmd10_comunes_dia_deduccion_trans','cnmd10_comunes_escala_bolivares_asig','cnmd10_comunes_escala_bolivares_asig_2');
...
function verifica($cod_nomina=null, $cod_trans=null){
	...
	if($cod_trans!=null && $cod_trans!=null && $cod_trans!=""){
		...
		$veri=$this->cnmd10_comunes_escala_bolivares_asig->findAll($this->condicion()." and cod_tipo_nomina=".$cod_nomina." and cod_transaccion=".$cod_trans);
		$this->set('opciones',$veri);
		if($veri){
			$v=$this->cnmd10_comunes_escala_bolivares_asig->execute("SELECT * FROM cnmd10_comunes_escala_sueldo_bolivares_asig WHERE ".$this->condicion()." and cod_tipo_nomina=".$cod_nomina." and cod_transaccion=".$cod_trans);
...
function grilla($var=null,$var2=null){
	...
	if($var2!=""){
	$cond= $this->SQLCA();
			$v=$this->cnmd10_comunes_escala_bolivares_asig_2->execute("SELECT * FROM cnmd10_comunes_escala_sueldo_bolivares_asig_2 WHERE ".$cond." and cod_tipo_nomina=".$var." and cod_transaccion=".$var2." ORDER BY escala DESC");
			if($v!=null){
				$escala=$v[0][0]["escala"];
		...
		$datos = $this->cnmd10_comunes_escala_bolivares_asig_2->findAll($cond." and cod_tipo_nomina=".$var." and cod_transaccion=".$var2,null,'escala ASC');
...
function agrega_input($var=null,$var2=null){
	$this->layout="ajax";
		...
		$this->set('nomina',$var);
		$this->set('trans',$var2);
		$prueba=$this->cnmd10_comunes_escala_bolivares_asig->findAll($this->SQLCA()." and cod_tipo_nomina=".$var." and cod_transaccion=".$var2,null,null);
		$cond= $this->SQLCA();
		if(!$prueba){
			echo "<script>";
...
function guardar_items(){
	$this->layout="ajax";
	  ...
	  $cod_tipo_nomina = $this->data['cnmp10_asignacion_bolivares_escala_sueldo']['cod_nomina'];
	  $cod_tipo_transaccion = 1;
	  $cod_transaccion = $this->data['cnmp10_asignacion_bolivares_escala_sueldo']['cod_trans'];
	  $monto = $this->Formato1($this->data['cnmp10_asignacion_bolivares_escala_sueldo']['monto']);
	  $escala=$this->data['cnmp10_asignacion_bolivares_escala_sueldo']['escala'];
	   $desde_sueldo=$this->Formato1($this->data['cnmp10_asignacion_bolivares_escala_sueldo']['desde_sueldo']);
	    $hasta_sueldo=$this->Formato1($this->data['cnmp10_asignacion_bolivares_escala_sueldo']['hasta_sueldo']);

		$this->set('escala',$escala);
		$this->set('hasta_sueldo',$hasta_sueldo);

	    if($this->data['cnmp10_asignacion_bolivares_escala_sueldo']['hasta_sueldo']!=""){
	    	$sql_insert2 = "INSERT INTO cnmd10_comunes_escala_sueldo_bolivares_asig_2 VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_tipo_transaccion', '$cod_transaccion','$escala', '$desde_sueldo', '$hasta_sueldo', '$monto')";
			$sw = $this->cnmd10_comunes_escala_bolivares_asig_2->execute($sql_insert2);
			if($sw>1){
				$datos = $this->cnmd10_comunes_escala_bolivares_asig_2->findAll($this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion,null,'escala ASC');
				$this->set("datos",$datos);
...
function guardar(){
	...
	$cod_condicion = $this->data['cnmp10_asignacion_bolivares_escala_sueldo']['condicion'];
	$escala = $this->data['cnmp10_asignacion_bolivares_escala_sueldo']['escala'];
	$desde_sueldo = $this->Formato1($this->data['cnmp10_asignacion_bolivares_escala_sueldo']['desde_sueldo']);
	$hasta_sueldo = $this->Formato1($this->data['cnmp10_asignacion_bolivares_escala_sueldo']['hasta_sueldo']);
	  ...
	  $cod_inst = $this->Session->read('SScodinst');
	  $cod_dep = $this->Session->read('SScoddep');
	  $ubicacion_escenario = strtoupper('Asignaciones Comunes -  en Bolivares utilizando una escala de sueldo');
	$prueba=$this->cnmd10_comunes_escala_bolivares_asig->findAll($this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion,null,null);

	if(!$prueba){
		 $monto = $this->Formato1($this->data['cnmp10_asignacion_bolivares_escala_sueldo']['monto']);

		if($this->data['cnmp10_asignacion_bolivares_escala_sueldo']['monto']==0 && $desde_sueldo==0 && $hasta_sueldo==""){
			$this->set('errorMessage', 'Debe completar todos los campos');
			return;
		}else{
			if  (!empty($this->data['cnmp10_asignacion_bolivares_escala_sueldo'])){

				if(empty($this->data['cnmp10_asignacion_bolivares_escala_sueldo']['tipo_trans'])){
					$codi_tipo_transaccion = 0;
				}else{
					$codi_tipo_transaccion = $this->data['cnmp10_asignacion_bolivares_escala_sueldo']['tipo_trans'];
				}

			if($cod_condicion==1){
				$codi_transaccion = 0;
			}else{
				if(empty($this->data['cnmp10_asignacion_bolivares_escala_sueldo']['select4'])){
					 $this->set('errorMessage', 'Debe seleccionar el codigo de transaccion');
					 return;
				}else{
					 $codi_transaccion = $this->data['cnmp10_asignacion_bolivares_escala_sueldo']['select4'];
				}

			}

				if(empty($this->data['cnmp10_asignacion_bolivares_escala_sueldo']['escenario'])){
					$activar_frecuencia_eventual = 2;
				}else{
					$activar_frecuencia_eventual = $this->data['cnmp10_asignacion_bolivares_escala_sueldo']['escenario'];
				}

				$sql_insert = "INSERT INTO cnmd10_comunes_escala_sueldo_bolivares_asig VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_tipo_transaccion', '$cod_transaccion','$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
				$sw = $this->cnmd10_comunes_escala_bolivares_asig->execute($sql_insert);
				$sql_insert2 = "INSERT INTO cnmd10_comunes_escala_sueldo_bolivares_asig_2 VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_tipo_transaccion', '$cod_transaccion','$escala', '$desde_sueldo', '$hasta_sueldo', '$monto')";
				$sw = $this->cnmd10_comunes_escala_bolivares_asig_2->execute($sql_insert2);
				if($sw > 1){
			...
			$this->data['cnmp10_asignacion_bolivares_escala_sueldo'] = array();
			}//FIN ELSE VALIDA MONTO CERO
	}else{

	}
$datos=$this->cnmd10_comunes_escala_bolivares_asig_2->findAll($this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1 and cod_transaccion=".$cod_transaccion,null,null);
	 if($datos){
	 	 $this->set('datos',$datos);
	 }else{
	 	 $this->set('datos','');
...
function modificar(){
	...
	$cod_tipo_nomina = $this->data['cnmp10_asignacion_bolivares_escala_sueldo']['cod_nomina'];
	$cod_transaccion = $this->data['cnmp10_asignacion_bolivares_escala_sueldo']['cod_trans'];
//	$cod_frecuencia = $this->data['cnmp10_asignacion_bolivares_escala_sueldo']['frecuencia'];
	$cod_condicion = $this->data['cnmp10_asignacion_bolivares_escala_sueldo']['condicion'];
	$a=0;
	...
	if(empty($this->data['cnmp10_asignacion_bolivares_escala_sueldo']['tipo_trans'])){
		...
		if($cod_condicion==1){
			...
			if($a!=1){
				$this->cnmd10_comunes_escala_bolivares_asig->execute("update cnmd10_comunes_escala_sueldo_bolivares_asig set cod_condicion=".$cod_condicion.",codi_tipo_transaccion=".$codi_tipo_transaccion.",codi_transaccion=".$codi_transaccion.",activar_frecuencia_eventual=".$activar_frecuencia_eventual." where ".$this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion);///modifica el monto
		   		 $this->set('Message_existe', 'El registro se ha modificado');
			}

			$datos=$this->cnmd10_comunes_escala_bolivares_asig->findAll($this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1 and cod_transaccion=".$cod_transaccion,null,null);
			 if($datos){
...
function eliminar($cod_nomina=null, $cod_trans=null){
	$this->layout="ajax";
	    ...
	    $verificar=$this->cnmd10_comunes_escala_bolivares_asig_2->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1 and cod_transaccion=".$cod_transaccion);
		if($verificar!=null){
			if(($cod_tipo_nomina!="") && ($cod_transaccion!="")){
				$sql_eliminar="DELETE FROM cnmd10_comunes_escala_sueldo_bolivares_asig WHERE ".$this->condicion()." and cod_tipo_nomina='$cod_tipo_nomina' and cod_tipo_transaccion=1 and cod_transaccion='$cod_transaccion'";
				$sw = $this->cnmd10_comunes_escala_bolivares_asig->execute($sql_eliminar);
				$sql_eliminar2="DELETE FROM cnmd10_comunes_escala_sueldo_bolivares_asig_2 WHERE ".$this->condicion()." and cod_tipo_nomina='$cod_tipo_nomina' and cod_tipo_transaccion=1 and cod_transaccion='$cod_transaccion'";
				$sw2 = $this->cnmd10_comunes_escala_bolivares_asig_2->execute($sql_eliminar2);
				if($sw>1){
					...
					$sw = $this->cnmd10_comunes_escala_bolivares_asig_2->execute($sql_del_control);
...
function transferir($var1=null,$var2=null){
	$this->layout="ajax";
		$carga=$this->cnmd10_comunes_escala_bolivares_asig_2->findAll($this->condicion()." and cod_tipo_nomina=".$var1." and cod_tipo_transaccion=1 and cod_transaccion=".$var2);
		if($carga){
	...
	$datos=$this->cnmd10_comunes_escala_bolivares_asig->findAll($this->SQLCA()." and cod_tipo_transaccion=1 and cod_transaccion=".$var2." and cod_tipo_nomina!=".$var1,null,null);
			 if($datos){
			 	 $this->set('opciones',$datos);
			 }else{
			 	 $this->set('opciones','');
...
function guardar_transferir(){
	$this->layout="ajax";
	  ...
	  $cod_tipo_nomina = $this->data['cnmp10_asignacion_bolivares_escala_sueldo']['cod_nomina'];
	  $cod_transaccion = $this->data['cnmp10_asignacion_bolivares_escala_sueldo']['cod_trans'];
	  $cod_transferir = $this->data['cnmp10_asignacion_bolivares_escala_sueldo']['cod_transferir'];
	  $cod_tipo_transaccion=1;
	  ...
	  if(!$ubicacion){
	  $data=$this->cnmd10_comunes_escala_bolivares_asig->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1 and cod_transaccion=".$cod_transaccion);
	  $data2=$this->cnmd10_comunes_escala_bolivares_asig_2->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1 and cod_transaccion=".$cod_transaccion);
	  $ubicacion_escenario = strtoupper('Asignaciones Comunes -  en Bolivares utilizando una escala de sueldo');
	  foreach($data as $row){
			$cod_frecuencia = $row['cnmd10_comunes_escala_bolivares_asig']['cod_frecuencia'];
			$cod_condicion = $row['cnmd10_comunes_escala_bolivares_asig']['cod_condicion'];
			$codi_tipo_transaccion = $row['cnmd10_comunes_escala_bolivares_asig']['codi_tipo_transaccion'];
			$codi_transaccion = $row['cnmd10_comunes_escala_bolivares_asig']['codi_transaccion'];
			$activar_frecuencia_eventual = $row['cnmd10_comunes_escala_bolivares_asig']['activar_frecuencia_eventu'];
			$sql_insert = "INSERT INTO cnmd10_comunes_escala_sueldo_bolivares_asig VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_transferir', '$cod_tipo_transaccion', '$cod_transaccion','$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
			$sw = $this->cnmd10_comunes_escala_bolivares_asig->execute($sql_insert);
			if($sw>1){
				echo "<script>";
				...
				if($sw1 > 1){
					foreach($data2 as $row){
						$escala = $row['cnmd10_comunes_escala_bolivares_asig_2']['escala'];
						$desde = $row['cnmd10_comunes_escala_bolivares_asig_2']['desde_sueldo'];
						$hasta = $row['cnmd10_comunes_escala_bolivares_asig_2']['hasta_sueldo'];
						$monto= $row['cnmd10_comunes_escala_bolivares_asig_2']['monto'];
						$sql_insert2 = "INSERT INTO cnmd10_comunes_escala_sueldo_bolivares_asig_2 VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_transferir', '$cod_tipo_transaccion', '$cod_transaccion','$escala','$desde','$hasta','$monto')";
						$sw2 = $this->cnmd10_comunes_escala_bolivares_asig_2->execute($sql_insert2);
					}// fin foreach data2
				}//fin $sw1
			}//fin $sw
	  }//fin foreach data
...
//////////////////////////////AGREGAR DESDE AQUI////////////////////
	...
	$datos=$this->cnmd10_comunes_escala_bolivares_asig->findAll($this->SQLCA()." and cod_tipo_transaccion=1 and cod_transaccion=".$cod_transaccion." and cod_tipo_nomina!=".$cod_tipo_nomina,null,null);
			 if($datos){
			 	 $this->set('opciones',$datos);
			 }else{
			 	 $this->set('opciones','');
...
function transaccion($var=null){
	...
	if($var!='guarda'){
		$nomina=$var;
		 ...
		 $nomina = $this->data['cnmp10_asignacion_bolivares_escala_sueldo']['cod_nomina'];
	}
	$datos=$this->cnmd10_comunes_escala_bolivares_asig->findAll($this->SQLCA()." and cod_tipo_transaccion=1 and cod_tipo_nomina=".$nomina,null,null);
			 if($datos){
			 	 $this->set('opciones',$datos);
			 }else{
			 	 $this->set('opciones','');
