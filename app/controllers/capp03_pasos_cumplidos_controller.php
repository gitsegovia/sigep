<?php
class Capp03PasosCumplidosController extends AppController {
   var $name = 'capp03_pasos_cumplidos';
...
function busqueda($tipo=null,$pagina=null,$pista=null){
	...
	if($tipo==1){
		...
		if(isset($pagina)){
			...
			$sql="select a.cod_presi,a.cod_entidad,a.cod_tipo_inst,a.cod_inst,a.cod_dep,a.ano,a.numero_control,a.paso,a.username,a.operacion,a.observaciones,a.fecha_flujo,a.hora,
				 (select b.denominacion from cugd02_departamento b where b.cod_tipo_institucion=(select c.cod_tipo_inst from usuarios c where c.username=a.username) and
				 b.cod_institucion=(select d.cod_inst from usuarios d where d.username=a.username) and b.cod_dependencia=(select e.cod_dep from usuarios e where e.username=a.username) and
				 b.cod_dir_superior=(select f.cod_dir_superior from  usuarios f where f.username=a.username) and b.cod_coordinacion=(select g.cod_coordinacion from usuarios g where g.username=a.username) and
				 b.cod_secretaria=(select h.cod_secretaria from usuarios h where h.username=a.username) and  b.cod_direccion=(select i.cod_direccion from usuarios i where i.username=a.username) and
				 b.cod_division=(select j.cod_division from usuarios j where j.username=a.username) and b.cod_departamento=(select k.cod_departamento from usuarios k where k.username=a.username)) as departamento from capd04_flujo a where a.cod_presi=".$this->verifica_SS(1)." and a.cod_entidad=".$this->verifica_SS(2)." and a.cod_tipo_inst=".$this->verifica_SS(3)." and a.cod_inst=".$this->verifica_SS(4)." and a.cod_dep=".$this->verifica_SS(5)." and a.ano=".$ano." and a.numero_control=".$x[0]["capd03_documentos"]["numero_control"]." order by a.paso asc";
		...
		if($tipo!=3){
			$url                  =  "/capp03_pasos_cumplidos/buscar_datos";
		$width_aux            =  "750px";
		$height_aux           =  "400px";
		$title_aux            =  "Buscar";
		$resizable_aux        =  false;
