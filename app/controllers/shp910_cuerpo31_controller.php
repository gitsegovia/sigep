<?php
 class Shp910Cuerpo31Controller extends AppController{
...
function agregar_grilla(){
	...
	$rif_cedula    		       	= $this->data['shp910_cuerpo31']['rif_constribuyente'];
	$ano 						= $this->data['shp910_cuerpo31']['ano_deuda'];
	$cod_numero_catastral_placas='0';
	$datos = $this->shd003_codigo_ingresos->findAll('cod_ingreso=3');
...
*/
	 	   	$mes    			= $this->data['shp910_cuerpo31']['mes'];
   			$numero_recibo     	= $this->data['shp910_cuerpo31']['numero_recibo'];
   			$deuda_vigente	    = $this->Formato1($this->data['shp910_cuerpo31']['deuda_vigente']);
   			if($this->data['shp910_cuerpo31']['recargo']!=null){
   				$recargo			= $this->Formato1($this->data['shp910_cuerpo31']['recargo']);
   			}else{
				$recargo=0;
   			}

   			if($this->data['shp910_cuerpo31']['multa']!=null){
   				$multa			= $this->Formato1($this->data['shp910_cuerpo31']['multa']);
   			}else{
				$multa=0;
   			}

   			if($this->data['shp910_cuerpo31']['intereses']!=null){
   				$intereses			= $this->Formato1($this->data['shp910_cuerpo31']['intereses']);
   			}else{
				$intereses=0;
   			}
			if($this->data['shp910_cuerpo31']['descuentos']!=null){
   				$descuentos			= $this->Formato1($this->data['shp910_cuerpo31']['descuentos']);
   			}else{
				$descuentos=0;
   			}
   			$total 				= $this->Formato1($this->data['shp910_cuerpo31']['total']);
   			$cancelado 			= $this->data['shp910_cuerpo31']['cancelado'];
...
function buscar_detalles($ano){
	...
	if($datos3 !=null){
		$a=$this->Formato2($datos3[0]['shd900_planillas_deuda_cobro_cuerpo']['deuda_ano_anterior']);
	}else{
		$a='';
	}
	echo'<script>';
...
function guardar(){
	...
	$cod_partida 	= $datos[0]['shd003_codigo_ingresos']['cod_partida'];
  	...
  	$part='cod_partida='.$cod_partida.' and cod_generica='.$cod_generica.' and cod_especifica='.$cod_especifica.' and cod_sub_espec='.$cod_subespec.' and cod_auxiliar='.$cod_auxiliar;
	$rif_cedula    		       	= $this->data['shp910_cuerpo31']['rif_constribuyente'];
	$deuda_anterior				= $this->Formato1($this->data['shp910_cuerpo31']['deuda_anterior']);
	$upd="update shd900_planillas_deuda_cobro_cuerpo set deuda_ano_anterior=$deuda_anterior where rif_cedula='".$rif_cedula."' and ".$this->SQLCA()." and ".$part;
	$sw = $this->shd900_planillas_deuda_cobro_cuerpo->execute($upd);
	$this->set('Message_existe', 'LOS DATOS FUERON GUARDADOS');
	$this->index();
