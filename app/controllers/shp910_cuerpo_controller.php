<?php
 class Shp910CuerpoController extends AppController{
...
function agregar_grilla(){
	...
	$rif_cedula    		       	= $this->data['shp910_cuerpo']['rif_constribuyente'];
	$ano 						= $this->data['shp910_cuerpo']['ano_deuda'];
	$cod_numero_catastral_placas='0';
	$datos = $this->shd003_codigo_ingresos->findAll('cod_ingreso=1');
...
/*  	$cont = $this->shd900_planillas_deuda_cobro_cuerpo->findCount($this->SQLCA()." and rif_cedula='".$rif_cedula."' and ".$part);
	if($cont==0){
		...
		$sw = $this->shd900_planillas_deuda_cobro_cuerpo->execute($upd);
	 	   	...
	 	   	$mes    			= $this->data['shp910_cuerpo']['mes'];
   			$numero_recibo     	= $this->data['shp910_cuerpo']['numero_recibo'];
   			$deuda_vigente	    = $this->Formato1($this->data['shp910_cuerpo']['deuda_vigente']);
   			if($this->data['shp910_cuerpo']['recargo']!=null){
   				$recargo			= $this->Formato1($this->data['shp910_cuerpo']['recargo']);
   			}else{
				$recargo=0;
   			}

   			if($this->data['shp910_cuerpo']['multa']!=null){
   				$multa			= $this->Formato1($this->data['shp910_cuerpo']['multa']);
   			}else{
				$multa=0;
   			}

   			if($this->data['shp910_cuerpo']['intereses']!=null){
   				$intereses			= $this->Formato1($this->data['shp910_cuerpo']['intereses']);
   			}else{
				$intereses=0;
   			}
			if($this->data['shp910_cuerpo']['descuentos']!=null){
   				$descuentos			= $this->Formato1($this->data['shp910_cuerpo']['descuentos']);
   			}else{
				$descuentos=0;
   			}
   			$total 				= $this->Formato1($this->data['shp910_cuerpo']['total']);
   			$cancelado 			= $this->data['shp910_cuerpo']['cancelado'];
...
/*function guardar(){
	...
	$cod_partida 	= $datos[0]['shd003_codigo_ingresos']['cod_partida'];
  	...
  	$part='cod_partida='.$cod_partida.' and cod_generica='.$cod_generica.' and cod_especifica='.$cod_especifica.' and cod_sub_espec='.$cod_subespec.' and cod_auxiliar='.$cod_auxiliar;
	$rif_cedula    		       	= $this->data['shp910_cuerpo']['rif_constribuyente'];
	$deuda_anterior				= $this->Formato1($this->data['shp910_cuerpo']['deuda_anterior']);
	$upd="update shd900_planillas_deuda_cobro_cuerpo set deuda_ano_anterior=$deuda_anterior where rif_cedula='".$rif_cedula."' and ".$this->SQLCA()." and ".$part;
	$sw = $this->shd900_planillas_deuda_cobro_cuerpo->execute($upd);
	$this->set('Message_existe', 'LOS DATOS FUERON GUARDADOS');
	$this->index();
