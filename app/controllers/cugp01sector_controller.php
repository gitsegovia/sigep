<?php
 ...
 class Cugp01sectorController extends AppController{



	var $name = 'cugp01sector';

	var $uses = array('cugd01_republica', 'cugd01_estados', 'cugd01_municipios', 'cugd01_parroquias', 'cugd01_centropoblados',  'ccnd03_censo_sector', 'v_concejo_comunales_usuarios', 'cugd90_municipio_defecto');
...
function index($vari_sector=null){
	...
	$denominacion = $this->ccnd03_censo_sector->generateList($condicion, 'denominacion ASC', null, '{n}.ccnd03_censo_sector.cod_sector', '{n}.ccnd03_censo_sector.denominacion');
...
function agregar_sec($valor_sec=null){
	$this->layout = "ajax";
	$this->set('cod_valor_sec', $valor_sec);
	$cod_presi = $this->Session->read('SScodpresi');
	$cod_entidad = $this->Session->read('SScodentidad');
	$cod_tipo_inst = $this->Session->read('SScodtipoinst');
	$cod_inst = $this->Session->read('SScodinst');
	...
	if($valor_sec=="otros"){
		$cant_sector = $this->ccnd03_censo_sector->findAll($condicion, 'cod_sector', 'cod_sector DESC', null);
		$cont_sec = $cant_sector != null ? $cant_sector[0]['ccnd03_censo_sector']['cod_sector']+1 : 1;
		$this->set('codigo_sector', $cont_sec);
		echo "<script>
...
function index_ant($estado=null, $municipio=null, $parroquia=null, $centro_poblado=null, $selet=null, $boton=null){
	...
	if($estado==null && $municipio==null && $selet==null && $boton==null){
			$opcion = 'si';
				...
				if($selet!=null && $selet!='otros' && $selet!="republica"){
						...
						$sql_selet = "cod_republica=".$this->Session->read('cod_presi_geografico')." and cod_estado=".$estado." and cod_municipio=".$municipio." and cod_parroquia=".$parroquia." and cod_centro=".$centro_poblado." and cod_sector=".$selet."";

						if($this->ccnd03_censo_sector->findCount($sql_selet) != 0){

				 			$data = $this->ccnd03_censo_sector->findAll($sql_selet, null, null, null);

				 			$denominacion =  $this->ccnd03_censo_sector->generateList($sql_re, 'denominacion ASC', null, '{n}.ccnd03_censo_sector.cod_sector', '{n}.ccnd03_censo_sector.denominacion');

				 			$data_aux = $data;

				 			foreach($data_aux as $data_aux_pre){}

				 			if($boton=='modificar'){$this->set('selecion_vialidad_2', $selet); $selet=$data_aux_pre['ccnd03_censo_sector']['denominacion']; }
							...
							$datos_1 = $this->ccnd03_censo_sector->findAll($sql_aux_cod, null, " cod_sector DESC");

							if(!isset($datos_1[0]["ccnd03_censo_sector"]["cod_sector"])){$datos_1[0]["ccnd03_censo_sector"]["cod_sector"]=0;}

							$this->set('cod_vialidad_aux',$datos_1[0]["ccnd03_censo_sector"]["cod_sector"]+1);
						...
						if(($parroquia!=null && $centro_poblado!=null) && ($parroquia!="no" && $centro_poblado!="no")){
							...
							if($this->ccnd03_censo_sector->findCount($sql_vialidad) != 0){
								...
								$consulta_vialidad = $this->ccnd03_censo_sector->generateList($sql_vialidad, 'denominacion ASC', null, '{n}.ccnd03_censo_sector.cod_sector', '{n}.ccnd03_censo_sector.denominacion');
						   		...
						   		$denominacion =  $this->ccnd03_censo_sector->generateList($sql_vialidad, 'denominacion ASC', null, '{n}.ccnd03_censo_sector.cod_sector', '{n}.ccnd03_censo_sector.denominacion');
...
function grabar(){
	...
	$cod_republica = $this->data['cugp01sector']['cod_republica'];
	$cod_estado = $this->data['cugp01sector']['cod_estado'];
	$cod_municipio = $this->data['cugp01sector']['cod_municipio'];
	$cod_parroquia = $this->data['cugp01sector']['cod_parroquia'];
	$cod_centro = $this->data['cugp01sector']['cod_centro'];
	$cod_sector = $this->data['cugp01sector']['cod_sector'];
	$denominacion_sec = $this->data['cugp01sector']['denominacion'];

	$sql_re = "cod_republica=".$cod_republica." and cod_estado=".$cod_estado." and cod_municipio=".$cod_municipio." and cod_parroquia=".$cod_parroquia." and cod_centro=".$cod_centro."  and cod_sector=".$cod_sector."";

	if($this->ccnd03_censo_sector->findCount($sql_re) == 0){
		$var_nota = "Registrado";
		$sql_sec = "INSERT INTO ccnd03_censo_sector (cod_republica, cod_estado, cod_municipio, cod_parroquia, cod_centro, cod_sector, denominacion) VALUES ( '".$cod_republica."',  '".$cod_estado."',  '".$cod_municipio."',  '".$cod_parroquia."', '".$cod_centro."',  '".$cod_sector."', '".$denominacion_sec."')";
	}else{
		$var_nota = "Actualizado";
		$sql_sec = "UPDATE ccnd03_censo_sector SET  denominacion = '".$denominacion_sec."' WHERE ".$sql_re;
	}

	$ejecuta = $this->ccnd03_censo_sector->execute($sql_sec);
	if($ejecuta>1){
		$this->set('Message_existe','El Sector fue '.$var_nota.' Exitosamente');
	}else{
		$this->set('errorMessage','No se pudo guardar - Intente Nuevamente');
	}

	$this->index($cod_sector);
	$this->render('index');
...
function modificar($valor_sec=null){
	$this->layout = "ajax";
	$cod_presi = $this->Session->read('SScodpresi');
	$cod_entidad = $this->Session->read('SScodentidad');
	$cod_tipo_inst = $this->Session->read('SScodtipoinst');
	...
	$condicion = "cod_republica = ".$cod_presi." and cod_estado = ".$cod_entidad." and cod_municipio = ".$cod_tipo_inst." and cod_parroquia = ".$cod_inst." and cod_centro = ".$cod_dep;
		...
		$denomi = $this->ccnd03_censo_sector->findAll($condicion." and cod_sector='$valor_sec'", 'denominacion', null, null);
		$denominac = $denomi != null ? $denomi[0]['ccnd03_censo_sector']['denominacion'] : '';
		$valor_secc = $valor_sec != null ? $valor_sec : null;
		$this->set('valor_secc', $valor_secc);
	...
	$this->set('Message_existe','Puede modificar el sector');

}


function eliminar_sec($valor_sect=null){
	$this->layout = "ajax";
	$cod_presi = $this->Session->read('SScodpresi');
	$cod_entidad = $this->Session->read('SScodentidad');
	$cod_tipo_inst = $this->Session->read('SScodtipoinst');
	...
	$sql_dele = "DELETE FROM ccnd03_censo_sector WHERE ".$condicion." and cod_sector='$valor_sect'";
	$ejecuta_e = $this->ccnd03_censo_sector->execute($sql_dele);
	if($ejecuta_e>1){
		$this->set('Message_existe','El Sector fue Eliminado Exitosamente');
	}else{
		$this->set('errorMessage','No se pudo eliminar el sector - Intente Nuevamente');
	}
...
function grabar_ant($estado=null, $municipio=null, $parroquia=null, $centro_poblado=null, $selet=null, $boton=null){
	...
	$denominacion='';
	 ...
	 if($estado==null){$estado=$this->data['cugp01sector']['cod_estado'];}

	 if($municipio==null){$municipio=$this->data['cugp01sector']['cod_municipio'];}

	 if($parroquia==null){$parroquia=$this->data['cugp01sector']['cod_parroquia'];}
 	 if($centro_poblado==null){$centro_poblado=$this->data['cugp01sector']['cod_centro'];}

	 if($selet==null){$selet=$this->data['cugp01sector']['cod_vialidad'];}



//pr($this->data);

$sql_re = "cod_republica=".$this->Session->read('cod_presi_geografico')." and cod_estado=".$estado." and cod_municipio=".$municipio." and cod_parroquia=".$parroquia." and cod_centro=".$centro_poblado."  and cod_sector=".$selet."";

if($this->ccnd03_censo_sector->findCount($sql_re) == 0){

$sql = "INSERT INTO ccnd03_censo_sector (cod_republica, cod_estado, cod_municipio, cod_parroquia, cod_centro, cod_sector, denominacion)   VALUES  ( '".$this->data['cugp01sector']['cod_republica']."',  '".$this->data['cugp01sector']['cod_estado']."',  '".$this->data['cugp01sector']['cod_municipio']."',  '".$this->data['cugp01sector']['cod_parroquia']."', '".$this->data['cugp01sector']['cod_centro']."',  '".$this->data['cugp01sector']['cod_vialidad']."', '".$this->data['cugp01sector']['denominacion']."')";

$this->ccnd03_censo_sector->execute($sql);

}else{

$sql ="UPDATE ccnd03_censo_sector SET  denominacion = '".$this->data['cugp01sector']['denominacion']."'   where cod_republica=".$this->Session->read('cod_presi_geografico')." and cod_estado=".$estado." and cod_municipio=".$municipio." and cod_parroquia=".$parroquia."  and cod_centro=".$centro_poblado."  and cod_sector=".$selet."";
...
function eliminar($estado=null, $municipio=null, $parroquia=null, $centro_poblado=null, $selet=null, $boton=null){
	...
	$sql= "DELETE  FROM  ccnd03_censo_sector  WHERE cod_republica=".$this->Session->read('cod_presi_geografico')." and cod_estado=".$estado." and cod_municipio=".$municipio." and cod_parroquia=".$parroquia." and cod_centro=".$centro_poblado."  and cod_sector=".$selet." ";
...
function consulta($pag_num=null) {
 		...
 		$this->layout = "ajax";
                    ...
                    $Tfilas=$this->ccnd03_censo_sector->findCount(null);
...
//			        	$pagina=1;
						...
						$this->set('ultimo',$Tfilas);
			     	    ...
			     	    $data = $this->ccnd03_censo_sector->findAll(null, null, 'cod_republica, cod_estado, cod_municipio, cod_parroquia, cod_centro, cod_sector  ASC', 1, $pag_num, null);
        ...
        if(isset($data[0]["ccnd03_censo_sector"]["cod_republica"])){



        	$sql_1 = "cod_republica=".$data[0]["ccnd03_censo_sector"]["cod_republica"];

        	$sql_2 = "cod_republica=".$data[0]["ccnd03_censo_sector"]["cod_republica"]." and cod_estado=".$data[0]["ccnd03_censo_sector"]["cod_estado"];

        	$sql_3 = "cod_republica=".$data[0]["ccnd03_censo_sector"]["cod_republica"]." and cod_estado=".$data[0]["ccnd03_censo_sector"]["cod_estado"]." and cod_municipio=".$data[0]["ccnd03_censo_sector"]["cod_municipio"];

        	$sql_4 = "cod_republica=".$data[0]["ccnd03_censo_sector"]["cod_republica"]." and cod_estado=".$data[0]["ccnd03_censo_sector"]["cod_estado"]." and cod_municipio=".$data[0]["ccnd03_censo_sector"]["cod_municipio"]." and cod_parroquia=".$data[0]["ccnd03_censo_sector"]["cod_parroquia"];

        	$sql_5 = "cod_republica=".$data[0]["ccnd03_censo_sector"]["cod_republica"]." and cod_estado=".$data[0]["ccnd03_censo_sector"]["cod_estado"]." and cod_municipio=".$data[0]["ccnd03_censo_sector"]["cod_municipio"]." and cod_parroquia=".$data[0]["ccnd03_censo_sector"]["cod_parroquia"]." and cod_centro=".$data[0]["ccnd03_censo_sector"]["cod_centro"];
...
function agregar_ventana(){
    ...
    $vialidad=$this->ccnd03_censo_sector->generateList($sql,'cod_sector ASC', null, '{n}.ccnd03_censo_sector.cod_sector', '{n}.ccnd03_censo_sector.denominacion');
...
function agregar_vialidad($opcion=null){
	...
	$ver=$this->ccnd03_censo_sector->execute("select * from ccnd03_censo_sector where ".$conditions." order by cod_sector desc limit 1");

	if($ver!=null)

		$cod_vialidad=$ver[0][0]['cod_sector']+1;
	...
	if($opcion!='agregar'){
		...
		$deno=$this->ccnd03_censo_sector->field('denominacion', $conditions ="cod_republica=".$this->Session->read('CC_republica')." and cod_estado=".$this->Session->read('CC_estado')." and cod_municipio=".$this->Session->read('CC_municipio')." and cod_parroquia=".$this->Session->read('CC_parroquia')." and cod_centro=".$this->Session->read('CC_centro')." and cod_sector=".$opcion, $order ="cod_sector ASC");
...
function guardar(){
	...
	if(!empty($this->data['cugp01sector']['cod_vialidad']) && !empty($this->data['cugp01sector']['denominacion'])){



		$selet=$this->data['cugp01sector']['cod_vialidad'];





		$sql_re = "cod_republica=".$cod_republica."and cod_estado=".$estado." and cod_municipio=".$municipio." and cod_parroquia=".$parroquia." and cod_centro=".$centro_poblado."  and cod_sector=".$selet."";

		if($this->ccnd03_censo_sector->findCount($sql_re) == 0){

			$sql = "INSERT INTO ccnd03_censo_sector (cod_republica, cod_estado, cod_municipio, cod_parroquia, cod_centro, cod_sector, denominacion)   VALUES  ( '".$cod_republica."',  '".$estado."',  '".$municipio."',  '".$parroquia."', '".$centro_poblado."',  '".$this->data['cugp01sector']['cod_vialidad']."', '".$this->data['cugp01sector']['denominacion']."')";
			...
			$sql ="UPDATE ccnd03_censo_sector SET  denominacion = '".$this->data['cugp01sector']['denominacion']."'   where cod_republica=".$cod_republica." and cod_estado=".$estado." and cod_municipio=".$municipio." and cod_parroquia=".$parroquia."  and cod_centro=".$centro_poblado."  and cod_sector=".$selet."";
