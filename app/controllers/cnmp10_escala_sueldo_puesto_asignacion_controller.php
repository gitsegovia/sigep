 ...
 * sisap
 */
class Cnmp10EscalaSueldoPuestoAsignacionController extends AppController {
   var $name = 'cnmp10_escala_sueldo_puesto_asignacion';
   var $uses = array('Cnmd01',  'cnmd03_transacciones', 'cnmd10_control_escenarios','Cnmd02_empleados_puestos','Cnmd02_obreros_puestos','cnmd02_varios_puestos','cnmd10_comunes_puestos_bolivares_asig','cnmd10_comunes_puestos_bolivares_asig_2','v_cnmd05_cargos');
...
function carga_transaccion($nomina=null){
	...
	$sql="select
		  ...
		  from cnmd10_comunes_puestos_bolivares_asig a	where ".$this->SQLCA()." and cod_tipo_nomina='$nomina' order by cod_transaccion";

	$transacciones=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute($sql);
...
function seleccion_trans($nomina=null,$transaccion=null,$otro=null){
	...
	if($transaccion!=null){
		...
		$sql="select
			  ...
			  from cnmd10_comunes_puestos_bolivares_asig a	where ".$this->SQLCA()." and a.cod_tipo_nomina='$nomina' and a.cod_transaccion='$transaccion' order by a.cod_transaccion";

		$verifica=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute($sql);
		if($verifica!=null){
			...
			$clasi=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute("select clasificacion_personal from cnmd01 where ".$this->SQLCA()." and cod_tipo_nomina=".$nomina);
		...
		$clasi=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute("select clasificacion_personal from cnmd01 where ".$this->SQLCA()." and cod_tipo_nomina=".$nomina);
				...
				$sql="select
					 ...
					 from cnmd10_comunes_puestos_bolivares_asig_2 a where ".$this->SQLCA()." and cod_tipo_nomina=".$nomina." and cod_transaccion=".$transaccion." order by a.cod_puesto asc";
				$datos=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute($sql);
...
function seleccion_busqueda($nomina=null,$transaccion=null,$puesto=null){
	...
	$deno=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute("select * from ".$table." where b.cod_puesto=".$puesto);
	$this->set('denominacion',$deno[0][0][$campo]);
	$this->set('cod_puesto',$puesto);

	$datos=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute("select * from cnmd10_comunes_puestos_bolivares_asig_2 where ".$this->SQLCA()." and cod_tipo_nomina=".$nomina." and cod_transaccion=".$transaccion." and cod_puesto=".$puesto);
	if($datos!=null){
...
function guardar(){
    ...
    if(empty($this->data['cnmp10']['cod_puesto'])){
    	...
    	$this->set('errorMessage','Ingrese el monto a asignar');
    }else{
	    ...
	    $ubicacion_escenario = strtoupper('Asignaciones Comunes - en Bolivares según el puesto que ocupa');
	    $this->set('nomina',$cod_tipo_nomina);
	    $this->set('transaccion',$cod_transaccion);

		$verifica=$this->cnmd10_comunes_puestos_bolivares_asig->FindCount($this->SQLCA()." and cod_tipo_nomina='$cod_tipo_nomina' and cod_transaccion='$cod_transaccion'");
		if($verifica==0){/////////// AQUI COMPRUEBO SI YA EXISTE EL ESCENARIO,SI NO EXISTE REGISTRO TODOS LOS DATOS INCLUYENDO LOS RADIOS SINO REGISTRO SOLO LA ESCALA
				if($this->data['cnmp10']['condicion']==2 && empty($this->data['cnmp10']['tipo_trans'])){
					...
					$insert1 = "Begin;INSERT INTO cnmd10_comunes_puestos_bolivares_asig VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_tipo_transaccion', '$cod_transaccion','$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
					$sw1 = $this->cnmd10_comunes_puestos_bolivares_asig->execute($insert1);
					if($sw1>1){
						$insert2 = "INSERT INTO cnmd10_comunes_puestos_bolivares_asig_2 VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_tipo_transaccion', '$cod_transaccion','$puesto', '$monto')";
						$sw2 = $this->cnmd10_comunes_puestos_bolivares_asig_2->execute($insert2);
						if($sw2>1){
							...
							$sw3 = $this->cnmd10_comunes_puestos_bolivares_asig_2->execute($insert3);
							if($sw3>1){
								$this->cnmd10_comunes_puestos_bolivares_asig_2->execute('COMMIT');
								$this->set('Message_existe','Registro exitoso');
								$this->set('guardado','si');
							}else{
								$this->cnmd10_comunes_puestos_bolivares_asig_2->execute('ROLLBACK');
...
function guardar_transferir($nomina=null,$transaccion=null){
	...
	$cod_dep = $this->Session->read('SScoddep');
	  ...
	  if($ubicacion==null){
		  $data=$this->cnmd10_comunes_puestos_bolivares_asig->execute("select * from cnmd10_comunes_puestos_bolivares_asig where ".$this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1 and cod_transaccion=".$cod_transaccion);
		  $data2=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute("select * from cnmd10_comunes_puestos_bolivares_asig_2 where ".$this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1 and cod_transaccion=".$cod_transaccion);
		  $ubicacion_escenario = strtoupper('Asignaciones Comunes - en Bolivares según el puesto que ocupa');
		  ...
		  $insert1 = "Begin;INSERT INTO cnmd10_comunes_puestos_bolivares_asig VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_transferir', '$cod_tipo_transaccion', '$cod_transaccion','$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
		  $sw      = $this->cnmd10_comunes_puestos_bolivares_asig_2->execute($insert1);
		  if($sw>1){
			  for($j=0;$j<count($data2);$j++){
				  $puesto  = $data2[$j][0]['cod_puesto'];
				  $monto   = $data2[$j][0]['monto'];
				  $insert2 = "INSERT INTO cnmd10_comunes_puestos_bolivares_asig_2 VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_transferir', '$cod_tipo_transaccion', '$cod_transaccion','$puesto', '$monto')";
				  $sw2     = $this->cnmd10_comunes_puestos_bolivares_asig_2->execute($insert2);
			  }
			  if($sw2>1){
...
function otros_escenarios($nomina=null,$transaccion=null){
	...
	$sql="select
			 ...
			 from cnmd10_comunes_puestos_bolivares_asig a where ".$this->SQLCA()." and a.cod_transaccion=".$transaccion." and a.cod_tipo_nomina!=".$nomina." order by a.cod_tipo_nomina asc";
	$otras_nominas=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute($sql);
...
function eliminar($nomina=null,$transaccion=null){
	...
	if(isset($nomina) && isset($transaccion)){
		$this->set('nomina',$nomina);
		$delete1="Begin;DELETE FROM cnmd10_comunes_puestos_bolivares_asig WHERE ".$this->SQLCA()." and cod_tipo_nomina='$nomina' and cod_transaccion='$transaccion'";
		$sw=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute($delete1);
		if($sw>1){
			$delete2="DELETE FROM cnmd10_comunes_puestos_bolivares_asig_2 WHERE ".$this->SQLCA()." and cod_tipo_nomina='$nomina' and cod_transaccion='$transaccion'";
			$sw2=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute($delete2);
			if($sw2>1){
				...
				$sw3=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute($delete3);
				if($sw3>1){
					$this->cnmd10_comunes_puestos_bolivares_asig_2->execute('COMMIT');
...
function modificar($var=null,$nomina=null,$transaccion=null){//////AQUI SI VAR ES 1 SOLO ENVIO UN MENSAJE PARA QUE MODIFIQUE Y HABILITE EL BOTON GUARDAR SINO VAR ES 2 Y PROCEDE A MODIFICAR(SOLO MODIFICA LOS RADIOS)
	...
	if($var==2 && (isset($nomina) && isset($transaccion))){
		if($this->data['cnmp10']['condicion']==2 && empty($this->data['cnmp10']['tipo_trans'])){
			...
			$ver=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute("update cnmd10_comunes_puestos_bolivares_asig set cod_condicion=".$cod_condicion.",codi_tipo_transaccion=".$codi_tipo_transaccion.",codi_transaccion=".$codi_transaccion.",activar_frecuencia_eventual=".$activar_frecuencia_eventual." where ".$this->SQLCA()." and cod_tipo_nomina=".$nomina." and cod_transaccion=".$transaccion);
		   	if($ver>1){
...
function modificar_items($cod_nomina=null, $cod_trans=null,$puesto=null,$i=null){
	...
	$clasi=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute("select clasificacion_personal from cnmd01 where ".$this->SQLCA()." and cod_tipo_nomina=".$cod_nomina);
	...
	$sql="select
		 ...
		 a.monto,
		 (select ".$campo." from ".$table." where b.cod_puesto=a.cod_puesto) as deno_puesto
		 from cnmd10_comunes_puestos_bolivares_asig_2 a where ".$this->SQLCA()." and cod_tipo_nomina=".$cod_nomina." and cod_transaccion=".$cod_trans." and cod_puesto=".$puesto." order by a.cod_puesto asc";
	$datos=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute($sql);
	$this->set('datos',$datos);
	$this->set('k',$i);
...
function eliminar_items($cod_nomina=null, $cod_trans=null,$puesto=null){
	...
	if($this->cnmd10_comunes_puestos_bolivares_asig_2->FindCount($this->SQLCA()." and cod_tipo_nomina='$cod_nomina' and cod_tipo_transaccion=1 and cod_transaccion='$cod_trans'")==1){
		$this->set('errorMessage', 'NO PODRA ELIMINAR TODOS LOS PUESTOS');
	}else{
		$sql_eliminar2="DELETE FROM cnmd10_comunes_puestos_bolivares_asig_2 WHERE ".$this->SQLCA()." and cod_tipo_nomina='$cod_nomina' and cod_tipo_transaccion=1 and cod_transaccion='$cod_trans' and cod_puesto='$puesto'";
		$sw2 = $this->cnmd10_comunes_puestos_bolivares_asig_2->execute($sql_eliminar2);
		if($sw2 > 1){
//			$this->set('Message_existe', 'LA TRANSACCION SE ELIMINO CON EXITO');
	...
	$sql="select
		 ...
		 a.monto,
		 (select ".$campo." from ".$table." where b.cod_puesto=a.cod_puesto) as deno_puesto
		 from cnmd10_comunes_puestos_bolivares_asig_2 a where ".$this->SQLCA()." and cod_tipo_nomina=".$cod_nomina." and cod_transaccion=".$cod_trans." order by a.cod_puesto asc";
	$datos=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute($sql);
	$this->set('datos',$datos);
...
function guardar_editar($cod_nomina=null, $cod_trans=null,$puesto=null,$i=null){
	...
	if(!empty($this->data['cnmp10']['monto'.$i])){
		...
		$ver=$this->cnmd10_comunes_puestos_bolivares_asig_2->execute("update cnmd10_comunes_puestos_bolivares_asig_2 set monto=".$cantidad." where ".$this->SQLCA()." and cod_tipo_nomina=".$cod_nomina." and cod_transaccion=".$cod_trans." and cod_puesto=".$puesto);
	}else{
