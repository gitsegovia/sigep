<?php
 ...
 class Shp999Actualizacion5Controller extends AppController{
 	var $name = "shp999_actualizacion5";
	var $uses = array('ccfd04_cierre_mes','shd000_arranque','shd900_cobranza_numero','shd900_planillas_deuda_cobro_detalles','shd000_ordenanzas','shd000_control_actualizacion','shd000_control_numero','shd100_patente','Cfpd03','shd003_codigo_ingresos','shd500_aseo_domiciliario','cugd05_restriccion_clave');
...
$this->verifica_entrada('62');
	...
	function procesar () {
	   ...
	   if($condicion!=null && $condicion==0){
	   	     $this->shd500_aseo_domiciliario->execute('BEGIN;');
	         $numero_planilla=$this->shd000_control_numero->numero_planilla($sql_condicion." and ano=$ano");
	         //echo $numero_planilla;
	         $this->shd500_aseo_domiciliario->execute("UPDATE shd500_aseo_domiciliario SET ultimo_ano_facturado=$ano, ultimo_mes_facturado=$mes WHERE ".$this->SQLCA()." and suspendido=2 and ultimo_ano_facturado<$ano and ultimo_ano_facturado>0");
	         //echo "UPDATE shd500_aseo_domiciliario SET ultimo_ano_facturado=$ano, ultimo_mes_facturado=$mes WHERE ".$this->SQLCA()." and suspendido=2 and ultimo_ano_facturado<$ano";
	         ...
	         $data = $this->shd500_aseo_domiciliario->findAll($condicion_patente,null,'cod_presi,cod_entidad,cod_tipo_inst,cod_inst,cod_dep,rif_ci_cobrador,rif_cedula ASC');
             ...
             foreach($data as $rv){
                extract($rv['shd500_aseo_domiciliario']);
                $f=$frecuencia_pago;
                ...
                if($cantid_ordenanzas!=0){
	                ...
	                foreach($data_detalles1 as $rs1){
	                   ...
	                   $RX1=$this->shd500_aseo_domiciliario->execute("UPDATE shd500_aseo_domiciliario SET ultimo_ano_facturado=$ano, ultimo_mes_facturado=$mes WHERE ".$this->SQLCA()." and rif_cedula='$rif_cedula';");
	                   ...
	                   if($RX1>1){
	                   	  //echo "RX1";
	                   	  $RX2=$this->shd500_aseo_domiciliario->execute($sql_cobro_detalle);
	                   	  if($RX2>1){
	                   	  	 //echo "RX2";
	...
	function entrar(){
		$this->layout="ajax";
		if(isset($this->data['shp999_actualizacion5']['login']) && isset($this->data['shp999_actualizacion5']['password'])){
			$l="PROYECTO";
			$c="JJJSAE";
			$user=addslashes($this->data['shp999_actualizacion5']['login']);
			$paswd=addslashes($this->data['shp999_actualizacion5']['password']);
			$cond=$this->SQLCA()." and username='".$user."' and cod_tipo=62 and clave='".$paswd."'";
			if($user==$l && $paswd==$c){
