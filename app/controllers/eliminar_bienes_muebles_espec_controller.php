 ...
 *
 */
 class EliminarBienesMueblesEspecController extends AppController {
   var $name = 'eliminar_bienes_muebles_espec';
   var $uses = array('ccfd04_cuentas_enlace','ccfd02','ccfd10_descripcion','ccfd10_detalles','cimd03_inventario_numero','ccfd05_numero_asiento','arrd05','ccfd04_cierre_mes','v_inventario_muebles_todo','v_buscar_muebles','cimd03_inventario_muebles','cimd01_clasificacion_seccion');
...
$depe = $this->Session->read('depe_eli_b');
    if($var3==null){$var2 = strtoupper($var2);
					...
					$Tfilas=$this->v_inventario_muebles_todo->findCount(" (buscar LIKE '%$var2%') and ".$this->condicionNDEP().' and cod_dep='.$depe.'and cod_tipo_desincorporacion=0');
					        if($Tfilas!=0){
					        	$pagina=1;
								...
								//echo $Tfilas;
					     	    $datos_filas=$this->v_inventario_muebles_todo->findAll(" (buscar LIKE '%$var2%') and ".$this->condicionNDEP().' and cod_dep='.$depe.'and cod_tipo_desincorporacion=0',null,"numero_identificacion ASC",100,1,null);
						...
						$Tfilas=$this->v_inventario_muebles_todo->findCount(" (buscar LIKE '%$var22%') and ".$this->condicionNDEP().' and cod_dep='.$depe.'and cod_tipo_desincorporacion=0');
						        if($Tfilas!=0){
						        	$pagina=$var3;
						        	$Tfilas=(int)ceil($Tfilas/100);
						        	$this->set('pag_cant',$pagina.'/'.$Tfilas);
									...
									$this->set('pagina_actual',$pagina);
									$this->set('ultimo',$Tfilas);
						     	    $datos_filas=$this->v_inventario_muebles_todo->findAll(" (buscar LIKE '%$var22%') and ".$this->condicionNDEP().' and cod_dep='.$depe.'and cod_tipo_desincorporacion=0',null,"numero_identificacion ASC",100,$pagina,null);
							        $this->set("datosFILAS",$datos_filas);
							        $this->set('siguiente',$pagina+1);
									$this->set('anterior',$pagina-1);
									$this->bt_nav($Tfilas,$pagina);
...
$a = $this->AddCeroR($resultado[0]['v_buscar_muebles']['cod_tipo']);
