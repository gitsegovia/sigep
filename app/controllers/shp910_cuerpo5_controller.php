<?php
 class Shp910Cuerpo51Controller extends AppController{
...
function agregar_grilla(){
	...
	$rif_cedula    		       	= $this->data['shp910_cuerpo51']['rif_constribuyente'];
	$ano 						= $this->data['shp910_cuerpo51']['ano_deuda'];
	$deuda_anterior				= $this->Formato1($this->data['shp910_cuerpo51']['deuda_anterior']);
	if($deuda_anterior==null){
		$deuda_anterior=0;
	}
	$cod_numero_catastral_placas='0';
	$datos = $this->shd003_codigo_ingresos->findAll('cod_ingreso=5');
	...
	if($cont==0){
		...
		$sw = $this->shd910_cuerpo->execute($upd);
	 	   	...
	 	   	$mes    			= $this->data['shp910_cuerpo51']['mes'];
   			$numero_recibo     	= $this->data['shp910_cuerpo51']['numero_recibo'];
   			$deuda_vigente	    = $this->Formato1($this->data['shp910_cuerpo51']['deuda_vigente']);
   			if($this->data['shp910_cuerpo51']['recargo']!=null){
   				$recargo			= $this->Formato1($this->data['shp910_cuerpo51']['recargo']);
   			}else{
				$recargo=0;
   			}

   			if($this->data['shp910_cuerpo51']['multa']!=null){
   				$multa			= $this->Formato1($this->data['shp910_cuerpo51']['multa']);
   			}else{
				$multa=0;
   			}

   			if($this->data['shp910_cuerpo51']['intereses']!=null){
   				$intereses			= $this->Formato1($this->data['shp910_cuerpo51']['intereses']);
   			}else{
				$intereses=0;
   			}
			if($this->data['shp910_cuerpo51']['descuentos']!=null){
   				$descuentos			= $this->Formato1($this->data['shp910_cuerpo51']['descuentos']);
   			}else{
				$descuentos=0;
   			}
   			$total 				= $this->Formato1($this->data['shp910_cuerpo51']['total']);
   			$cancelado 			= $this->data['shp910_cuerpo51']['cancelado'];
...
function buscar_detalles($ano){
	...
	if($datos3 !=null){
		$a=$this->Formato2($datos3[0]['shd910_cuerpo']['deuda_ano_anterior']);
	}else{
		$a='';
	}
	echo'<script>';
       echo" document.getElementById('deuda_anterior').value          = '".$a."'; ";
	echo'</script>';
