<?php
 class Shp000ControlActualizacionPlanillaController extends AppController{
	var $uses = array('shd000_arranque','shd000_control_actualizacion','cugd05_restriccion_clave');
 	var $helpers = array('Html','Ajax','Javascript', 'Sisap');
 	var $name = "shp000_control_actualizacion_planilla";
...
$this->verifica_entrada('70');
	...
	if($ver!=null){
		...
		$sql2="select * FROM v_shd000_control_arranque_cierre WHERE ".$this->SQLCA()." and ano_actualizado=".$ver[0][0]['ano_arranque']." and mes_actualizado=".$ver[0][0]['mes_arranque']." order by cod_ingreso asc";
...
function entrar(){
	...
	if(isset($this->data['cscp02_solicitud_numero']['login']) && isset($this->data['cscp02_solicitud_numero']['password'])){
		...
		$cond=$this->SQLCA()." and username='".$user."' and cod_tipo=70 and clave='".$paswd."'";
		if(($user==$l && $paswd==$c)){
			$this->set('autor_valido',true);
