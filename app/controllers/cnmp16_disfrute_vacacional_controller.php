<?php
class Cnmp16DisfruteVacacionalController extends AppController{

	var $name = 'cnmp16_disfrute_vacacional';
 	var $uses = array('cnmd15_disfrute_vaca_vacaciones','Cnmd01');
...
function escala_inputs($var=null){
	$this->layout="ajax";
	$v=$this->cnmd15_disfrute_vaca_vacaciones->execute("SELECT * FROM cnmd15_disfrute_vaca_vacaciones WHERE ".$this->SQLCA()." and cod_tipo_nomina=".$var." ORDER BY escala DESC");
			if($v!=null){
				$escala=$v[0][0]["escala"];
				$escala = $escala =="" ? 1 : $escala+1;
				//$x=$this->cnmd15_bono_vaca->execute("SELECT * FROM cnmd15_bono_vaca WHERE ".$this->SQLCA()." and cod_tipo_nomina=".$var." ORDER BY desde_antiguedad DESC");
...
function grilla($var=null){
	$this->layout="ajax";
	$datos=$this->cnmd15_disfrute_vaca_vacaciones->FindAll($this->SQLCA()." and cod_tipo_nomina=".$var,null,' ORDER BY escala ASC');
...
function guardar(){
	...
	$escala=$this->data["cnmp15_disfrute"]["escala"];
	$dia1=$this->data["cnmp15_disfrute"]["dia1"];
	$mes1=$this->data["cnmp15_disfrute"]["mes1"];
	$ano1=$this->data["cnmp15_disfrute"]["ano1"];
	$dia2=$this->data["cnmp15_disfrute"]["dia2"];
	$mes2=$this->data["cnmp15_disfrute"]["mes2"];
	$ano2=$this->data["cnmp15_disfrute"]["ano2"];
	$desde=$this->data["cnmp15_disfrute"]["desde"];
	$hasta=$this->data["cnmp15_disfrute"]["hasta"];
	$dias = $this->Formato1($this->data["cnmp15_disfrute"]["dias"]);
	...
	if($escala!="" && $dia1!="" && $mes1!="" && $ano1!="" && $dia2!="" && $mes2!="" && $ano2!="" && $desde!="" && $hasta!="" && $dias!=""){
			$sql_insert = "INSERT INTO cnmd15_disfrute_vaca_vacaciones VALUES(".$this->verifica_SS(1).", ".$this->verifica_SS(2).", ".$this->verifica_SS(3).",".$this->verifica_SS(4).",".$this->verifica_SS(5).",".$this->Session->read('nomina').",".$escala.",".$dia1.",".$mes1.",".$ano1.",".$dia2.",".$mes2.",".$ano2.",".$desde.",".$hasta.",".$dias.",".$basico.",".$descuento.")";
			$sw1 = $this->cnmd15_disfrute_vaca_vacaciones->execute($sql_insert);
			if($sw1>1){
	...
	$datos=$this->cnmd15_disfrute_vaca_vacaciones->FindAll($this->SQLCA()." and cod_tipo_nomina=".$this->Session->read('nomina'),null,' ORDER BY escala ASC');
...
function eliminar(){
	$this->layout="ajax";
	//echo $nomina=$this->data["cnmp15_disfrute"]["cod_nomina"];
	$nomina=$this->Session->read('nomina');
	if($nomina!=""){
		if($this->cnmd15_disfrute_vaca_vacaciones->FindAll($this->SQLCA()." and cod_tipo_nomina=".$nomina)){
			$sw1=$this->cnmd15_disfrute_vaca_vacaciones->execute("delete from cnmd15_disfrute_vaca_vacaciones where ".$this->SQLCA()." and cod_tipo_nomina=".$nomina);
			if($sw1>1){
		...
		$this->set('errorMessage','debe seleccionar el codogo a eliminar');
	}
	echo "<script>";
...
function modificar($nomina=null,$escala=null,$desde_dia=null,$desde_mes=null,$desde_ano=null,$hasta_dia=null,$hasta_mes=null,$hasta_ano=null,$desde_antiguedad=null,$hasta_antiguedad=null,$dias=null,$i=null,$basicos=null,$descuentos=null){
	...
	$sq_datos_campos = $this->cnmd15_disfrute_vaca_vacaciones->execute("SELECT escala, hasta_dia, hasta_mes, hasta_ano FROM cnmd15_disfrute_vaca_vacaciones WHERE ".$this->SQLCA()." AND cod_tipo_nomina=".$nomina." AND escala = (SELECT (MAX(escala)-1) FROM cnmd15_disfrute_vaca_vacaciones) LIMIT 1;");
...
function guardar_modificar($nomina=null,$escala=null,$desde_dia=null,$desde_mes=null,$desde_ano=null,$hasta_dia=null,$hasta_mes=null,$hasta_ano=null,$desde_antiguedad=null,$hasta_antiguedad=null,$diass=null,$i=null,$basicomo=null,$descuentomo=null){
	...
	$escala=$this->data["cnmp15_disfrute"]["escalas"];
	$dia1=$this->data["cnmp15_disfrute"]["dias1"];
	$mes1=$this->data["cnmp15_disfrute"]["mess1"];
	$ano1=$this->data["cnmp15_disfrute"]["anos1"];
	$dia2=$this->data["cnmp15_disfrute"]["dias2"];
	$mes2=$this->data["cnmp15_disfrute"]["mess2"];
	$ano2=$this->data["cnmp15_disfrute"]["anos2"];
	$desde=$this->data["cnmp15_disfrute"]["desdes"];
	$hasta=$this->data["cnmp15_disfrute"]["hastas"];
	$dias = $this->Formato1($this->data["cnmp15_disfrute"]["diass"]);
	$basicoa=$this->data["cnmp15_bono"]["basicos"];
	$descuentoa=$this->data["cnmp15_bono"]["descuentos"];

	if($escala!="" && $dia1!="" && $mes1!="" && $ano1!="" && $dia2!="" && $mes2!="" && $ano2!="" && $desde!="" && $hasta!="" && $dias!=""){
		$sw=$this->cnmd15_disfrute_vaca_vacaciones->execute("update cnmd15_disfrute_vaca_vacaciones set hasta_dia=".$dia2." , hasta_mes=".$mes2." , hasta_ano=".$ano2." , dias=".$dias." , basico=".$basicoa." , descuento=".$descuentoa." where ".$this->SQLCA()." and cod_tipo_nomina=".$nomina." and escala=".$escala);
		if($sw>1){
		...
		$datos=$this->cnmd15_disfrute_vaca_vacaciones->FindAll($this->SQLCA()." and cod_tipo_nomina=".$nomina,null,' ORDER BY escala ASC');
		$this->set('datos',$datos);
	}else{
		$this->set('errorMessage','Debe ingresar datos en todos los campos');
		$datos=$this->cnmd15_disfrute_vaca_vacaciones->FindAll($this->SQLCA()." and cod_tipo_nomina=".$nomina,null,' ORDER BY escala ASC');
