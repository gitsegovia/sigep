...
class Cnmp10AsignacionComunesPorcentajeSexoController extends AppController {
   var $name = 'cnmp10_asignacion_comunes_porcentaje_sexo';
   var $uses = array('Cnmd01',  'cnmd03_transacciones', 'cnmd10_control_escenarios','cnmd10_comunes_asignacion_porcentaje_sexo');
...
function otras_nominas($var1=null,$var2=null){
	...
	$cod_dep                  =       $this->Session->read('SScoddep');
	$sql='';
	$datos=$this->cnmd10_comunes_asignacion_porcentaje_sexo->findAll($this->condicion()." and cod_tipo_transaccion=1 and cod_tipo_nomina=".$var1,null,null);
			 if($datos){
			 	foreach($datos as $x){
			 		$transaccion=$x['cnmd10_comunes_asignacion_porcentaje_sexo']['cod_transaccion'];
			 		if($sql==''){
			 			$sql.=$transaccion;
			 		}else{
			 			$sql.=",".$transaccion;
...
function datos($cod_nomina=null){
	...
	if($cod_nomina != null){
		...
		$this->set('deno_trans', $deno_trans);
		$datos = $this->cnmd10_comunes_asignacion_porcentaje_sexo->findAll($conditions = $this->condicion()." and cod_tipo_nomina='$cod_nomina'",null, $order = 'cod_tipo_nomina, cod_transaccion', $limit = null, $page = null, $recursive = null);
...
function guardar(){
    ...
    $cod_inst = $this->Session->read('SScodinst');
    $cod_dep = $this->Session->read('SScoddep');
    $cod_tipo_nomina = $this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['cod_nomina'];
    $cod_tipo_transaccion = 1;
    $cod_transaccion = $this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['cod_trans'];
    $cod_frecuencia =$this->Session->read('frecuencia');
	$cod_condicion = $this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['condicion'];
	$montof = $this->porcentaje_5_2($this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['monto_f']);
	$montom = $this->porcentaje_5_2($this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['monto_m']);
    $ubicacion_escenario = strtoupper('Asignaciones Comunes - en Porcentaje segun el sexo');
	$prueba=$this->cnmd10_comunes_asignacion_porcentaje_sexo->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion,null,null);

	if(!$prueba){
		if($montof==0 && $montom==0){
			$this->set('errorMessage', 'Verifique que los montos sean validos');
			return;
		}else{
			if  (!empty($this->data['cnmp10_asignacion_comunes_porcentaje_sexo'])){

				if(empty($this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['tipo_trans'])){
					$codi_tipo_transaccion = 0;
				}else{
					$codi_tipo_transaccion = $this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['tipo_trans'];
				}

			if($cod_condicion==1){
				$codi_transaccion = 0;
			}else{
				if(empty($this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['select4'])){
					...
					echo'</script>';
					 ...
					 $codi_transaccion = $this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['select4'];
				}

			}

				if(empty($this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['escenario'])){
					$activar_frecuencia_eventual = 2;
				}else{
					$activar_frecuencia_eventual = $this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['escenario'];
				}

				$sql_insert = "INSERT INTO cnmd10_comunes_asignacion_porcentaje_sexo VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_tipo_transaccion', '$cod_transaccion', '$montof','$montom', '$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
				$sw = $this->cnmd10_comunes_asignacion_porcentaje_sexo->execute($sql_insert);
				if($sw > 1){
					...
					$sw = $this->cnmd10_comunes_asignacion_porcentaje_sexo->execute($sql_control);
					echo'<script>';
			...
			$this->data['cnmp10_asignacion_comunes_porcentaje_sexo'] = array();
			}//FIN ELSE VALIDA MONTO CERO
	}else{
		$this->set('errorMessage', 'estos datos ya existen registrados');
	}

	$datos=$this->cnmd10_comunes_asignacion_porcentaje_sexo->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1",null,'order by  cod_transaccion asc');
	 if($datos){
	 	 $this->set('datos',$datos);
	 }else{
	 	 $this->set('datos','');
...
function modificar(){
	...
	$a=0;
    $cod_tipo_nomina = $this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['cod_nomina'];
	$cod_transaccion = $this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['cod_trans_grilla'];
	$monto_f = $this->porcentaje_5_2($this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['monto_f']);
	$monto_m = $this->porcentaje_5_2($this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['monto_m']);
    $cod_condicion = $this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['condicion'];
	...
	if($monto_f==0 && $monto_m==0){
		$this->set('errorMessage', 'POR FAVOR INGRESE PORCENTAJES VALIDOS');
		echo'<script>';
		   echo"document.getElementById('transaccion').value = ''; ";
		   echo"document.getElementById('denominacion').value = ''; ";
		   echo"document.getElementById('monto').value = ''; ";
		...
		if($a!=1){
			$this->cnmd10_comunes_asignacion_porcentaje_sexo->execute("update cnmd10_comunes_asignacion_porcentaje_sexo set porcentaje_femenino=".$monto_f.",porcentaje_masculino=".$monto_m.",cod_condicion=".$this->data['cnmp10_asignacion_comunes_porcentaje_sexo']['condicion'].",codi_tipo_transaccion=".$codi_tipo_transaccion.",codi_transaccion=".$codi_transaccion.",activar_frecuencia_eventual=".$activar_frecuencia_eventual." where ".$this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion);///modifica el monto
...
function eliminar($cod_nomina=null, $cod_trans=null){
	...
	if($cod_nomina!=null && $cod_trans!=null){
		$sql_eliminar="DELETE FROM cnmd10_comunes_asignacion_porcentaje_sexo WHERE ".$this->condicion()." and cod_tipo_nomina='$cod_nomina' and cod_tipo_transaccion=1 and cod_transaccion='$cod_trans'";
		$sw = $this->cnmd10_comunes_asignacion_porcentaje_sexo->execute($sql_eliminar);
		if($sw>1){
			...
			$sw = $this->cnmd10_comunes_asignacion_porcentaje_sexo->execute($sql_del_control);
...
function ver($cod_nomina=null, $cod_trans=null, $codi=null,$condicion=null){
	$this->layout="ajax";
	if($cod_nomina!=null && $cod_trans!=null){
		 $datos2 = $this->cnmd10_comunes_asignacion_porcentaje_sexo->findAll($this->condicion()." and cod_tipo_nomina=".$cod_nomina,null,'order by  cod_transaccion asc');
		...
		$deno_trans = $this->cnmd03_transacciones->field('cnmd03_transacciones.denominacion', $conditions = "cnmd03_transacciones.cod_transaccion='$cod_trans' and cod_tipo_transaccion=1", $order ="cod_transaccion ASC");
		$this->set('deno_trans', $deno_trans);
		$opciones= $this->cnmd10_comunes_asignacion_porcentaje_sexo->findAll($conditions = $this->condicion()." and cod_tipo_nomina='$cod_nomina' and cod_transaccion='$cod_trans'", $order = null, $limit = null, $page = null, $recursive = null);
		$this->set('opciones', $opciones);
...
function transferir($var1=null){
	$this->layout="ajax";
		$carga=$this->cnmd10_comunes_asignacion_porcentaje_sexo->findAll($this->condicion()." and cod_tipo_nomina=".$var1." and cod_tipo_transaccion=1");
		if($carga){
...
function guardar_transferir(){
	$this->layout="ajax";
	  ...
	  $bandera=0;
	  $bandera=0;
	 $data=$this->cnmd10_comunes_asignacion_porcentaje_sexo->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1");
	 $ubicacion_escenario = strtoupper('Asignaciones Comunes - en Porcentaje segun el sexo');
	  foreach($data as $row){
			$cod_transaccion1 = $row['cnmd10_comunes_asignacion_porcentaje_sexo']['cod_transaccion'];
			...
			$cod_frecuencia = $row['cnmd10_comunes_asignacion_porcentaje_sexo']['cod_frecuencia'];
			$cod_condicion = $row['cnmd10_comunes_asignacion_porcentaje_sexo']['cod_condicion'];
			$codi_tipo_transaccion = $row['cnmd10_comunes_asignacion_porcentaje_sexo']['codi_tipo_transaccio'];
			$codi_transaccion = $row['cnmd10_comunes_asignacion_porcentaje_sexo']['codi_transaccion'];
			$activar_frecuencia_eventual = $row['cnmd10_comunes_asignacion_porcentaje_sexo']['activar_frecuencia_e'];
			...
			if(!$datos){
				$sql_insert = "INSERT INTO cnmd10_comunes_asignacion_porcentaje_sexo VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_transferir', '$cod_tipo_transaccion', '$cod_transaccion1','$monto_f','$monto_m','$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
				$sw = $this->cnmd10_comunes_asignacion_porcentaje_sexo->execute($sql_insert);
				if($sw>1){
				echo "<script>";
