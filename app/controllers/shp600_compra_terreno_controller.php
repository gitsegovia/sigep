<?php
 ...
 class shp600CompraTerrenoController extends AppController{
 	var $name = "shp600_compra_terreno";
	var $uses = array('v_shd600_compra_terreno','shd600_compra_terreno','v_shd600_aprobacion_arrendamiento','shd600_aprobacion_arrendamiento','v_shd600_solicitud_arrendamiento','shd600_solicitud_arrendamiento','v_shd400_propiedad','catd02_ficha_tipologia','catd02_ficha_datos','v_catd02_ficha_datos','shd400_propiedad','shd300_propaganda','v_shd100_patente_actividades','v_shd100_patente','v_shd100_solicitud','shd100_patente','shd001_registro_contribuyentes','shd100_patente_actividades','shd100_solicitud',
...
function index($var=null){
	$this->layout = "ajax";
 	$this->data = null;
 	$rif_cedula 	= $this->shd002_cobradores->generateList(null, 'rif_ci ASC', null, '{n}.shd002_cobradores.rif_ci', '{n}.shd002_cobradores.nombre_razon');
...
$this->layout="ajax";
    ...
    if($var3==null){$var2 = strtoupper($var2);
					...
					$cond_p = "SELECT a.numero_solicitud, a.rif_cedula, a.nombre_razon, a.cod_ficha FROM v_shd600_aprobacion_arrendamiento a WHERE NOT EXISTS (SELECT b.numero_solicitud FROM v_shd600_compra_terreno b WHERE b.cod_presi = a.cod_presi and b.cod_entidad = a.cod_entidad and b.cod_tipo_inst = a.cod_tipo_inst and b.cod_inst = a.cod_inst and b.cod_dep = a.cod_dep and b.rif_cedula = a.rif_cedula and b.numero_solicitud = a.numero_solicitud) and
...
function num_ficha1($rif_cedula=null,$numero_solicitud=null,$cod_ficha=null){
	$this->layout="ajax";
...
function num_fichax2($numero=null){
	$this->layout="ajax";
	$numero=strtoupper($numero);
	$datos1=$this->v_shd600_aprobacion_arrendamiento->findAll($this->SQLCA()." and numero_solicitud='".$numero."'");
	if($datos1 != null){//echo 'si';
	//$cod_ficha=$datos1[0]['shd600_solicitud_arrendamiento']['cod_ficha'];
	//$datos=$this->catd02_ficha_datos->findAll($this->SQLCA()." and cod_ficha='".$cod_ficha."'");
...
function guardar(){
  ...
  $cod_inst 			= $cod_inst;
  $cod_dep 				= $cod_dep;
  $rif_cedula   				= $this->data['shp600_compra_terreno']['rif_constribuyente'];
  $numero_solicitud 	= $this->data['shp600_compra_terreno']['numero_solicitud'];
  $fecha_compra 		= $this->data['shp600_compra_terreno']['fecha_compra'];
  $datos_compra 		= $this->data['shp600_compra_terreno']['datos_venta'];
  $monto 				= $this->Formato1($this->data['shp600_compra_terreno']['monto_venta']);
  $terreno_vendido 		= $this->data['shp600_compra_terreno']['terreno_vendido'];
	$SQL_INSERT ="INSERT INTO shd600_compra_terreno (cod_presi, cod_entidad, cod_tipo_inst, cod_inst, cod_dep,rif_cedula, numero_solicitud, fecha_compra, datos_compra, monto)";
	$SQL_INSERT .=" VALUES ($cod_presi, $cod_entidad, $cod_tipo_inst, $cod_inst, $cod_dep,'".$rif_cedula."', '".$numero_solicitud."', '".$fecha_compra."', '".$datos_compra."', $monto)";
	$cond=$this->SQLCA();
	$update_aprobacion="update shd600_aprobacion_arrendamiento set terreno_vendido=1, pago_todo=1, suspendido=1 where numero_solicitud = '".$numero_solicitud."' and rif_cedula='".$rif_cedula."' and $cond";

	$sw = $this->shd600_compra_terreno->execute($SQL_INSERT);
		if($sw>1){
			$sww = $this->shd600_aprobacion_arrendamiento->execute($update_aprobacion);
...
function consultar($pagina=null){
         ...
         if(isset($pagina) && $pagina!=null){
          	 ...
          	 $this->set('pagina',$pagina);
          	  $Tfilas=$this->v_shd600_compra_terreno->findCount($this->SQLCA());
          	 if($Tfilas==0){
          	 ...
          	 if($Tfilas!=0){
          	 $this->set('pag_cant',$pagina.'/'.$Tfilas);
          	 $datos2=$this->v_shd600_compra_terreno->findAll($this->SQLCA(),null,'rif_cedula,numero_solicitud ASC',1,$pagina,null);
			 $rif_cedula = $datos2[0]['v_shd600_compra_terreno']['rif_cedula'];
          	 $cod_ficha = $datos2[0]['v_shd600_compra_terreno']['cod_ficha'];
...
function modificar($rif_cedula=null,$numero_solicitud=null,$pagina=null){
	$this->layout = "ajax";
	$datos2=$this->v_shd600_compra_terreno->findAll($this->SQLCA()." and numero_solicitud='".$numero_solicitud."' and rif_cedula='".$rif_cedula."'");
	$rif_cedula = $datos2[0]['v_shd600_compra_terreno']['rif_cedula'];
    $cod_ficha = $datos2[0]['v_shd600_compra_terreno']['cod_ficha'];
    $datos=$this->catd02_ficha_datos->findAll("cedula_rif='".$rif_cedula."' and cod_ficha=$cod_ficha and ".$this->SQLCA());
    $this->set('datos2',$datos2);
...
function guardar_modificar($rif_cedula=null,$numero_solicitud=null,$pagina=null){
	...
	$fecha_compra 		= $this->data['shp600_compra_terreno']['fecha_compra'];
  	...
  	$monto 				= $this->Formato1($this->data['shp600_compra_terreno']['monto_venta']);
	$cond=$this->SQLCA();
	$guardar="update shd600_compra_terreno set fecha_compra='".$fecha_compra."', datos_compra='".$datos_compra."', monto=$monto where numero_solicitud='".$numero_solicitud."' and rif_cedula='".$rif_cedula."' and $cond";
	$sw = $this->shd600_compra_terreno->execute($guardar);
		if($sw>1){
...
function eliminar($rif_cedula=null,$numero_solicitud=null,$pagina=null){
 	...
 	$update_aprobacion="update shd600_aprobacion_arrendamiento set terreno_vendido=2, pago_todo=2, suspendido=2 where ". $cond;
	$sw2 = $this->shd600_aprobacion_arrendamiento->execute($update_aprobacion);
 	$this->shd600_compra_terreno->execute("DELETE FROM shd600_compra_terreno  WHERE ".$cond);
 	$y=$this->shd600_compra_terreno->findCount($this->SQLCA());
 	if($pagina>$y){
 		$pagina=$pagina-1;
 	}
 	if($y!=0){
...
function fecha_venta($var=null, $var2=null, $var3=null, $var4=null){$this->layout = "ajax";
		echo "<script>";
	    	echo "document.getElementById('fecha_venta').value='".$var."/".$var2."/".$var3."';   ";
		echo "</script>";
}
...
$this->layout="ajax";
    if($var3==null){$var2 = strtoupper($var2);
					$this->Session->write('pista', $var2);
					$Tfilas=$this->v_shd600_compra_terreno->findCount($this->SQLCA()." and ((rif_cedula LIKE '%$var2%') or (numero_solicitud LIKE '%$var2%') or (quitar_acentos(nombre_razon) LIKE quitar_acentos('%$var2%')))");
					        if($Tfilas!=0){
					        	$pagina=1;
					        	$Tfilas=(int)ceil($Tfilas/50);
					        	$this->set('pag_cant',$pagina.'/'.$Tfilas);
								...
								$this->set('pagina_actual',$pagina);
								$this->set('ultimo',$Tfilas);
					     	    $datos_filas=$this->v_shd600_compra_terreno->findAll($this->SQLCA()." and ((rif_cedula LIKE '%$var2%') or (numero_solicitud LIKE '%$var2%') or (quitar_acentos(nombre_razon) LIKE quitar_acentos('%$var2%')))",null,"rif_cedula ASC",50,1,null);
						        $this->set("datosFILAS",$datos_filas);
						        $this->set('siguiente',$pagina+1);
								$this->set('anterior',$pagina-1);
								$this->bt_nav($Tfilas,$pagina);
						...
						$Tfilas=$this->v_shd600_compra_terreno->findCount($this->SQLCA()." and ((rif_cedula LIKE '%$var2%') or (numero_solicitud LIKE '%$var2%') or (quitar_acentos(nombre_razon) LIKE quitar_acentos('%$var2%')))");
						        if($Tfilas!=0){
									...
									$this->set('pagina_actual',$pagina);
									$this->set('ultimo',$Tfilas);
							        $datos_filas=$this->v_shd600_compra_terreno->findAll($this->SQLCA()." and ((rif_cedula LIKE '%$var2%') or (numero_solicitud LIKE '%$var2%') or (quitar_acentos(nombre_razon) LIKE quitar_acentos('%$var2%')))",null,"rif_cedula ASC",50,$pagina,null);
							        $this->set("datosFILAS",$datos_filas);
							        $this->set('siguiente',$pagina+1);
									$this->set('anterior',$pagina-1);
									$this->bt_nav($Tfilas,$pagina);
...
function consulta2($rif_cedula=null,$numero_solicitud=null){
    ...
    $veri=$this->v_shd600_compra_terreno->findCount($c);
    if($veri > 0){
		$datos2=$this->v_shd600_compra_terreno->findAll($this->SQLCA()." and numero_solicitud='".$numero_solicitud."' and rif_cedula='".$rif_cedula."'");
		$rif_cedula = $datos2[0]['v_shd600_compra_terreno']['rif_cedula'];
    	$cod_ficha = $datos2[0]['v_shd600_compra_terreno']['cod_ficha'];
