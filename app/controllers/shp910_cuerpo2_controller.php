...
function agregar_grilla(){
	...
	$rif_cedula    		       	= $this->data['shp910_cuerpo2']['rif_constribuyente'];
	$ano 						= $this->data['shp910_cuerpo2']['ano_deuda'];
	$deuda_anterior				= $this->Formato1($this->data['shp910_cuerpo2']['deuda_anterior']);
	if($deuda_anterior==null){
		$deuda_anterior=0;
	}
	$cod_numero_catastral_placas= $this->data['shp910_cuerpo2']['placa'];
	...
	$cod_partida 	= $datos[0]['shd003_codigo_ingresos']['cod_partida'];
  	...
  	$cont = $this->shd910_cuerpo->findCount($this->SQLCA()." and rif_cedula='".$rif_cedula."' and ".$part);
	if($cont==0){
	 	$SQL_INSERT ="INSERT INTO shd910_cuerpo (cod_presi, cod_entidad, cod_tipo_inst, cod_inst, cod_dep, cod_partida, cod_generica,
	 	...
	 	$sw = $this->shd910_cuerpo->execute($SQL_INSERT);
	}else{
		$upd="update shd910_cuerpo set deuda_ano_anterior=$deuda_anterior where rif_cedula='".$rif_cedula."' and ".$this->SQLCA()." and ".$part;
		$sw = $this->shd910_cuerpo->execute($upd);
	}
	 	   	$mes    			= $this->data['shp910_cuerpo2']['mes'];
   			$numero_recibo     	= $this->data['shp910_cuerpo2']['numero_recibo'];
   			$deuda_vigente	    = $this->Formato1($this->data['shp910_cuerpo2']['deuda_vigente']);
   			if($this->data['shp910_cuerpo2']['recargo']!=null){
   				$recargo			= $this->Formato1($this->data['shp910_cuerpo2']['recargo']);
   			}else{
				$recargo=0;
   			}

   			if($this->data['shp910_cuerpo2']['multa']!=null){
   				$multa			= $this->Formato1($this->data['shp910_cuerpo2']['multa']);
   			}else{
				$multa=0;
   			}

   			if($this->data['shp910_cuerpo2']['intereses']!=null){
   				$intereses			= $this->Formato1($this->data['shp910_cuerpo2']['intereses']);
   			}else{
				$intereses=0;
   			}
			if($this->data['shp910_cuerpo2']['descuentos']!=null){
   				$descuentos			= $this->Formato1($this->data['shp910_cuerpo2']['descuentos']);
   			}else{
				$descuentos=0;
   			}
   			$total 				= $this->Formato1($this->data['shp910_cuerpo2']['total']);
   			$cancelado 			= $this->data['shp910_cuerpo2']['cancelado'];
			$sql ="INSERT INTO shd910_detalles (cod_presi, cod_entidad, cod_tipo_inst, cod_inst, cod_dep, cod_partida, cod_generica, cod_especifica, cod_sub_espec,
  					cod_auxiliar, rif_cedula, cod_numero_catastral_placas, ano, mes, numero_recibo, deuda_vigente, monto_recargo,
 	 				monto_multa, monto_intereses, monto_descuento, cancelado)";
			$sql.="VALUES ($cod_presi, $cod_entidad, $cod_tipo_inst, $cod_inst, $cod_dep, $cod_partida, $cod_generica, $cod_especifica, $cod_subespec,
  					...
  					$multa, $intereses, $descuentos, $cancelado);";
			$sw1 = $this->shd910_detalles->execute($sql);
...
echo'</script>';
$datos2 = $this->shd910_detalles->findAll($this->SQLCA()." and rif_cedula='".$rif_cedula."' and ano=".$ano." and ".$part);
...
function buscar_detalles($ano){
	...
	$cod_partida 	= $datos[0]['shd003_codigo_ingresos']['cod_partida'];
  	...
  	$part='cod_partida='.$cod_partida.' and cod_generica='.$cod_generica.' and cod_especifica='.$cod_especifica.' and cod_sub_espec='.$cod_subespec.' and cod_auxiliar='.$cod_auxiliar;
	$datos2 = $this->shd910_detalles->findAll($this->SQLCA()." and rif_cedula='".$rif_cedula."' and ano=".$ano." and ".$part);
	$datos3 = $this->shd910_cuerpo->findAll($this->SQLCA()." and rif_cedula='".$rif_cedula."' and ".$part);
	if($datos3 !=null){
		$a=$datos3[0]['shd910_cuerpo']['cod_numero_catastral_placas'];
		$b=$this->Formato2($datos3[0]['shd910_cuerpo']['deuda_ano_anterior']);

		echo'<script>';
       		echo" document.getElementById('placa').value           = '".$a."'; ";
       		echo" document.getElementById('deuda_anterior').value  = '".$b."'; ";
		echo'</script>';
	}else{
		echo'<script>';
       		echo" document.getElementById('placa').value           = ''; ";
       		echo" document.getElementById('deuda_anterior').value  = ''; ";
		echo'</script>';
	}
...
function eliminar_grilla($ano=null,$mes=null,$rif_cedula=null){
	...
	$cod_partida 	= $datos[0]['shd003_codigo_ingresos']['cod_partida'];
  	...
  	$cod_auxiliar 	= $datos[0]['shd003_codigo_ingresos']['cod_auxiliar'];
  	$part='cod_partida='.$cod_partida.' and cod_generica='.$cod_generica.' and cod_especifica='.$cod_especifica.' and cod_sub_espec='.$cod_subespec.' and cod_auxiliar='.$cod_auxiliar;
	$this->shd910_detalles->execute("DELETE FROM shd910_detalles  WHERE ".$this->SQLCA()." and rif_cedula='".$rif_cedula."' and ano=".$ano." and mes=".$mes." and ".$part);
 	$datos2 = $this->shd910_detalles->findAll($this->SQLCA()." and rif_cedula='".$rif_cedula."' and ano=".$ano." and ".$part);
 	$this->set('accion',$datos2);
