 ...
 * sisap
 */
class Cnmp10AsignacionIndividualDiasController extends AppController {
   var $name = 'cnmp10_asignacion_individual_dias';
   var $uses = array('Cnmd01',  'cnmd03_transacciones', 'cnmd10_control_escenarios','cnmd10_individual_dias','cnmd10_individual_dias_cantidad','cnmd06_fichas','cnmd06_datos_personales','v_cnmd05','v_cnmd06_fichas_datos_personales');
...
function verifica($cod_nomina=null, $cod_trans=null){
	...
	if($cod_trans!=null && $cod_trans!=""){
		...
		$veri=$this->cnmd10_individual_dias->findAll($this->condicion()." and cod_tipo_nomina=".$cod_nomina." and cod_transaccion=".$cod_trans);
		$this->set('opciones',$veri);
		if($veri){
			$v=$this->cnmd10_individual_dias->execute("SELECT * FROM cnmd10_individual_dias WHERE ".$this->condicion()." and cod_tipo_nomina=".$cod_nomina." and cod_transaccion=".$cod_trans);
	...
	if($cod!=""){
		switch($var){
			...
			case 'cedula':
				...
				$ver1=$this->cnmd10_individual_dias->FindAll($this->SQLCA()." and cod_tipo_nomina=".$nom." and cod_transaccion=".$trans);
				if(!$ver1){
					echo "<script>";
						echo "document.getElementById('agregar').disabled='disabled';";
					echo "</script>";
				}else{
					$ver=$this->cnmd10_individual_dias_cantidad->FindAll($cond." and cod_transaccion=".$trans);
					if($ver){
						echo "<script>";
							echo "document.getElementById('agregar').disabled='disabled';";
						echo "</script>";
...
function grilla($var=null,$var2=null){
	$this->layout="ajax";
	if($var2!=""){
			$datos=$this->cnmd10_individual_dias_cantidad->FindAll($this->SQLCA()." and cod_tipo_nomina=".$var." and cod_transaccion=".$var2." ORDER BY cod_ficha ASC");
			if($datos){
...
function guardar_items(){
	$this->layout="ajax";
	    ...
	    $cod_inst = $this->Session->read('SScodinst');
	    $cod_dep = $this->Session->read('SScoddep');
	    $cod_tipo_nomina = $this->data['cnmp10_asignacion_individual_dias']['cod_nomina'];
	    $cod_tipo_transaccion = 1;
	    $cod_transaccion = $this->data['cnmp10_asignacion_individual_dias']['cod_trans'];
		$cod_cargo = $this->data['cnmp10_asignacion_individual_dias']['select_cargo'];
		$cod_ficha =$this->data['cnmp10_asignacion_individual_dias']['select_ficha'];
//		$cantidad = $this->porcentaje_5_2($this->data['cnmp10_asignacion_individual_dias']['cantidad1']);
		$cantidad = $this->Formato1($this->data['cnmp10_asignacion_individual_dias']['cantidad1']);

	    if($this->data['cnmp10_asignacion_individual_dias']['cantidad1']!=""){
	    	$prueba=$this->cnmd10_individual_dias->findAll($this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion,null,null);
	    		if($prueba){
	    			$prueba1=$this->cnmd10_individual_dias_cantidad->findAll($this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion." and cod_cargo=".$cod_cargo." and cod_ficha=".$cod_ficha,null,null);
	    			if(!$prueba1){
	    				$sql_insert2 = "INSERT INTO cnmd10_individual_dias_cantidad VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_cargo', '$cod_ficha','$cod_tipo_transaccion', '$cod_transaccion', '$cantidad')";
						$sw = $this->cnmd10_individual_dias_cantidad->execute($sql_insert2);
						if($sw>1){
							$datos = $this->cnmd10_individual_dias_cantidad->findAll($this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion,null,'cod_ficha ASC');
							$this->set("datos",$datos);
...
function guardar(){
	...
	$cod_condicion = $this->data['cnmp10_asignacion_individual_dias']['condicion'];
	$cod_cargo = $this->data['cnmp10_asignacion_individual_dias']['select_cargo'];
	$cod_ficha =$this->data['cnmp10_asignacion_individual_dias']['select_ficha'];
//	$cantidad = $this->porcentaje_5_2($this->data['cnmp10_asignacion_individual_dias']['cantidad1']);
	$cantidad = $this->Formato1($this->data['cnmp10_asignacion_individual_dias']['cantidad1']);
    $prueba=$this->cnmd10_individual_dias->findAll($this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion,null,null);
		...
		if(!$prueba){
			if($this->data['cnmp10_asignacion_individual_dias']['cantidad1']==0){
			  ...
			  $cod_inst = $this->Session->read('SScodinst');
			  $cod_dep = $this->Session->read('SScoddep');
			  $ubicacion_escenario = strtoupper('ASIGNACION INDIVIDUAL EN DIAS SEGUN CANTIDAD QUE LE CORRESPONDE');
			  if(!empty($this->data['cnmp10_asignacion_individual_dias'])){
					if(empty($this->data['cnmp10_asignacion_individual_dias']['tipo_trans'])){
						$codi_tipo_transaccion = 0;
					}else{
						$codi_tipo_transaccion = $this->data['cnmp10_asignacion_individual_dias']['tipo_trans'];
					}

					if($cod_condicion==1){
						$codi_transaccion = 0;
					}else{
						if(empty($this->data['cnmp10_asignacion_individual_dias']['select4'])){
							 $this->set('errorMessage', 'Debe seleccionar el codigo de transaccion');
						}else{
							 $codi_transaccion = $this->data['cnmp10_asignacion_individual_dias']['select4'];
						}//empty
					}//fin cod_condicion


					if(empty($this->data['cnmp10_asignacion_individual_dias']['escenario'])){
						$activar_frecuencia_eventual = 2;
					}else{
						$activar_frecuencia_eventual = $this->data['cnmp10_asignacion_individual_dias']['escenario'];
					}//fin empty

					$sql_insert = "INSERT INTO cnmd10_individual_dias VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_tipo_transaccion', '$cod_transaccion','$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
					$sw = $this->cnmd10_individual_dias->execute($sql_insert);
					if($sw>1){
						$sql_insert2 = "INSERT INTO cnmd10_individual_dias_cantidad VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_cargo', '$cod_ficha','$cod_tipo_transaccion', '$cod_transaccion', '$cantidad')";
						$sw1 = $this->cnmd10_individual_dias_cantidad->execute($sql_insert2);
						if($sw1 > 1){
							...
							$sw = $this->cnmd10_individual_dias_cantidad->execute($sql_control);
								echo "<script>";
									echo "document.getElementById('cedula').value='';";
									echo "document.getElementById('primer_ape').value='';";
									echo "document.getElementById('segudo_ape').value='';";
		...
		$this->set('errorMessage',' ESTA TRANSACCION YA EXISTE REGISTRADA');
	}
	 $datos=$this->cnmd10_individual_dias_cantidad->findAll($this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion,null,null);
	 if($datos){
	 	 $this->set('datos',$datos);
	 	 $grilla=$this->v_cnmd06_fichas_datos_personales->FindAll($this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina);
		 $this->set('grilla',$grilla);
...
function modificar(){
	...
	$cod_tipo_nomina = $this->data['cnmp10_asignacion_individual_dias']['cod_nomina'];
	$cod_transaccion = $this->data['cnmp10_asignacion_individual_dias']['cod_trans'];
//	$cod_frecuencia = $this->data['cnmp10_asignacion_individual_dias']['frecuencia'];
	$cod_condicion = $this->data['cnmp10_asignacion_individual_dias']['condicion'];
	$a=0;
	if(empty($this->data['cnmp10_asignacion_individual_dias']['tipo_trans'])){
		$codi_tipo_transaccion = 0;
...
function eliminar($cod_nomina=null, $cod_trans=null,$cargo=null,$ficha=null){
	$this->layout="ajax";
		...
		if(isset($cod_nomina) && isset($cod_trans) && isset($cargo) && isset($ficha)){
			$sql_eliminar2="DELETE FROM cnmd10_individual_dias_cantidad WHERE ".$this->SQLCA()." and cod_tipo_nomina='$cod_nomina' and cod_tipo_transaccion=1 and cod_transaccion='$cod_trans' and cod_cargo='$cargo' and cod_ficha='$ficha'";
			$sw2 = $this->cnmd10_individual_dias_cantidad->execute($sql_eliminar2);
			if($sw2 > 1){
		    ...
		    $cod_transaccion = $this->data['cnmp10_asignacion_individual_dias']['cod_trans'];
			 ...
			 $verificar=$this->cnmd10_individual_dias->findAll($this->SQLCA()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1 and cod_transaccion=".$cod_transaccion);
			if($verificar!=null){
				if(($cod_tipo_nomina!="") && ($cod_transaccion!="")){
					$sql_eliminar="DELETE FROM cnmd10_individual_dias WHERE ".$this->SQLCA()." and cod_tipo_nomina='$cod_tipo_nomina' and cod_tipo_transaccion=1 and cod_transaccion='$cod_transaccion'";
					$sw = $this->cnmd10_individual_dias->execute($sql_eliminar);
					if($sw>1){
						...
						$sw = $this->cnmd10_individual_dias->execute($sql_del_control);
...
function transferir($var1=null,$var2=null){
	$this->layout="ajax";
		$carga=$this->cnmd10_individual_dias->findAll($this->condicion()." and cod_tipo_nomina=".$var1." and cod_tipo_transaccion=1 and cod_transaccion=".$var2);
		if($carga){
	...
	$datos=$this->cnmd10_individual_dias->findAll($this->SQLCA()." and cod_tipo_transaccion=1 and cod_transaccion=".$var2." and cod_tipo_nomina!=".$var1,null,null);
			 if($datos){
			 	 $this->set('opciones',$datos);
			 }else{
			 	 $this->set('opciones','');
...
function guardar_transferir(){
	$this->layout="ajax";
	  ...
	  $cod_tipo_nomina = $this->data['cnmp10_asignacion_individual_dias']['cod_nomina'];
	  $cod_transaccion = $this->data['cnmp10_asignacion_individual_dias']['cod_trans'];
	  $cod_transferir = $this->data['cnmp10_asignacion_individual_dias']['cod_transferir'];
	  $cod_tipo_transaccion=1;
	  ...
	  if(!$ubicacion){
	  $data=$this->cnmd10_individual_dias->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1 and cod_transaccion=".$cod_transaccion);
	  $data2=$this->cnmd10_individual_dias_cantidad->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1 and cod_transaccion=".$cod_transaccion);
	  $ubicacion_escenario = strtoupper('ASIGNACION INDIVIDUAL EN PORCENTAJE SEGUN CANTIDAD  HORAS TRABAJADAS');
	  foreach($data as $row){
			$cod_frecuencia = $row['cnmd10_individual_dias']['cod_frecuencia'];
			$cod_condicion = $row['cnmd10_individual_dias']['cod_condicion'];
			$codi_tipo_transaccion = $row['cnmd10_individual_dias']['codi_tipo_transaccion'];
			$codi_transaccion = $row['cnmd10_individual_dias']['codi_transaccion'];
			$activar_frecuencia_eventual = $row['cnmd10_individual_dias']['activar_frecuencia_eventual'];
			$porcentaje = $row['cnmd10_individual_dias']['porcentaje'];
			$sql_insert = "INSERT INTO cnmd10_individual_dias VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_transferir', '$cod_tipo_transaccion', '$cod_transaccion','$porcentaje','$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
			$sw = $this->cnmd10_individual_dias->execute($sql_insert);
			if($sw>1){
				echo "<script>";
				...
				if($sw1 > 1){
					foreach($data2 as $row){
						...
						$sql_insert2 = "INSERT INTO cnmd10_individual_dias_cantidad VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_transferir', '$cod_cargo', '$cod_ficha','$cod_tipo_transaccion', '$cod_transaccion', '$cantidad')";
						$sw2 = $this->cnmd10_individual_dias_cantidad->execute($sql_insert2);
					}// fin foreach data2
				}//fin $sw1
			}//fin $sw
	  }//fin foreach data
...
//////////////////////////////AGREGAR DESDE AQUI////////////////////
	...
	$datos=$this->cnmd10_individual_dias->findAll($this->SQLCA()." and cod_tipo_transaccion=1 and cod_transaccion=".$cod_transaccion." and cod_tipo_nomina!=".$cod_tipo_nomina,null,null);
			 if($datos){
			 	 $this->set('opciones',$datos);
			 }else{
			 	 $this->set('opciones','');
...
function seleccion_busqueda($opcion=null,$nomina=null,$cargo=null,$ficha=null){
	...
	$veri=$this->cnmd10_individual_dias->FindCount($this->condicion()." and cod_tipo_nomina=".$nomina." and cod_transaccion=".$cod_trans);
	if($veri==0){
		$disabled='disabled';
	}else{
		$datos=$this->cnmd10_individual_dias_cantidad->FindCount($this->SQLCA()." and cod_tipo_nomina=".$nomina." and cod_transaccion=".$cod_trans." and cod_cargo=".$cargo." and cod_ficha=".$ficha,null,null);
		if($datos==0){
			$disabled='';
		}else{
			$disabled='disabled';
...
function transaccion($var=null){
	...
	if($var!='guarda'){
		$nomina=$var;
		 ...
		 $nomina = $this->data['cnmp10_asignacion_individual_dias']['cod_nomina'];
	}
	$datos=$this->cnmd10_individual_dias->findAll($this->SQLCA()." and cod_tipo_transaccion=1 and cod_tipo_nomina=".$nomina,null,null);
			 if($datos){
			 	 $this->set('opciones',$datos);
			 }else{
			 	 $this->set('opciones','');
