<?php
class Cnmp01AbonoEliminaController extends AppController {
   var $name = 'cnmp01_abono_elimina';
   var $uses = array('cnmd03_transaccion','cnmd07_abono_cuenta','v_cnmd07_abono_cuenta','Cnmd01', 'cnmd05', 'cfpd02_sub_prog', 'cfpd02_proyecto', 'cfpd02_activ_obra','ccfd04_cierre_mes');
...
function radio($tipo=null,$codigo=null){
	...
	$lista = $this->v_cnmd07_abono_cuenta->generateList($this->SQLCA().' and cod_tipo_nomina='.$codigo.' and cod_tipo_transaccion_padre='.$tipo, 'cod_transaccion_padre ASC', null, '{n}.v_cnmd07_abono_cuenta.cod_transaccion_padre', '{n}.v_cnmd07_abono_cuenta.deno_transaccion');
	if($this->v_cnmd07_abono_cuenta->findCount($this->SQLCA().' and cod_tipo_nomina='.$codigo.' and cod_tipo_transaccion_padre='.$tipo)!=0){
		$this->concatena($lista, 'transa');
	}else{
...
function grilla($tipo=null,$codigo=null){
	...
	$cod=$this->SQLCA().' and cod_tipo_nomina='.$cod_tipo_nomina.' and cod_tipo_transaccion_padre='.$tipo.' and cod_transaccion_padre='.$codigo;
	$b = $this->v_cnmd07_abono_cuenta->findAll($cod,null,'cod_cargo,cod_puesto ASC');
	$this->set('datos',$b);
}
...
function procesar(){
	$this->layout = "ajax";
//	pr($this->data);
	$codigo = $this->data['cnmp01_abono_elimina']['cod_tipo_nomina'];
	$tipo = $this->data['cnmp01_abono_elimina']['tipo'];
	$cod_transa = $this->data['cnmp01_abono_elimina']['cod_transa'];
	$cod=$this->SQLCA().' and cod_tipo_nomina='.$codigo.' and cod_tipo_transaccion_padre='.$tipo.' and cod_transaccion_padre='.$cod_transa;
	$elimina='delete from cnmd07_abono_cuenta where '.$cod;
	//echo $elimina;
	$b = $this->cnmd07_abono_cuenta->execute($elimina);
...
function guardar(){
	...
	$tipo = $this->data['cnmp01_abono_elimina']['tipo'];
	$cod_transa = $this->data['cnmp01_abono_elimina']['cod_transa'];
	$operacion = $this->data['cnmp01_abono_elimina']['operacion'];
	$cuanto = $this->data['cnmp01_abono_elimina']['cuanto'];
	$cod=$this->SQLCA().' and cod_tipo_nomina='.$codigo.' and cod_tipo_transaccion_padre='.$tipo.' and cod_transaccion_padre='.$cod_transa;
	if($operacion == 1){
	$actualizar='update cnmd07_abono_cuenta set monto_cuota= monto * '.$cuanto.' where '.$cod;
	}else if($operacion == 2){
	$actualizar='update cnmd07_abono_cuenta set monto_cuota= monto / '.$cuanto.' where '.$cod;
	//echo $actualizar;
	}
