<?php
 class Caop03registrocotizacionController extends AppController{
	 var $name='caop03_registro_cotizacion';
     var $uses = array('ccfd04_cierre_mes','v_cscd03_cotizacion','cscd01_catalogo','cugd02_direccionsuperior','cpcd02','ccfd03_instalacion',
                       ...
                       'cscd02_solicitud_numero','cscd02_solicitud_encabezado','cscd02_solicitud_cuerpo','cscd03_cotizacion_encabezado','cfpd07_obras_cuerpo',
...
function index2(){
 ...
 $lista = $this->cscd02_solicitud_encabezado->generateList($condicion." and (cod_obra!='' and cod_obra!='0' and cod_obra IS NOT NULL) ", ' numero_solicitud ASC', null, '{n}.cscd02_solicitud_encabezado.numero_solicitud', '{n}.cscd02_solicitud_encabezado.numero_solicitud');
...
$cod_obra = $num["cscd02_solicitud_encabezado"]["cod_obra"];
...
function guardar(){
   ...
   $i_lenght = $this->data['caop03_registro_cotizacion']['cuenta_i'];
   ...
   $solicitud_cotizacion_ano      =   $this->data['caop03_registro_cotizacion']['solicitud_cotizacion_ano'];
   $solicitud_cotizacion_numero   =   $this->data['caop03_registro_cotizacion']['solicitud_cotizacion_numero'];
   $solicitud_cotizacion_fecha    =   $this->data['caop03_registro_cotizacion']['solicitud_cotizacion_fecha'];
   $cotizacion_ano                =   $this->data['caop03_registro_cotizacion']['cotizacion_ano'];
   $cotizacion_numero             =   $this->data['caop03_registro_cotizacion']['cotizacion_numero'];
   $cotizacion_fecha              =   $this->data['caop03_registro_cotizacion']['cotizacion_fecha'];
   $fecha_proceso                 =   date('Y-m-d');
   $rif_numero                    =   $this->data['caop03_registro_cotizacion']['rif_numero'];
   $rif_nombre                    =   $this->data['caop03_registro_cotizacion']['rif_nombre'];
   $rif_direccion                 =   $this->data['caop03_registro_cotizacion']['rif_direccion'];
...
if(!empty($solicitud_cotizacion_ano) && !empty($solicitud_cotizacion_numero) && !empty($solicitud_cotizacion_fecha) && !empty($cotizacion_ano) && !empty($cotizacion_numero) && !empty($cotizacion_fecha) && !empty($rif_numero)){
  ...
  if($solicitud_cotizacion_ano!="" && $solicitud_cotizacion_numero!="" && $solicitud_cotizacion_fecha!="" && $cotizacion_ano!="" && $cotizacion_numero!="" && $cotizacion_fecha!="" && $rif_numero!=""){
       ...
       if($this->cscd02_solicitud_encabezado->findCount($this->condicion()." and  ano_solicitud = '".$solicitud_cotizacion_ano."' and  numero_solicitud='$solicitud_cotizacion_numero' and numero_cotizacion='0' ")!=0){
							       ...
							       $sql1  = "BEGIN; INSERT INTO cscd03_cotizacion_encabezado (cod_presi, cod_entidad, cod_tipo_inst, cod_inst, cod_dep, rif, ano_cotizacion, numero_cotizacion, fecha_cotizacion, ano_solicitud, numero_solicitud, fecha_proceso, ano_ordencompra, numero_ordencompra, cod_obra) ";
							       $sql1 .= "VALUES ('".$this->Session->read('SScodpresi')."', '".$this->Session->read('SScodentidad')."', '".$this->Session->read('SScodtipoinst')."', '".$this->Session->read('SScodinst')."', '".$this->Session->read('SScoddep')."', '".$rif_numero."', '".$cotizacion_ano."', '".$cotizacion_numero."', '".$cotizacion_fecha."', '".$solicitud_cotizacion_ano."', '".$solicitud_cotizacion_numero."', '".$fecha_proceso."', '0', '0', '".$caop02_codigo_obra."');";
							 	...
							 	if($sw >1 ){
										...
										$sql2="INSERT INTO cscd03_cotizacion_cuerpo (cod_presi, cod_entidad, cod_tipo_inst, cod_inst, cod_dep, rif, ano_cotizacion, numero_cotizacion, codigo_prod_serv, descripcion, cod_medida, cantidad, precio_unitario, cod_sector, cod_programa, cod_sub_prog, cod_proyecto, cod_partida, cod_generica, cod_especifica, cod_sub_espec) VALUES ";
									  	 for($i=0; $i<$i_lenght; $i++){
									      	 $var[$i]['codigo_prod_serv']= $this->data['caop03_registro_cotizacion']['codigo_prod_serv_'.$i];
									      	 $var[$i]['cod_medida']      = $this->data['caop03_registro_cotizacion']['cod_medida_'.$i];
									      	 $var[$i]['cantidad']        = str_replace(',', '.', $this->data['caop03_registro_cotizacion']['cantidad_'.$i]);
									       	 $var[$i]['precio']          = $this->Formato_3_in($this->data['caop03_registro_cotizacion']['precio_'.$i]);
									       	 $var[$i]['descripcion']     = $this->data['caop03_registro_cotizacion']['descripcion_'.$i];
									       	 $var[$i]['cod_sector']     = $this->data['caop03_registro_cotizacion']['cod_sector_'.$i];
									       	 $var[$i]['cod_programa']     = $this->data['caop03_registro_cotizacion']['cod_programa_'.$i];
									       	 $var[$i]['cod_sub_prog']     = $this->data['caop03_registro_cotizacion']['cod_sub_prog_'.$i];
									       	 $var[$i]['cod_proyecto']     = $this->data['caop03_registro_cotizacion']['cod_proyecto_'.$i];
									       	 $var[$i]['cod_partida']     = $this->data['caop03_registro_cotizacion']['cod_partida_'.$i];
									       	 $var[$i]['cod_generica']     = $this->data['caop03_registro_cotizacion']['cod_generica_'.$i];
									       	 $var[$i]['cod_especifica']     = $this->data['caop03_registro_cotizacion']['cod_especifica_'.$i];
									       	 $var[$i]['cod_sub_espec']     = $this->data['caop03_registro_cotizacion']['cod_sub_espec_'.$i];
...
function show_rif($pista=null){
	...
	if($pista != null){
		...
		if($this->cpcd02->findCount("mayus_acentos(denominacion) LIKE '%$pista%' OR mayus_acentos(rif) LIKE '%$pista%'") > 0){
			$proveedor= $this->cpcd02->generateList($conditions = "mayus_acentos(denominacion) LIKE '%$pista%' OR mayus_acentos(rif) LIKE '%$pista%'", $order = null, $limit = null, '{n}.cpcd02.rif', '{n}.cpcd02.denominacion');
...
*/
 ...
 if(isset($_SESSION['ano_compra'])){$ano = $_SESSION['ano_compra'];}else{$ano = $this->ano_ejecucion();}
   ...
   $array = $this->v_cscd03_cotizacion->findAll($condicion.'and ano_solicitud = '.$ano, 'DISTINCT numero_solicitud, rif, numero_cotizacion, cod_obra');
 ...
 foreach($array as $aux){
 	...
 	if(isset($aux['v_cscd03_cotizacion']['cod_obra'])){
 		$numero[$i]['cod_obra'] = $aux['v_cscd03_cotizacion']['cod_obra'];
 	}
 	$i++;
 }
 $i--;
...
$numero_solicitud = $numero[$pag_num]['numero_solicitud'];
 ...
 $lista = $this->v_cscd03_cotizacion->findAll($condicion. ' and ano_solicitud = '.$ano.' and numero_solicitud = '.$numero[$pag_num]['numero_solicitud'], null, 'codigo_prod_serv DESC', null);
	 	...
	 	$cod_obra = $numero[$pag_num]['cod_obra'];
	    $this->Session->write("caop02_codigo_obra", strtoupper_sisap($cod_obra));
		$cfpd07_obras_cuerpo     =  $this->cfpd07_obras_cuerpo->findAll($this->condicion()." and ano_estimacion=".$ano." and upper(cod_obra)='".strtoupper($cod_obra)."' ");
		$denominacion_obra       = "";
		$cfpd07_obras_cuerpo_aux = $cfpd07_obras_cuerpo;
...
function buscar(){
	...
	$condicion = 'cod_presi='.$this->Session->read('SScodpresi').'  and  cod_entidad='.$this->Session->read('SScodentidad').' and cod_tipo_inst='.$this->Session->read('SScodtipoinst').' and  cod_inst='.$this->Session->read('SScodinst').' and cod_dep='.$this->Session->read('SScoddep').'';
    if(isset($_SESSION['ano_compra'])){$ano = $_SESSION['ano_compra'];}else{$ano = $this->ano_ejecucion();}
	$rif= strtoupper($this->data['caop03_registro_cotizacion']['rif']);
	$numero_cotizacion = $this->data['caop03_registro_cotizacion']['num_cotizacion']."";
	$array = $this->v_cscd03_cotizacion->findAll($condicion.'and ano_solicitud = '.$ano."", 'DISTINCT numero_solicitud, rif, numero_cotizacion, cod_obra');
	$cont = count($array);
    $i = 0;
 	$pag_num=0;
...
// 	pr($this->data['caop03_registro_cotizacion']);
...
//    echo $rif_q."=".$rif."||".$num_cotizacion_q."=".$numero_cotizacion."|| el pag_num es: ".$i."<br/>";
 	...
 	if(($rif_q == $rif) && ($num_cotizacion_q == $numero_cotizacion)){
 		echo "<script>";
...
function cotizar($rif=null){
	...
	if(isset($_SESSION['ano_compra'])){$ano = $_SESSION['ano_compra'];}else{$ano = $this->ano_ejecucion();}
	if($rif!=null){
		$lista = $this->cscd03_cotizacion_encabezado->generateList($conditions = $this->condicion()."  and ano_cotizacion='".$ano."'  and mayus_acentos(rif)=mayus_acentos('$rif') "." and (cod_obra!='' and cod_obra!='0' and cod_obra IS NOT NULL)", $order = 'numero_cotizacion ASC', $limit = null, '{n}.cscd03_cotizacion_encabezado.numero_cotizacion', '{n}.cscd03_cotizacion_encabezado.numero_cotizacion');
		//echo $conditions;
...
function eliminar($var1=null, $var2=null, $var3=null, $var4=null, $var5=null){
   ...
   $solicitud_cotizacion_ano      =   $this->data['caop03_registro_cotizacion']['solicitud_cotizacion_ano_222'];
   $solicitud_cotizacion_numero   =   $this->data['caop03_registro_cotizacion']['solicitud_cotizacion_numero_222'];
   $solicitud_cotizacion_fecha    =   $this->data['caop03_registro_cotizacion']['solicitud_cotizacion_fecha_222'];
   $cotizacion_ano                =   $this->data['caop03_registro_cotizacion']['cotizacion_ano_222'];
   $cotizacion_numero             =   $this->data['caop03_registro_cotizacion']['cotizacion_numero_222'];
   $cotizacion_fecha              =   $this->data['caop03_registro_cotizacion']['cotizacion_fecha_222'];
   $fecha_proceso                 =   date('Y-m-d');
   $rif_numero                    =   $this->data['caop03_registro_cotizacion']['rif_numero_222'];
   $rif_nombre                    =   $this->data['caop03_registro_cotizacion']['rif_nombre_222'];
   $rif_direccion                 =   $this->data['caop03_registro_cotizacion']['rif_direccion_222'];
	   ...
	   //echo $cond3."<br/>".$cont_solicitud;
		if( $cont_solicitud== 0){
			$sw = $this->cscd02_solicitud_encabezado->execute($sql_insert_solicitud_encabezado.$sql_insert_solicitud_cuerpo);
		}else{
			$sw =2;
		}
...
$i = 1 ;
         ...
         $var[$i]['codigo_prod_serv']=$this->data['caop03_registro_cotizacion']['codigo_prod_serv'];
      	 $var[$i]['cod_medida']=$this->data['caop03_registro_cotizacion']['cod_medida'];
      	 $var[$i]['cantidad']= str_replace(',', '.', $this->data['caop03_registro_cotizacion']['cantidad']);
       	 $var[$i]['precio']=$this->Formato_3_in($this->data['caop03_registro_cotizacion']['precio']);
       	 $var[$i]['descripcion']=$this->data['caop03_registro_cotizacion']['descripcion'];
       	 $var[$i]['ano_cotizacion']=$this->data['caop03_registro_cotizacion']['ano_cotizacion'];
       	 $var[$i]['numero_cotizacion']=$this->data['caop03_registro_cotizacion']['numero_cotizacion'];
       	 $numero_cotizacion = $this->data['caop03_registro_cotizacion']['numero_cotizacion'];
