 ...
 */
 ...
 class Arrp01EstadosController extends AppController{

	var $name = 'arrp01_estados';
	var $uses = array('arrd01','arrd02','Usuario');
...
function datos($cod_presi=null){
	$this->layout="ajax";
	if($cod_presi!=''){
		$datos=$this->arrd02->execute("select * from arrd02 where cod_presi='$cod_presi' order by cod_entidad asc");
		if($datos!=null){
...
function guardar(){
	...
	if(empty($this->data['arrp00']['cod_republica'])){
		...
		$this->set('errorMessage', 'Debe ingresar el código del estado');

	}else if(empty($this->data['arrp00']['denominacion'])){
		$this->set('errorMessage', 'Debe ingresar la denominación del estado');

	}else{
		$cod_presi=$this->data['arrp00']['cod_republica'];
		$codigo=$this->data['arrp00']['codigo'];
		$denominacion=$this->data['arrp00']['denominacion'];
		$datos=$this->arrd02->execute("select * from arrd02 where cod_presi='$cod_presi' and cod_entidad='$codigo' order by cod_entidad asc");
		if($datos==null){
			 $sql = "INSERT INTO arrd02 VALUES ('$cod_presi','$codigo','$denominacion')";
		   	 $sw=$this->arrd02->execute($sql);
		   	 if($sw>1){
				$this->set('Message_existe', 'REGISTRO EXITOSO');
				echo" <script> ver_documento('/arrp01_estados/datos/$cod_presi','grilla'); </script>";
	   		}else{
...
function modificar($cod_presi=null,$cod_entidad=null,$i=null){
	$this->layout="ajax";

	$datos=$this->arrd02->execute("select * from arrd02 where cod_presi='$cod_presi' and cod_entidad='$cod_entidad' order by cod_presi asc");
	$this->set('datos',$datos);
...
function guardar_modificar($cod_presi=null,$cod_entidad=null,$i=null){
	$this->layout="ajax";
	...
	if(empty($this->data['arrp00']['denominacion'.$i])){
		...
		$denominacion=$this->data['arrp00']['denominacion'.$i];
		 ...
		 $sql = "update arrd02 set denominacion='".$denominacion."' where cod_presi='$cod_presi' and cod_entidad='$cod_entidad'";
	   	 $sw=$this->arrd02->execute($sql);
	   	 if($sw>1){
...
function eliminar($cod_presi=null,$cod_entidad=null){
	$this->layout="ajax";

	 $sql = "delete from  arrd02 where cod_presi='$cod_presi' and cod_entidad='$cod_entidad'";
   	 $sw=$this->arrd02->execute($sql);
   	 if($sw>1){
