<?php
 ...
 class Caop01CompromisoController extends AppController{
 	...
 	var $uses = array('ccfd04_cierre_mes','v_cfpd05_denominaciones','cfpd21','cfpd21_numero_asiento_compromiso',
                      ...
                      'cepd03_ordenpago_cuerpo', 'cugd02_direccion', 'v_cscd01_catalogo_deno_und','cobd01_contratoobras_cuerpo','cfpd07_obras_cuerpo',
...
$this->layout="ajax";
    ...
    $sql = $this->SQLCA()." and ano_documento=".$year_buscar." and cod_obra!=''";
...
function index2($var=null){///////////////<<--INDEX 2
	...
	// *** SECRETARIA ****
      ...
      if(isset($var)){
	    ...
	    $SScoddep                 =       $this->Session->read('SScoddep');
	    $Modulo                   =       $this->Session->read('Modulo');
		$condicion = 'cod_presi='.$this->Session->read('SScodpresi').'  and  cod_entidad='.$this->Session->read('SScodentidad').' and cod_tipo_inst='.$this->Session->read('SScodtipoinst').' and  cod_inst='.$this->Session->read('SScodinst').' and cod_dep='.$SScoddep.' and ano_estimacion='.$ano;
	    $lista_ob = $this->cfpd07_obras_cuerpo->generateList($condicion.' and ano_estimacion='.$ano.'   and (estimado_presu - ((monto_contratado+ aumento_obras) - disminucion_obras)) != 0     ', ' cod_obra ASC', null, '{n}.cfpd07_obras_cuerpo.cod_obra', '{n}.cfpd07_obras_cuerpo.denominacion');
...
function limpiar_no_disponible ($var = null) {
   $this->layout="ajax";
...
function mostrar_partidas_obra2($var = null, $cod_sector=null, $cod_programa=null, $cod_sub_prog=null, $cod_proyecto=null, $cod_activ_obra=null, $cod_partida=null, $cod_generica=null, $cod_especifica=null, $cod_sub_espec=null, $cod_auxiliar=null, $monto_aux=null) {
   $this->layout="ajax";
...
if($var!=null){
	...
	$partida = " and cod_sector='$cod_sector' and cod_programa='$cod_programa' and cod_sub_prog='$cod_sub_prog' and cod_proyecto='$cod_proyecto' and cod_activ_obra='$cod_activ_obra' and cod_partida='$cod_partida' and cod_generica='$cod_generica' and cod_especifica='$cod_especifica' and cod_sub_espec='$cod_sub_espec' and cod_auxiliar='$cod_auxiliar' ";
	$cfpd07_obras_cuerpo   =  $this->cfpd07_obras_cuerpo->findAll($condicion." and ano_estimacion=".$ano." and upper(cod_obra)='".strtoupper($var)."' ");
	$cfpd07_obras_partidas =  $this->cfpd07_obras_partidas->findAll($condicion." and ano_estimacion=".$ano." and upper(cod_obra)='".strtoupper($var)."' ".$partida, null, 'cod_sector,cod_programa,cod_sub_prog,cod_proyecto,cod_activ_obra, cod_partida,cod_generica,cod_especifica,cod_sub_espec,cod_auxiliar ASC');
    ...
    $codigos_direccion=$this->Session->read('CodigosDireccion');
 	...
 	$catalogo= $this->v_cscd01_catalogo_deno_und->generateList($this->condicion()." and cod_sector=".$cod_sector." and cod_programa=".$cod_programa." and cod_sub_prog=".$cod_sub_prog." and cod_proyecto=".$cod_proyecto, 'cod_snc ASC', null, '{n}.v_cscd01_catalogo_deno_und.codigo_prod_serv', '{n}.v_cscd01_catalogo_deno_und.denominacion');
		...
		case 'obras':
			...
			$resultado=$this->cugd02_direccion->findAll($cond,array('cod_sector','cod_programa','cod_sub_prog','cod_proyecto'));
			$this->Session->write('CodigosDireccion',$resultado);
			$this->set('SELECT','obra');
			$this->set('codigo','obra');
			$this->set('seleccion','');
			$this->set('n',5);
			$this->set('no','no');
			$this->set('otro','otro');
		    $codigos_direccion=$resultado;
		 	$cod_sector=$codigos_direccion[0]["cugd02_direccion"]["cod_sector"];
		 	$cod_programa=$codigos_direccion[0]["cugd02_direccion"]["cod_programa"];
		 	$cod_sub_prog=$codigos_direccion[0]["cugd02_direccion"]["cod_sub_prog"];
		 	$cod_proyecto=$codigos_direccion[0]["cugd02_direccion"]["cod_proyecto"];
		    ...
		    $SScoddep                 =       $this->Session->read('SScoddep');
		    $Modulo                   =       $this->Session->read('Modulo');
			$condicion = 'cod_presi='.$this->Session->read('SScodpresi').'  and  cod_entidad='.$this->Session->read('SScodentidad').' and cod_tipo_inst='.$this->Session->read('SScodtipoinst').' and  cod_inst='.$this->Session->read('SScodinst').' and cod_dep_original='.$SScoddeporig.' and ano_estimacion='.$ano; //." and cod_sector=".$cod_sector." and cod_programa=".$cod_programa." and cod_sub_prog=".$cod_sub_prog." and cod_proyecto=".$cod_proyecto;
...
function eliminar_items ($id) {
	...
	$_SESSION["items"][$id]['id']='no_mostrar';
	...
	for($z=0;$z<count($vec);$z++){
	  if($vec[$z]['id']=="no_mostrar" && $vec[$z]['id']!="0"){}else{ $conta++;
		  $cod_presi                 =     $cod_presi;
...
function eliminar_items_ ($id) {
	...
	for($z=0;$z<count($vec);$z++){
	  if($vec[$z]['id']=="no" && $vec[$z]['id']!="0"){}else{ $conta++;
		  $cod_presi                 =     $cod_presi;
		  $cod_entidad               =     $cod_entidad;
		  $cod_tipo_inst             =     $cod_tipo_inst;
		  $cod_inst                  =     $cod_inst;
...
function ver_disponibilidad($cod_obra=null,$i=null, $a=null, $b=null){	$this->layout="ajax";


    $ano            = $_SESSION["items"][$i][0];
    $vec[1] = $_SESSION["items"][$i][1];
    $vec[2] = $_SESSION["items"][$i][2];
    $vec[3] = $_SESSION["items"][$i][3];
    $vec[4] = $_SESSION["items"][$i][4];
    $vec[5] = $_SESSION["items"][$i][5];
    $vec[6] = $_SESSION["items"][$i][6];
    $vec[7] = $_SESSION["items"][$i][7];
    $vec[8] = $_SESSION["items"][$i][8];
    $vec[9] = $_SESSION["items"][$i][9];
    $vec[10]= $_SESSION["items"][$i][10];
    $sql_aux = $this->SQLCA()." and ano_estimacion=".$ano." and upper(cod_obra)='".strtoupper($cod_obra)."' and cod_sector=".$vec[1]."   and   cod_programa=".$vec[2]."   and   cod_sub_prog=".$vec[3]."   and   cod_proyecto=".$vec[4]."   and   cod_activ_obra=".$vec[5]."   and   cod_partida=".$vec[6]."   and   cod_generica=".$vec[7]."   and   cod_especifica=".$vec[8]."   and   cod_sub_espec=".$vec[9]."   and   cod_auxiliar=".$vec[10].";";
    $data = $this->cepd01_compromiso_partidas->execute("SELECT monto,aumento_obras,disminucion_obras,monto_contratado FROM cfpd07_obras_partidas WHERE ".$sql_aux);

    extract($data[0][0]);
    $disponibilidad = ($data[0][0]['monto']+$data[0][0]['aumento_obras'])-($data[0][0]['disminucion_obras']+$data[0][0]['monto_contratado']);
...
if($b>$disponibilidad){
       $this->set('msg_error', 'El monto es mayor a la disponibilidad '.$this->Formato2($disponibilidad).' '.MONEDA2);
       $this->set('i', $i);
       $_SESSION["items"][$i][11] = $disponibilidad;
    }else{
...
function guardar($pagina=null){
	...
	if(isset($this->data["cepp01_compromiso"])){
			 ...
			 $username=$this->Session->read('nom_usuario');
			 $cod_obra=$this->data['cepp01_compromiso']['codigo_obra'];
             ...
             $R2=$this->cepd01_compromiso_cuerpo->execute("INSERT INTO cepd01_compromiso_cuerpo (".$camposT2.") VALUES (".$this->SQLCAIN($ano).",".$numero_documento.",".$tipo_documento.",'".$fecha_documento."',".$tipo_recurso.",'".$rif."','".$cedula."',".$this->data["cepp01_compromiso"]["cod_dir_superior"].",".$this->data["cepp01_compromiso"]["cod_coordinacion"].",".$this->data["cepp01_compromiso"]["cod_secretaria"].",".$this->data["cepp01_compromiso"]["cod_direccion"].",'".$concepto."',".$monto.",1,0,0,0,0,'".$username."',0,0,'0',0,0,0,0,0,0,'".$beneficiario."',".$condicion_juridica.",'".$fecha_proceso_registro."','".$fecha_proceso_anulacion."','".$cod_obra."')");
	...
	if($vec[$i]['id']=="no_mostrar" && $vec[$i]['id']!="0"){
	  ...
	  $new_array[$i][11]=     $vec[$i][11];
					...
					foreach($new_array as $vec){
											            $sql_aux = $this->SQLCA()." and ano_estimacion=".$ano." and upper(cod_obra)='".strtoupper($cod_obra)."' and cod_sector=".$vec[1]."   and   cod_programa=".$vec[2]."   and   cod_sub_prog=".$vec[3]."   and   cod_proyecto=".$vec[4]."   and   cod_activ_obra=".$vec[5]."   and   cod_partida=".$vec[6]."   and   cod_generica=".$vec[7]."   and   cod_especifica=".$vec[8]."   and   cod_sub_espec=".$vec[9]."   and   cod_auxiliar=".$vec[10].";";
							    $data_disp = $this->cepd01_compromiso_partidas->execute("SELECT monto,aumento_obras,disminucion_obras,monto_contratado FROM cfpd07_obras_partidas WHERE ".$sql_aux);

							    $disponible_partida = ($data_disp[0][0]['monto']+$data_disp[0][0]['aumento_obras'])-($data_disp[0][0]['disminucion_obras']+$data_disp[0][0]['monto_contratado']);
										...
										if($PASAR==0){
															  ...
															  $numero_compromiso= $this->cfpd21_numero_asiento_compromiso->field('cfpd21_numero_asiento_compromiso.numero_compromiso', $conditions = $this->condicionNDEP()." and ano_compromiso='$ann'", $order =null);
															     ...
															     foreach($new_array as $vec){
																		   ...
																		   $dnco = $this->motor_presupuestario($cp, $to, $td, $ta, $fecha_documento, $mt, $ccp, $ann, $numero_documento, null, null, null, null, null, null, null, $numero_compromiso, null, null, null, $x);
																			...
																			if($dnco != false){
																				...
																				$sql_aux = $this->SQLCA()." and ano_estimacion=".$ano." and upper(cod_obra)='".strtoupper($cod_obra)."' and cod_sector=".$vec[1]."   and   cod_programa=".$vec[2]."   and   cod_sub_prog=".$vec[3]."   and   cod_proyecto=".$vec[4]."   and   cod_activ_obra=".$vec[5]."   and   cod_partida=".$vec[6]."   and   cod_generica=".$vec[7]."   and   cod_especifica=".$vec[8]."   and   cod_sub_espec=".$vec[9]."   and   cod_auxiliar=".$vec[10].";";
              	                                                                $sw4 = $this->cepd01_compromiso_partidas->execute("UPDATE cfpd07_obras_partidas SET monto_contratado=monto_contratado+".$mt." where ".$sql_aux);


																			     $sql_aux = $this->SQLCA()." and ano=".$ano." and cod_sector=".$vec[1]."   and   cod_programa=".$vec[2]."   and   cod_sub_prog=".$vec[3]."   and   cod_proyecto=".$vec[4]."   and   cod_activ_obra=".$vec[5]."   and   cod_partida=".$vec[6]."   and   cod_generica=".$vec[7]."   and   cod_especifica=".$vec[8]."   and   cod_sub_espec=".$vec[9]."   and   cod_auxiliar=".$vec[10].";";
																				 $sw455   = $this->cepd01_compromiso_partidas->execute("UPDATE cfpd05 SET precompromiso_obras=precompromiso_obras-".$mt." where ".$sql_aux);
										                       ...
										                       $valor_motor_contabilidad =  $this->motor_contabilidad_fiscal(
                                                                 ...
                                                                 $sql_aux = $this->SQLCA()." and ano_estimacion='".$ano."' and upper(cod_obra)='".strtoupper($cod_obra)."' ;  ";
                                                                 $sw3 = $this->cepd01_compromiso_cuerpo->execute("UPDATE cfpd07_obras_cuerpo SET monto_contratado=monto_contratado+".$monto." where ".$sql_aux);
...
function guardar_anulacion ($var=null,$ano = null,$pagina_actual = null) {
	...
	if(isset($this->data["cepp01_compromiso"])){
			 ...
			 $beneficiario       =  $this->data["cepp01_compromiso"]["beneficiario"];
			 $cod_obra           =  $this->data["cepp01_compromiso"]["cod_obra"];
                   ...
                   if($msj==true){
                        ...
                        foreach($partidas_compromiso as $vec){
								   ...
								   $dnco = $this->motor_presupuestario($cp, $to ,$td, $ta, date("d/m/Y"), $mt, $ccp, $ano, $numero_documento, null, null, null, null, null, null, null, $rnco, null, null, null, null);
                                    ...
                                    $sql_aux = $this->SQLCA()." and ano=".$ano." and cod_sector=".$vec["cepd01_compromiso_partidas"]["cod_sector"]."   and   cod_programa=".$vec["cepd01_compromiso_partidas"]["cod_programa"]."   and   cod_sub_prog=".$vec["cepd01_compromiso_partidas"]["cod_sub_prog"]."   and   cod_proyecto=".$vec["cepd01_compromiso_partidas"]["cod_proyecto"]."   and   cod_activ_obra=".$vec["cepd01_compromiso_partidas"]["cod_activ_obra"]."   and   cod_partida=".$vec["cepd01_compromiso_partidas"]["cod_partida"]."   and   cod_generica=".$vec["cepd01_compromiso_partidas"]["cod_generica"]."   and   cod_especifica=".$vec["cepd01_compromiso_partidas"]["cod_especifica"]."   and   cod_sub_espec=".$vec["cepd01_compromiso_partidas"]["cod_sub_espec"]."   and   cod_auxiliar=".$vec["cepd01_compromiso_partidas"]["cod_auxiliar"].";";
									$sw455   = $this->cepd01_compromiso_partidas->execute("UPDATE cfpd05 SET precompromiso_obras=precompromiso_obras+".$mt." where ".$sql_aux);
									...
									if($dnco != false){
										...
										//$this->Session->write("MSJ",$MSJ);
										$para_msj = true;
										$sql_aux = $this->SQLCA()." and ano_estimacion=".$ano." and upper(cod_obra)='".strtoupper($cod_obra)."' and cod_sector=".$vec["cepd01_compromiso_partidas"]["cod_sector"]."   and   cod_programa=".$vec["cepd01_compromiso_partidas"]["cod_programa"]."   and   cod_sub_prog=".$vec["cepd01_compromiso_partidas"]["cod_sub_prog"]."   and   cod_proyecto=".$vec["cepd01_compromiso_partidas"]["cod_proyecto"]."   and   cod_activ_obra=".$vec["cepd01_compromiso_partidas"]["cod_activ_obra"]."   and   cod_partida=".$vec["cepd01_compromiso_partidas"]["cod_partida"]."   and   cod_generica=".$vec["cepd01_compromiso_partidas"]["cod_generica"]."   and   cod_especifica=".$vec["cepd01_compromiso_partidas"]["cod_especifica"]."   and   cod_sub_espec=".$vec["cepd01_compromiso_partidas"]["cod_sub_espec"]."   and   cod_auxiliar=".$vec["cepd01_compromiso_partidas"]["cod_auxiliar"].";";
              	                        $sw4 = $this->cepd01_compromiso_partidas->execute("UPDATE cfpd07_obras_partidas SET monto_contratado=monto_contratado-".$mt." where ".$sql_aux);
									}else{
		...
		if(isset($pagina)){
			...
			if(isset($this->data['cepp01_compromiso']['ano_consulta'])){
				...
				$Ano=$this->ano_ejecucion();
						 ...
						 $Tfilas=$this->cepd01_compromiso_cuerpo->findCount($this->SQLCA()." and ano_documento=".$Ano." and cod_obra!=''");
						 //echo $Tfilas;
						 if($Tfilas!=0){
						 $this->set('pag_cant',$pagina.'/'.$Tfilas);
						 $this->set('ultimo',$Tfilas);
						 $dataCompromiso=$this->cepd01_compromiso_cuerpo->findAll($this->SQLCA()." and ano_documento=".$Ano." and cod_obra!=''",null,'ano_documento,numero_documento ASC',1,$pagina,null);
