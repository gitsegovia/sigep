 ...
 *
 */
 class Cnmp06DatosEducativosController extends AppController {
   var $name = 'cnmp06_datos_educativos';
...
function codi_nivel($codigo=null){
	$this->layout = "ajax";
	if($codigo!=null){
...
function deno_nivel($codigo=null){
	$this->layout = "ajax";
	if($codigo!=null){
...
function select3($select=null,$var=null) { //select codigos presupuestarios
	...
	switch($select){
		case 'pais':
		  ...
		  $this->set('seleccion','');
		  $this->set('n',1);
		  $lista=  $this->cugd01_republica->generateList($cond, 'denominacion ASC', null, '{n}.cugd01_republica.cod_republica', '{n}.cugd01_republica.denominacion]');
...
function modificar($cedula=null,$consecutivo=null,$cod_nivel_educacion=null){
	  $this->layout = "ajax";
...
function guardar_modificar($cedula=null,$consecutivo=null,$cod_nivel_educacion=null){
	$this->layout = "ajax";
	if(!empty($this->data)){
...
function eliminar($cedula=null,$consecutivo=null,$cod_nivel_educacion=null){
	...
	$cond ="cedula=".$cedula." and consecutivo=".$consecutivo." and cod_nivel_educacion=".$cod_nivel_educacion;
	  ...
	  $y=$this->cnmd06_datos_educativos->findCount();
	  if($y!=0){
	  $this->consultar();
      $this->render("consultar");
		}else if($y==0){
