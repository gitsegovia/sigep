<?php
class Cnmp15BonoVacaDiasAdicController extends AppController{

	var $name = 'cnmp15_bono_vaca_dias_adic';
 	var $uses = array('cnmd15_bono_vaca_dias_adic','Cnmd01');
...
function escala_inputs($var=null){
	$this->layout="ajax";
	$v=$this->cnmd15_bono_vaca_dias_adic->execute("SELECT * FROM cnmd15_bono_vaca_dias_adic WHERE ".$this->SQLCA()." and cod_tipo_nomina=".$var." ORDER BY escala DESC");
			if($v!=null){
				$escala=$v[0][0]["escala"];
...
function grilla($var=null){
	$this->layout="ajax";
	$datos=$this->cnmd15_bono_vaca_dias_adic->FindAll($this->SQLCA()." and cod_tipo_nomina=".$var,null,' ORDER BY escala ASC');
...
function guardar(){
	...
	if($escala!="" && $dia1!="" && $mes1!="" && $ano1!="" && $dia2!="" && $mes2!="" && $ano2!="" && $desde!="" && $hasta!="" && $dias!=""){
			$sql_insert = "INSERT INTO cnmd15_bono_vaca_dias_adic VALUES(".$this->verifica_SS(1).", ".$this->verifica_SS(2).", ".$this->verifica_SS(3).",".$this->verifica_SS(4).",".$this->verifica_SS(5).",".$this->Session->read('nomina').",".$escala.",".$dia1.",".$mes1.",".$ano1.",".$dia2.",".$mes2.",".$ano2.",".$desde.",".$hasta.",".$dias.",".$basico.",".$descuento.")";
			$sw1 = $this->cnmd15_bono_vaca_dias_adic->execute($sql_insert);
			if($sw1>1){
	...
	$datos=$this->cnmd15_bono_vaca_dias_adic->FindAll($this->SQLCA()." and cod_tipo_nomina=".$this->Session->read('nomina'),null,' ORDER BY escala ASC');
...
function eliminar(){
	...
	if($nomina!=""){
		if($this->cnmd15_bono_vaca_dias_adic->FindAll($this->SQLCA()." and cod_tipo_nomina=".$nomina)){
			$sw1=$this->cnmd15_bono_vaca_dias_adic->execute("delete from cnmd15_bono_vaca_dias_adic where ".$this->SQLCA()." and cod_tipo_nomina=".$nomina);
			if($sw1>1){
...
function modificar($nomina=null,$escala=null,$desde_dia=null,$desde_mes=null,$desde_ano=null,$hasta_dia=null,$hasta_mes=null,$hasta_ano=null,$desde_antiguedad=null,$hasta_antiguedad=null,$dias=null,$i=null,$basicos=null,$descuentos=null){
	...
	$sq_datos_campos = $this->cnmd15_bono_vaca_dias_adic->execute("SELECT escala, hasta_dia, hasta_mes, hasta_ano FROM cnmd15_bono_vaca_dias_adic WHERE ".$this->SQLCA()." AND cod_tipo_nomina=".$nomina." AND escala = (SELECT (MAX(escala)-1) FROM cnmd15_bono_vaca_dias_adic) LIMIT 1;");
...
function guardar_modificar($nomina=null,$escala=null,$desde_dia=null,$desde_mes=null,$desde_ano=null,$hasta_dia=null,$hasta_mes=null,$hasta_ano=null,$desde_antiguedad=null,$hasta_antiguedad=null,$diass=null,$i=null,$basicomo=null,$descuentomo=null){
	...
	if($escala!="" && $dia1!="" && $mes1!="" && $ano1!="" && $dia2!="" && $mes2!="" && $ano2!="" && $desde!="" && $hasta!="" && $dias!=""){
		$sw=$this->cnmd15_bono_vaca_dias_adic->execute("update cnmd15_bono_vaca_dias_adic set hasta_dia=".$dia2." , hasta_mes=".$mes2." , hasta_ano=".$ano2." , dias=".$dias." , basico=".$basicoa." , descuento=".$descuentoa." where ".$this->SQLCA()." and cod_tipo_nomina=".$nomina." and escala=".$escala);
		if($sw>1){
		...
		$datos=$this->cnmd15_bono_vaca_dias_adic->FindAll($this->SQLCA()." and cod_tipo_nomina=".$nomina,null,' ORDER BY escala ASC');
		$this->set('datos',$datos);
	}else{
		$this->set('errorMessage','Debe ingresar datos en todos los campos');
		$datos=$this->cnmd15_bono_vaca_dias_adic->FindAll($this->SQLCA()." and cod_tipo_nomina=".$nomina,null,' ORDER BY escala ASC');
