...
class Cnmp09ActivaTrabaRetiradosController extends AppController {
   var $name = 'cnmp09_activa_traba_retirados';
   var $uses = array('Cnmd01', 'cnmd06_fichas','cnmd06_datos_personales','v_cnmd06_fichas_datos_personales','ccfd04_cierre_mes');
...
function mostrar_grilla($nomina=null){
	...
	$sql="  select cod_tipo_nomina,cod_cargo,cod_ficha,cedula_identidad,primer_apellido,segundo_apellido,primer_nombre,segundo_nombre,condicion_actividad,fecha_retiro FROM v_cnmd06_fichas_datos_personales  WHERE ".$this->SQLCA()." and cod_tipo_nomina=".$nomina." and condicion_actividad=6 and substr(fecha_retiro::text,0,5)::integer=".$ano." order by cod_ficha asc";
...
function mostrar_activos($nomina=null){
	$this->layout="ajax";
	$cod_presi = $this->Session->read('SScodpresi');
    $cod_entidad = $this->Session->read('SScodentidad');
    $cod_tipo_inst = $this->Session->read('SScodtipoinst');
	...
	$sql="  select cod_tipo_nomina,cod_cargo,cod_ficha,cedula_identidad,primer_apellido,segundo_apellido,primer_nombre,segundo_nombre,condicion_actividad FROM v_cnmd06_fichas_datos_personales  WHERE ".$this->SQLCA()." and cod_tipo_nomina=".$nomina." and condicion_actividad=1 order by cod_ficha asc";
	$result=$this->v_cnmd06_fichas_datos_personales->execute($sql);
	if($result!=null){
		$this->set('datos',$result);
	}else{
...
function procesar($nomina=null){
	$this->layout = "ajax";
	//debo filtrar aqui por el año de ejecucion, falta eso agregarselo ok
	$ano=$this->ano_ejecucion();
	$sql="  select cod_tipo_nomina,cod_cargo,cod_ficha,cedula_identidad,primer_apellido,segundo_apellido,primer_nombre,segundo_nombre,condicion_actividad,fecha_retiro FROM v_cnmd06_fichas_datos_personales  WHERE ".$this->SQLCA()." and cod_tipo_nomina=".$nomina." and condicion_actividad=6 and substr(fecha_retiro::text,0,5)::integer=".$ano." order by cod_ficha asc";
	$datos=$this->v_cnmd06_fichas_datos_personales->execute($sql);

	//aqui debo verificar si son todos los trabajadores activos o el que el usuario elija
	$sql1="  select cod_tipo_nomina,cod_cargo,cod_ficha,cedula_identidad,primer_apellido,segundo_apellido,primer_nombre,segundo_nombre,condicion_actividad,fecha_retiro FROM v_cnmd06_fichas_datos_personales  WHERE ".$this->SQLCA()." and cod_tipo_nomina=".$nomina." and condicion_actividad=1 order by cod_ficha asc";
	$datos1=$this->v_cnmd06_fichas_datos_personales->execute($sql1);
	...
	for($p=0;$p<count($datos1);$p++){
		$this->cnmd06_fichas->execute("update cnmd06_fichas set condicion_actividad=8 where ".$this->SQLCA()." and cod_tipo_nomina=".$datos1[$p][0]['cod_tipo_nomina']." and cod_cargo=".$datos1[$p][0]['cod_cargo']." and cod_ficha=".$datos1[$p][0]['cod_ficha']);
	}
