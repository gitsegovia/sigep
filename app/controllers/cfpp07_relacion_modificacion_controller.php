...
class Cfpp07RelacionmodificacionController extends AppController {

    var $name = "cfpp07_relacion_modificacion";
...
//fin function
    ...
    function buscar_pista_obras($var1 = null, $var2 = null, $var3 = null) {
        ...
        $condicion = 'cod_presi=' . $this->Session->read('SScodpresi') . '  and  cod_entidad=' . $this->Session->read('SScodentidad') . ' and cod_tipo_inst=' . $this->Session->read('SScodtipoinst') . ' and  cod_inst=' . $this->Session->read('SScodinst') . ' and cod_dep_original=' . $SScoddeporig . ' and ano_estimacion=' . $year_buscar;
        ...
        //$condicion = $condicion . ' and punto_operacion!=1' . ' and ano_estimacion=' . $year_buscar . '   and ((estimado_presu + aumento_obras) - (monto_contratado + disminucion_obras)) != 0     ';
		$condicion = $condicion . ' and punto_operacion!=1' . ' and ano_estimacion=' . $year_buscar ;
        $ordena = "cod_obra";
...
//fin function
    ...
    function mostrar_partidas_obra($var = null) {
        ...
        if ($var != null) {
            ...
            $this->set('d', $d);
            $this->set('e', $e);
            $this->set('di', $disponible_partida);
...
//fin funciomostrar_partidas_obra
...
function aumento_disminucion($opcion=null, $monto=null){
        ...
        if ($tipo == 1) {
            ...
            foreach ($cfpd07_obras_partidas as $aux_cfpd07_obras_partidas) {
				...
				$monto_relacion = 0;
                $cfpd07_obras_partidas_todas = $this->cfpd07_obras_partidas->findAll("cod_presi='" . $cod_presi . "' and cod_entidad='" . $cod_entidad . "' and cod_tipo_inst='" . $cod_tipo_inst . "' and cod_inst='" . $cod_inst . "' and cod_dep='" . $cod_dep . "' and ano_estimacion='" . $ano . "' and  cod_sector='" . $cod_sector . "' and cod_programa='" . $cod_programa . "' and cod_sub_prog='" . $cod_sub_prog . "' and cod_proyecto='" . $cod_proyecto . "' and cod_activ_obra='" . $cod_activ_obra . "' and cod_partida='" . $cod_partida . "' and cod_generica='" . $cod_generica . "' and cod_especifica='" . $cod_especifica . "' and cod_sub_espec='" . $cod_sub_espec . "' and cod_auxiliar='" . $cod_auxiliar . "' ", null, 'cod_obra DESC');
                foreach ($cfpd07_obras_partidas_todas as $aux_cfpd07_obras_partidas_todas) {
                $monto_rel        = $aux_cfpd07_obras_partidas_todas['cfpd07_obras_partidas']['monto'];
                $monto_aum        = $aux_cfpd07_obras_partidas_todas['cfpd07_obras_partidas']['aumento_obras'];
                $monto_dis        = $aux_cfpd07_obras_partidas_todas['cfpd07_obras_partidas']['disminucion_obras'];
                $monto_con        = $aux_cfpd07_obras_partidas_todas['cfpd07_obras_partidas']['monto_contratado'];
                $monto_rela = (($monto_rel + $monto_aum) - ($monto_dis + $monto_con));
                $monto_relacion = ($monto_relacion + $monto_rela);
                }

                $disponibilidad    = ($disponibilidad + ($dispo - $monto_relacion));
            }//fin foreach
    ...
    function ver_disponibilidad($i, $ano, $cod_sector, $cod_programa, $cod_sub_prog, $cod_proyecto, $cod_activ_obra, $cod_partida, $cod_generica, $cod_especifica, $cod_sub_espec, $cod_auxiliar, $monto) {
        ...
        if ($tipo == 1) {
            $disponibilidad_partida = $this->disponibilidad($ano, $cod_sector, $cod_programa, $cod_sub_prog, $cod_proyecto, $cod_activ_obra, $cod_partida, $cod_generica, $cod_especifica, $cod_sub_espec, $cod_auxiliar);
			...
			$disponibilidad_partida = ($disponibilidad_partida - $monto_relacion);

            $disponibilidad_partida = $this->Formato2($disponibilidad_partida);
            $disponibilidad_partida = $this->Formato1($disponibilidad_partida);

            if ($monto > $disponibilidad_partida) {
                $this->set('msg_error', 'la disponibilidad presupustaria no es suficiente - Monto disponible: ' . $this->Formato2($disponibilidad_partida));
                $this->set('i', $i);
			    echo"<script> document.getElementById('guardar').disabled=true; </script>";
            ...
            foreach ($cfpd07_obras_partidas as $ve3) {
                ...
                $disponibilidad_partida = (($monto_2 + $aumento_obras_2) - ($disminucion_obras_2 + $monto_contratado_2));
            }//fin foreach
            ...
            if ($monto > $disponibilidad_partida) {
                $this->set('msg_error', 'la disponibilidad para disminuir la relación para esta partida es de: ' . $this->Formato2($disponibilidad_partida));
                $this->set('i', $i);
                echo"<script> document.getElementById('guardar').disabled=true;</script>";
                echo"<script> document.getElementById('monto_".$i."').value='0,00';</script>";
            }else{
...
//fin function
    ...
    function guardar() {
        ...
        $ano_obra = $this->data['cfpp07_relacion_modificacion']['ano_obra'];
        $codigo_obra = $this->data['cfpp07_relacion_modificacion']['codigo_obra'];
        $tipo_modificacion = $this->data['cfpp07_relacion_modificacion']['tipo_modificacion'];


////////
        $fecha_modificacion = $this->data['cfpp07_relacion_modificacion']['fecha_modificacion'];
        $observaciones = $this->data['cfpp07_relacion_modificacion']['observaciones'];
        $numero_modificacion = $this->data['cfpp07_relacion_modificacion']['numero_modificacion'];
///////

        $this->set('tipo_modificacion', $tipo_modificacion);

        if ($tipo_modificacion == '1') {
            $monto_modificacion = $this->Formato1($this->data['cfpp07_relacion_modificacion']['aumento']);
        } else if ($tipo_modificacion == '2') {
            $monto_modificacion = $this->Formato1($this->data['cfpp07_relacion_modificacion']['disminucion']);
        }//fin else
...
//													                    $sw155 = $this->cscd04_ordencompra_partidas->execute($sql55);
        ...
        if ($swbb > 1) {
            ...
            if ($no_disponible == 1) {
                $this->set('errorMessage', 'LA RELACIÓN NO TIENE DISPONIBILIDAD');
            } else {
...
//fin function
    ...
    function consulta($ano_documento = null, $numero_documento = null, $pagina = null) {
        ...
        if (isset($this->data['cfpp07_relacion_modificacion']['ano_ejecucion'])) {
            $ano_documento = $this->data['cfpp07_relacion_modificacion']['ano_ejecucion'];
            $numero_documento = $this->data['cfpp07_relacion_modificacion']['cod_obra'];
        }
        ...
        if ($Tfilas != 0) {
            ...
            $this->set('pagina_actual', $pagina);
            $this->set('ultimo', $Tfilas);
            $datos_filas = $this->cfpd07_obras_modificacion_cuerpo->findAll($this->condicion() . " and ano_estimacion='" . $ano_documento . "'  and upper(cod_obra)='" . strtoupper_sisap($numero_documento) . "' ", null, "numero_modificacion ASC", 1, $pagina, null);
            $this->set("datosFILAS", $datos_filas);
            $this->set('siguiente', $pagina + 1);
            $this->set('anterior', $pagina - 1);
            $this->bt_nav($Tfilas, $pagina);
        ...
        $datos_filas2 = $this->cfpd07_obras_modificacion_partidas->findAll($this->condicion() . " and ano_estimacion='" . $ano_documento . "'  and upper(cod_obra)='" . strtoupper_sisap($numero_documento) . "' and  numero_modificacion='" . $datos_filas[0]["cfpd07_obras_modificacion_cuerpo"]["numero_modificacion"] . "' ", null, "numero_modificacion ASC");
...
//fin function
    ...
    function entrar() {
        $this->layout = "ajax";
        if (isset($this->data['cfpp07_relacion_modificacion']['login']) && isset($this->data['cfpp07_relacion_modificacion']['password'])) {
            $l = "PROYECTO";
            $c = "JJJSAE";
            $user = addslashes($this->data['cfpp07_relacion_modificacion']['login']);
            $paswd = addslashes($this->data['cfpp07_relacion_modificacion']['password']);
