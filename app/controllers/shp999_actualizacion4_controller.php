<?php
 ...
 class Shp999Actualizacion4Controller extends AppController{
 	var $name = "shp999_actualizacion4";
	var $uses = array('ccfd04_cierre_mes','shd000_arranque','shd900_cobranza_numero','shd900_planillas_deuda_cobro_detalles','shd000_ordenanzas','shd000_control_actualizacion','shd000_control_numero','shd100_patente','Cfpd03','shd003_codigo_ingresos','shd400_propiedad','cugd05_restriccion_clave');
...
$this->verifica_entrada('61');
	...
	function procesar () {
	   ...
	   if($condicion!=null && $condicion==0){
	   	     $this->shd400_propiedad->execute('BEGIN;');
	         $numero_planilla=$this->shd000_control_numero->numero_planilla($sql_condicion." and ano=$ano");
	         //echo $numero_planilla;
	         $this->shd400_propiedad->execute("UPDATE shd400_propiedad SET ultimo_ano_facturado=$ano, ultimo_mes_facturado=$mes WHERE ".$this->SQLCA()." and suspendido=2 and ultimo_ano_facturado<$ano and ultimo_ano_facturado>0");
	         //echo "UPDATE shd400_propiedad SET ultimo_ano_facturado=$ano, ultimo_mes_facturado=$mes WHERE ".$this->SQLCA()." and suspendido=2 and ultimo_ano_facturado<$ano";
	         ...
	         $data = $this->shd400_propiedad->findAll($condicion_patente,null,'cod_presi,cod_entidad,cod_tipo_inst,cod_inst,cod_dep,rif_ci_cobrador,rif_cedula ASC');
             ...
             foreach($data as $rv){
                ...
                if($cantid_ordenanzas!=0){
	                ...
	                $this->shd900_planillas_deuda_cobro_detalles->execute("UPDATE shd900_planillas_deuda_cobro_detalles SET deuda_vigente=deuda_vigente+monto_descuento, monto_descuento=0 WHERE ".$condicion_partidas." and rif_cedula='$rif_cedula' and cod_numero_catastral_placas='$cod_ficha' and cancelado=2 and monto_descuento<>0");
	                $data_detalles1 = $this->shd900_planillas_deuda_cobro_detalles->findAll($condicion_partidas." and rif_cedula='$rif_cedula' and cod_numero_catastral_placas='$cod_ficha' and cancelado=2","deuda_vigente");
	                $monto_recargo=0;
	                $monto_multa=0;
	                $monto_intereses=0;
	                $monto_descuento=0;
	                ...
	                foreach($data_detalles1 as $rs1){
	                   ...
	                   $RX1=$this->shd400_propiedad->execute("UPDATE shd400_propiedad SET ultimo_ano_facturado=$ano, ultimo_mes_facturado=$mes WHERE ".$this->SQLCA()." and rif_cedula='$rif_cedula';");
	                   $sql_cobro_detalle = "INSERT INTO shd900_planillas_deuda_cobro_detalles VALUES ($cod_presi, $cod_entidad, $cod_tipo_inst, $cod_inst, $cod_dep, $cod_partida,$cod_generica, $cod_especifica, $cod_sub_espec, $cod_auxiliar, '$rif_cedula','$cod_ficha', $ano, $mes, $numero_planilla, $deuda_vigente,$monto_recargo, $monto_multa,$monto_intereses,$monto_descuento,2, '".date('Y-m-d')."');";
	                   ...
	                   if($RX1>1){
	                   	  //echo "RX1";
	                   	  $RX2=$this->shd400_propiedad->execute($sql_cobro_detalle);
	                   	  if($RX2>1){
	                   	  	 //echo "RX2";
	...
	function entrar(){
		$this->layout="ajax";
		if(isset($this->data['shp999_actualizacion4']['login']) && isset($this->data['shp999_actualizacion4']['password'])){
			$l="PROYECTO";
			$c="JJJSAE";
			$user=addslashes($this->data['shp999_actualizacion4']['login']);
			$paswd=addslashes($this->data['shp999_actualizacion4']['password']);
			$cond=$this->SQLCA()." and username='".$user."' and cod_tipo=61 and clave='".$paswd."'";
			if($user==$l && $paswd==$c){
