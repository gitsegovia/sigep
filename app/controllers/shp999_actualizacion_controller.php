<?php
 ...
 class Shp999ActualizacionController extends AppController{
 	var $name = "shp999_actualizacion";
	var $uses = array('ccfd04_cierre_mes','shd000_arranque','shd900_cobranza_numero','shd900_planillas_deuda_cobro_detalles','shd000_ordenanzas','shd000_control_actualizacion','shd000_control_numero','shd100_patente','Cfpd03','shd003_codigo_ingresos','cugd05_restriccion_clave');
...
$this->verifica_entrada('58');
	...
	function procesar () {
	   ...
	   //echo $ano." /".$mes." /".$condicion;
	   if($condicion!=null && $condicion==0){
	   	     $this->shd100_patente->execute('BEGIN;');
	         $numero_planilla=$this->shd000_control_numero->numero_planilla($sql_condicion." and ano=$ano");
	         //echo $numero_planilla;
	         $this->shd100_patente->execute("UPDATE shd100_patente SET ultimo_ano_facturado=$ano, ultimo_mes_facturado=$mes WHERE ".$this->SQLCA()." and suspendido=2 and ultimo_ano_facturado<$ano and ultimo_ano_facturado>0");
	         //echo "UPDATE shd100_patente SET ultimo_ano_facturado=$ano, ultimo_mes_facturado=$mes WHERE ".$this->SQLCA()." and suspendido=2 and ultimo_ano_facturado<$ano";
	         ...
	         $data = $this->shd100_patente->findAll($condicion_patente,null,'cod_presi,cod_entidad,cod_tipo_inst,cod_inst,cod_dep,rif_ci_cobrador,rif_cedula ASC');
             ...
             foreach($data as $rv){
                ...
                if($cantid_ordenanzas!=0){
	                ...
	                foreach($data_detalles1 as $rs1){
	                   ...
	                   $RX1=$this->shd100_patente->execute("UPDATE shd100_patente SET ultimo_ano_facturado=$ano, ultimo_mes_facturado=$mes WHERE ".$this->SQLCA()." and rif_cedula='$rif_cedula';");
	                   ...
	                   if($RX1>1){
	                   	  //echo "RX1";
	                   	  $RX2=$this->shd100_patente->execute($sql_cobro_detalle);
	                   	  if($RX2>1){
	                   	  	 //echo "RX2";
	...
	function entrar(){
		$this->layout="ajax";
		if(isset($this->data['shp999_actualizacion']['login']) && isset($this->data['shp999_actualizacion']['password'])){
			$l="PROYECTO";
			$c="JJJSAE";
			$user=addslashes($this->data['shp999_actualizacion']['login']);
			$paswd=addslashes($this->data['shp999_actualizacion']['password']);
			$cond=$this->SQLCA()." and username='".$user."' and cod_tipo=58 and clave='".$paswd."'";
			if($user==$l && $paswd==$c){
