 ...
 *
 */
 class InfoCnmp06DatosPersonalesController extends AppController {
   var $name = 'info_cnmp06_datos_personales';
   var $uses = array('cugd10_imagenes','cnmd06_colores','cnmd06_clubes','cnmd06_datos_personales','cnmd06_deportes','cnmd06_profesiones',
                     'cugd01_republica','cugd01_estados','cugd01_municipios','cugd01_parroquias','cugd01_centropoblados','cnmd06_profesiones',
                     'cnmd06_especialidades','cnmd06_deportes','cnmd06_religiones','cnmd06_hobby','cnmd06_oficio', 'datos_personales_super_busqueda');
...
function checkSession(){
 ...
 function verifica_SS($i){
  ...
  function index($id=null){
 	$this->layout ="ajax";
 	$identidad = $_SESSION['infogobierno']['cedula_identidad'];
 	$verificar_c = $this->cnmd06_datos_personales->findCount('cedula_identidad='.$identidad);
	if($verificar_c == 1){
		$this->busca_foto($identidad);
	    $this->render("busca_foto");
	}else if($verificar_c == 0){
    ...
    $this->concatena($listaclubes, 'club');
    ...
    $this->concatena($listadeporte, 'deporte');
    ...
    $this->concatena($listahobby, 'hobby');
   ...
   // }else{
    //	if($this->Session->read('cedula_pestana_expediente')==""){$id=0;}else{$id=$this->Session->read('cedula_pestana_expediente');}
    //	$this->set('cedula', "");
 ...
 function select3($select=null,$var=null) { //select codigos presupuestarios
	...
	switch($select){
		...
		case 'municipio':
		  ...
		  if(!isset($_SESSION["rep"])){$this->Session->write('rep',1);}
...
function denociudad2($var=null){
	$this->layout = "ajax";
	$rep =  $this->Session->read('rep2');
	$est=$this->Session->read('est2');
...
function guardar(){
	...
	if(!empty($this->data)){
                          ...
                          if($correo_electronico==""){$correo_electronico=0;}

						  if($numero_inscripcion_sso==""){$numero_inscripcion_sso=0;}
						  if($numero_inscripcion_lph==""){$numero_inscripcion_lph=0;}
						  if($grado_licencia_conducir==""){$grado_licencia_conducir=0;}
						  if($numero_licencia_conducir==""){$numero_licencia_conducir=0;}
						  $naturalizado = "";
...
'".$cero."')";
	 ...
	 if($CC>1){
	 	 $this->set('msj', array('Registro creado con exito.','exito'));
...
if($correo_electronico==""){$correo_electronico=0;}
  ...
  cod_centropoblado_origen=$cod_centropoblado_origen,cod_estado_habitacion=$cod_estado_habitacion,cod_municipio_habitacion=$cod_municipio_habitacion,cod_parroquia_habitacion=$cod_parroquia_habitacion,cod_centropoblado_habitacion=$cod_centropoblado_habitacion where cedula_identidad=".$cedula_identidad;
     ...
     $this->data=null;
			 ...
			 if($vvv>1){
			 	$this->set('msj', array('Registro modificado con exito.','exito'));
			 	 ...
			 	 $this->set('msj', array('Disculpe, El Registro no fue Modificado.','error'));
				    	$this->index();
				      //  $this->render("index");
				    }//fin else
...
if($pagina!=null){
          	 ...
          	 if($Tfilas==0){
          	 	$this->index();
          		//$this->render("index");
          	 }else if($pagina>$Tfilas){$pagina = $pagina-2;}
...
function modificar($cedula=null,$pagina=null){
	  ...
	  $this->layout = "ajax";
		    ...
		    if($vec!=0){
          	 ...
          	 $data=$this->cnmd06_datos_personales->findAll($cond);$i = 0;
          	 foreach($data as $row){
   ...
   $var[$i]['cod_centropoblado_habitacion']=$row['cnmd06_datos_personales']['cod_centropoblado_habitacion'];
			 ...
			 $municipios2=$this->cugd01_municipios->findAll($c33);
          	 $this->set('municipios2',$municipios2);
 ...
 function eliminar($cedula,$pagina){
 	$this->layout = "ajax";
		if(isset($cedula)){
		 	$cond=" cedula_identidad=".$cedula;
...
function lista_encontrados($pagina){
	//echo $pagina;
 		$this->layout = "ajax";
...
function subir_imagen_2 ($var=null){
	//$this->layout="";
	if(isset($var)){
