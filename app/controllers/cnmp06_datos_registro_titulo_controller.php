<?php
 class Cnmp06DatosRegistroTituloController extends AppController {
   var $name = 'cnmp06_datos_registro_titulo';
...
function cod_colegio($codigo=null){
	$this->layout = "ajax";
	if($codigo!=null){
...
function deno_colegio($codigo=null){
	$this->layout = "ajax";
	if($codigo!=null){
...
function modificar($cedula=null,$consecutivo=null,$cod_profesion=null){
	  $this->layout = "ajax";
		    ...
		    $listaespecialidad=  $this->cnmd06_especialidades->generateList($cond2, 'denominacion ASC', null, '{n}.cnmd06_especialidades.cod_especialidad', '{n}.cnmd06_especialidades.denominacion');
          	$this->concatena_tres_digitos($listaespecialidad, 'cod_especialidades');
...
function guardar_modificar($cedula=null,$consecutivo=null,$cod_profesiones=null){
	$this->layout = "ajax";
	if(!empty($this->data)){
	 $cod_profesion=$this->data['cnmp06_datos_registro_titulo']['cod_profesion'];
	 $cod_especialidad=$this->data['cnmp06_datos_registro_titulo']['cod_especialidad'];
...
function eliminar($cedula=null,$consecutivo=null,$cod_profesion=null){
	...
	$cond ="cedula=".$cedula." and consecutivo=".$consecutivo." and cod_profesion=".$cod_profesion;
	  ...
	  $y=$this->cnmd06_datos_registro_titulo->findCount();
	  if($y!=0){
	  $this->consultar();
      $this->render("consultar");
		}else if($y==0){
