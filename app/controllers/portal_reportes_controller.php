<?php
class PortalReportesController extends AppController {
   var $name = 'PortalReportes';
...
function relacion_obras_proyectadas($var1=null,$ano=null,$clave_acceso = null){
      set_time_limit(0);
      $logo = '1_11_30_11';
...
function obras_particular ($ano = null) {
   $this->layout="xmlportal";
   if(!$ano){
      $sql= " ";
   }else{
...
function relacion_obras_proyectadas_segun_snc($var1=null, $var2=null, $var3=null, $clave_acceso=null){
      set_time_limit(0);
	  $logo = '1_11_30_11';
      $clave_acceso_local = md5($logo);
      $this->set('logo',$logo);
	  if($var1==1){ $this->layout = "xmlportal";
      	$datos  = $this->arrd01->execute(" SELECT DISTINCT ano_estimacion  FROM cfpd07_obras_cuerpo ORDER BY ano_estimacion DESC");
      	$datos2 = $this->arrd01->execute(" SELECT DISTINCT ano_arranque    FROM ccfd03_instalacion  ORDER BY ano_arranque DESC");
			if(count($datos)!=0){
				foreach($datos as $n){
...
function solicitudes_ayudas ($cedula=null,$clave_acceso =null) {
   $this->layout="xmlportal";
   $logo = '1_11_30_11';
   $clave_acceso_local = md5($logo);
   if($cedula && $clave_acceso && $clave_acceso==$clave_acceso_local){
	...
	if(count($result)>0){
        ...
        $this->set('profesion',  $this->cnmd06_profesiones->field('denominacion', $conditions = 'cod_profesion='.$result[0][0]['cod_profesion'], $order ="cod_profesion ASC"));
		$this->set('oficio',     $this->cnmd06_oficio->field('denominacion', $conditions = 'cod_oficio='.$result[0][0]['cod_oficio'], $order ="cod_oficio ASC"));
