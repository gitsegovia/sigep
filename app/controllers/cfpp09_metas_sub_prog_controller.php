<?php
 class Cfpp09MetasSubProgController extends AppController{

     var $uses = array('ccfd04_cierre_mes','cfpd09_metas_subprog','cfpd09_metas_programa','cfpd09_metas_sector', 'cfpd02_sector', 'cfpd02_programa', 'cfpd05', 'cfpd05_auxiliar', 'cfpd02_sub_prog', 'cfpd02_proyecto', 'cfpd02_activ_obra', 'cfpd01_ano_grupo', 'cfpd01_ano_partida', 'cfpd01_ano_generica', 'cfpd01_ano_especifica', 'cfpd01_ano_sub_espec', 'cfpd01_ano_auxiliar', 'cfpd01_formulacion','v_programas_metas');
...
//	$listaSector=$this->cfpd02_sector->generateList($this->SQLCA(), 'cod_sector ASC', null, '{n}.cfpd02_sector.cod_sector', '{n}.cfpd02_sector.cod_sector');
...
function datos($var=null){
	...
	$v=$this->cfpd09_metas_subprog->execute("select metas,unidad_medida,cantidad from cfpd09_metas_subprog where ".$this->SQLCA()." and ano=".$ano." and cod_sector=".$sec." and cod_programa=".$prog." and cod_sub_prog=".$var);
	if($v){
		$meta=$v[0][0]["metas"];
...
function select3($select=null,$var=null) { //select codigos presupuestarios
	...
	switch($select){
		...
		//  echo $cond;
//		  $lista=$this->cfpd09_metas_programa->generateList($cond, 'cod_programa ASC', null, '{n}.cfpd09_metas_programa.cod_programa', '{n}.cfpd09_metas_programa.cod_programa');
...
//          $this->AddCero('vector', $lista);
		...
		case 'actividad':
		  ...
		  $lista=  $this->cfpd09_metas_activ_obra->generateList($cond, 'cod_activ_obra ASC', null, '{n}.cfpd09_metas_activ_obra.cod_activ_obra', '{n}.cfpd09_metas_activ_obra.cod_activ_obra');
          $this->AddCero('vector', $lista);
		break;
	}//fin wsitch
	}else{
...
function guardar(){
	...
	$ano=$year[0]['cfpd01_formulacion']['ano_formular'];
	if(!empty($this->data)){
		$a=$this->data['cfpp09_metas_sub_prog']['cod_sector'];
		$b=$this->data['cfpp09_metas_sub_prog']['cod_programa'];
		$c=$this->data['cfpp09_metas_sub_prog']['cod_subprograma'];
		$f=$this->data['cfpp09_metas_sub_prog']['metas'];
		$g=$this->data['cfpp09_metas_sub_prog']['unidad_medida'];
		$ca=$this->data['cfpp09_metas_sub_prog']['cantidad'];
		$ano=$this->data['cfpp09_metas_sub_prog']['ano_formulacion'];
	}
	...
	$SQL_INSERT ="INSERT INTO cfpd09_metas_subprog (cod_presi, cod_entidad,cod_tipo_inst,cod_inst,cod_dep,ano,cod_sector,cod_programa,cod_sub_prog,metas,unidad_medida,cantidad)";
	$SQL_INSERT .=" VALUES (".$aa[1].",".$aa[2].",".$aa[3].",".$aa[4].",".$aa[5].",".$ano.",".$a.",".$b.",".$c.",'".$f."','".$g."','".$ca."')";
	// echo $SQL_INSERT;
	$condicion="cod_sector=".$a." and cod_programa=".$b." and cod_sub_prog=".$c."and ano=".$ano." and ".$this->SQLCA();
	$tc=$this->cfpd09_metas_subprog->findCount($condicion);
	if($tc==0){
		$x=$this->cfpd09_metas_subprog->execute($SQL_INSERT);
		$this->data=null;
		$this->set('mensaje', 'Meta agregada con &eacute;xito.');
		$this->index();
		$this->render("index");
	}else if($tc!=0){
...
function consultar($pagina=null){
		...
		$ano=$year[0]['cfpd01_formulacion']['ano_formular'];
         if($pagina!=null){
          	 $pagina=$pagina;
          	  $Tfilas=$this->cfpd09_metas_subprog->findCount('ano='.$ano);
          	 // echo $Tfilas;
          	 if($Tfilas==0){
          	 	$this->index();
          		$this->render("index");
          	 ...
          	 $datacpcp01=$this->cfpd09_metas_subprog->findAll($ano,null,'cod_sector',1,$pagina,null);
          	 $sec=$this->cfpd02_sector->findAll('ano='.$ano);
          	 $this->set('sec',$sec);
          	 $prog=$this->cfpd02_programa->findAll('ano='.$ano);
          	 $this->set('prog',$prog);
          	 $subp=$this->cfpd02_sub_prog->findAll('ano='.$ano);
          	 $this->set('subp',$subp);
          	 $proy=$this->cfpd02_proyecto->findAll('ano='.$ano);
          	 $this->set('proy',$proy);
 	...
 	$pagina=1;
          	 ...
          	 if($Tfilas!=0){
          	 $this->set('pag_cant',$pagina.'/'.$Tfilas);
          	 $datacpcp01=$this->cfpd09_metas_subprog->findAll(null,null,'cod_sector DESC',1,$pagina,null);
          	  $sec=$this->cfpd02_sector->findAll('ano='.$ano);
          	 $this->set('sec',$sec);
          	 $prog=$this->cfpd02_programa->findAll('ano='.$ano);
          	 $this->set('prog',$prog);
...
function consultar($pagina=null){//echo 'pagina es '.$pagina.' y ano2 es '.$ano2;
        ...
        if($pagina!=null){
			$pagina=$pagina;
          	$Tfilas=$this->cfpd09_metas_subprog->findCount($this->SQLCA().' and ano='.$ano);
          	if($Tfilas==0){
			...
			if($Tfilas!=0){
				$this->set('pag_cant',$pagina.'/'.$Tfilas);
				$datacpcp01=$this->cfpd09_metas_subprog->findAll($this->SQLCA().' and ano='.$ano,null,'cod_sector,cod_programa,cod_sub_prog ASC',1,$pagina,null);
				$sec=$this->cfpd02_sector->findAll($this->SQLCA().' and ano='.$ano.' and cod_sector='.$datacpcp01[0]['cfpd09_metas_subprog']['cod_sector']);
				$prog=$this->cfpd02_programa->findAll($this->SQLCA().' and ano='.$ano.' and cod_sector='.$datacpcp01[0]['cfpd09_metas_subprog']['cod_sector'].' and cod_programa='.$datacpcp01[0]['cfpd09_metas_subprog']['cod_programa']);
				$subp=$this->cfpd02_sub_prog->findAll($this->SQLCA().' and ano='.$ano.' and cod_sector='.$datacpcp01[0]['cfpd09_metas_subprog']['cod_sector'].' and cod_programa='.$datacpcp01[0]['cfpd09_metas_subprog']['cod_programa'].' and cod_sub_prog='.$datacpcp01[0]['cfpd09_metas_subprog']['cod_sub_prog']);
				...
				$this->set('cod_sub_prog',$subp[0]['cfpd02_sub_prog']['cod_sub_prog']);
				$this->set('deno_sub_prog',$subp[0]['cfpd02_sub_prog']['denominacion']);
				//$sec=$this->cfpd02_sector->findAll($this->SQLCA().' and ano='.$ano);
				//$this->set('sec',$sec);
				//$prog=$this->cfpd02_programa->findAll($this->SQLCA().' and ano='.$ano);
				//$this->set('prog',$prog);
				//$subp=$this->cfpd02_sub_prog->findAll($this->SQLCA().' and ano='.$ano);
				//$this->set('subp',$subp);
				$this->set('DATOS',$datacpcp01);
				$this->set('siguiente',$pagina+1);
				$this->set('anterior',$pagina-1);
...
function modificar($a=null,$b=null,$c=null,$siguiente=null){
	$this->layout = "ajax";
   	$cod_presi = $this->Session->read('SScodpresi');
	$cod_entidad = $this->Session->read('SScodentidad');
	$cod_tipo_inst = $this->Session->read('SScodtipoinst');
   ...
   $sql=" cod_sector=".$a." and cod_programa=".$b." and cod_sub_prog=".$c." and ano=".$ano." and ".$this->SQLCA();

   $a=$this->cfpd09_metas_subprog->findAll($sql);
   $sec=$this->cfpd02_sector->findAll($this->SQLCA().' and ano='.$ano.' and cod_sector='.$a[0]['cfpd09_metas_subprog']['cod_sector']);
   $prog=$this->cfpd02_programa->findAll($this->SQLCA().' and ano='.$ano.' and cod_sector='.$a[0]['cfpd09_metas_subprog']['cod_sector'].' and cod_programa='.$a[0]['cfpd09_metas_subprog']['cod_programa']);
   $subp=$this->cfpd02_sub_prog->findAll($this->SQLCA().' and ano='.$ano.' and cod_sector='.$a[0]['cfpd09_metas_subprog']['cod_sector'].' and cod_programa='.$a[0]['cfpd09_metas_subprog']['cod_programa'].' and cod_sub_prog='.$a[0]['cfpd09_metas_subprog']['cod_sub_prog']);
   $this->set('DATOS',$a);
   $this->set('cod_sector',$sec[0]['cfpd02_sector']['cod_sector']);
   $this->set('deno_sector',$sec[0]['cfpd02_sector']['denominacion']);
   $this->set('cod_programa',$prog[0]['cfpd02_programa']['cod_programa']);
 ...
 function guardar_modificar($a=null,$b=null,$c=null,$pagina=null){
	$this->layout = "ajax";
	$cod_presi = $this->Session->read('SScodpresi');
	$cod_entidad = $this->Session->read('SScodentidad');
	$cod_tipo_inst = $this->Session->read('SScodtipoinst');
	...
	$ano=$year[0]['cfpd01_formulacion']['ano_formular'];
	if(!empty($this->data)){
		$metas=$this->data['cfpp09_metas_sub_prog']['metas'];
		$unidad_medida=$this->data['cfpp09_metas_sub_prog']['unidad_medida'];
		$cantidad=$this->data['cfpp09_metas_sub_prog']['cantidad'];
		$sql=" cod_sector=".$a." and cod_programa=".$b." and cod_sub_prog=".$c." and ano=".$ano." and ".$this->SQLCA();
		$sql2 ="update cfpd09_metas_subprog set metas ='$metas',unidad_medida='$unidad_medida',cantidad='$cantidad' where ".$sql;
	    $vvv=$this->cfpd09_metas_subprog->execute($sql2);
	    $this->data=null;
 ...
 function eliminar($a=null,$b=null,$c=null,$hola=null){
	$this->layout = "ajax";
		$p=1;
		$cod_presi = $this->Session->read('SScodpresi');
		$cod_entidad = $this->Session->read('SScodentidad');
		...
		if(!isset($a)){
			...
			$c=$this->data['cfpp09_metas_sub_prog']['cod_subprograma'];
			$p=0;
			$this->data['cfpp09_metas_sub_prog']=null;
		}
	$sql=" cod_sector=".$a." and cod_programa=".$b." and cod_sub_prog=".$c." and ano=".$ano." and ".$this->SQLCA();
    $x=$this->cfpd09_metas_subprog->findCount($sql);
	if($x!=0){
		$sql1 ="DELETE  FROM  cfpd09_metas_subprog where ".$sql;
		$this->cfpd09_metas_subprog->execute($sql1);
		$this->set('mensaje', 'Meta Eliminada con &eacute;xito.');
		//echo $x;
	  $y=$this->cfpd09_metas_subprog->findCount($this->SQLCA().' and ano='.$ano);
	  if($p==0){
		$this->index();
      	$this->render("index");
	  }else{
 ...
 function lista_encontrados () {
	$this->layout="ajax";
	if(!empty($this->data["cfpp09_metas_sub_prog"])){
		$sector=$this->data["cfpp09_metas_sub_prog"]["cod_sector"];
		$programa=$this->data["cfpp09_metas_sub_prog"]["cod_programa"];
		$subprograma=$this->data["cfpp09_metas_sub_prog"]["cod_sub_prog"];
		$cod =" and cod_sector=".$this->data["cfpp09_metas_sub_prog"]["cod_sector"];
		$cod .=" and cod_programa=".$this->data["cfpp09_metas_sub_prog"]["cod_programa"];
		$cod .=" and cod_sub_prog=".$this->data["cfpp09_metas_sub_prog"]["cod_sub_prog"];
		$cod_presi = $this->Session->read('SScodpresi');
		$cod_entidad = $this->Session->read('SScodentidad');
		$cod_tipo_inst = $this->Session->read('SScodtipoinst');
		$cod_inst = $this->Session->read('SScodinst');
	    ...
	    $this->set('anof',$anof);
	    $this->set('ano',$ano);
	    $Tfilas=$this->cfpd09_metas_subprog->findCount($this->SQLCA().$cod.' and ano='.$ano.' and cod_sector='.$sector.' and cod_programa='.$programa.' and cod_sub_prog='.$subprograma);
		if($Tfilas!=0){
	    	$DATOS_res = $this->cfpd09_metas_subprog->findAll($this->SQLCA().$cod.' and ano='.$ano.' and cod_sector='.$sector.' and cod_programa='.$programa.' and cod_sub_prog='.$subprograma, null, 'cod_sector, cod_programa, cod_sub_prog ASC',1,1, null);
 ...
 function modificar_buscar($a,$b,$c){
 	$this->layout = "ajax";
	$cod_presi = $this->Session->read('SScodpresi');
	$cod_entidad = $this->Session->read('SScodentidad');
	$cod_tipo_inst = $this->Session->read('SScodtipoinst');
...
}//fin clase cfpp09_metas_sub_prog_metas_sector_Controller
