...
class Cnmp09DiasCancelarAusenciaLaboralController extends AppController {
   var $name = 'cnmp09_dias_cancelar_ausencia_laboral';
   var $uses = array('Cnmd01', 'cnmd06_fichas','cnmd06_datos_personales','cnmd09_dias_trabajados_falta','v_cnmd06_fichas_datos_personales');
...
function carga_datos($nomina=null,$ficha=null){
	...
	if($this->cnmd09_dias_trabajados_falta->FindCount($this->SQLCA()." and cod_tipo_nomina=".$nomina." and cod_ficha=".$ficha)!=0){//falta filtrar por el cargo tambien aqui
		$disabled='disabled';
	}else{
		$disabled='';
	}
...
function seleccion_busqueda($opcion=null,$nomina=null,$cargo=null,$ficha=null){
	...
	if($this->cnmd09_dias_trabajados_falta->FindCount($this->SQLCA()." and cod_tipo_nomina=".$nomina." and cod_cargo=".$cargo." and cod_ficha=".$ficha)!=0){
		$disabled='disabled';
	}else{
		$disabled='';
	}
...
function mostrar_grilla($nomina=null){
	...
	$sql="  select a.cod_presi,
			...
			a.dias
			from cnmd09_dias_trabajados_falta a where
 ...
 function guardar($nomina=null,$cargo=null,$ficha=null) {
	...
	if(!empty($this->data['cnmp09_deduccion']['dias_guarda'])){
		$sql = "INSERT INTO cnmd09_dias_trabajados_falta VALUES ('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$nomina','$cargo','$ficha','$dias')";
		if($this->cnmd09_dias_trabajados_falta->execute($sql)>1){
...
/*
	$data=$this->cnmd09_dias_trabajados_falta->findAll($this->SQLCA()." and cod_tipo_nomina=".$nomina,null,"cod_profesion, cod_especialidad ASC",null,null,null);
	if($data!=null){
		$this->set('datos',$data);
	}else{
		$this->set('datos','');
...
function eliminar($nomina=null,$cargo=null,$ficha=null){
	  ...
	  $sw = $this->cnmd09_dias_trabajados_falta->execute("DELETE FROM cnmd09_dias_trabajados_falta  WHERE ".$this->SQLCA()." and cod_tipo_nomina='$nomina' and cod_cargo='$cargo' and cod_ficha='$ficha' ");
    ...
    $cod_dep                  =       $this->Session->read('SScoddep');
 	if(!empty($this->data['cnmp09_deduccion']['dias'.$i])){
		$dias=$this->data['cnmp09_deduccion']['dias'.$i];
  	    $sql="update cnmd09_dias_trabajados_falta set dias='$dias' where ".$this->SQLCA()." and cod_tipo_nomina='$nomina' and cod_cargo='$cargo' and cod_ficha='$ficha'";
