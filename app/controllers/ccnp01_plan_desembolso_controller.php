<?php
 ...
 class ccnp01PlanDesembolsoController extends AppController {
    var $name    = 'ccnp01_plan_desembolso';
	var $uses    = array('ccnd02_proyectos','ccnd01_tipo_directivo','ccnd02_proyectos_actividades','ccnd02_proyectos_actividad_desembolso');
...
//////////////////////////////////////////////////////////////////////////////////////////////
	...
	$sql="select
			...
			where ".$conditions." and a.cod_actividad not in(select b.cod_actividad from ccnd02_proyectos_actividad_desembolso b where ".$conditions." and a.cod_actividad=b.cod_actividad)
...
*/
	...
	$this->set('numero_actividad_registrada',$this->ccnd02_proyectos_actividad_desembolso->FindCount($conditions));

	$this->data=null;

	if($this->ccnd02_proyectos_actividad_desembolso->FindCount($conditions)!=0){
		$this->consulta();
	...
	if($x[0][0]['plan_desembolso']!='0'){
		$this->set('plan',$x[0][0]['plan_desembolso']);
...
//	pr($this->data);
	if(empty($this->data['ccnp01']['plan_ejecucion'])){
		$this->set('errorMessage', 'ingrese el plan de inversión y de desembolso');
		...
		$filtro2 = "cod_republica=".$cod_republica." and cod_estado=".$cod_estado." and cod_municipio=".$cod_municipio." and cod_parroquia=".$cod_parroquia." and cod_centro=".$cod_centro." and cod_concejo=".$cod_concejo." and ano=".$ano." and cod_proyecto='".$cod_proyecto."'";
				...
				if($bandera!=0){
					$sql_insert1 = "update ccnd02_proyectos set plan_desembolso='$plan_ejecucion' where ".$filtro2;
					$this->ccnd02_proyectos->execute($sql_insert1);
					if($this->ccnd02_proyectos_actividad_desembolso->FindCount($filtro)==0){
						$sql_insert = "INSERT INTO ccnd02_proyectos_actividad_desembolso VALUES('$cod_republica','$cod_estado','$cod_municipio','$cod_parroquia','$cod_centro','$cod_concejo','$ano','$cod_proyecto','$cod_actividad','$campo_semanas')";
						$sw = $this->ccnd02_proyectos_actividad_desembolso->execute($sql_insert);
						if($sw>1){
							$this->set('Message_existe', 'registro exitoso');
							if(isset($pagina)){
								echo" <script> ver_documento('/ccnp01_plan_desembolso/consulta/$pagina','tab_pestana_descripcion_proyecto_5'); </script>";
							}else{
								echo" <script> ver_documento('/ccnp01_plan_desembolso/datos/$cod_actividad','carga'); </script>";
							}
						}else{
						...
						$sql_insert = "update ccnd02_proyectos_actividad_desembolso set semanas='$campo_semanas' where ".$filtro;
						$sw = $this->ccnd02_proyectos_actividad_desembolso->execute($sql_insert);
						if($sw>1){
...
*/
	...
	$equipo=$this->ccnd02_proyectos_actividad_desembolso->execute("select * from ccnd02_proyectos_actividad_desembolso where ".$filtro);
	if($equipo!=null){
		...
		$elimina=$this->ccnd02_proyectos_actividad_desembolso->execute("select * from ccnd02_proyectos_actividad_desembolso where ".$filtro2." order by cod_actividad desc");
	...
	$this->set('numero_actividad_registrada',$this->ccnd02_proyectos_actividad_desembolso->FindCount($filtro2));
...
function eliminar($var=null,$pagina=null){
	...
	$sw=$this->ccnd02_proyectos_actividad_desembolso->execute("delete from ccnd02_proyectos_actividad_desembolso where ".$filtro);
	if($this->ccnd02_proyectos_actividad_desembolso->FindCount($filtro2)==0){
		$sql_insert = "update ccnd02_proyectos set plan_desembolso='0' where ".$filtro2;
		$sw = $this->ccnd02_proyectos_actividad_desembolso->execute($sql_insert);
	}
	if($sw>1){
		...
		if(isset($pagina)){
			...
			echo" <script> ver_documento('/ccnp01_plan_desembolso/datos/$var','carga'); </script>";
		}
...
function consulta($pagina=null) {
	...
	if(isset($pagina)){
		$Tfilas=$this->ccnd02_proyectos_actividad_desembolso->findCount($conditions);
        if($Tfilas!=0){
        	$x=$this->ccnd02_proyectos_actividad_desembolso->findAll($conditions,null,"cod_actividad ASC",1,$pagina,null);
...
*/
	...
	$filtro  = "cod_republica=".$cod_republica." and cod_estado=".$cod_estado." and cod_municipio=".$cod_municipio." and cod_parroquia=".$cod_parroquia." and cod_centro=".$cod_centro." and cod_concejo=".$cod_concejo." and ano=".$ano." and cod_proyecto='".$cod_proyecto."' and cod_actividad=".$x[0]["ccnd02_proyectos_actividad_desembolso"]["cod_actividad"];
	$filtro2= "cod_republica=".$cod_republica." and cod_estado=".$cod_estado." and cod_municipio=".$cod_municipio." and cod_parroquia=".$cod_parroquia." and cod_centro=".$cod_centro." and cod_concejo=".$cod_concejo." and ano=".$ano." and cod_proyecto='".$cod_proyecto."'";
	...
	$this->set('cod_actividad',$x[0]["ccnd02_proyectos_actividad_desembolso"]["cod_actividad"]);
