...
class Cstp30DebitoIvaController extends AppController{

    var $name = "cstp30_debito_iva";
 	var $uses = array('cstd30_debito_cuerpo', 'cstd30_debito_partidas', 'cstd07_retenciones_cuerpo_iva', 'cstd07_retenciones_partidas_iva',
                      ...
                      'cfpd05', 'cugd04', 'cfpd23', 'cepd03_ordenpago_facturas', 'cugd02_institucion', 'cugd02_dependencia', 'cstd03_movimientos_manuales',
...
//    cstd30_debito_cuerpo
  ...
  $ano_movimiento                         =         $this->data['cstp30_debito_iva']['ano'];
  $ann = $ano_movimiento;
  $cod_entidad_bancaria                   =         $this->data['cstp30_debito_iva']['entidad'];
  $cod_sucursal                           =         $this->data['cstp30_debito_iva']['sucursal'];
  $cuenta_bancaria                        =         $this->data['cstp30_debito_iva']['cuenta'];
  $numero_debito                          =         $this->data['cstp30_debito_iva']['numero_debito'];
  $fecha_debito                           =         $this->Cfecha($this->data['cstp30_debito_iva']['fecha'], 'A-M-D');
  $fd                                     =         $this->data['cstp30_debito_iva']['fecha'];
  $concepto                               =         $this->data['cstp30_debito_iva']['concepto'];
  //$status_debito                        =         $this->data['cstp30_debito_iva']['status_debito'];
  //$clase_beneficiario                   =         $this->data['cstp30_debito_iva']['clase_beneficiario'];
...
if(isset($this->data['cstp30_debito_iva']['ano_orden_pago_'.$var_aux])){

						if($ve2[0]['ano_orden_pago']    == $this->data['cstp30_debito_iva']['ano_orden_pago_'.$var_aux] &&
						   $ve2[0]['numero_orden_pago'] == $this->data['cstp30_debito_iva']['numero_orden_pago_'.$var_aux]
						){
						...
						ORDER BY
						           ...
						           cod_auxiliar ASC; ");
																		...
																		$fecha_debito_comp = $datos_ordenes[0]['cepd03_ordenpago_cuerpo']['fecha_cheque'];

																		$mes_comprobante  =  $fecha_debito_comp[5].$fecha_debito_comp[6];
																		$dia_comprobante  =  $fecha_debito_comp[8].$fecha_debito_comp[9];
																		...
																		$concate = $this->AddCeroR2(substr($ve['cstd07_retenciones_partidas_iva']['cod_partida'], -2) , substr($ve['cstd07_retenciones_partidas_iva']['cod_partida'], 0, 1 ) ).'.'.$this->AddCeroR2($ve['cstd07_retenciones_partidas_iva']['cod_generica']).'.'.$this->AddCeroR2($ve['cstd07_retenciones_partidas_iva']['cod_especifica']).'.'.$this->AddCeroR2($ve['cstd07_retenciones_partidas_iva']['cod_sub_espec']);
																		  ...
																		  $monto                               =         $this->Formato1($this->data['cstp30_debito_iva']['monto_'.$jj]);
																		  $ano_orden_pago                      =         $ve['cstd07_retenciones_partidas_iva']['ano_orden_pago'];
																		  $numero_orden_pago                   =         $ve['cstd07_retenciones_partidas_iva']['numero_orden_pago'];
...
if($this->cstd02_cuentas_bancarias->execute("UPDATE cstd02_cuentas_bancarias SET cheque_dia=".$cheque_dia.", cheque_mes=".$cheque_mes.", cheque_ano=".$cheque_ano.", monto_cheque_por_emitir=".$monto_cheque_por_emitir.", disponibilidad_libro=".$disponibilidad_libro."   WHERE cod_presi='".$cod_presi."' and cod_entidad='".$cod_entidad."' and  cod_tipo_inst='".$cod_tipo_inst."' and  cod_inst='".$cod_inst."' and cod_dep='".$cod_dep."' and cod_entidad_bancaria = '".$cod_entidad_bancaria."' and   cod_sucursal='".$cod_sucursal."' and  cuenta_bancaria='".$cuenta_bancaria."'; ")>=1){}else{$opcion = 'no';}//fin else
     ...
     if($opcion=="si"){
		...
		$this->index(); $this->render('index');}//fin else


}else{	$this->cstd30_debito_cuerpo->execute("ROLLBACK;");  $this->set('errorMessage', 'Los datos no pueden ser almacenado verifique el n&uacute;mero de comprobante egreso'); $this->index(); $this->render('index'); }//fin else
}else{	$this->cstd30_debito_cuerpo->execute("ROLLBACK;");  $this->set('errorMessage', 'Los datos ya existen'); $this->index(); $this->render('index'); }//fin else
...
function guardar_anulacion2($var=null) {
  ...
  $condicion = "cod_presi = ".$cod_presi." and cod_entidad = ".$cod_entidad." and cod_tipo_inst = ".$cod_tipo_inst." and cod_inst = ".$cod_inst." and cod_dep = ".$cod_dep;
	...
	if(isset($this->data["cstp30_debito_iva"])){


		$condicion = 'cod_presi='.$this->Session->read('SScodpresi').'  and  cod_entidad='.$this->Session->read('SScodentidad').' and cod_tipo_inst='.$this->Session->read('SScodtipoinst').' and  cod_inst='.$this->Session->read('SScodinst').' and cod_dep='.$this->Session->read('SScoddep').' ';

		      $tipo_documento           =  251;
			  $concepto_anulacion       =  $this->data["cstp30_debito_iva"]["concepto_anulacion"];
			  $fecha_proceso_anulacion  =  date("d/m/Y");
			  $condicion_documento      =  2;//cuando se guarda es Activo=1
			  $fecha_debito            =   $this->data["cstp30_debito_iva"]["fecha_debito"];
			  $fd = $fecha_debito;
			  $cod_sucursal            =   $this->data["cstp30_debito_iva"]["cod_sucursal"];
			  $ano_movimiento          =   $this->data["cstp30_debito_iva"]["ano_movimiento"];
			  $cuenta_bancaria         =   $this->data["cstp30_debito_iva"]["cuenta_bancaria"];
			  $numero_debito           =   $this->data["cstp30_debito_iva"]["numero_debito"];
			  $cod_entidad_bancaria    =   $this->data["cstp30_debito_iva"]["cod_entidad_bancaria"];
...
$monto_debito    =   $this->data["cstp30_debito_iva"]["monto_debito"];
$beneficiario    =   $this->data["cstp30_debito_iva"]["beneficiario"];
$rif_cedula      =   $this->data["cstp30_debito_iva"]["rif_cedula"];
