 ...
 * sisap
 */
class Cnmp10comunesbolivaresasigController extends AppController {
   var $name = 'cnmp10_comunes_bolivares_asig';
   var $uses = array('Cnmd01',  'cnmd03_transacciones', 'cnmd10_control_escenarios', 'v_cnmd10_bolivares_asig_trans','cnmd10_bolivares_asig');
...
function select_trans($opc=null,$cod_nomina=null){
	...
	if($cod_nomina==null && $opc!=null){
		$cnmd03 = $this->cnmd03_transacciones->generateList3($conditions ='cod_tipo_transaccion = 1', $order = 'cod_transaccion ASC', $limit = null, '{n}.cnmd03_transacciones.cod_transaccion', '{n}.cnmd03_transacciones.denominacion');

		if($cnmd03){
		$this->concatena0($cnmd03, 'transaccion','0');
	...
	if($opc=='radio'){
		...
		$cnmd03 = $this->cnmd03_transacciones->generateList3($conditions ='cod_tipo_transaccion ='.$cod_nomina, $order = 'cod_transaccion ASC', $limit = null, '{n}.cnmd03_transacciones.cod_transaccion', '{n}.cnmd03_transacciones.denominacion');
		if($cnmd03){
...
function otras_nominas($var1=null,$var2=null){
	...
	$cod_dep                  =       $this->Session->read('SScoddep');
	$sql='';
	$datos=$this->cnmd10_bolivares_asig->findAll($this->condicion()." and cod_tipo_transaccion=1 and cod_tipo_nomina=".$var1,null,null);
			 if($datos){
			 	foreach($datos as $x){
			 		$transaccion=$x['cnmd10_bolivares_asig']['cod_transaccion'];
			 		if($sql==''){
			 			$sql.=$transaccion;
			 		}else{
			 			$sql.=",".$transaccion;
				...
				$opciones=$this->cnmd10_bolivares_asig->execute($query);
				if($opciones){
...
function datos($cod_nomina=null){
	...
	if($cod_nomina != null){
		...
		$this->set('deno_trans', $deno_trans);
		$datos = $this->cnmd10_bolivares_asig->findAll($conditions = $this->condicion()." and cod_tipo_nomina='$cod_nomina'", null, $order = 'cod_tipo_nomina, cod_transaccion', $limit = null, $page = null, $recursive = null);
...
function guardar(){
	...
	$monto = $this->Formato1($this->data['cnmp10_comunes_bolivares_asig']['monto']);
    $ubicacion_escenario = strtoupper('Asignaciones Comunes - en Bolivares');
	$prueba=$this->cnmd10_bolivares_asig->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion,null,null);

	if(!$prueba){
		if($this->Formato1($this->data['cnmp10_comunes_bolivares_asig']['monto'])==0){
			...
			if($cod_condicion==1){
				...
				$sql_insert = "INSERT INTO cnmd10_comunes_bolivares_asignacion VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_tipo_transaccion', '$cod_transaccion', '$monto', '$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
				$sw = $this->cnmd10_bolivares_asig->execute($sql_insert);
				if($sw > 1){
					...
					$sw = $this->cnmd10_bolivares_asig->execute($sql_control);
					echo'<script>';
	...
	$datos=$this->cnmd10_bolivares_asig->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1",null,'order by  cod_transaccion asc');
	 if($datos){
	 	 $this->set('datos',$datos);
	 }else{
	 	 $this->set('datos','');
...
//	$cod_frecuencia = $this->data['cnmp10_comunes_bolivares_asig']['frecuencia'];
	...
	if($monto==0){
		...
		echo'<script>';
		   ...
		   echo"document.getElementById('deno_trans_grilla').value = ''; ";
		   echo"document.getElementById('monto').value = ''; ";
		   echo"document.getElementById('monto').text = ''; ";
		   echo"document.getElementById('save').disabled ='disabled'; ";
		echo'</script>';
	}else{
		if($a!=1){
		$this->cnmd10_bolivares_asig->execute("update cnmd10_comunes_bolivares_asignacion set monto=".$monto.",cod_condicion=".$cod_condicion.",codi_tipo_transaccion=".$codi_tipo_transaccion.",codi_transaccion=".$codi_transaccion.",activar_frecuencia_eventual=".$activar_frecuencia_eventual." where ".$this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion);///modifica el monto
...
function eliminar($cod_nomina=null, $cod_trans=null,$i=null){
	$this->layout="ajax";
	if($cod_nomina!=null && $cod_trans!=null){
		$sql_eliminar="DELETE FROM cnmd10_comunes_bolivares_asignacion WHERE ".$this->condicion()." and cod_tipo_nomina='$cod_nomina' and cod_tipo_transaccion=1 and cod_transaccion='$cod_trans'";
		$sw = $this->cnmd10_bolivares_asig->execute($sql_eliminar);
		if($sw>1){
			...
			$sw = $this->cnmd10_bolivares_asig->execute($sql_del_control);
...
function ver($cod_nomina=null, $cod_trans=null, $codi=null,$condicion=null){
	$this->layout="ajax";
	if($cod_nomina!=null && $cod_trans!=null){
		$datos2 = $this->v_cnmd10_bolivares_asig_trans->findAll($this->condicion()." and cod_tipo_nomina='$cod_nomina'",null,'order by  cod_transaccion asc');
		...
		$opciones= $this->cnmd10_bolivares_asig->findAll($conditions = $this->condicion()." and cod_tipo_nomina='$cod_nomina' and cod_transaccion='$cod_trans'", $fields ='cod_frecuencia, cod_condicion, codi_tipo_transaccion, codi_transaccion, activar_frecuencia_eventual', $order = null, $limit = null, $page = null, $recursive = null);
		$this->set('opciones', $opciones);
...
function transferir($var1=null){
	$this->layout="ajax";
		$carga=$this->cnmd10_bolivares_asig->findAll($this->condicion()." and cod_tipo_nomina=".$var1." and cod_tipo_transaccion=1");
		if($carga){
...
function guardar_transferir(){
	$this->layout="ajax";
	  ...
	  $bandera=0;
	  $bandera=0;
	 $data=$this->cnmd10_bolivares_asig->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1");
	 $ubicacion_escenario = strtoupper('Asignaciones Comunes - en Bolivares');
	  foreach($data as $row){
			$cod_transaccion1 = $row['cnmd10_bolivares_asig']['cod_transaccion'];
			$monto = $row['cnmd10_bolivares_asig']['monto'];
			$cod_frecuencia = $row['cnmd10_bolivares_asig']['cod_frecuencia'];
			$cod_condicion = $row['cnmd10_bolivares_asig']['cod_condicion'];
			$codi_tipo_transaccion = $row['cnmd10_bolivares_asig']['codi_tipo_transaccion'];
			$codi_transaccion = $row['cnmd10_bolivares_asig']['codi_transaccion'];
			$activar_frecuencia_eventual = $row['cnmd10_bolivares_asig']['activar_frecuencia_eventual'];
			...
			if(!$datos){
				$sql_insert = "INSERT INTO cnmd10_comunes_bolivares_asignacion VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_transferir', '$cod_tipo_transaccion', '$cod_transaccion1','$monto','$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
				$sw = $this->cnmd10_bolivares_asig->execute($sql_insert);
				if($sw>1){
				echo "<script>";
