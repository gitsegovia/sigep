 ...
 */
 ...
 class Shp900CobranzaNumeroController extends AppController{
	var $name = "shp900_cobranza_numero";
 	var $uses = array('shd900_cobranza_numero','ccfd04_cierre_mes','cugd05_restriccion_clave','shd000_arranque');
...
$this->verifica_entrada('56');
	 ...
	 $max=$this->shd900_cobranza_numero->execute("SELECT MAX(numero_comprobante) as numero_mayor FROM shd900_cobranza_numero WHERE ".$this->SQLCA()." and ano_comprobante=".$ano);
     ...
     if(isset($var)){
     	if($var=="guardado"){
        	...
        	if($var=="xy"){
				$this->set('msg_error1', $msg_error1 = 'NECESITA CREAR O DESCONGELAR LOS NUMEROS DE COMPROBANTE PARA CONTINUAR');
			}else{
	...
	$Tfilas=$this->shd900_cobranza_numero->findCount($this->SQLCA()." and ano_comprobante=".$ano);
    if($Tfilas!=0){
    	$pagina=1;
		...
		$this->set('ultimo',$Tfilas);
 	    $datos_filas=$this->shd900_cobranza_numero->findAll($this->SQLCA()." and ano_comprobante=".$ano,null,"numero_comprobante ASC",1000,1,null);
    ...
    $this->set('modelo','shd900_cobranza_numero');
}//fin index


function guardar(){
	$this->layout="ajax";
	$modelo = 'shd900_cobranza_numero';
	if(isset($this->data[$modelo])){
			 ...
			 $camposT3="cod_presi,cod_entidad,cod_tipo_inst,cod_inst,cod_dep,ano_comprobante,numero_comprobante,situacion";
			 $values="";
			 $monto=0;
			 ...
			 $R3=$this->shd900_cobranza_numero->execute("INSERT INTO shd900_cobranza_numero (".$camposT3.") VALUES ".$valores.";");
             if($R3 > 1){
             	$msg="guardado";
             }else{
                 $msg=false;
...
function cambiar_situacion ($codigo,$ano,$var) {
	$this->layout="ajax";
    $resultado=$this->shd900_cobranza_numero->execute("UPDATE shd900_cobranza_numero SET situacion=1 WHERE ".$this->SQLCA()." and numero_comprobante=".$codigo." and ano_comprobante=".$ano);
	if($resultado>1){
        $this->set("Message_existe","Situacion del n&uacute;mero actualizada con exito");
        $m="si";
	}else{
...
function cambiar_situacion_celdacompleta($codigo,$ano,$var,$opc){
	...
	if($opc==1){//anular el numero de solicitud.
	    $resultado=$this->shd900_cobranza_numero->execute("UPDATE shd900_cobranza_numero SET situacion=4 WHERE ".$this->SQLCA()." and numero_comprobante=".$codigo." and ano_comprobante=".$ano);
	    $consulta_numero =$this->shd900_cobranza_numero->execute("SELECT ano_comprobante, numero_comprobante, situacion FROM shd900_cobranza_numero WHERE ".$this->SQLCA()." and numero_comprobante=".$codigo." and ano_comprobante=".$ano);
		if($resultado>1){
		...
		$this->set('codigo',$consulta_numero[0][0]["numero_comprobante"]);
		$this->set('ano',$consulta_numero[0][0]["ano_comprobante"]);
		$this->set('i',$var);
		$this->set('opc',$opc);

	}elseif($opc==2){//colocar como emitido el numero de solicitud.
	    $resultado=$this->shd900_cobranza_numero->execute("UPDATE shd900_cobranza_numero SET situacion=3 WHERE ".$this->SQLCA()." and numero_comprobante=".$codigo." and ano_comprobante=".$ano);
	    $consulta_numero =$this->shd900_cobranza_numero->execute("SELECT ano_comprobante, numero_comprobante, situacion FROM shd900_cobranza_numero WHERE ".$this->SQLCA()." and numero_comprobante=".$codigo." and ano_comprobante=".$ano);
		if($resultado>1){
	    ...
	    $resultado=$this->shd900_cobranza_numero->execute("UPDATE shd900_cobranza_numero SET situacion=5 WHERE ".$this->SQLCA()." and numero_comprobante=".$codigo." and ano_comprobante=".$ano);
	    $consulta_numero =$this->shd900_cobranza_numero->execute("SELECT ano_comprobante, numero_comprobante, situacion FROM shd900_cobranza_numero WHERE ".$this->SQLCA()." and numero_comprobante=".$codigo." and ano_comprobante=".$ano);
		if($resultado>1){
...
$this->layout = "ajax";
     ...
     if(isset($pagina)){
        ...
        if($Tfilas!=0){
			...
			$this->set('pag_cant',$pagina.'/'.$Tfilas);
			$this->set('ultimo',$Tfilas);
     	    $datos_filas=$this->shd900_cobranza_numero->findAll($this->SQLCA()." and ano_comprobante=".$ano,null,"numero_comprobante ASC",1000,$pagina,null);
	        $this->set("datosFILAS",$datos_filas);
	        $this->set('siguiente',$pagina+1);
			$this->set('anterior',$pagina-1);
			$this->bt_nav($Tfilas,$pagina);
...
function entrar(){
	...
	if(isset($this->data['cscp02_solicitud_numero']['login']) && isset($this->data['cscp02_solicitud_numero']['password'])){
		...
		$cond=$this->SQLCA()." and username='".$user."' and cod_tipo=56 and clave='".$paswd."'";
		if(($user==$l && $paswd==$c)){
			$this->set('autor_valido',true);
