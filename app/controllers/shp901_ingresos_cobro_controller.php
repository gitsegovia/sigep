<?php
 ...
 class Shp901IngresosCobroController extends AppController{
 	var $name = "shp901_ingresos_cobro";
	var $uses = array('v_shd001_registro_contribuyentes','shd001_registro_contribuyentes','shd901_ingresos_cobro','shd900_ingresos_numero',
                       'shd100_actividades', 'cnmd06_profesiones','cugd01_republica', 'cugd01_estados','ccfd04_cierre_mes','shd999_cobranza_acumulada',
                      'cugd01_municipios', 'cugd01_parroquias', 'cugd01_centropoblados', 'cugd01_vialidad', 'cugd01_vereda','v_shd901_ingresos_cobro',
                      'cstd02_cuentas_bancarias','cstd01_entidades_bancarias','cstd01_sucursales_bancarias','v_consulta_ingreso','v_ingresos_cobro','shd002_cobradores','shd000_arranque','cfpd03');
...
function index($var=null){///////////////<<--INDEX
     ...
     $maxi=$this->shd900_ingresos_numero->findCount($this->SQLCA()." and ano_comprobante=".$ano." and situacion=1");
      ...
      if($maxi==0){
         $this->set("errorMessage","Verifique el n&uacute;mero de control de compromisos");
      	 ...
      	 $this->redirect("/shp900_ingresos_numero/");
      }
}//fin index
...
function index2($var=null){
    ...
    $numero_comprobante=$this->shd900_ingresos_numero->field('shd900_ingresos_numero.numero_comprobante', $this->SQLCA()." and ano_comprobante='$ano' and situacion=1", $order ="numero_comprobante ASC");
    if(!empty($numero_comprobante)){
  		$this->set('numero_comprobante',          $numero_comprobante);
  		$this->set('ano_comprobante',          $ano);
  		$this->shd900_ingresos_numero->execute('UPDATE shd900_ingresos_numero set situacion=2 WHERE '.$this->SQLCA()." and ano_comprobante='$ano' and numero_comprobante = '$numero_comprobante'");
    }else{
  		$this->set('msg_error1', $msg_error1 = 'NECESITA CREAR LOS NUMEROS DE COMPROBANTE PARA CONTINUAR');
  		$this->redirect('/shp900_ingresos_numero');
	    return;
    }
    $this->concatena($this->shd002_cobradores->generateList(null, 'rif_ci ASC', null, '{n}.shd002_cobradores.rif_ci', '{n}.shd002_cobradores.nombre_razon'), 'lista_cobradores');
...
function guardar(){
	...
	extract($this->data['shp901_ingresos_cobro']);
	...
	$sql_insert        = "INSERT INTO shd901_ingresos_cobro VALUES (".implode(',',$values).");";
    ...
    $sql_update_acumulada = "UPDATE shd999_cobranza_acumulada SET
       ...
       monto_pagos_efectivo=monto_pagos_efectivo+$monto_efectivo  WHERE ".$condicion_acumulado;
        $this->shd900_ingresos_numero->execute("BEGIN;");
		$sw = $this->shd900_ingresos_numero->execute($sql_insert);
		if($sw>1){
		    ...
		    if($X1>1){
		    	...
		    	if($contar_cfpd03==0){
		    		$sw2 = $this->shd900_ingresos_numero->execute($sql_insert_cfpd03);
		    	}else{
		    		$sw2 = $this->shd900_ingresos_numero->execute("UPDATE cfpd03 SET monto_facturado=monto_facturado+$monto_total , monto_cobrado=monto_cobrado+$monto_total WHERE ".$condicion_cfpd03);
		    	}
		    	if($sw2>1){
		    		$this->set('exito', 'LOS DATOS FUERON GUARDADOS');
			        $this->shd900_ingresos_numero->execute('UPDATE shd900_ingresos_numero set situacion=3 WHERE '.$this->SQLCA()." and ano_comprobante='$ano_comprobante' and numero_comprobante = '$numero_comprobante'");
		            $this->set('guadado',true);
		    		$this->shd900_ingresos_numero->execute("COMMIT;");
		    	}else{
		    		$this->shd900_ingresos_numero->execute("ROLLBACK;");
		    	}
		    }else{
                $this->shd900_ingresos_numero->execute("ROLLBACK;");
		    }
...
function consultar($pagina=null){//echo 'si llego';
         ...
         if(isset($pagina) && $pagina!=null){
          	 ...
          	 $Tfilas=$this->v_ingresos_cobro->findCount($condicion);
          	 if($Tfilas==0){
          	 ...
          	 if($Tfilas!=0){
          	 $this->set('pag_cant',$pagina.'/'.$Tfilas);
          	 $datos=$this->v_ingresos_cobro->findAll($condicion,null,'numero_comprobante ASC',1,$pagina,null);
          	 $this->set('datos',$datos);
          	 //$this->set('datos2',$datos2);
          	 $this->set('siguiente',$pagina+1);
          	 $this->set('anterior',$pagina-1);
             $this->bt_nav($Tfilas,$pagina);
               $cond =" rif_ci='".$datos[0]['v_ingresos_cobro']['rif_ci_cobrador']."'";
			   $cobrador = $this->shd002_cobradores->findAll($cond);
			   $this->set('nombre_razon',$cobrador[0]['shd002_cobradores']['nombre_razon']);
             }
            // echo"hola";
...
function eliminar($ano_comprobante=null,$numero_comprobante=null,$pagina=null){
 	...
 	$data = $this->v_shd901_ingresos_cobro->findAll($cond);
    extract($data[0]['v_shd901_ingresos_cobro']);
    ...
    $sql_update_acumulada = "UPDATE shd999_cobranza_acumulada SET
       ...
       monto_pagos_efectivo=monto_pagos_efectivo-$monto_efectivo  WHERE ".$condicion_acumulado;
        ...
        $this->shd900_ingresos_numero->execute("BEGIN;");
        $res=$this->v_shd901_ingresos_cobro->execute("UPDATE shd901_ingresos_cobro SET condicion_documento=2, fecha_anulacion='".date('Y-m-d')."',ano_anulacion=".date('Y').", username_anulacion='$username_anulacion'  WHERE ".$cond);
	    //echo $sql_update_acumulada;
	    if($res>1){
           $res=$this->v_shd901_ingresos_cobro->execute($sql_update_acumulada);
...
function buscar_pista_recibo($var2=null, $var3=null){
    ...
    if($var3==null){
		...
		$Tfilas=$this->v_ingresos_cobro->findCount("denominacion_busqueda LIKE '%$var2%'");
		        if($Tfilas!=0){
		        	$pagina=1;
		        	$Tfilas=(int)ceil($Tfilas/50);
		        	$this->set('pag_cant',$pagina.'/'.$Tfilas);
					...
					$this->set('pagina_actual',$pagina);
					$this->set('ultimo',$Tfilas);
		     	    $datos_filas=$this->v_ingresos_cobro->findAll("denominacion_busqueda LIKE '%$var2%'",null,"ano_comprobante, numero_comprobante ASC",50,1,null);
			        $this->set("datosFILAS",$datos_filas);
			        $this->set('siguiente',$pagina+1);
					$this->set('anterior',$pagina-1);
					$this->bt_nav($Tfilas,$pagina);
		...
		$Tfilas=$this->v_ingresos_cobro->findCount("denominacion_busqueda LIKE '%$var22%'");
		        if($Tfilas!=0){
		        	$pagina=$var3;
		        	$Tfilas=(int)ceil($Tfilas/50);
		        	$this->set('pag_cant',$pagina.'/'.$Tfilas);
					...
					$this->set('pagina_actual',$pagina);
					$this->set('ultimo',$Tfilas);
		     	    $datos_filas=$this->v_ingresos_cobro->findAll("denominacion_busqueda LIKE '%$var22%'",null,"ano_comprobante, numero_comprobante ASC",50,$pagina,null);
			        $this->set("datosFILAS",$datos_filas);
			        $this->set('siguiente',$pagina+1);
					$this->set('anterior',$pagina-1);
					$this->bt_nav($Tfilas,$pagina);
...
function planilla () {
   ...
   $this->set('datos1',$this->v_ingresos_cobro->findAll($cond));
...
function salir($numero_comprobante=null){
	...
	//$ano=$this->ano_ejecucion();
	$ano = $this->shd000_arranque->ano($this->SQLCA());
    $this->shd900_ingresos_numero->execute('UPDATE shd900_ingresos_numero set situacion=1 WHERE '.$this->SQLCA()." and ano_comprobante='$ano' and numero_comprobante = '$numero_comprobante'");
