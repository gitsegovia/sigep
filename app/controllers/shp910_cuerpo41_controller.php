<?php
 class Shp910Cuerpo41Controller extends AppController{
...
function agregar_grilla(){
	...
	$rif_cedula    		       	= $this->data['shp910_cuerpo41']['rif_constribuyente'];
	$ano 						= $this->data['shp910_cuerpo41']['ano_deuda'];
	$cod_numero_catastral_placas= $this->data['shp910_cuerpo41']['placa'];
	$datos = $this->shd003_codigo_ingresos->findAll('cod_ingreso=4');
...
/*
	...
	*/
	 	   	$mes    			= $this->data['shp910_cuerpo41']['mes'];
   			$numero_recibo     	= $this->data['shp910_cuerpo41']['numero_recibo'];
   			$deuda_vigente	    = $this->Formato1($this->data['shp910_cuerpo41']['deuda_vigente']);
   			if($this->data['shp910_cuerpo41']['recargo']!=null){
   				$recargo			= $this->Formato1($this->data['shp910_cuerpo41']['recargo']);
   			}else{
				$recargo=0;
   			}

   			if($this->data['shp910_cuerpo41']['multa']!=null){
   				$multa			= $this->Formato1($this->data['shp910_cuerpo41']['multa']);
   			}else{
				$multa=0;
   			}

   			if($this->data['shp910_cuerpo41']['intereses']!=null){
   				$intereses			= $this->Formato1($this->data['shp910_cuerpo41']['intereses']);
   			}else{
				$intereses=0;
   			}
			if($this->data['shp910_cuerpo41']['descuentos']!=null){
   				$descuentos			= $this->Formato1($this->data['shp910_cuerpo41']['descuentos']);
   			}else{
				$descuentos=0;
   			}
   			$total 				= $this->Formato1($this->data['shp910_cuerpo41']['total']);
   			$cancelado 			= $this->data['shp910_cuerpo41']['cancelado'];
...
function buscar_detalles($ano){
	...
	$cod_partida 	= $datos[0]['shd003_codigo_ingresos']['cod_partida'];
  	...
  	$cod_auxiliar 	= $datos[0]['shd003_codigo_ingresos']['cod_auxiliar'];
  	$part='cod_partida='.$cod_partida.' and cod_generica='.$cod_generica.' and cod_especifica='.$cod_especifica.' and cod_sub_espec='.$cod_subespec.' and cod_auxiliar='.$cod_auxiliar;
	$datos2 = $this->shd900_planillas_deuda_cobro_detalles->findAll($this->SQLCA()." and rif_cedula='".$rif_cedula."' and ano=".$ano." and ".$part);
	$datos3 = $this->shd900_planillas_deuda_cobro_detalles->findAll($this->SQLCA()." and rif_cedula='".$rif_cedula."' and ".$part);
	if($datos3 !=null){
...
function guardar(){
	$this->layout = "ajax";
	$rif_cedula    		       	= $this->data['shp910_cuerpo41']['rif_constribuyente'];
	$deuda_anterior				= $this->Formato1($this->data['shp910_cuerpo41']['deuda_anterior']);
	$datos = $this->shd003_codigo_ingresos->findAll('cod_ingreso=4');
	$cod_partida 	= $datos[0]['shd003_codigo_ingresos']['cod_partida'];
  	$cod_generica 	= $datos[0]['shd003_codigo_ingresos']['cod_generica'];
  	$cod_especifica = $datos[0]['shd003_codigo_ingresos']['cod_especifica'];
