...
class Cstp07CancelacionesIslrController extends AppController{



    var $name = "cstp07_cancelaciones_islr";
  var $uses = array('v_cstd01_bancos','v_cstd01_sucursales','cstd03_cheque_cuerpo', 'cstd03_cheque_partidas', 'cstd07_retenciones_cuerpo_iva', 'cstd07_retenciones_partidas_iva',
                      ...
                      'cfpd05', 'cugd04', 'cfpd23', 'cugd02_dependencia', 'cepd03_ordenpago_facturas',
...
function guardar(){
  ...
  $ano_movimiento                         =         $this->data['cstp07_cancelaciones_islr']['ano'];
  $ann = $ano_movimiento;
  $cod_entidad_bancaria                   =         $this->data['cstp07_cancelaciones_islr']['entidad'];
  $cod_sucursal                           =         $this->data['cstp07_cancelaciones_islr']['sucursal'];
  $cuenta_bancaria                        =         $this->data['cstp07_cancelaciones_islr']['cuenta'];
  $numero_cheque                          =         $this->data['cstp07_cancelaciones_islr']['numero_cheque'];
  $fecha_cheque                           =         $this->Cfecha($this->data['cstp07_cancelaciones_islr']['fecha'], 'A-M-D');
  $fd                                     =         $this->data['cstp07_cancelaciones_islr']['fecha'];
  $concepto                               =         $this->data['cstp07_cancelaciones_islr']['concepto'];
  //$status_cheque                          =         $this->data['cstp07_cancelaciones_islr']['status_cheque'];
  //$clase_beneficiario                     =         $this->data['cstp07_cancelaciones_islr']['clase_beneficiario'];
            ...
            if(!empty($fecha_cheque)){
              if(!empty($concepto)){
                if(!empty($rif_cedula)){
                        ...
                        if($this->cstd03_cheque_cuerpo->findCount("cod_presi='".$cod_presi."'  and cod_entidad='".$cod_entidad."' and cod_tipo_inst='".$cod_tipo_inst."' and  cod_inst='".$cod_inst."' and  cod_dep='".$cod_dep."' and ano_movimiento='".$ano_movimiento."'  and  cod_entidad_bancaria='".$cod_entidad_bancaria."' and cod_sucursal='".$cod_sucursal."' and cuenta_bancaria='".$cuenta_bancaria."' and numero_cheque='".$numero_cheque."' ") == 0){
                          ...
                          if($beneficiario!=""){
                            if($this->cstd06_comprobante_cuerpo_egreso->findCount("cod_presi='".$cod_presi."'  and cod_entidad='".$cod_entidad."' and cod_tipo_inst='".$cod_tipo_inst."' and  cod_inst='".$cod_inst."' and  cod_dep='".$cod_dep."' and ano_comprobante_egreso='".$ano_movimiento."'  and  numero_comprobante_egreso='".$numero_comprobante_egreso."'  ") == 0){
                              ...
                              $dato = $this->cepd03_ordenpago_cuerpo->execute("select
                                                    a.cod_presi,
                                                    a.cod_entidad,
                                                    a.cod_tipo_inst,
                                                    a.cod_inst,
                                                    ...
                                                    a.monto                 as   monto_cuerpo_retencion,
                                                      ...
                                                      c.autorizado,
                                                      c.cod_tipo_pago,
                                                      c.codigo_retencion_islr,
                                                      c.cod_actividad,
                                                      c.porcentaje_islr,
                                                      c.monto_descontar_impuesto
                              ...
                              if(isset($dato)){
                                if($dato!=null){
                                  foreach($dato as $ve2){
                                    ...
                                    $codigo_retencion_islr     =  $ve2[0]['codigo_retencion_islr'];
                                    $cod_actividad             =  $ve2[0]['cod_actividad'];
                                    $porcentaje_islr           =  $ve2[0]['porcentaje_islr'];
                                    $monto_descontar_impuesto  =  $ve2[0]['monto_descontar_impuesto'];


                                    if(isset($this->data['cstp07_cancelaciones_islr']['ano_orden_pago_'.$var_aux])){

                                      if($ve2[0]['ano_orden_pago']    == $this->data['cstp07_cancelaciones_islr']['ano_orden_pago_'.$var_aux] && $ve2[0]['numero_orden_pago'] == $this->data['cstp07_cancelaciones_islr']['numero_orden_pago_'.$var_aux]){
                                        ...
                                        $sql_cstd06_comprobante_poremitir_egreso.= "VALUES ('".$cod_presi."', '".$cod_entidad."', '".$cod_tipo_inst."', '".$cod_inst."', '".$cod_dep."', '".$username."', '".$ano_comprobante_egreso."', '".$numero_comprobante_egreso."', '".$ve2[0]['ano_orden_pago']."', '".$ve2[0]['clase_orden']."', '".$ve2[0]['numero_orden_pago']."','10'); ";
    
                                        if($this->cstd06_comprobante_poremitir_egreso->execute($sql_cstd06_comprobante_poremitir_egreso)>=1){}else{$opcion = 'no';}//fin else

                                        $dato2 = $this->cepd03_ordenpago_cuerpo->execute("select
                                                a.cod_presi,
                                                a.cod_entidad,
                                                a.cod_tipo_inst,
                                                a.cod_inst,
                                        ...
                                        if(isset($datos_orden_pago_partidas)){
                                          if($datos_orden_pago_partidas!=null){
                                            ...
                                            foreach($datos_orden_pago_partidas as $ve){
                                              ...
                                              $monto          = $this->Formato1($this->data['cstp07_cancelaciones_islr']['monto_'.$jj]);
                                            ...
                                            }//fin foreach datos_orden_partidas linea 1092
                                          }//fin
                                        }//fin
                                        ...
                                        $datos_cepd03_ordenpago_facturas = $this->cepd03_ordenpago_facturas->findAll($condicion. ' and  ano_orden_pago='.$ve2[0]['ano_orden_pago'].'  and  numero_orden_pago='.$ve2[0]['numero_orden_pago'], null, "numero_factura, numero_control ASC");
                                        ...
                                        foreach($datos_cepd03_ordenpago_facturas as $aux_datos_cepd03_ordenpago_facturas){
                                          ...
                                          $rif_institucion =  str_replace("-","",$rif_institucion);

                                          if($inicio_cadena==""){
                                            $inicio_cadena='<?xml version="1.0" encoding="ISO-8859-1" standalone="no"?>'."\n".'<RelacionRetencionesISLR RifAgente="'.$rif_institucion.'" Periodo="'.$periodo.'">';
                                          }
                                          if($codigo_retencion_islr==""){$codigo_retencion_islr=0;}
                                            $cadena .= '<DetalleRetencion>
                                                  <RifRetenido>'.$rif_empresa.'</RifRetenido>
                                                  <NumeroFactura>'.mascara($numero_factura,10).'</NumeroFactura>
                                                  <NumeroControl>'.$numero_control.'</NumeroControl>
                                                  <CodigoConcepto>'.$codigo_retencion_islr.'</CodigoConcepto>
                                                  <MontoOperacion>'.$monto_sub_total.'</MontoOperacion>
                                                  <PorcentajeRetencion>'.$porcentaje_islr.'</PorcentajeRetencion>
                                            </DetalleRetencion>'."\n";

                                          $datosXml[$contador][1]=$rif_empresa;
                                          $datosXml[$contador][2]=mascara($numero_factura,10);
                                          $datosXml[$contador][3]=$numero_control;
                                          $datosXml[$contador][4]=$codigo_retencion_islr;
                                          $datosXml[$contador][5]=$monto_sub_total;
                                          $datosXml[$contador][6]=$porcentaje_islr;
                                          $datosXml[$contador][7]=date("d/m/Y");
                                          $contador++;
                                            
                                          if($fin_cadena==""){
                                            $fin_cadena='</RelacionRetencionesISLR>';
                                          }
                                  ...
                                  }//fin foreach dato as ve2 linea 951
                                }//fin dato!=null

                                $_SESSION["DATAXMLEX"]=$datosXml;

                                $this->set('titulo',"RelacionRetencionesISLR RifAgente=".$rif_institucion." Periodo=".$periodo);
                              ...
                              /////////////////////FIN ORDENES DE PAGO////////////////////////////////
                              $this->wFile_xml('isrl_'.$cod_dep.'_'.date('d_m_Y'), $cadena);
                              if(file_exists('../webroot/descargas/isrl_'.$cod_dep.'_'.date('d_m_Y').'.xml')){
                                  chmod('../webroot/descargas/isrl_'.$cod_dep.'_'.date('d_m_Y').'.xml', 0777);
                              }
                              $this->set('name', 'isrl_'.$cod_dep.'_'.date('d_m_Y'));
                              ...
                              if($opcion=="si"){
                                ...
                                $this->cstd03_cheque_cuerpo->execute("ROLLBACK;");
                                $this->set('errorMessage', 'La datos no fueron almacenados correctamente..');
                              }//fin if else opcion si liena 1267
                            
                            }else{  

                              $this->cstd03_cheque_cuerpo->execute("ROLLBACK;"); 
                              $this->set('errorMessage', 'Los datos no pueden ser almacenado verifique el n&uacute;mero de comprobante egreso');  
                            }//fin if else numero_comprobante_egreso linea 792
                          
                          }else{  
                            $this->cstd03_cheque_cuerpo->execute("ROLLBACK;"); 
                            $this->set('errorMessage', 'Los datos no pueden ser almacenados, faltan datos beneficiario');
                          }//fin if else beneficiario!="" linea 791

                        }else{
                          $this->cstd03_cheque_cuerpo->execute("ROLLBACK;");
                          $this->set('errorMessage', 'Los datos ya existen'); 
                        }//fin if else datos_cheque linea 783
                  ...
                  $this->set('errorMessage', 'No existe el rif del beneficiario'); 
                  $this->index(); $this->render("index"); 
                }
              }else{ $this->set('errorMessage', 'Los datos no pueden ser almacenados7');}
            }else{  $this->set('errorMessage', 'Los datos no pueden ser almacenados6');}
          }else{ $this->set('errorMessage', 'Los datos no pueden ser almacenados5');}
        }else{  $this->set('errorMessage', 'Los datos no pueden ser almacenados4');}
      }else{ $this->set('errorMessage', 'Los datos no pueden ser almacenados3');}
    }else{ $this->set('errorMessage', 'Los datos no pueden ser almacenados2');}
  }else{  $this->set('errorMessage', 'Los datos no pueden ser almacenados1');}
...
$this->set('datos_cheque_partidas', $datos_cheque_partidas);
  ...
  $anhos_cheque_ordenes=$numero[0]['ano_movimiento'];

  foreach ($datos_cheque_ordenes as $value) {

    $pos = strpos($anhos_cheque_ordenes, $value['cstd03_cheque_ordenes']['ano_orden_pago']);

    if ($pos === false) {
      $anhos_cheque_ordenes .= ",".$value['cstd03_cheque_ordenes']['ano_orden_pago'];
    }
  }

  $ordenes_cheque_ordenes="";

  foreach ($datos_cheque_ordenes as $value) {
    if($ordenes_cheque_ordenes==""){
      $ordenes_cheque_ordenes = $value['cstd03_cheque_ordenes']['numero_orden_pago'];
    }else{
      $ordenes_cheque_ordenes .=  ",".$value['cstd03_cheque_ordenes']['numero_orden_pago'];
    }
  }
...
$datos_orden_pago_cuerpo = $this->cepd03_ordenpago_cuerpo->findAll($condicion." and ano_orden_pago IN(".$anhos_cheque_ordenes.") and numero_orden_pago IN (".$ordenes_cheque_ordenes.")");
...
if(!empty($this->data["cstp07_cancelaciones_islr"]["fecha_cheque"]) && !empty($this->data["cstp07_cancelaciones_islr"]["concepto_anulacion"]) && !empty($this->data["cstp07_cancelaciones_islr"]["cod_sucursal"]) && !empty($this->data["cstp07_cancelaciones_islr"]["cuenta_bancaria"]) && !empty($this->data["cstp07_cancelaciones_islr"]["cod_entidad_bancaria"]) && !empty($this->data["cstp07_cancelaciones_islr"]["numero_cheque"])){

    $condicion = 'cod_presi='.$this->Session->read('SScodpresi').'  and  cod_entidad='.$this->Session->read('SScodentidad').' and cod_tipo_inst='.$this->Session->read('SScodtipoinst').' and  cod_inst='.$this->Session->read('SScodinst').' and cod_dep='.$this->Session->read('SScoddep').' ';

          $tipo_documento           =  251;
        $concepto_anulacion       =  $this->data["cstp07_cancelaciones_islr"]["concepto_anulacion"];
        $fecha_proceso_anulacion  =  date("d/m/Y");
        $condicion_documento      =  2;//cuando se guarda es Activo=1
        $fecha_cheque             =   $this->data["cstp07_cancelaciones_islr"]["fecha_cheque"];
        $fd = $fecha_cheque;
        $cod_sucursal            =   $this->data["cstp07_cancelaciones_islr"]["cod_sucursal"];

        $ano_movimiento_canc     =   $this->data["cstp07_cancelaciones_islr"]["ano_movimiento"];
        $ano_movimiento          =   $this->ano_ejecucion();

        $cuenta_bancaria         =   $this->data["cstp07_cancelaciones_islr"]["cuenta_bancaria"];
        $numero_cheque           =   $this->data["cstp07_cancelaciones_islr"]["numero_cheque"];
        $cod_entidad_bancaria    =   $this->data["cstp07_cancelaciones_islr"]["cod_entidad_bancaria"];
...
$monto_cheque    =   $this->data["cstp07_cancelaciones_islr"]["monto_cheque"];
$beneficiario    =   $this->data["cstp07_cancelaciones_islr"]["beneficiario"];
$rif_cedula      =   $this->data["cstp07_cancelaciones_islr"]["rif_cedula"];
