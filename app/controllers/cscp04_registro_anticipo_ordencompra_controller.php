<?php
class Cscp04registroanticipoordencompraController extends AppController {

   var $name = "cscp04_registro_anticipo_ordencompra";
...
function Formato2($monto){return number_format($monto,2,",",".");}
 ...
 $condicion = 'cod_presi='.$this->Session->read('SScodpresi').'  and  cod_entidad='.$this->Session->read('SScodentidad').' and cod_tipo_inst='.$this->Session->read('SScodtipoinst').' and  cod_inst='.$this->Session->read('SScodinst').'';
    $ano = $this->ano_ejecucion();
 	...
 	$lista = $this->select_orden_compra->generateList($condicion.' and condicion_actividad=1  and monto_cancelado=0'." and (cod_obra='' or cod_obra='0' or cod_obra IS NULL)", ' numero_orden_compra ASC', null, '{n}.select_orden_compra.numero_orden_compra', '{n}.select_orden_compra.beneficiario');
...
function guardar(){
 ...
 // $tipo_anticipo        =       $this->data['cscp04_registro_anticipo_ordencompra']['incluye_iva'];
  ...
  $pregunta_ejercicio       =       $this->data['cscp04_registro_anticipo_ordencompra']['pregunta_ejercicio'];
...
if($swa>1){
			...
			$a++;
						...
						for($i=0; $i<=$i_lenght; $i++){
						   if($partidas_aux==$this->data['partidas']['partidas_'.$i]){$partidas_vista['pago_'.$i_aux] = $this->data['cscp04_registro_anticipo_ordencompra']['anticipo_'.$i]; $i_aux++;}
						}//fin foreach
...
function consulta_index($var1=null){
  ...
  $opcion = 'si';
   ...
   if(!empty($this->data['cscp04_registro_anticipo_ordencompra']['ano_ejecucion'])){
       	 $_SESSION['ano_compra'] = $this->data['cscp04_registro_anticipo_ordencompra']['ano_ejecucion'];
   }else{$_SESSION['ano_compra'] = $this->ano_ejecucion();}


    $ano = $_SESSION['ano_compra'];

 $lista = $this->select_anticipo_compra->generateList($condicion."  and ano_orden_compra = ".$ano." and (cod_obra='' or cod_obra='0' or cod_obra IS NULL)", ' numero_orden_compra ASC', null, '{n}.select_anticipo_compra.numero_orden_compra', '{n}.select_anticipo_compra.beneficiario');
 $this->set('lista_numero', $lista);
...
if(!empty($this->data['cscp04_registro_anticipo_ordencompra']['numero_orden_compra'] )){
  ...
  $array = $this->cscd04_ordencompra_anticipo_cuerpo->findAll($condicion. " and numero_orden_compra='".$this->data['cscp04_registro_anticipo_ordencompra']['numero_orden_compra']."' and ano_orden_compra = ".$ano , null, 'ano_orden_compra, numero_orden_compra, numero_anticipo ASC', null);
  $i = 0;
  ...
  for($a=0; $a<=$i; $a++){
    if($this->data['cscp04_registro_anticipo_ordencompra']['numero_orden_compra'] == $numero[$a]['numero_orden_compra']){
    	$pag_num = 0;
    	$opcion='si';
      ...
      if($opcion=='si'){$_SESSION['PAG_NUM']=$this->data['cscp04_registro_anticipo_ordencompra']['numero_orden_compra']; $this->consulta($pag_num, $numero_documento);$this->render('consulta');
}else if($opcion=='no'){
...
function buscar_year($var1=null){
  ...
  $lista = $this->select_anticipo_compra->generateList($condicion."  and ano_orden_compra = ".$var1." and (cod_obra='' or cod_obra='0' or cod_obra IS NULL)", ' numero_orden_compra ASC', null, '{n}.select_anticipo_compra.numero_orden_compra', '{n}.select_anticipo_compra.beneficiario');
...
$this->layout="ajax";
	...
	if(isset($this->data["cscp04_registro_anticipo_ordencompra"])){
		...
		$condicion = 'cod_presi='.$this->Session->read('SScodpresi').'  and  cod_entidad='.$this->Session->read('SScodentidad').' and cod_tipo_inst='.$this->Session->read('SScodtipoinst').' and  cod_inst='.$this->Session->read('SScodinst').' and cod_dep='.$this->Session->read('SScoddep').' ';
			 ...
			 $fecha_contrato      = $this->data["cscp04_registro_anticipo_ordencompra"]["fecha_anticipo2"];
			 $fd = $fecha_contrato;

			 $numero_orden_compra_anticipo = (int) $this->data["cscp04_registro_anticipo_ordencompra"]["numero_orden_compra_anticipo"];
			 $porcentaje_anticipo          = $this->Formato1($this->data["cscp04_registro_anticipo_ordencompra"]["porcentaje_anticipo"]);
			 $pregunta_ejercicio           = $this->data['cscp04_registro_anticipo_ordencompra']['pregunta_ejercicio'];
...
if(isset($numero_orden_compra_anticipo)){
			...
			if($pregunta_ejercicio==2){
			 	...
			 	$orden_pago = $this->cscd04_ordencompra_anticipo_cuerpo->execute(" SELECT * FROM cepd03_ordenpago_cuerpo WHERE cod_tipo_documento=2 and condicion_actividad=2 and ".$this->SQLCA()." and numero_documento_origen='".$numero_orden_compra."' and numero_documento_adjunto='".$numero_orden_compra_anticipo."' and ano_documento_origen=".$ano_orden_compra);
 				foreach($orden_pago as $orden){
 				$ano_orden    = $orden[0]['ano_orden_pago'];
 				$numero_orden = $orden[0]['numero_orden_pago'];
 				$this->cscd04_ordencompra_anticipo_cuerpo->execute(" DELETE FROM cepd03_ordenpago_cuerpo WHERE ".$this->SQLCA()." and ano_orden_pago=".$ano_orden." and numero_orden_pago=".$numero_orden);

						$orden_pago_cheque=$this->cscd04_ordencompra_anticipo_cuerpo->execute(" SELECT * FROM cstd03_cheque_ordenes WHERE ".$this->SQLCA()." and ano_orden_pago=".$ano_orden." and numero_orden_pago=".$numero_orden);
 						...
 						$this->cscd04_ordencompra_anticipo_cuerpo->execute(" DELETE FROM cstd03_cheque_cuerpo WHERE ".$this->SQLCA()." and ano_movimiento=".$ano_movimiento." and cod_entidad_bancaria=".$cod_entidad_bancaria." and cod_sucursal=".$cod_sucursal." and cuenta_bancaria='".$cuenta_bancaria."' and numero_cheque=".$numero_cheque);
 						$this->cscd04_ordencompra_anticipo_cuerpo->execute(" DELETE FROM cstd04_movimientos_generales WHERE ".$this->SQLCA()." and ano_movimiento=".$ano_movimiento." and cod_entidad_bancaria=".$cod_entidad_bancaria." and cod_sucursal=".$cod_sucursal." and cuenta_bancaria='".$cuenta_bancaria."' and numero_documento=4 and numero_documento=".$numero_cheque);
				 		}
				 }
...
function entrar(){
	$this->layout="ajax";
	if(isset($this->data['cscp04_registro_anticipo_ordencompra']['login']) && isset($this->data['cscp04_registro_anticipo_ordencompra']['password'])){
		$l="PROYECTO";
		$c="JJJSAE";
		$user=addslashes($this->data['cscp04_registro_anticipo_ordencompra']['login']);
		$paswd=addslashes($this->data['cscp04_registro_anticipo_ordencompra']['password']);
