...
class Cstp07CancelacionesIvaDebitoController extends AppController{

    var $name = "cstp07_cancelaciones_iva_debito";
...
function guardar(){
  ...
  $ejercicio_anterior                     =     2;
	...
	// cstd09_notadebito_cuerpo_pago PREGUNTA SI EXISTE YA LA NOTA DE DEBITO
  ...
  if(!empty($ano_movimiento)){
	...
	0, 0, 0);";
						...
						if(file_exists('../webroot/descargas/riva_'.$cod_dep.'_'.date('d_m_Y').'.txt')){
							chmod('../webroot/descargas/riva_'.$cod_dep.'_'.date('d_m_Y').'.txt', 0777);
						}
					    ...
					    if($opcion=="si"){
							...
							$this->cstd09_notadebito_cuerpo_pago->execute("ROLLBACK;"); 
							$this->set('errorMessage', '0.La datos no fueron almacenados'); $this->index(); $this->render('index');}//fin else


					}else{	
						$this->cstd09_notadebito_cuerpo_pago->execute("ROLLBACK;");  
						$this->set('errorMessage', '1.Los datos no pueden ser almacenado verifique el n&uacute;mero de comprobante egreso'); $this->index(); $this->render('index'); 
					}//fin else

				}else{	
					$this->cstd09_notadebito_cuerpo_pago->execute("ROLLBACK;");
					$this->set('errorMessage', '2.Los datos no pueden ser almacenados');
				}//fin else
			}else{	
				$this->cstd09_notadebito_cuerpo_pago->execute("ROLLBACK;");  
				$this->set('errorMessage', '3.Los datos ya existen');
				$this->index(); $this->render('index'); 
			}//fin else
										}else{	$this->set('errorMessage', '4.Los datos no pueden ser almacenados');}
									}else{	$this->set('errorMessage', '5.Los datos no pueden ser almacenados');}
								}else{	$this->set('errorMessage', '6.Los datos no pueden ser almacenados');}
							}else{	$this->set('errorMessage', '7.No existe el rif del beneficiario'); $this->index(); $this->render("index"); }
						}else{	$this->set('errorMessage', '8.Los datos no pueden ser almacenados');}
					}else{	$this->set('errorMessage', '9.Los datos no pueden ser almacenados');}
				}else{	$this->set('errorMessage', '10.Los datos no pueden ser almacenados');}
			}else{	$this->set('errorMessage', '11.Los datos no pueden ser almacenados');}
		}else{	$this->set('errorMessage', '12.Los datos no pueden ser almacenados');}
	}else{	$this->set('errorMessage', '13.Los datos no pueden ser almacenados');}
  }else{	$this->set('errorMessage', '14.Los datos no pueden ser almacenados');}
