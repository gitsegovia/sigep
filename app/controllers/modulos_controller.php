...
//
//  OPTIMIZACION DEL FUNCIONAMIENTO DEL MENU CON LA BASE DE DATOS
//
/*
foreach ($mod as $key => $value) {
	if($modulo=='0'){
		if($key==0){
			if((isset($_SESSION["concejo_comunal"])  || ($cod_presi==1 && $cod_entidad==1 && $cod_tipo_inst==1 && $cod_inst==1 && $cod_dep==1))  && $_SESSION["nom_usuario"]!="ADMIN" ){
   				$vmodulos[$key]='modulos/#';
   			}else{
   				$vmodulos[$key]='administradors/uso_general';
   			}
		}else{
			$vmodulos[$key]='administradors/standard/'.strtolower($value);
		}
	}else{
		foreach ($modulo as $key_modulo => $value_modulo) {
			if($value==$value_modulo){
				$vmodulos[$key]='administradors/standard/'.strtolower($value);
			}else{
				$vmodulos[$key]='administradors/standard/'.strtolower("negar_menu");
			}
		}
	}
}
...
//
// FIN DE LA OPTIMIZACION ing. Jose Segovia
//
*/

//
// 	 FUNCION ORIGINAL PARA LA CREACION DEL MENU DEL SISTEMA
