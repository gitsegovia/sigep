 ...
 * sisap
 */
class Cnmp10EscalaIndividualDiasController extends AppController {
   var $name = 'cnmp10_escala_individual_dias';
   var $uses = array('Cnmd01',  'cnmd03_transacciones', 'cnmd10_control_escenarios','cnmd10_individual_dias','cnmd10_individual_dias_cantidad','v_cnmd06_fichas_datos_personales');
...
function carga_transaccion($nomina=null){
	...
	$sql="select
		  ...
		  from cnmd10_individual_dias a	where ".$this->SQLCA()." and cod_tipo_nomina='$nomina' order by cod_transaccion";

	$transacciones=$this->cnmd10_individual_dias->execute($sql);
...
function seleccion_trans($nomina=null,$transaccion=null,$otro=null){
	...
	if($transaccion!=null){
		...
		$sql="select
			  ...
			  from cnmd10_individual_dias a	where ".$this->SQLCA()." and a.cod_tipo_nomina='$nomina' and a.cod_transaccion='$transaccion' order by a.cod_transaccion";

		$verifica=$this->cnmd10_individual_dias_cantidad->execute($sql);
		if($verifica!=null){
		...
		////////////////////////////////AQUI LA GRILLA////////////////////////////////////
			$grilla="select
			  ...
			  from cnmd10_individual_dias_cantidad a where ".$this->SQLCA()." and a.cod_tipo_nomina=".$nomina." and cod_transaccion=".$transaccion." ORDER BY (select b.cedula_identidad from v_cnmd06_fichas_datos_personales b where b.cod_presi=a.cod_presi and b.cod_entidad=a.cod_entidad and b.cod_tipo_inst=a.cod_tipo_inst and b.cod_inst=a.cod_inst and b.cod_dep=a.cod_dep and b.cod_tipo_nomina=a.cod_tipo_nomina and b.cod_cargo=a.cod_cargo and b.cod_ficha=a.cod_ficha) ASC";
			  $datos=$this->cnmd10_individual_dias_cantidad->execute($grilla);
...
function seleccion_busqueda($opcion=null,$nomina=null,$transaccion=null,$cargo=null,$ficha=null){
	...
	$datos=$this->cnmd10_individual_dias_cantidad->FindCount($this->SQLCA()." and cod_tipo_nomina=".$nomina." and cod_transaccion=".$transaccion." and cod_cargo=".$cargo." and cod_ficha=".$ficha,null,null);
	if($datos!=0){
...
function guardar(){
    ...
    if(empty($this->data['cnmp10']['cod_cargo'])){
    	...
    	$this->set('errorMessage','Ingrese una cantidad de dias');
    }else{
	    ...
	    $ubicacion_escenario  = strtoupper('ASIGNACIÓN INDIVIDUAL EN DIAS SEGÚN CANTIDAD QUE LE CORRESPONDE');
	    $this->set('nomina',$cod_tipo_nomina);
	    $this->set('transaccion',$cod_transaccion);

		$verifica=$this->cnmd10_individual_dias->FindCount($this->SQLCA()." and cod_tipo_nomina='$cod_tipo_nomina' and cod_transaccion='$cod_transaccion'");
		if($verifica==0){/////////// AQUI COMPRUEBO SI YA EXISTE EL ESCENARIO,SI NO EXISTE REGISTRO TODOS LOS DATOS INCLUYENDO LOS RADIOS SINO REGISTRO SOLO LA ESCALA
				if($this->data['cnmp10']['condicion']==2 && empty($this->data['cnmp10']['tipo_trans'])){
					...
					$insert1 = "Begin;INSERT INTO cnmd10_individual_dias VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_tipo_transaccion', '$cod_transaccion','$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
					$sw1 = $this->cnmd10_individual_dias->execute($insert1);
					if($sw1>1){
						$insert2 = "INSERT INTO cnmd10_individual_dias_cantidad VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina','$cod_cargo','$cod_ficha', '$cod_tipo_transaccion', '$cod_transaccion','$cantidad')";
						$sw2 = $this->cnmd10_individual_dias_cantidad->execute($insert2);
						if($sw2>1){
							...
							$sw3 = $this->cnmd10_individual_dias->execute($insert3);
							if($sw3>1){
								$this->cnmd10_individual_dias->execute('COMMIT');
								$this->set('Message_existe','Registro exitoso');
								$this->set('guardado','si');
							}else{
								$this->cnmd10_individual_dias->execute('ROLLBACK');
...
function guardar_transferir($nomina=null,$transaccion=null){
	...
	$cod_dep = $this->Session->read('SScoddep');
	  ...
	  if($ubicacion==null){
		  $data=$this->cnmd10_individual_dias->execute("select * from cnmd10_individual_dias where ".$this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1 and cod_transaccion=".$cod_transaccion);
		  $data2=$this->cnmd10_individual_dias_cantidad->execute("select * from cnmd10_individual_dias_cantidad where ".$this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=1 and cod_transaccion=".$cod_transaccion);
		  $ubicacion_escenario = strtoupper('Asignaciones Comunes - en Dias calculado segun escala de años de servicios');
		  ...
		  $insert1 = "Begin;INSERT INTO cnmd10_individual_dias VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_transferir', '$cod_tipo_transaccion', '$cod_transaccion','$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
		  $sw      = $this->cnmd10_individual_dias->execute($insert1);
		  if($sw>1){
			  for($j=0;$j<count($data2);$j++){
				  ...
				  $insert2 = "INSERT INTO cnmd10_individual_dias_cantidad VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_transferir', '$cod_tipo_transaccion', '$cod_transaccion','$escala','$desde','$hasta', '$monto')";
				  $sw2     = $this->cnmd10_individual_dias_cantidad->execute($insert2);
			  }
			  if($sw2>1){
...
function otros_escenarios($nomina=null,$transaccion=null){
	...
	$sql="select
			 ...
			 from cnmd10_individual_dias a where ".$this->SQLCA()." and a.cod_transaccion=".$transaccion." and a.cod_tipo_nomina!=".$nomina." order by a.cod_tipo_nomina asc";
	$otras_nominas=$this->cnmd10_individual_dias_cantidad->execute($sql);
...
function eliminar($nomina=null,$transaccion=null){
	...
	if(isset($nomina) && isset($transaccion)){
		$this->set('nomina',$nomina);
		$delete1="Begin;DELETE FROM cnmd10_individual_dias WHERE ".$this->SQLCA()." and cod_tipo_nomina='$nomina' and cod_transaccion='$transaccion'";
		$sw=$this->cnmd10_individual_dias->execute($delete1);
		if($sw>1){
			$delete2="DELETE FROM cnmd10_individual_dias_cantidad WHERE ".$this->SQLCA()." and cod_tipo_nomina='$nomina' and cod_transaccion='$transaccion'";
			$sw2=$this->cnmd10_individual_dias_cantidad->execute($delete2);
			if($sw2>1){
				...
				$sw3=$this->cnmd10_individual_dias_cantidad->execute($delete3);
				if($sw3>1){
					$this->cnmd10_individual_dias->execute('COMMIT');
...
function eliminar_items($cod_nomina=null, $cod_trans=null,$cargo=null,$ficha=null){
	...
	if($this->cnmd10_individual_dias_cantidad->FindCount($this->SQLCA()." and cod_tipo_nomina='$cod_nomina' and cod_tipo_transaccion=1 and cod_transaccion='$cod_trans'")==1){
...
//			$this->set('Message_existe', 'LA TRANSACCION SE ELIMINO CON EXITO');
	...
	////////////////////////////////AQUI LA GRILLA////////////////////////////////////
			$grilla="select
			  ...
			  (select b.primer_nombre from v_cnmd06_fichas_datos_personales b where b.cod_presi=a.cod_presi and b.cod_entidad=a.cod_entidad and b.cod_tipo_inst=a.cod_tipo_inst and b.cod_inst=a.cod_inst and b.cod_dep=a.cod_dep and b.cod_tipo_nomina=a.cod_tipo_nomina and b.cod_cargo=a.cod_cargo and b.cod_ficha=a.cod_ficha) as primer_nombre,
			  (select b.segundo_nombre from v_cnmd06_fichas_datos_personales b where b.cod_presi=a.cod_presi and b.cod_entidad=a.cod_entidad and b.cod_tipo_inst=a.cod_tipo_inst and b.cod_inst=a.cod_inst and b.cod_dep=a.cod_dep and b.cod_tipo_nomina=a.cod_tipo_nomina and b.cod_cargo=a.cod_cargo and b.cod_ficha=a.cod_ficha) as segundo_nombre
			  from cnmd10_individual_dias_cantidad a where ".$this->SQLCA()." and a.cod_tipo_nomina=".$cod_nomina." and cod_transaccion=".$cod_trans." order by cod_cargo,cod_ficha";
			  $datos=$this->cnmd10_individual_dias_cantidad->execute($grilla);
			  $this->set('datos',$datos);
...
function modificar_items($cod_nomina=null, $cod_trans=null,$cargo=null,$ficha=null,$i=null){
	...
	$grilla="select
			  ...
			  (select b.primer_nombre from v_cnmd06_fichas_datos_personales b where b.cod_presi=a.cod_presi and b.cod_entidad=a.cod_entidad and b.cod_tipo_inst=a.cod_tipo_inst and b.cod_inst=a.cod_inst and b.cod_dep=a.cod_dep and b.cod_tipo_nomina=a.cod_tipo_nomina and b.cod_cargo=a.cod_cargo and b.cod_ficha=a.cod_ficha) as primer_nombre,
			  (select b.segundo_nombre from v_cnmd06_fichas_datos_personales b where b.cod_presi=a.cod_presi and b.cod_entidad=a.cod_entidad and b.cod_tipo_inst=a.cod_tipo_inst and b.cod_inst=a.cod_inst and b.cod_dep=a.cod_dep and b.cod_tipo_nomina=a.cod_tipo_nomina and b.cod_cargo=a.cod_cargo and b.cod_ficha=a.cod_ficha) as segundo_nombre
			  from cnmd10_individual_dias_cantidad a where ".$this->SQLCA()." and a.cod_tipo_nomina=".$cod_nomina." and cod_transaccion=".$cod_trans." and cod_cargo=".$cargo." and cod_ficha=".$ficha." order by cod_cargo,cod_ficha";
    $datos=$this->cnmd10_individual_dias_cantidad->execute($grilla);
    $this->set('datos',$datos);
	$this->set('k',$i);
}
...
function guardar_editar($cod_nomina=null, $cod_trans=null,$cargo=null,$ficha=null,$i=null){
	...
	if(!empty($this->data['cnmp10']['cantidad'.$i])){
		...
		$ver=$this->cnmd10_individual_dias_cantidad->execute("update cnmd10_individual_dias_cantidad set cantidad=".$cantidad." where ".$this->SQLCA()." and cod_tipo_nomina=".$cod_nomina." and cod_transaccion=".$cod_trans." and cod_cargo=".$cargo." and cod_ficha=".$ficha);
	}else{
		$this->set('errorMessage','Debe ingresar la cantidad de dias');
	}
