<?php
class Cscp05ordencompranotaentregaController extends AppController {

   var $name = "cscp05_ordencompra_nota_entrega";
...
$this->layout = "ajax";
 ...
 $lista = $this->select_orden_compra->generateList($this->condicion()."  and ano_orden_compra='".$this->ano_ejecucion()."' AND condicion_actividad=1 AND tipo_orden IN (1, 3) AND (entrega_completa=1 or entrega_completa=0)"." and (cod_obra='' or cod_obra='0' or cod_obra IS NULL)", ' numero_orden_compra ASC', null, '{n}.select_orden_compra.numero_orden_compra', '{n}.select_orden_compra.beneficiario');
...
function agregar_todos_productos($ano_cotizacion=null,$numero_cotizacion=null, $rif=null, $tp = null ){
  ...
  if($lista_cscd03_cotizacion_cuerpo!=null){
    foreach($lista_cscd03_cotizacion_cuerpo as $ve){ $opcion = "no";
		...
		$ve['cscd03_cotizacion_cuerpo']['cantidad'] = $cantidad_faltante;
          if($ve['cscd03_cotizacion_cuerpo']['codigo_prod_serv'] != 3228 && $ve['cscd03_cotizacion_cuerpo']['cod_partida'] != 403 && $cantidad_faltante!=0){
...
$ano_orden_compra    = $this->data['cscp05_ordencompra_nota_entrega']['ano_orden_compra'];
$numero_orden_compra = $this->data['cscp05_ordencompra_nota_entrega']['numero_orden_compra'];
$fecha_cotizacion    = $this->data['cscp05_ordencompra_nota_entrega']['fecha_cotizacion'];
$ano_orden_compra_pago    = $this->data['cscp05_ordencompra_nota_entrega']['ano_orden_compra_pago'];
$numero_orden_compra_pago = $this->data['cscp05_ordencompra_nota_entrega']['numero_orden_compra_pago'];
$fecha_pago    = $this->data['cscp05_ordencompra_nota_entrega']['fecha_pago'];
$rif   = $this->data['cscp05_ordencompra_nota_entrega']['rif'];
$observaciones    =   $this->data['cscp05_ordencompra_nota_entrega']['observaciones'];
$entrega_completa    = $this->data['cscp05_ordencompra_nota_entrega']['entrega_completa'];


$tipo_orden_compra = $this->cscd03_cotizacion_cuerpo->execute("SELECT tipo_orden FROM cscd04_ordencompra_encabezado WHERE  ano_orden_compra='$ano_orden_compra' and numero_orden_compra='$numero_orden_compra' and ".$this->condicion().";");
...
if($tipo_orden_compra[0][0]['tipo_orden'] == 3){
  ...
  $cantidad_entregada_aux1 = $conta_cantdida[0][0]['cantidad_entregada'];
...
for($i=1; $i<=$cont; $i++){  
  $cantidad_entregada_aux1 += $this->data['cscp05_ordencompra_notaentrega']['cantidad_'.($i-1)]; 
}//fin for
...
if($sw1 > 1){
	...
	for($i=1; $i<=$cont; $i++){
			...
			$cantidad_entregada = $this->data['cscp05_ordencompra_notaentrega']['cantidad_'.($i-1)];
	...
	$this->cscd05_ordencompra_nota_entrega_cuerpo->execute("ROLLBACK;");
	 ...
	 $ano = $this->ano_ejecucion();
	 $condicion = 'cod_presi='.$this->Session->read('SScodpresi').'  and  cod_entidad='.$this->Session->read('SScodentidad').' and cod_tipo_inst='.$this->Session->read('SScodtipoinst').' and  cod_inst='.$this->Session->read('SScodinst').' and cod_dep='.$this->Session->read('SScoddep').' and  ano_orden_compra='.$ano.'';
	 $lista = $this->cscd04_ordencompra_encabezado->generateList($this->condicion()." AND condicion_actividad=1 AND tipo_orden IN (1, 3) AND (entrega_completa=1)", ' numero_orden_compra ASC', null, '{n}.cscd04_ordencompra_encabezado.numero_orden_compra', '{n}.cscd04_ordencompra_encabezado.rif');
	 //print_r($lista);
	 if($lista != null){
...
if(!empty($this->data['cscp05_ordencompra_nota_entrega']['ano_ejecucion'])){
$_SESSION['ano_compra'] = $this->data['cscp05_ordencompra_nota_entrega']['ano_ejecucion'];
}else{$_SESSION['ano_compra'] = $this->ano_ejecucion();}

$ano = $_SESSION['ano_compra'];

$this->set('ano', $ano);
$listaCompra = $this->select_nota_entrega->generateList($conditions = $this->condicion()."  and ano_orden_compra='".$ano."' "." and (cod_obra='' or cod_obra='0' or cod_obra IS NULL)", $order = 'numero_orden_compra ASC', $limit = null, '{n}.select_nota_entrega.numero_orden_compra', '{n}.select_nota_entrega.beneficiario');
...
if(!empty($this->data['cscp05_ordencompra_nota_entrega']['numero_orden_compra'] )){
for($a=0; $a<=$i; $a++){
if($this->data['cscp05_ordencompra_nota_entrega']['numero_orden_compra'] == $numero[$a]['numero_orden_compra']){$pag_num = $a; $opcion='si'; break;}else{$opcion='no';}
}//fin for
...
function buscar_year($var1=null){
  ...
  $listaCompra = $this->select_nota_entrega->generateList($condicion."  and ano_orden_compra='".$var1."' "." and (cod_obra='' or cod_obra='0' or cod_obra IS NULL)", $order = 'numero_orden_compra ASC', $limit = null, '{n}.select_nota_entrega.numero_orden_compra', '{n}.select_nota_entrega.beneficiario');
    ...
    if($pagina!=null){
        	$pagina=$pagina;
 			...
 			$this->set('pagina',$pagina);
              ...
              if($datos[0]['v_cscd05_ordencompra_nota_entrega']['tipo_orden'] == 3){
                  $condicion = 'cod_presi='.$this->Session->read('SScodpresi').'  and  cod_entidad='.$this->Session->read('SScodentidad').' and cod_tipo_inst='.$this->Session->read('SScodtipoinst').' and  cod_inst='.$this->Session->read('SScodinst').' and cod_dep='.$this->Session->read('SScoddep').' and  ano_cotizacion='.$ano_cotizacion." and numero_cotizacion='$numero_cotizacion' and upper(rif)=upper('$rif') and cod_partida != 403";
              }else{
                $condicion = 'cod_presi='.$this->Session->read('SScodpresi').'  and  cod_entidad='.$this->Session->read('SScodentidad').' and cod_tipo_inst='.$this->Session->read('SScodtipoinst').' and  cod_inst='.$this->Session->read('SScodinst').' and cod_dep='.$this->Session->read('SScoddep').' and  ano_cotizacion='.$ano_cotizacion." and numero_cotizacion='$numero_cotizacion' and upper(rif)=upper('$rif')";
              }
