 ...
 * sisap
 */
class Cnmp10ComunesBolivaresDeduccionController extends AppController {
   var $name = 'cnmp10_comunes_bolivares_deduccion';
   var $uses = array('Cnmd01',  'cnmd03_transacciones', 'cnmd10_control_escenarios', 'v_cnmd10_bolivares_asig_trans','cnmd10_bolivares_asig','cnmd10_comunes_bolivares_deduccion','v_cnmd10_bolivares_deducciones_trans');
...
function index(){
	...
	$this->data['cnmp10_comunes_bolivares_deduccion']['monto']=null;
}
...
function otras_nominas($var1=null){
	...
	$cod_dep                  =       $this->Session->read('SScoddep');
	$sql='';
	$datos=$this->cnmd10_comunes_bolivares_deduccion->findAll($this->condicion()." and cod_tipo_transaccion=2 and cod_tipo_nomina=".$var1,null,null);
			 if($datos){
			 	foreach($datos as $x){
			 		$transaccion=$x['cnmd10_comunes_bolivares_deduccion']['cod_transaccion'];
			 		if($sql==''){
			 			$sql.=$transaccion;
			 		}else{
			 			$sql.=",".$transaccion;
...
function datos($cod_nomina=null){
	...
	if($cod_nomina != null){
		...
		$this->set('deno_trans', $deno_trans);
		$datos = $this->cnmd10_comunes_bolivares_deduccion->findAll($conditions = $this->condicion()." and cod_tipo_nomina='$cod_nomina'",null, $order = 'cod_tipo_nomina, cod_transaccion', $limit = null, $page = null, $recursive = null);
...
function guardar(){
	$this->layout="ajax";
		...
		$cod_condicion = $this->data['cnmp10_comunes_bolivares_deduccion']['condicion'];
		$monto = $this->Formato1($this->data['cnmp10_comunes_bolivares_deduccion']['monto']);
	    $ubicacion_escenario = strtoupper('Deducciones Comunes - En Bolivares');
	$prueba=$this->cnmd10_comunes_bolivares_deduccion->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion,null,null);

	if(!$prueba){
		if($this->Formato1($this->data['cnmp10_comunes_bolivares_deduccion']['monto'])==0){
		$this->set('errorMessage', 'Debe ingresar un monto');
				return;
	}else{
		if  (!empty($this->data['cnmp10_comunes_bolivares_deduccion'])){

			if(empty($this->data['cnmp10_comunes_bolivares_deduccion']['tipo_trans'])){
				$codi_tipo_transaccion = 0;
			}else{
				$codi_tipo_transaccion = $this->data['cnmp10_comunes_bolivares_deduccion']['tipo_trans'];
			}

		if($cod_condicion==1){
			$codi_transaccion = 0;
		}else{
			if(empty($this->data['cnmp10_comunes_bolivares_deduccion']['select4'])){
				 $this->set('errorMessage', 'Debe seleccionar el codigo de transaccion');
				 return;
			}else{
				 $codi_transaccion = $this->data['cnmp10_comunes_bolivares_deduccion']['select4'];
			}

		}


			if(empty($this->data['cnmp10_comunes_bolivares_deduccion']['escenario'])){
				$activar_frecuencia_eventual = 2;
			}else{
				$activar_frecuencia_eventual = $this->data['cnmp10_comunes_bolivares_deduccion']['escenario'];
			}
			$sql_insert = "INSERT INTO cnmd10_comunes_bolivares_deduccion VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_tipo_nomina', '$cod_tipo_transaccion', '$cod_transaccion', '$monto', '$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
			$sw = $this->cnmd10_comunes_bolivares_deduccion->execute($sql_insert);
			if($sw > 1){
				...
				$sw = $this->cnmd10_comunes_bolivares_deduccion->execute($sql_control);
				echo'<script>';
		...
		$this->data['cnmp10_comunes_bolivares_deduccion'] = array();
		}//FIN ELSE VALIDA MONTO CERO
	}else{
		$this->set('errorMessage', 'estos datos ya existen registrados');
	}

	$datos=$this->cnmd10_comunes_bolivares_deduccion->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=2",null,'order by  cod_transaccion asc');
	 if($datos){
	 	 $this->set('datos',$datos);
	 }else{
	 	 $this->set('datos','');
...
function modificar(){
	...
	$a=0;
	$cod_tipo_nomina = $this->data['cnmp10_comunes_bolivares_deduccion']['cod_nomina'];
	$cod_transaccion = $this->data['cnmp10_comunes_bolivares_deduccion']['cod_trans_grilla'];
	$monto = $this->Formato1($this->data['cnmp10_comunes_bolivares_deduccion']['monto']);
//	$cod_frecuencia = $this->data['cnmp10_comunes_bolivares_deduccion']['frecuencia'];
	$cod_condicion = $this->data['cnmp10_comunes_bolivares_deduccion']['condicion'];
...
if($cod_tipo_nomina!='' && $cod_transaccion!=''){
	...
	if($monto==0){
		...
		if($a!=1){
			$this->cnmd10_comunes_bolivares_deduccion->execute("update cnmd10_comunes_bolivares_deduccion set monto=".$monto.",cod_condicion=".$cod_condicion.",codi_tipo_transaccion=".$codi_tipo_transaccion.",codi_transaccion=".$codi_transaccion.",activar_frecuencia_eventual=".$activar_frecuencia_eventual." where ".$this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_transaccion=".$cod_transaccion);///modifica el monto
	...
	$datos=$this->v_cnmd10_bolivares_deducciones_trans->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=2",null,'order by cod_transaccion asc');
	 if($datos){
	 	 $this->set('datos',$datos);
	 }else{
	 	 $this->set('datos','');
	...
	$this->set('errorMessage', 'debe seleccionar una transaccion a modificar');
	$this->set('datos',null);
}
}//FIN MODIFICAR
...
function eliminar($cod_nomina=null, $cod_trans=null){
	...
	if($cod_nomina!=null && $cod_trans!=null){
		$sql_eliminar="DELETE FROM cnmd10_comunes_bolivares_deduccion WHERE ".$this->condicion()." and cod_tipo_nomina='$cod_nomina' and cod_tipo_transaccion=2 and cod_transaccion='$cod_trans'";
		$sw = $this->cnmd10_comunes_bolivares_deduccion->execute($sql_eliminar);
		if($sw>1){
			...
			$sw = $this->cnmd10_comunes_bolivares_deduccion->execute($sql_del_control);
...
function monto($cod_nomina=null, $cod_trans=null){
	if($cod_nomina!=null && $cod_trans!=null){
		...
		$monto = $this->cnmd10_comunes_bolivares_deduccion->field('cnmd10_comunes_bolivares_deduccion.monto', $conditions = $this->condicion()." and cod_tipo_nomina='$cod_nomina' and cod_transaccion='$cod_trans'", $order ="cod_tipo_nomina, cod_transaccion ASC");
...
function ver($cod_nomina=null, $cod_trans=null, $codi=null,$condicion=null){
	...
	if($cod_nomina!=null && $cod_trans!=null){
		...
		$cod_trans;
		$codi;
		$datos2 = $this->v_cnmd10_bolivares_deducciones_trans->findAll($this->condicion()." and cod_tipo_nomina='$cod_nomina'",null,'order by  cod_transaccion asc');
		...
		$opciones= $this->cnmd10_comunes_bolivares_deduccion->findAll($conditions = $this->condicion()." and cod_tipo_nomina='$cod_nomina' and cod_transaccion='$cod_trans'", $fields ='cod_frecuencia, cod_condicion, codi_tipo_transaccion, codi_transaccion, activar_frecuencia_eventual', $order = null, $limit = null, $page = null, $recursive = null);
		$this->set('opciones', $opciones);
...
function transferir($var1=null){
	$this->layout="ajax";
		$carga=$this->cnmd10_comunes_bolivares_deduccion->findAll($this->condicion()." and cod_tipo_nomina=".$var1." and cod_tipo_transaccion=2");
		if($carga){
...
function guardar_transferir(){
	$this->layout="ajax";
	 ...
	 $data=$this->cnmd10_comunes_bolivares_deduccion->findAll($this->condicion()." and cod_tipo_nomina=".$cod_tipo_nomina." and cod_tipo_transaccion=2");
	 $ubicacion_escenario = strtoupper('Deducciones Comunes - En Bolivares');
	  foreach($data as $row){
			$cod_transaccion1 = $row['cnmd10_comunes_bolivares_deduccion']['cod_transaccion'];
			$monto = $row['cnmd10_comunes_bolivares_deduccion']['monto'];
			$cod_frecuencia = $row['cnmd10_comunes_bolivares_deduccion']['cod_frecuencia'];
			$cod_condicion = $row['cnmd10_comunes_bolivares_deduccion']['cod_condicion'];
			$codi_tipo_transaccion = $row['cnmd10_comunes_bolivares_deduccion']['codi_tipo_transaccion'];
			$codi_transaccion = $row['cnmd10_comunes_bolivares_deduccion']['codi_transaccion'];
			$activar_frecuencia_eventual = $row['cnmd10_comunes_bolivares_deduccion']['activar_frecuencia_eventual'];
			...
			if(!$datos){
				$sql_insert = "INSERT INTO cnmd10_comunes_bolivares_deduccion VALUES('$cod_presi', '$cod_entidad', '$cod_tipo_inst', '$cod_inst', '$cod_dep', '$cod_transferir', '$cod_tipo_transaccion', '$cod_transaccion1','$monto','$cod_frecuencia', '$cod_condicion', '$codi_tipo_transaccion', '$codi_transaccion', '$activar_frecuencia_eventual')";
				$sw = $this->cnmd10_comunes_bolivares_deduccion->execute($sql_insert);
				if($sw>1){
				echo "<script>";
